...
**Slug**: `arkts-apis-image-imagesource`

**DocID**: `dfe531c146304a128ac6cca6cbc228c6`

**NodeID**: `00020177579591076116007c9e868e07`
...
支持的图片格式，包括：png，jpeg，bmp，gif，webp，dng，heic
12+
（不同硬件设备支持情况不同）。
...
创建ImageSource时的序号。默认值为0，表示第一张图片。当取值为N时，表示第N+1张图片。单帧图片场景中index取值只能为0，动图等多帧图片场景中index的取值范围为：[0, (帧数-1)]。
...
回调函数。当获取图片信息成功，err为undefined，data为获取到的图片信息；否则为错误对象。
...
该接口仅支持JPEG、PNG和HEIF
12+
（不同硬件设备支持情况不同）文件，且需要包含exif信息。其中可以通过supportedFormats属性查询是否支持HEIF格式的exif读写。
...
Promise对象，返回图片属性值，如获取失败则返回属性默认值。
...
Promise对象，返回图片属性值，如获取失败则返回null。
...
返回图片exif中指定属性键的值（如获取失败则返回属性默认值），各个数据值作用请参考
PropertyKey
。
...
Bad source. e.g.,1. Image has invalid width or height. 2. Image source incomplete. 3. Read image data failed. 4. Codec create failed.
...
Unsupported metadata. For example, key is not supported.
...
调用该接口修改属性会改变属性字节长度，建议通过传入文件描述符来创建
ImageSource
实例或通过传入的uri创建
ImageSource
实例。
...
true表示数据更新完成，当前buffer内存放最后一段数据；false表示数据还未更新完成，需要继续更新。
...
即当前buffer中的数据首地址，相对于整个图片文件首地址的偏移量。单位：字节。
...
回调函数，当更新增量数据成功，err为undefined，否则为错误对象。
...
Unsupported options. For example, unsupported desiredPixelFormat causes a failure in converting an image into the desired pixel format.
...
Unsupported options. For example, index is invalid.
...
从API version 15开始，推荐使用
createPixelMapUsingAllocator
，该接口可以指定输出pixelMap的内存类型
AllocatorType
，详情请参考
图片解码内存优化(ArkTS)
。
...
回调函数，当创建PixelMap对象成功，err为undefined，data为获取到的PixelMap对象；否则为错误对象。
...
回调函数，当创建PixelMap对象数组成功，err为undefined，data为获取到的PixelMap对象数组；否则为错误对象。
...
用于图像解码的内存类型。默认值为AllocatorType.AUTO。
...
Image too large. This status code is thrown when an error occurs during the process of checking size.
...
根据指定的分配器同步创建一个基于图像解码参数的PixelMap对象。接口使用详情请参考
图片解码内存优化(ArkTS)
。
...
回调函数，当获取图像延迟时间数组成功，err为undefined，data为获取到的图像延时时间数组；否则为错误对象。
...
回调函数，当获取图像帧数成���，err为undefined，data为获取到的图像帧数；否则为错误对象。
...
回调函数，当资源释放成功，err为undefined，否则为错误对象。
...
回调函数，当获取图片属性值成功，err为undefined，data为获取到的图片属性值；否则为错误对象。
...
回调函数，当修改图片属性值成功，err为undefined，否则为错误对象。

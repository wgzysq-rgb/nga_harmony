...
**Slug**: `arkts-apis-image-pixelmap`

**DocID**: `7b0457fe80084674b33f3de66f41f627`

**NodeID**: `00020177579591076121547907ae21ef`

---

Interface (PixelMap)

图像像素类，用于读取或写入图像数据以及获取图像信息。在调用PixelMap的方法前，需要先通过
image.createPixelMap
创建一个PixelMap实例。目前PixelMap序列化大小最大128MB，超过会送显失败。大小计算方式为（宽*高*
每像素占用字节数
）。

从API version 11开始，PixelMap支持通过
Worker
跨线程调用。当PixelMap通过
Worker
跨线程后，原线程的PixelMap的所有接口均不能调用，否则将报错501 服务器不具备完成请求的功能。

在调用PixelMap的方法前，可以通过
image.createPixelMap
传入像素数据创建一个PixelMap对象，也可以通过
ImageSource
进行图片解码创建PixelMap对象。
...
图片使用的内存往往较大，在PixelMap对象使用完成后，应主动调用
release
方法及时释放内存。释放时应确保该对象的所有异步方法均执行完成，且后续不再使用该对象。
...
图像像素是否可被编辑。true表示可被编辑，false表示不可被编辑。为false时，图像的渲染和传输性能更好。
...
图像的行数据是否已进行内存对齐。true表示已进行内存对齐，每行数据的末尾可能有空白字节填充以满足对齐要求；false表示未进行内存对齐，每行数据紧密排列，末尾无空白字节填充。
...
缓冲区，函数执行结束后获取的图像像素数据写入到该内存区域内。缓冲区大小由
getPixelBytesNumber
接口获取。
...
回调函数。当读取像素数据到ArrayBuffer成功，err为undefined，否则为错误对象。
...
固定按照BGRA_8888格式，读取PixelMap指定区域内的图像像素数据，并写入
PositionArea
.pixels缓冲区中，该区域由
PositionArea
...
回调函数。当读取区域内的图片数据成功，err为undefined，否则为错误对象。
...
固定按照BGRA_8888格式，读取
PositionArea
.pixels缓冲区中的图像像素数据，并写入PixelMap指定区域内，该区域由
PositionArea
.region指定。使用Promise异步回调。
...
缓冲区，函数执行时会将该缓冲区中的图像像素数据写入到PixelMap。缓冲区大小由
getPixelBytesNumber
接口获取。
...
回调函数。当缓冲区中的图像像素数据写入PixelMap成功，err为undefined，否则为错误对象。
...
回调函数。当获取图像像素信息成功，err为undefined，data为获取到的图像像素信息；否则为错误对象。
...
回调函数。当设置透明比率成功，err为undefined，否则为错误对象。
...
回调函数，当创建PixelMap成功，err为undefined，data为获取到的PixelMap对象；否则为错误对象。
...
回调函数。当对图片进行缩放成功，err为undefined，否则为错误对象。
...
采用的缩放算法，默认值为AntiAliasingLevel.NONE。
...
采用的缩放算法。默认值是AntiAliasingLevel.NONE。
...
The PixelMap has been released.
...
Image YUV And ASTC types are not supported.
...
回调函数。当对图片进行位置变换成功，err为undefined，否则为错误对象。
...
回调函数。当对图片进行旋转成功，err为undefined，否则为错误对象。
...
回调函数，当对图片翻转成功，err为undefined，否则为错误对象。
...
回调函数。当对图片进行裁剪成功，err为undefined，否则为错误对象。
...
If the image parameter invalid.
...
目标色彩空间，支持SRGB、DCI_P3、DISPLAY_P3、ADOBE_RGB_1998。
...
回调函数。当对图像像素颜色进行色彩空间转换成功，err为undefined，否则为错误对象。
...
Failed to initialize the internal object.
...
Memory copy failed. Possibly caused by invalid metadata value.
...
从MessageSequence中获取PixelMap，如需使用同步方式创建PixelMap可使用：
...
回调函数。当对PixelMap对象释放成功，err为undefined，否则为错误对象。
...
目标像素格式，用于YUV和RGB类型互转，或者ASTC_4x4类型转为RGBA_8888类型。目前仅支持NV12/NV21与RGB888/RGBA8888/RGB565/BGRA8888/RGBAF16互转，YCRCB_P010/YCBCR_P010与RGBA1010102互转，ASTC_4x4转为RGBA_8888。
...
pixelmap内存标识符，只允许DMA和ASHMEM内存形式的pixelmap设置。DMA内存设置名字长度取值范围为[1, 255]，ASHMEM内存设置名字长度取值范围为[1, 244]，单位字节。
...
Parameter error. Possible causes: 1.The length of the input parameter is too long. 2.Parameter verification failed.
...
PixelMap是否已被释放。返回true表示已被释放，否则返回false。

...
**Slug**: `arkts-apis-image-imagepacker`

**DocID**: `f563b1ea2d6f4309a74c7f4ecd12fa82`

**NodeID**: `000201775795910761134690194c976a`
...
当前支持的格式有：jpeg、webp、png、heic
12+
、gif
18+
（不同硬件设备支持情况不同，可通过ImagePacker的supportedFormats属性查看）。
...
图片编码支持的格式，包括：jpeg、webp、png、heic
12+
、gif
18+
（不同硬件设备支持情况不同）。
...
回调函数，当释放图片编码实例成功，err为undefined，否则为错误对象。
...
回调函数，当编码进文件成功，err为undefined，否则为错误对象。
...
回调函数，当编码图片进文件成功，err为undefined，否则为错误对象。
...
从API version 6开始支持，从API version 13开始废弃，建议使用
packToData
代替。
...
回调函数，当图片编码成功，err为undefined，data为获取到的压缩或编码数据；否则为错误对象。
...
从API version 8开始支持，从API version 13开始废弃，建议使用
packToData
代替。

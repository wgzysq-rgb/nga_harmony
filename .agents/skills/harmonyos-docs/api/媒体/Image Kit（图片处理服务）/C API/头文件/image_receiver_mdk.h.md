...
**DocID**: `083537170d2f4501b24b406990791aa1`

**NodeID**: `0002017757959107612544a04963ed78`
...
通过应用层ImageReceiver对象初始化native层
ImageReceiverNative
对象。
...
[h2]OH_Image_Receiver_On_Callback()
...
[h2]OH_Image_Receiver_CreateImageReceiver()
...
[h2]OH_Image_Receiver_InitImageReceiverNative()
...
操作成功则返回ImageReceiverNative指针；如果操作失败，则返回nullptr。

参考：

OH_Image_Receiver_Release

[h2]OH_Image_Receiver_GetReceivingSurfaceId()
...
native层的ImageReceiverNative指针。

char* id

指向字符缓冲区的指针，用于获取字符串的id。
...
[h2]OH_Image_Receiver_ReadLatestImage()
...
注意，此接口需要在
OH_Image_Receiver_On_Callback
回调后调用，才能正常的接收到数据。并且使用此接口返回Image对象创建的
ImageNative
使用完毕后需要调用
OH_Image_Release
方法释放，释放后才可以继续接收新的数据。
...
[h2]OH_Image_Receiver_ReadNextImage()
...
[h2]OH_Image_Receiver_GetCapacity()
...
[h2]OH_Image_Receiver_GetFormat()

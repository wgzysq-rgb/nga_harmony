...
**DocID**: `f5b036e6d19e4a088bc7d223aec900bc`

**NodeID**: `000201775795910761022a6098bbf4c2`
...
通过音频编解码器名称创建音频编解码器实例，使用此接口的前提是知道编解码器的确切名称，编解码器的名称可以通过
...
获取编解码器输出数据的OH_AVFormat信息。
 ...
 需要注意的是，返回值所指向的OH_AVFormat实例需要开发者调用
OH_AVFormat_Destroy
接口手动释放。
...
回调将报告可用的输入缓冲区和对应的索引值。
 ...
 一旦具有指定索引的缓冲区被提交给音频编解码器，该缓冲区将无法再次访问，直到再次收到
OH_AVCodecOnNeedInputBuffer
回调，收到相同索引时此缓冲区才可使用。
...
在设置的超时时间内，尝试查询对应音频编解码器可用的输出缓冲区的索引值。获取到索引值后，通过
...
[h2]OH_AudioCodec_RegisterCallback()
...
指向OH_AVFormat的指针，给出要编解码的音频轨道的描述。
...
AV_ERR_OK：执行成功。
 ...
 AV_ERR_UNKNOWN：发生内���错误，建议检查日志。

[h2]OH_AudioCodec_Start()
...
[h2]OH_AudioCodec_GetOutputDescription()
...
[h2]OH_AudioCodec_PushInputBuffer()
...
AV_ERR_OK：执行成功。
 ...
 AV_ERR_INVALID_VAL：输入的index已使用或无效，需使用其他
OH_AVCodecOnNeedInputBuffer
回调返回的index。

 AV_ERR_INVALID_STATE：编解码器状态错误，调用OH_AudioCodec_PushInputBuffer前需确保按顺序成功调用
...
[h2]OH_AudioCodec_FreeOutputBuffer()
...
AV_ERR_OK：执行成功。
 ...
 AV_ERR_INVALID_VAL：输入参数为nullptr或无效。缓冲区索引应该由
OH_AVCodecOnNewOutputBuffer
给出。
...
输出参数。指向布尔类型的指针，true：编解码器实例有效，false：编解码器实例无效。
...
[h2]OH_AudioCodec_SetDecryptionConfig()
...
[h2]OH_AudioCodec_QueryInputBuffer()
...
[h2]OH_AudioCodec_GetInputBuffer()
...
[h2]OH_AudioCodec_QueryOutputBuffer()
...
AV_ERR_OK：执行成功。
 ...
 AV_ERR_STREAM_CHANGED：解码输出流格式发生变化, 可以通过调用
OH_AudioCodec_GetOutputDescription
接口获取新的流信息。

 AV_ERR_TRY_AGAIN_LATER：执行失败，超时时间内获取不到可用的缓冲区。

[h2]OH_AudioCodec_GetOutputBuffer()

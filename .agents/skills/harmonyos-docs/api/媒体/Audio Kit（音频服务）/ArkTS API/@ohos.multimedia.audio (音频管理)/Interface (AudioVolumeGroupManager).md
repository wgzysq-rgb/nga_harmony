...
**Slug**: `arkts-apis-audio-audiovolumegroupmanager`

**DocID**: `b6b494e02bbe4b2fb808fd6ab00e82af`

**NodeID**: `0002017757959107612098d09d0f7b5d`
...
回调函数。当获取指定音量流静音状态成功，err为undefined，data为true表示静音，false表示非静音；否则为错误对象。
...
事件回调类型，支持的事件为'ringerModeChange'，当铃声模式发生变化时，触发该事件。
...
事件回调类型，支持的事件为'ringerModeChange'，当取消监听铃声模式变化事件时，触发该事件。
...
系统麦克风静音状态。返回true表示静音，返回false表示非静音。
...
事件回调类型，支持的事件为'micStateChange'，当检测到系统麦克风状态发生改变时，触发该事件。
...
事件回调类型，支持的事件为'micStateChange'，当取消监听系统麦克风状态更改事件时，触发该事件。
...
Parameter error. Possible causes: 1.Mandatory parameters missing; 2.Incorrect parameter types.
...
固定音量模式开关状态。返回true表示固定音量模式，返回false表示非固定音量模式。
...
回调函数。当获取音量增益dB值成功，err为undefined，data为获取到的音量增益dB值；否则为错误对象。
...
System error. Return by callback.
...
System error. Return by promise.
...
Promise对象，返回对应设备的电平值，大小在[0, 1]之间。
...
需要权限：
 ohos.permission.MANAGE_AUDIO_CONFIG，该权限仅系统应用可申请。

...
**Slug**: `arkts-apis-audio-audiosessionmanager`

**DocID**: `58f375c918104000be950f79a5ea263a`

**NodeID**: `000201775795910761152d5f60b0ae7e`
...
Parameter error. Possible causes: 1.Mandatory parameters unspecified. 2.Incorrect parameter types.
...
System error. Returned by promise.
...
音频会话是否处于激活状态。true表示已激活，false表示已停用。
...
事件回调类型，支持的事件为'audioSessionDeactivated'，当音频会话停用时，触发该事件。
...
事件回调类型，支持的事件为'audioSessionDeactivated'，当取消监听音频会话停用事件时，触发该事件。
...
事件回调类型，支持的事件为'audioSessionStateChanged'，当音频会话状态变更时，触发该事件。
...
本接口适用于以下情况：当设置的
AudioSessionScene
为VoIP场景时，激活AudioSession后立即生效。若
AudioSessionScene
为非VoIP场景，激活AudioSession时不会生效，仅在启动播放的
StreamUsage
...
Allocate memory failed. Return by promise.
...
事件回调类型，支持的事件为'currentOutputDeviceChanged'，当前输出设备变更时触发。
...
事件回调类型，支持的事件为'currentOutputDeviceChanged'，当前输出设备发生变化时，触发该事件。
...
Parameter verification failed, for example, the selected device does not exist.
...
获得通过
selectMediaInputDevice
设置的媒体输入设备。如果没有设置，返回一个deviceType属性为INVALID的设备。
...
事件回调类型，支持的事件为'currentInputDeviceChanged'，当前输入设备发生变化时，触发该事件。

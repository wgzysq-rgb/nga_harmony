...
**Slug**: `arkts-apis-audio-audiovolumemanager`

**DocID**: `4067df4e1a3f452ab5916180c1272964`

**NodeID**: `000201775795910761219928b1f96642`
...
在使用AudioVolumeManager的接口之前，需先通过
getVolumeManager
...
音量组id，默认使用DEFAULT_VOLUME_GROUP_ID。
...
回调函数。当获取音频组音量管理器实例成功，err为undefined，data为获取到的音频组音量管理器实例；否则为错误对象。
...
Crash or blocking occurs in system process.
...
事件回调类型，支持的事件为'volumeChange'，当系统音量发生变化时，触发该事件。
...
事件回调类型，支持的事件为'volumeChange'，当取消监听系统音量变化事件时，触发该事件。
...
事件回调类型，支持的事件为'appVolumeChange'，当应用级音量发生变化时，触发该事件。
...
事件回调类型，支持的事件为'appVolumeChange'，当取消监听当前应用的应用级音量变化事件时，触发该事件。
...
音频流是否为静音状态，true表示音频流已静音，false表示音频流未静音。
...
事件回调类型，支持的事件为'streamVolumeChange'，当系统音量发生变化时，触发该事件。

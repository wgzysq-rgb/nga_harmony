...
**Slug**: `arkts-apis-audio-audioloopback`

**DocID**: `3a348fa78bfc4353807e74db350f70a6`

**NodeID**: `000201775795910761232bd706dd301c`
...
Parameter verification failed, form 0.0 to 1.0.
...
事件回调类型，支持的事件为'statusChange'，当AudioLoopback的状态发生变化时，触发该事件。
...
事件回调类型，支持的事件为'statusChange'，当取消监听音频状态事件时，触发该事件。
...
表示是否启用音频返听器。true表示启用，false表示不启用。
...
Promise对象。返回true表示功能执行成功；返回false表示功能执行失败。
...
返回混响模式是否设置成功。true表示成功，false表示不成功。
...
返回均衡器类型是否设置成功。true表示成功，false表示不成功。

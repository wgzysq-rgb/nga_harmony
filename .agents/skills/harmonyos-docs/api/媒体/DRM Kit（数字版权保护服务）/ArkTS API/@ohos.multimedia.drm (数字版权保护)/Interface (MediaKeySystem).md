...
**Slug**: `arkts-apis-drm-mediakeysystem`

**DocID**: `84995b8cb02846538b2abbd5b2c8dda9`

**NodeID**: `0002017757959107610750321a725c64`
...
The parameter check failed. Possibly because: 1.Mandatory parameters are left unspecified. 2.Incorrect parameter types. 3.Parameter verification failed.
...
The parameter check failed. Possibly because: 1.Mandatory parameters are left unspecified. 2.Parameter verification failed, the param's length is zero or too big(exceeds 4096 Bytes).
...
The parameter check failed. Possibly because: 1.Mandatory parameters are left unspecified. 2.Parameter verification failed.
...
Promise对象，mediaKeySystem设备证书的请求。设备上如果已存在设备证书，会返回失败。
...
事件类型，通过
createMediaKeySystem
成功创建MediaKeySystem实例后可监听，需要设备证书时触发该事件。
...
回调函数，返回事件信息。只要有该事件返回就证明需请求设备证书。
...
The parameter check failed. Possibly because: 1.Mandatory parameters are left unspecified. 2.Incorrect parameter types.
...
The parameter check failed. Possibly because: 1.Mandatory parameters are left unspecified. 2.The param level exceeds reasonable range, please use value in ContentProtectionLevel.

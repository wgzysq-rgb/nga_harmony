...
**Slug**: `arkts-apis-drm-mediakeysession`

**DocID**: `74cc490dabe943ab964ada0c5c94a449`

**NodeID**: `000201775795910761048345ac2d1272`
...
The parameter check failed. Possibly because: 1.Mandatory parameters are left unspecified or too many parameters. 2.Incorrect parameter types. 3.Parameter verification failed.
...
是否需要安全解码，true表示需要安全解码，false表示不需要安全解码。
...
事件类型，固定为'keyRequired'，当播放DRM节目需要获取媒体密钥时触发。
...
监听事件，固定为'vendorDefined'。自定义事件发生时触发。
...
监听事件类型，固定为'expirationUpdate'。密钥过期更新时触发。
...
监听事件类型，固定为'keysChange'。密钥变化时触发。

callback

(keyInfo: 
KeysInfo[]
, newKeyAvailable: boolean) => void

是

回调函数，返回事件信息，包含密钥标识和密钥状态描述的列表及密钥是否可用。

...
**Slug**: `arkts-apis-avsession-avcastcontroller`

**DocID**: `1e89ffe62a6d44459bd851502aac0c10`

**NodeID**: `000201775795910761068f14fee39c26`
...
后，返回会话控制器实例。控制器可查看会话ID，并可完成对会话发送命令及事件，获取会话元数据，播放状态信息等操作。
...
Promise<Array<
hdrCapability.HDRFormat
>>

Promise对象。返回远端设备所支持的HDR能力。
...
parameter check failed. 1.Mandatory parameters are left unspecified. 2.Parameter verification failed.
...
回调函数。当命令发送成功，err为undefined，否则返回错误对象。
...
Session service exception.You are advised to:1.Scheduled retry.2.Destroy the current session or session controller and re-create it.
...
Promise对象，当处理许可证响应成功，无返回结果，否则返回错误对象。
...
回调函数。当命令执行成功，err为undefined，否则返回错误对象。
...
Promise对象，controller销毁成功，无结果返回，否则返回错误对象。
...
事件回调类型，支持事件'playbackStateChange'：当播放状态变化时，触发该事件。

filter

Array<keyof AVPlaybackState>|'all'

是

'all' 表示关注播放状态所有字段变化；Array<keyof AVPlaybackstate> 表示关注Array中的字段变化。
...
parameter check failed. 1.Mandatory parameters are left unspecified. 2.Incorrect parameter types.
...
avCastController.on('playbackStateChange', ['state', 'speed', 'loopMode'], (playbackState: avSession.AVPlaybackState) => {
 console.info(`on playbackStateChange state : ${playbackState.state}`);
});
...
取消对应的监听事件，支持事件'playbackStateChange'。
...
事件回调类型，支持事件'mediaItemChange'：当播放的媒体内容变化时，触发该事件。
...
回调函数，参数AVQueueItem是当前正在播放的媒体内容。
...
取消对应的监听事件，支持事件'mediaItemChange'。
...
事件回调类型，支持事件'playNext'：当播放下一首状态变化时，触发该事件。
...
事件回调类型，支持事件'playPrevious'：当播放上一首状态变化时，触发该事件。
...
事件回调类型，支持事件'requestPlay'：当请求播放状态变化时，触发该事件。
...
回调函数，参数AVQueueItem是当前正在播放的媒体内容。当监听事件注册成功，err为undefined，否则返回错误对象。
...
回调函数，参数AVQueueItem是当前正在播放的媒体内容。当监听事件取消成功，err为undefined，否则返回错误对象。该参数为可选参数，若不填写该参数，则认为取消所有相关会话的事件监听。
...
事件回调类型，支持事件'endOfStream'：当资源播放结束时，触发该事件。
...
回调函数。当监听事件注册成功，err为undefined，否则返回错误对象。
...
回调函数。当监听事件取消成功，err为undefined，否则返回错误对象。该参数为可选参数，若不填写该参数，则认为取消所有相关会话的事件监听。
...
事件回调类型，支持事件'seekDone'：当seek结束时，触发该事件。
...
事件回调类型，支持事件'validCommandChange'：当检测到会话的合法命令发生改变时，触发该事件。
...
取消对应的监听事件，支持事件'validCommandChange'。
...
avCastController.on('validCommandChange', (validCommands: avSession.AVControlCommandType[]) => {
 console.info(`validCommandChange : SUCCESS : size : ${validCommands.length}`);
 console.info(`validCommandChange : SUCCESS : validCommands : ${validCommands.values()}`);
});
...
事件回调类型，支持事件'videoSizeChange'：当检测到会话的合法命令发生改变时，触发该事件。
...
错误事件回调方法：远端播放过程中发生的错误，会提供错误码ID和错误信息。
...
事件回调类型，支持事件'keyRequest'：当DRM资源播放需要许可证时，触发该事件。
...
错误事件回调类型，支持的事件：'castControlGenericError'。
...
取消对应的监听事件，支持的事件是'castControlGenericError'。
...
错误事件回调类型，支持的事件：'castControlIoError'。
...
取消对应的监听事件，支持的事件是'castControlIoError'。
...
错误事件回调类型，支持的事件：'castControlParsingError'。
...
取消对应的监听事件，支持的事件是'castControlParsingError'。
...
错误事件回调类型，支持的事件：'castControlDecodingError'。
...
取消对应的监听事件，支持的事件是'castControlDecodingError'。
...
错误事件回调类型，支持的事件：'castControlAudioRendererError'。
...
取消对应的监听事件，支持的事件是'castControlAudioRendererError'。
...
错误事件回调类型，支持的事件：'castControlDrmError'。
...
取消对应的监听事件，支持的事件是'castControlDrmError'。
...
事件回调类型，支持'customDataChange'事件。媒体提供方发送自定义数据时触发。
...
取消对应的监听事件，支持的事件是'customDataChange'。
...
注册监听事件时的回调函数。该参数为可选参数，若不填写该参数，则认为取消会话所有与此事件相关的监听。

...
> **分区**: API参考  |  **Slug**: `errorcode-media`  |  **DocID**: `c768e01bc7754e1cb1924e559903058f`
...
# Media错误码
     ...
     **可能原因**
      ...
      - 网络超时。默认网络超时时间为15秒，从开始缓存的事件上报后开始计时，超时后会上报该错误信息。
      - 访问其他模块超时。
     ...
     **处理步骤**
     ...
     支持的规格请根据实际使用的模块功能，参考[Media Kit简介](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/media-kit-intro)中对应模块的规格介绍。
     ...
     **处理步骤**
      ...
      - 确认network_config.json配置文件中是否对相关域名进行HTTP明文访问拦截。
      - 如果不需要拦截，则参考[明文HTTP访问权限配置说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/http-request#明文http访问权限配置说明)对域名权限进行配置。
     ...
     **处理步骤**
      ...
      - 提示性错误码，用于客户端感知不支持SEEK_CONTINUOUS模式进行seek时的表现，客户端无需处理。
     ...
     **可能原因**
     ...
     若没有通过[PlaybackStrategy](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-i#playbackstrategy12)使能超分，则每次调用超分相关接口时会返回该错误码。

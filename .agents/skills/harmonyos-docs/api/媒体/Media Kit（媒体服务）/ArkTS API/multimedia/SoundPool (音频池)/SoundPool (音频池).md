...
**Slug**: `js-apis-inner-multimedia-soundpool`

**DocID**: `10eff5ba26514cbdab409f737ada4293`

**NodeID**: `00020177579591076100480f026d5294`
...
on('loadComplete')
：监听资源加载完成。建议开发者监听此回调以确保音频在加载完成后进行播放。

on('playFinishedWithStreamId')
：监听播放完成，同时返回播放结束的音频的streamId。
...
Parameter error. Possible causes: 1.Mandatory parameters are left unspecified. 2.Incorrect parameter types. 3.Parameter verification failed. Return by callback.
...
Parameter error. Possible causes: 1.Mandatory parameters are left unspecified. 2.Incorrect parameter types. 3.Parameter verification failed. Return by promise.
...
回调函数。当音频池stop回调成功，err为undefined，否则为错误对象。
...
回调函数。当setLoop的回调成功，err为undefined，否则为错误对象。
...
回调函数。当音频池setPriority方法回调成功，err为undefined，否则为错误对象。
...
回调函数。当音频池setRate方法回调成功，err为undefined，否则为错误对象。
...
右声道音量，设置范围为[0.0, 1.0]，当前右声道设置无效，以左声道为准。
...
回调函数。当音频池setVolume方法回调成功，err为undefined，否则为错误对象。
...
回调函数。当音频池unload方法回调成功，err为undefined，否则为错误对象。
...
回调函数。当音频池release方法回调成功，err为undefined，否则为错误对象。
...
支持的事件：'loadComplete'，对应的ID加载完成会触发此回调。
...
[h2]on('playFinishedWithStreamId')
18+
...
事件回调或者
on('playFinishedWithStreamId')
事件回调时，当音频播放完成的时候，都会触发注册的回调。

当同时注册
on('playFinished')
事件回调和
on('playFinishedWithStreamId')
事件回调时，当音频播放完成的时候，仅会触发'playFinishedWithStreamId'事件回调，不会触发'playFinished'事件回调。
...
支持的事件：'playFinishedWithStreamId'，音频流播放完成会触发此回调，并返回播放完成的音频的streamId。
...
[h2]off('playFinishedWithStreamId')
18+
...
取消注册的事件：'playFinishedWithStreamId'。
...
支持的事件：'playFinished'，音频流播放完成会触发此回调。
...
监听
SoundPool
的错误事件，该事件仅用于错误提示。使用callback异步回调。
...
监听
SoundPool
的错误事件，并返回包含错误码、错误发生阶段、资源ID和音频流ID的
ErrorInfo
。使用callback异步回调。
...
事件回调类型，支持的事件为'errorOccurred'，当用户或系统操作导致错误，触发该事件。
...
回调函数，返回错误事件回调方法。在使用播放器的过程中发生错误时，提供错误信息
ErrorInfo
。
...
错误事件回调方法。在使用播放器的过程中发生错误时，提供错误信息
ErrorInfo
，不设置callback时不提供相关信息。

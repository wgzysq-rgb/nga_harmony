...
**DocID**: `da581b2e9bbc40888dd7edec10a35fc7`

**NodeID**: `000201775795910761295bdf4edd75a2`
...
[h2]OH_LowPowerVideoSink_CreateByMime()
...
如果创建成功返回指向OH_LowPowerVideoSink实例的指针，否则返回空指针。

[h2]OH_LowPowerVideoSink_Configure()
...
指向OH_LowPowerVideoSink实例的指针。

const 
OH_AVFormat
* format

指向OH_AVFormat的指针，用于配置LowPowerVideoSink的参数。
...
[h2]OH_LowPowerVideoSink_SetParameter()
...
[h2]OH_LowPowerVideoSink_GetParameter()
...
指向OH_AVFormat的指针，为LowPowerVideoSink设置的参数。
...
[h2]OH_LowPowerVideoSink_SetVideoSurface()
...
[h2]OH_LowPowerVideoSink_Prepare()
...
[h2]OH_LowPowerVideoSink_StartDecoder()
...
[h2]OH_LowPowerVideoSink_RenderFirstFrame()
...
[h2]OH_LowPowerVideoSink_StartRenderer()
...
[h2]OH_LowPowerVideoSink_Resume()
...
[h2]OH_LowPowerVideoSink_Destroy()
...
[h2]OH_LowPowerVideoSink_SetSyncAudioSink()
...
[h2]OH_LowPowerVideoSink_SetTargetStartFrame()
...
OH_LowPowerVideoSink_OnTargetArrived方法，当目标帧渲染时触发该方法。

const int64_t timeoutMs

如果等待第一帧的时间超过timeoutMs，则直接调用onTargetArrived。
...
[h2]OH_LowPowerVideoSink_SetPlaybackSpeed()
...
[h2]OH_LowPowerVideoSink_ReturnSamples()
...
需要送LowPowerVideoSink消费的OH_AVSamplesBuffer，支持聚包输入。
...
[h2]OH_LowPowerVideoSink_GetLatestPts()
...
[h2]OH_LowPowerVideoSink_RegisterCallback()
...
指向OH_LowPowerVideoSinkCallback实例的指针。
...
[h2]OH_LowPowerVideoSinkCallback_Create()
...
返回指向OH_LowPowerVideoSinkCallback实例的指针。如果内存不足，则返回nullptr。

[h2]OH_LowPowerVideoSinkCallback_Destroy()
...
[h2]OH_LowPowerVideoSinkCallback_SetDataNeededListener()
...
OH_LowPowerVideoSink_OnDataNeeded方法，在DataNeeded事件触发时调用。
...
[h2]OH_LowPowerVideoSinkCallback_SetErrorListener()
...
OH_LowPowerVideoSink_OnError方法，在Error事件触发时调用。
...
[h2]OH_LowPowerVideoSinkCallback_SetRenderStartListener()
...
OH_LowPowerVideoSink_OnRenderStarted方法，在RenderStarted事件触发时调用。
...
[h2]OH_LowPowerVideoSinkCallback_SetStreamChangedListener()
...
OH_LowPowerVideoSink_OnStreamChanged方法，在StreamChanged事件触发时调用。
...
[h2]OH_LowPowerVideoSinkCallback_SetFirstFrameDecodedListener()
...
OH_LowPowerVideoSink_OnFirstFrameReady方法，在FirstFrameReady事件触发时调用。
...
[h2]OH_LowPowerVideoSinkCallback_SetEosListener()
...
OH_LowPowerVideoSink_OnEos方法，在Eos事件触发时调用。

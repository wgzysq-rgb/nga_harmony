...
**DocID**: `59515a195f5a48599e4f69901856cee1`

**NodeID**: `000201775795910761309500038fa74e`
...
LowPowerVideoSink需要数据时调用该方法，包含在
OH_LowPowerVideoSinkCallback
中。
...
LowPowerVideoSink到达目标点时调用该方法，包含在
OH_LowPowerVideoSinkCallback
中。
...
LowPowerVideoSink开始渲染时调用该方法，包含在
OH_LowPowerVideoSinkCallback
中。
...
LowPowerVideoSink第一帧解码成功时调用该方法，包含在
OH_LowPowerVideoSinkCallback
中。
...
LowPowerVideoSink播放完成时调用该方法，包含在
OH_LowPowerVideoSinkCallback
中。

函数说明

[h2]OH_LowPowerVideoSink_OnDataNeeded()
...
指向OH_AVSamplesBuffer实例的指针。

void *userData

用户执行回调所依赖的数据。

[h2]OH_LowPowerVideoSink_OnError()
...
业务操作过程中发生错误时返回的错误描述信息。

void* userData

用户执行回调所依赖的数据。

[h2]OH_LowPowerVideoSink_OnTargetArrived()
...
表示等待目标点是否超时。若为true，表示等待目标点超时；若为false，则表示未超时。

void* userData

用户执行回调所依赖的数据。

[h2]OH_LowPowerVideoSink_OnRenderStarted()
...
[h2]OH_LowPowerVideoSink_OnStreamChanged()
...
[h2]OH_LowPowerVideoSink_OnFirstFrameDecoded()

...
**DocID**: `6abe6a4f67494240a92e6c78f72b4e2e`

**NodeID**: `000201775795910761223afba5ba4a16`
...
当isMicrophone为true时，则打开麦克风，通过调用
OH_AVScreenCapture_StartScreenCapture
和
...
给指定的OH_AVScreenCapture实例设置捕获策略。
...
设置Picker显示模式。定义picker中显示的内容类型，模式更改会在下一次调用
...
[h2]OH_AVScreenCapture_StartScreenCapture()
...
[h2]OH_AVScreenCapture_StopScreenCapture()
...
[h2]OH_AVScreenCapture_StartScreenRecording()
...
[h2]OH_AVScreenCapture_StopScreenRecording()
...
[h2]OH_AVScreenCapture_AcquireAudioBuffer()
...
保存音频buffer的结构体，通过该结构体获取到音频buffer以及buffer的时间戳等信息。

OH_AudioCaptureSourceType
 type

音频buffer的类型，区分是麦克风录制的外部流还是系统内部播放音频的内录流。
...
[h2]OH_AVScreenCapture_AcquireVideoBuffer()
...
[h2]OH_AVScreenCapture_ReleaseAudioBuffer()
...
音频buffer的类型，区分麦克风录制的外部流还是系统内部播放音频的内录流。
...
[h2]OH_AVScreenCapture_ReleaseVideoBuffer()
...
[h2]OH_AVScreenCapture_SetCallback()
...
OH_AVScreenCaptureCallback的结构体，保存相关回调函数指针。
...
[h2]OH_AVScreenCapture_SetMicrophoneEnabled()
...
[h2]OH_AVScreenCapture_SetStateCallback()
...
.OH_SCREEN_CAPTURE_STATE_STARTED状态，告知应用启动录屏成功，并在屏幕显示录屏通知。如果启动录屏失败，则通过该状态处理回调方法上报失败状态信息（如，若麦克风不可用则上报
OH_AVScreenCaptureStateCode
.OH_SCREEN_CAPTURE_STATE_MIC_UNAVAILABLE状态），或通过错误处理回调方法
...
指向应用提供的自定义数据的指针，在状态处理回调方法被调用时作为入参回传。
...
[h2]OH_AVScreenCapture_SetDataCallback()
...
指向应用提供的自定义数据的指针，在数据处理回调方法被调用时作为入参回传。
...
[h2]OH_AVScreenCapture_SetErrorCallback()
...
调用该设置方法成功后，当OH_AVScreenCapture实例发生错误时，将不再调用通过
OH_AVScreenCapture_SetCallback
设置的错误处理回调方法
...
指向应用提供的自定义数据的指针，在错误处理回调方法被调用时作为入参回传。
...
[h2]OH_AVScreenCapture_StartScreenCaptureWithSurface()
...
[h2]OH_AVScreenCapture_SetCanvasRotation()
...
[h2]OH_AVScreenCapture_CreateContentFilter()
...
执行成功返回OH_AVScreenCapture_ContentFilter实例，否则返回空指针。

[h2]OH_AVScreenCapture_ReleaseContentFilter()
...
struct OH_AVScreenCapture_ContentFilter
 *filter

指向OH_AVScreenCapture_ContentFilter实例的指针。
...
[h2]OH_AVScreenCapture_ContentFilter_AddAudioContent()
...
OH_AVScreenCaptureFilterableAudioContent实例。
...
[h2]OH_AVScreenCapture_ExcludeContent()
...
[h2]OH_AVScreenCapture_ContentFilter_AddWindowContent()
...
执行成功返回AV_SCREEN_CAPTURE_ERR_OK，否则返回具体错误码。

[h2]OH_AVScreenCapture_ResizeCanvas()
...
该接口目前仅支持录屏取码流的场景，不支持录屏存文件的场景。并且调用该���口的调用者以及视频数据的消费者需要确保自身能够支持收到的视频数据分辨率发生变化。
...
[h2]OH_AVScreenCapture_SkipPrivacyMode()
...
[h2]OH_AVScreenCapture_SetMaxVideoFrameRate()
...
[h2]OH_AVScreenCapture_ShowCursor()
...
[h2]OH_AVScreenCapture_SetDisplayCallback()
...
[h2]OH_AVScreenCapture_CreateCaptureStrategy()
...
执行成功返回OH_AVScreenCapture_CaptureStrategy实例，否则返回空指针。

[h2]OH_AVScreenCapture_ReleaseCaptureStrategy()
...
[h2]OH_AVScreenCapture_SetCaptureStrategy()
...
[h2]OH_AVScreenCapture_StrategyForCanvasFollowRotation()
...
指向OH_AVScreenCapture_CaptureStrategy实例的指针。
...
[h2]OH_AVScreenCapture_StrategyForKeepCaptureDuringCall()
...
指向OH_AVScreenCapture_CaptureStrategy对象的指针。
...
[h2]OH_AVScreenCapture_SetCaptureContentChangedCallback()
...
AV_SCREEN_CAPTURE_ERR_OPERATE_NOT_PERMIT：不允许操作，设置��屏内容回调失败。

[h2]OH_AVScreenCapture_SetCaptureArea()
...
[h2]OH_AVScreenCapture_StrategyForPrivacyMaskMode()
...
[h2]OH_AVScreenCapture_SetSelectionCallback()
...
用户在界面确认后，通过该函数通知应用进行逻辑处理。

void *userData

应用传入的控制块指针，在返回时携带给应用。
...
[h2]OH_AVScreenCapture_GetCaptureTypeSelected()
...
指向OH_AVScreenCapture_UserSelectionInfo实例的指针。

int32_t* type

用户选择的捕获对象类型，0：代表屏幕，1：代表窗口。
...
[h2]OH_AVScreenCapture_GetDisplayIdSelected()
...
[h2]OH_AVScreenCapture_StrategyForBFramesEncoding()
...
[h2]OH_AVScreenCapture_StrategyForPickerPopUp()
...
设置为false，代表APP已经指定了录屏内容，录屏启动后无需弹出Picker；设置为true代表录屏启动后统一弹出Picker；不设置，代表使用系统推荐行为。
...
[h2]OH_AVScreenCapture_StrategyForFillMode()
...
[h2]OH_AVScreenCapture_PresentPicker()
...
[h2]OH_AVScreenCapture_SetCaptureAreaHighlight()
...
[h2]OH_AVScreenCapture_SetPickerMode()
...
Picker显示模式（请参阅OH_CapturePickerMode枚举）。
...
[h2]OH_AVScreenCapture_ExcludePickerWindows()
...
const int32_t *excludedWindowIDs

...
**DocID**: `81eaa74027e34ba0a391d88b467a4975`

**NodeID**: `000201775795910761322d8130fc6ca4`
...
视频元数据生成的风格模式。参数的具体取值请参考
VIDEO_METADATA_GENERATOR_STYLE_CONTROL
。
...
成功调用之后状态会变为
VideoProcessing_State
.VIDEO_PROCESSING_STATE_RUNNING。调用
OH_VideoProcessing_Stop
，所有的缓存buffer处理完成后，状态会变为
VideoProcessing_State
...
每个新输出buffer填充好数据之后该buffer的索引就会报告给用户。调用
...
const char * VIDEO_METADATA_GENERATOR_STYLE_CONTROL

指定视频元数据生成的风格模式。具体取值请参考
VideoMetadataGeneratorStyleControl
。调用
OH_AVFormat_SetIntValue
设置视频元数据生成的风格模式到AVFormat参数。调用
OH_VideoProcessing_SetParameter
设置当前视频元数据生成的风格模式。调用
OH_VideoProcessing_GetParameter
获取当前视频元数据生成的风格模式。
...
[h2]VideoDetailEnhancer_QualityLevel
...
不支持的处理类型，可以调用OH_VideoProcessing_IsXXXSupported来检查是否支持这种处理。
...
[h2]OH_VideoProcessingCallback_OnError()
...
[h2]OH_VideoProcessingCallback_OnState()
...
[h2]OH_VideoProcessingCallback_OnNewOutputBuffer()

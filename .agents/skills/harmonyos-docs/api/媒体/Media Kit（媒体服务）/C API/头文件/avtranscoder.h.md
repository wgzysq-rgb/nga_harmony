...
**DocID**: `2d455d934c5c4566bc433dc8442349f6`

**NodeID**: `000201775795910761182555876882d5`
...
如果AVTranscoder上报error事件，开发者需要通过
...
[h2]OH_AVTranscoderConfig_Create()
...
如果创建成功返回指向OH_AVTranscoder_Config实例的指针，否则返回空指针。

[h2]OH_AVTranscoderConfig_Release()
...
指向OH_AVTranscoder_Config实例的指针。传入的config指针必须为
...
[h2]OH_AVTranscoderConfig_SetSrcFD()
...
[h2]OH_AVTranscoderConfig_SetDstFD()
...
[h2]OH_AVTranscoderConfig_SetDstVideoType()
...
[h2]OH_AVTranscoderConfig_SetDstAudioType()
...
[h2]OH_AVTranscoderConfig_SetDstFileType()
...
[h2]OH_AVTranscoderConfig_SetDstAudioBitrate()
...
[h2]OH_AVTranscoderConfig_SetDstVideoBitrate()
...
[h2]OH_AVTranscoderConfig_SetDstVideoResolution()
...
输出视频帧的宽，支持范围[240-3840]，默认设置为源视频帧的宽。

int32_t height

输出视频帧的高，支持范围[240-2160]，默认设置为源视频帧的高。
...
[h2]OH_AVTranscoderConfig_EnableBFrame()
...
是否使能B帧编码。true表示使能B帧编码，false表示不使能B帧编码，默认为false。
...
如果创建成功返回指向OH_AVTranscoder实例的指针，否则返回空指针。

[h2]OH_AVTranscoder_Prepare()
...
指向OH_AVTranscoder实例的指针。传入的transcoder指针必须为
...
AV_ERR_OK：成功设置视频转码的参数设置，进入AVTRANSCODER_PREPARED状态。
...
AV_ERR_OK：成功开始转码，进入AVTRANSCODER_STARTED状态。
...
AV_ERR_OK：成功暂停转码，进入AVTRANSCODER_PAUSED状态。
...
AV_ERR_OK：成功恢复转码，进入AVTRANSCODER_STARTED状态。
...
AV_ERR_OK：成功取消转码，进入AVTRANSCODER_CANCELLED状态。
...
指向OH_AVTranscoder实例的指针。传入的transcoder指针必须为OH_AVTranscoder_Create创建的实例。
...
[h2]OH_AVTranscoder_SetStateCallback()
...
[h2]OH_AVTranscoder_SetErrorCallback()
...
[h2]OH_AVTranscoder_SetProgressUpdateCallback()

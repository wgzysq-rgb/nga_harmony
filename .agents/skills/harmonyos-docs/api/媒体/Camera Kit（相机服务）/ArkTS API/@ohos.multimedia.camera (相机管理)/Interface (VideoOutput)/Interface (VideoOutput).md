...
**Slug**: `arkts-apis-camera-videooutput`

**DocID**: `6aa2431850224068b07ae4637730032c`

**NodeID**: `00020177579591076136734a3b803678`
...
回调函数。当启动录制成功，err为undefined，否则为错误对象。错误码类型
CameraErrorCode
。
...
回调函数。当结束录制成功，err为undefined，否则为错误对象。
...
监听事件，固定为'frameStart'，videoOutput创建成功后可监听。底层第一次曝光时触发该事件并返回。
...
监听事件，固定为'frameStart'，videoOutput创建成功后可监听。
...
监听事件，固定为'frameEnd'，videoOutput创建成功后可监听。录像完全结束最后一帧时触发该事件并返回。
...
监听事件，固定为'frameEnd'，videoOutput创建成功后可监听。
...
监听事件，固定为'error'，videoOutput创建成功后可监听。录像接口调用出现错误时触发该事件并返回对应错误码，比如调用
start
，
...
接口调用前，先调用
getActiveFrameRate
接口查询当前VideoSession的帧率，若下发的帧率与当前帧率相等，则下发的帧率不会生效。
...
最小帧率，单位：fps。当传入的最大值小于最小值时，传参异常，接口不生效。
...
最大帧率，单位：fps。当传入的最小值大于最大值时，传参异常，接口不生效。
...
返回是否支持镜像录像，true表示支持，false表示不支持。若接口调用失败，返回undefined。
...
启用/关闭镜像录像。true为开启镜像录像，false为关闭镜像录像。

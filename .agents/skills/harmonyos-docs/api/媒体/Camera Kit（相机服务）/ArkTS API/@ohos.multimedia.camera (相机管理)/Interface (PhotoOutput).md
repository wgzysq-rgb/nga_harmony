...
**Slug**: `arkts-apis-camera-photooutput`

**DocID**: `4240f74f10e346c4b985d258d002947d`

**NodeID**: `0002017757959107613362c2082c971b`
...
回调函数。当以默认设置触发拍照成功，err为undefined，否则为错误对象。错误码类型
CameraErrorCode
。
...
拍照设置，传入undefined类型数据按默认设置触发一次拍照处理。
...
回调函数，用于获取结果。接口调用失败会返回相应错误码，错误码类型
CameraErrorCode
。
...
监听事件，固定为'photoAvailable'，photoOutput创建成功后可监听。
...
监听事件，固定为'captureStartWithInfo'，photoOutput创建成功后可监听。
...
返回是否支持动态照片拍照。true表示支持，false表示不支持。若接口调用失败，返回undefined。
...
使能动态照片拍照。true为开启动态照片，false为关闭动态照片。
...
监听事件，固定为'photoAssetAvailable'，photoOutput创建成功后可监听。
...
需要解监听的回调方法。如果callback不为空且与此对应的监听方法一致，不为匿名方法，则解注册该方法；如果callback为空，则解监听所有回调。
...
返回是否支持镜像拍照，true表示支持，false表示不支持。若接口调用失败，返回undefined。
...
是否启用动态照片镜像拍照。true为开启动态照片镜像拍照，false为关闭动态照片镜像拍照。
...
支持的动态照片短视频编码类型列表。若接口调用失败，返回undefined。
...
监听事件，固定为'frameShutter'，photoOutput创建成功后可监听。
...
回调函数，用于获取相关信息。该回调返回意味着可以再次下发拍照请求。
...
监听事件，固定为'captureEnd'。photoOutput创建成功后可监听。拍照完全结束可触发该事件发生并返回相应信息。
...
监听事件，固定为'captureEnd'，photoOutput创建成功后可监听。
...
监听事件，固定为'frameShutterEnd'，photoOutput创建成功后可监听。
...
监听事件，固定为'captureReady'，photoOutput创建成功后可监听。当下一张可拍时可触发该事件发生并返回相应信息。
...
监听事件，固定为'captureReady'，photoOutput创建成功后可监听。
...
监听事件，固定为'estimatedCaptureDuration'，photoOutput创建成功后可监听。拍照完全结束可触发该事件发生并返回相应信息。
...
回调函数，用于获取预估的单次拍照底层出sensor采集帧时间，单位：毫秒。如果上报-1，代表没有预估时间。
...
监听事件，固定为'estimatedCaptureDuration'，photoOutput创建成功后可监听。
...
监听事件，固定为'error'，photoOutput创建成功后可监听。拍照接口调用时出现错误触发该事件并返回错误信息。
...
监听事件，固定为'error'，photoOutput创建成功后可监听。
...
从 API version 10开始支持，从API version 11开始废弃。建议使用
on('captureStartWithInfo')
替代。
...
监听事件，固定为'captureStart'，photoOutput创建成功后可监听。每次拍照，底层开始曝光时触发该事件并返回。
...
监听事件，固定为'captureStart'，photoOutput创建成功后可监听。
...
是否支持指定的拍照画质优先策略。true表示支持，false表示不支持。
...
Camera service fatal error, reconfiguring streams is needed to recover from failure.

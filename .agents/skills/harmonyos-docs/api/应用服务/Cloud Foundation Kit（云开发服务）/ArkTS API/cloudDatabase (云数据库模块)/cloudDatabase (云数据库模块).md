...
**Slug**: `cloudfoundation-clouddatabase`

**DocID**: `8e6b1fe354c14540acd9bf85066252c2`

**NodeID**: `000201775795946267173696758dce30`
...
回调函数。当查询数据成功，err为undefined，data为查询到的结果数组；否则为错误对象。
...
表示值类型为布尔类型，可取true或者false。

Uint8Array

表示值类型为8位无符号整型数组，可取任意值。

Date

表示值类型为日期，值固定格式为“YYYY-MM-DD”。
...
使用orderByRandom()对数据进行排序时，建议与
limit()
配合使用。否则，当该对象类型的数据记录数量过多时，可能会影响查询效率，导致查询超时或失败。

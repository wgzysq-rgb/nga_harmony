...
**Slug**: `js-apis-geolocationmanager`

**DocID**: `d56b1015671f41198527f34886848a86`

**NodeID**: `000201775795946267037e469f69eb3c`
...
该参数生效的情况下，系统会对比GNSS或网络定位服务上报的位置信息与应用的位置信息申请。当位置信息
Location
中的精度值（accuracy）小于等于应用要求的精度值（maxAccuracy）时，位置信息会返回给应用；否则系统将丢弃本次收到的位置信息。
...
表示超时时间，单位是毫秒，最小���1000毫秒。默认值是5000。取值范围为大于等于1000。
...
表示应用在后台时位置权限校验失败导致持续定位失败。APP在后台定位时的位置权限申请方式参见
申请位置权限开发指导
。
...
Capability not supported. Failed to call ${geoLocationManager.on('locationChange')} due to limited device capabilities.
...
需要取消订阅的回调函数。该回调函数需要与on接口传入的回调函数保持一致。若无此参数，则取消当前类型的所有订阅。
...
Capability not supported. Failed to call ${geoLocationManager.off('locationChange')} due to limited device capabilities.
...
geoLocationManager.on('locationError')
12+
...
设置事件类型。type为“locationError”，表示持续定位过程中的错误码变化。
...
Capability not supported. Failed to call ${geoLocationManager.on('locationError')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.off('locationError')} due to limited device capabilities.
...
回调函数。返回true表示位置信息开关已经开启；返回false表示位置信息开关已经关闭。
...
Capability not supported. Failed to call ${geoLocationManager.on('locationEnabledChange')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.off('locationEnabledChange')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.on('cachedGnssLocationsChange')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.off('cachedGnssLocationsChange')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.on('satelliteStatusChange')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.off('satelliteStatusChange')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.on('nmeaMessage')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.off('nmeaMessage')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.on('gnssFenceStatusChange')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.off('gnssFenceStatusChange')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.on('countryCodeChange')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.off('countryCodeChange')} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.getCurrentLocation} due to limited device capabilities.
...
SingleLocationRequest为API12新增参数。若无此参数设置，则使用CurrentLocationRequest为默认值。
...
Capability not supported. Failed to call ${geoLocationManager.getLastLocation} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.isLocationEnabled} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.getAddressesFromLocation} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.getAddressesFromLocationName} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.isGeocoderAvailable} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.getCachedGnssLocationsSize} due to limited device capabilities.
...
回调函数。当操作成功，err为undefined，否则为错误对象。
...
Capability not supported. Failed to call ${geoLocationManager.flushCachedGnssLocations} due to limited device capabilities.
...
回调函数。当命令发送成功，err为undefined，否则为错误对象。
...
Capability not supported. Failed to call ${geoLocationManager.sendCommand} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.getCountryCode} due to limited device capabilities.
...
geoLocationManager.addGnssGeofence
12+
...
APP可以在入参
GnssGeofenceRequest
中传入回调函数用于接收地理围栏事件；也可以传入通知对象
NotificationRequest
，在系统识别到地理围栏事件发生时会弹出APP创建的通知。
...
Capability not supported. Failed to call ${geoLocationManager.addGnssGeofence} due to limited device capabilities.
...
geoLocationManager.removeGnssGeofence
12+
...
Capability not supported. Failed to call ${geoLocationManager.removeGnssGeofence} due to limited device capabilities.
...
Capability not supported. Failed to call ${geoLocationManager.getGeofenceSupportedCoordTypes} due to limited device capabilities.
...
geoLocationManager.getCurrentWifiBssidForLocating
14+
...
Capability not supported. Failed to call ${geoLocationManager.getCurrentWifiBssidForLocating()} due to limited device capabilities.
...
Failed to obtain the BSSID of the Wi-Fi hotspot. The Wi-Fi network is not connected.
...
geoLocationManager.on('bluetoothScanResultChange')
16+
...
本API需要开发者向
locationkit@huawei.com
发送邮件申请，请在邮件中写明使用该API的业务场景和获取蓝牙deviceId（MAC）的真实用途，申请审批通过后会方可使用。
...
需要权限
：ohos.permission.APPROXIMATELY_LOCATION 和 ohos.permission.LOCATION
...
设置事件类型。type为“bluetoothScanResultChange”，表示订阅蓝牙扫描信息上报事件。
...
Capability not supported. Failed to call ${geoLocationManager.on('bluetoothScanResultChange')} due to limited device capabilities.
...
设置事件类型。type为“bluetoothScanResultChange”，表示停止订阅蓝牙扫描信息上报事件。
...
Capability not supported. Failed to call ${geoLocationManager.off('bluetoothScanResultChange')} due to limited device capabilities.
...
geoLocationManager.isPoiServiceSupported
20+
...
Capability not supported. Failed to call ${geoLocationManager.getPoiInfo} due to limited device capabilities.
...
geoLocationManager.getDistanceBetweenLocations
20+
...
geoLocationManager.addBeaconFence
20+
...
应用可以在入参
BeaconFenceRequest
中传入回调函数用于接收围栏事件；也可以传入
FenceExtensionAbility
名称，在系统识别到围栏事件发生时通知应用。
...
Capability not supported. Failed to call ${geoLocationManager.addBeaconFence} due to limited device capabilities.
...
geoLocationManager.removeBeaconFence
20+
...
传入beaconFence参数，删除指定围栏；不传入参数，删除该应用所有围栏。
...
Capability not supported. Failed to call ${geoLocationManager.removeBeaconFence} due to limited device capabilities.
...
geoLocationManager.isBeaconFenceSupported
20+
...
geoLocationManager.isWlanBssidMatched
21+
...
请求匹配的BSSID列表。单个字符串的长度不超过64，数组的长度不超过1000。
...
RSSI阈值。只匹配RSSI大于此阈值的BSSID，取值范围为-10000至10000（单位：dBm）。
...
是否需要发起WLAN扫描。需要发起WLAN扫描设置为true。不需要发起WLAN扫描，使用最近一次WLAN扫描结果进行匹配设置为false。
...
表示匹配是否成功。当扫描结果中存在wlanBssidArray中的任意BSSID，且其RSSI值高于rssiThreshold时，返回true，否则返回false。
...
Capability not supported. Failed to call ${geoLocationManager.isWlanBssidMatched} due to limited device capabilities.

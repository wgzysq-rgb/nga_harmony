...
**Slug**: `map-map-mappolyline`

**DocID**: `270eee289cc54213b3717f3ec7b7f84d`

**NodeID**: `000201775795946267299131a9e6f225`
...
折线顶点的集合。默认情况下，折线不闭合；要形成闭合的折线，起点和终点必须相同，异常值不处理。
...
折线纹理。建议纹理使用没有背景色（透明色）的图片，异常值不处理。
...
是否使用贴图模式进行纹理绘制，异常值不处理。如果设置为false，建议纹理没有背景色（使用透明色）。
...
每个坐标对应的纹理索引。数组长度需要和points的数量保持一致，异常值不处理。

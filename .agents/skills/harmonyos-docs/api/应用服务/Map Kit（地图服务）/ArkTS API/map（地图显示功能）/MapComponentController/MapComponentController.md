...
**Slug**: `map-map-mapcomponentcontroller`

**DocID**: `c279df7695a64c4e971012b0d088a99f`

**NodeID**: `00020177579594626727158b3a0b3100`
...
在指定的持续时间内以动画的形式更新相机状态，并更新指定的marker。使用Promise异步回调。相机移动过程中不能被打断，否则
AnimateResult
的参数isCanceled返回值为true。
...
来约束相机目标，使用户移动地图时，相机目标不会移出此边界。当设置新的边界时，新边界将覆盖之前设置的边界。
...
“我的位置”
图层功能开关，默认使用系统的连续定位能力显示用户位置。开关打开后，“我的位置”按钮默认显示在地图的右下角。点击“我的位置”按钮，将会在屏幕中心显示当前定位，以蓝色圆点的形式呈现。
...
Map Kit默认使用系统的连续定位能力显示用户位置，如果您希望定制显示频率或者精准度，可以调用此接口设置
“我的位置”
坐标。
...
如果displayType使用FOLLOW_ROTATE需应用申请传感器权限：ohos.permission.ACCELEROMETER，具体可参考
声明权限
。
...
单指双击可将缩放级别提高1（放大）层级，用双指单击可将缩放级别降低1（缩小）层级。
...
设���地图和边界的距离来定义地图的可见区域。地图图层元素将适应填充，例如，缩放控件、指南针等将被移动到适应定义的区域，相机将相对于可见区域的中心移动。
...
地图和边界的距离，默认值为{ left: 0, right: 0, top: 0, bottom: 0 }。
...
mapCommon.CustomMapStyleOptions
...
设置指南针位置，即指南针左上角相对地图组件左上角的偏移量，单位：px。
...
Logo缩放比例，取值范围：[0.8, 1]。异常值返回错误码401。
...
中
mapCommon.MvtSource
参数的tileUrl方式），需要申请访问网络的权限：ohos.permission.INTERNET。
...
建议使用
MapEventManager.on(type: 'cameraChange')
。
...
回调函数，返回
mapCommon.LatLng
，监听地图相机状态变化事件。该回调不会在动画过程中触发，而是在动画结束时触发。
...
建议使用
MapEventManager.off(type: 'cameraChange')
。
...
建议使用
MapEventManager.on(type: 'cameraIdle')
。
...
建议使用
MapEventManager.off(type: 'cameraIdle')
。
...
建议使用
MapEventManager.on(type: 'cameraMoveCancel')
。
...
建议使用
MapEventManager.off(type: 'cameraMoveCancel')
。
...
建议使用
MapEventManager.on(type: 'cameraMove')
。
...
建议使用
MapEventManager.off(type: 'cameraMove')
。
...
建议使用
MapEventManager.on(type: 'cameraMoveStart')
。
...
建议使用
MapEventManager.off(type: 'cameraMoveStart')
。
...
建议使用
MapEventManager.on(type: 'mapClick')
。
...
建议使用
MapEventManager.off(type: 'mapClick')
。
...
建议使用
MapEventManager.on(type: 'mapLoad')
。
...
建议使用
MapEventManager.off(type: 'mapLoad')
。
...
建议使用
MapEventManager.on(type: 'mapLongClick')
。
...
建议使用
MapEventManager.off(type: 'mapLongClick')
。
...
监听
“我的位置”
按钮点击事件。使用callback异步回调。

建议使用
MapEventManager.on(type: 'myLocationButtonClick')
。
...
建议使用
MapEventManager.off(type: 'myLocationButtonClick')
。
...
建议使用
MapEventManager.on(type: 'myLocationClick')
。
...
建议使用
MapEventManager.off(type: 'myLocationClick')
。
...
建议使用
MapEventManager.on(type: 'poiClick')
。
...
建议使用
MapEventManager.off(type: 'poiClick')
。
...
建议使用
MapEventManager.on(type: 'markerClick')
。
...
建议使用
MapEventManager.off(type: 'markerClick')
。
...
建议使用
MapEventManager.on(type: 'markerDragStart')
。
...
建议使用
MapEventManager.off(type: 'markerDragStart')
。
...
建议使用
MapEventManager.on(type: 'markerDrag')
。
...
建议使用
MapEventManager.off(type: 'markerDrag')
。
...
建议使用
MapEventManager.on(type: 'markerDragEnd')
。
...
建议使用
MapEventManager.off(type: 'markerDragEnd')
。
...
建议使用
MapEventManager.on(type: 'circleClick')
。
...
建议使用
MapEventManager.off(type: 'circleClick')
。
...
建议使用
MapEventManager.on(type: 'polylineClick')
。
...
建议使用
MapEventManager.off(type: 'polylineClick')
。
...
建议使用
MapEventManager.on(type: 'polygonClick')
。
...
建议使用
MapEventManager.off(type: 'polygonClick')
。
...
建议使用
MapEventManager.on(type: 'infoWindowClick')
。
...
建议使用
MapEventManager.off(type: 'infoWindowClick')
。
...
建议使用
MapEventManager.on(type: 'infoWindowClose')
。
...
建议使用
MapEventManager.off(type: 'infoWindowClose')
。
...
建议使用
MapEventManager.on(type: 'pointAnnotationClick')
。
...
'pointAnnotationClick'：监听点注释点击事件。
...
建议使用
MapEventManager.off(type: 'pointAnnotationClick')
。
...
建议使用
MapEventManager.on(type: 'bubbleClick')
。
...
建议使用
MapEventManager.off(type: 'bubbleClick')
。
...
建议使用
MapEventManager.on(type: 'imageOverlayClick')
。
...
建议使用
MapEventManager.off(type: 'imageOverlayClick')
。
...
建议使用
MapEventManager.off(type: 'error')
。
...
元服务API：
从版本5.1.1(19)开始，该接口支持在元服务中使用。

系统能力：
SystemCapability.Map.Core

设备行为差异：
在API19及之后版本该接口在phone、tablet、2in1均可正常使用，在其他设备中返回801错误。
...
设置楼层调节控件的位置。需要先调用
setIndoorMapEnabled
方法来开启室内图功能，仅17级及以上地图层级可见室内图和楼层调节控件。
...
设备行为差异：
在API20及之后版本该接口在phone、tablet、2in1均可正常使用，在其他设备中返回801错误。

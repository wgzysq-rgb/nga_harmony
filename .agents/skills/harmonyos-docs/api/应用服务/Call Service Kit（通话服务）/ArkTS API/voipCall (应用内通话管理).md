...
**DocID**: `ec200bd2da1f4538bcf8458c4712a399`

**NodeID**: `00020177579594626707558b79061e46`
...
获取用户点击事件需使用
voipCall.on
在业务流程开始时提前订阅voipCallUiEvent事件，可于业务流程结束后使用
voipCall.off
结束订阅。
...
通话无效，比如传入的callId未通过校验等。

USER_ANSWER_CELLULAR_FIRST
...
用户头像图片，需导入图像处理模块，详情见
PixelMap
。

支持传入的最大图片大小为 221 x 221 像素，推荐传入的图片大小为 112 x 112 像素。通过 
PixelMap
的
getPixelBytesNumber
接口获取到的图片大小要小于196608。
...
isDialingAllowedDuringCarrierCall
...
AUDIO_EVENT_SPEAKER_ON

2

开启扬声器。

AUDIO_EVENT_SPEAKER_OFF
...
`Succeeded in reading callback. CallId: 
...
需要取消监听的回调函数，返回通话事件详细信息对象。若不填，则取消当前应用监听该事件的所有回调函数。
...
通知来电消息，如果应用来电消息建立失败，需调用
reportIncomingCallError
通知来电建立失败。需设置通话详细信息，见
VoipCallAttribute
。使用Promise异步回调。
...
应用内通话详细信息如用户头像、用户昵称、通话唯一标识等，详情请参见
VoipCallAttribute
...
'Get voip message successfully: %{public}s'
...
`DemoPushMessageAbility imageInfo: 
...
'Failed to report incoming call: %{public}d'
...
'Failed to report outgoing call: %{public}d'
...
`Succeeded in reporting call audio event change.`
...
该接口不能改变通话类型，例如，语音通话不能升级为视频通话，视频通话也不能降级为语音通话，如需上述升降级操作，请调用
voipCall.reportCallStateChange
。
...
`Succeeded in reporting call state change.`
...
`Succeeded in reporting incoming call error.`

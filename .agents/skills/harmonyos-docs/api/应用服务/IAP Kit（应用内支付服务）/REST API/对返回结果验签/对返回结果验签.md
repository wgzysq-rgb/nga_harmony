...
**DocID**: `64a539db8e8c436980ed12a361507f6c`

**NodeID**: `0002017757959462671540e1cdcfd834`
...
X.509 证书链，顺序为叶子证书、中间证书、根证书。

JWS解码和验签

解析JWS，获取Header、 Payload和Signature。

从header的x5c中获取证书链（依次是叶子证书、中间证书、根证书），使用
Huawei CBG Root CA G2
证书对证书链进行验证。

校验叶子证书的OID：1.3.6.1.4.1.2011.2.415.1.1。

证书校验通过则从叶子证书获取到PublicKey。

通过Header指定的算法和PublicKey进行JWT验签，可以使用各种开源库来验签，具体请参见
jwt.io
。
...
CA_CERT_FILE_PATH 
= 
"/path/to/cer/RootCaG2Ecdsa.cer"
;
// 

TODO: Need to replace it with the actual value, please refer to 
...
(!
HEADER_PARAM_ALG_ES256
.equals(decodedJWT.getAlgorithm())) {
...
"alg must be ES256"
 ...
 String[] x5cChain = decodedJWT.getHeaderClaim(
HEADER_PARAM_X5C
).asArray(String.
class
);
...
return new 
String(Base64.getUrlDecoder().decode(decodedJWT.getPayload()), StandardCharsets.UTF_8);
 }

private static 
PublicKey 
verifyChainAndGetPubKey
(String[] certificates) 
throws 
Exception {
...
"PKIX"
 ...
 CertPath certPath = certificateFactory.generateCertPath(certificateList.subList(
0
, 
X5C_CHAIN_LENGTH 
- 
...
Exception(e);
 ...
 Certificate iapCert = certificateList.get(
0
);
...
throw new 
Exception(
"leaf certificate must be X509 format"
);
 }
...
null 
||
 !x509Certificate.getNonCriticalExtensionOIDs().contains(
LEAF_CERT_OID
)) {
...
return 
certPathValidatorResult.getPublicKey();
 }
...
try 
(InputStream fis = Files.newInputStream(Paths.get(
CA_CERT_FILE_PATH
))) {
 CertificateFactory certificateFactory = CertificateFactory.getInstance(
"X.509"
...
throw new 
RuntimeException(
"root certificate must be X509 format"
);
 }
...
HashSet<>();
 trustAnchors.add(
new 
TrustAnchor((X509Certificate) trustCert, 
null
));
 parameters = 
new 

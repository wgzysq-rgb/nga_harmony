...
**DocID**: `52f1ed4ccdbd43248b5f45bd38c5e700`

**NodeID**: `00020177579594626720114d11a0729d`
...
包含已购订阅相关状态信息的JWS格式数据。可参见
对返回结果验签
解码验签获取相关订阅状态信息的JSON字符串，其包含的参数请参见
SubGroupStatusPayload
。

[h2]SubGroupStatusPayload
...
订阅组最近生效的历史订阅状态
SubscriptionStatus
的列表，按订阅切换时间升序 ，最多包含10条有效订阅状态信息的记录 。

比如A切换B，B切换C，这里包含A，B，C三个订阅状态信息。

[h2]SubscriptionStatus
...
购买token，在购买消耗型/非消耗型/非续期订阅商品场景中与具体购买订单一一对应，在订阅型商品场景中与订阅ID一一对应。最大长度256。
...
当前订阅最新的购买订单列表，包含续期、折算、延期等产生的购买订单。购买订单包含的参数请参见
PurchaseOrderPayload
。

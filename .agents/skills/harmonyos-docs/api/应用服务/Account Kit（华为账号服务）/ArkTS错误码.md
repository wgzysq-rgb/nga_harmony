...
> **分区**: API参考  |  **Slug**: `account-api-error-code`  |  **DocID**: `0c37e3d83beb437fb20c77c27ac25102`
...
# ArkTS错误码
     ...
     **可能原因**
             ...
             元服务开发过程中，调[AuthorizationWithHuaweiIDRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#authorizationwithhuaweiidrequest)接口时传了profile这个scope，且supportAtomicService参数未设置为true。

             元服务开发过程中，调[AuthorizationWithHuaweiIDRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#authorizationwithhuaweiidrequest)接口时传了phone这个scope。
     ...
     **处理步骤**
             ...
             更换或注册一个中国境内（香港特别行政区、澳门特别行政区、中国台湾除外）的华为账号重新登录授权，如果在一键登录场景下，应用需展示其他登录方式。

             在获取实名信息、实名信息校验、人脸核身等场景下，引导用户更换使用中国境内（香港特别行政区、澳门特别行政区、中国台湾除外）身份证实名的华为账号。
             ...
             更换或注册一个中国境内（香港特别行政区、澳门特别行政区、中国台湾除外）的成人华为账号，重新触发接口。
     ...
     **可能原因**
             ...
             已登录的华为账号注册地为中国境外、香港特别行政区、澳门特别行政区或中国台湾，不支持调用获取手机号一致性状态接口。

             儿童账号不支持在TV设备上调用华为账号一键登录接口。

     
     **处理步骤**

     
             更换或注册一个注册地为中国境内（香港特别行政区、澳门特别行政区、中国台湾除外）的账号，重新触发该接口。

             在TV设备上，登录成人账号调用华为账号一键登录接口。
     ...
     **处理步骤**
             ...
             应用需要控制手机号一致性接口的调用次数，保证24h内不超过50次。

             若开发者调测过程中出现该错误码，可通过重启设备恢复。
     ...
     **可能原因**
             ...
             调用[AuthorizationWithHuaweiIDRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#authorizationwithhuaweiidrequest)且forceAuthorization参数设置为false的场景，用户未对应用授权。

             idValue未按照[getMobileNumberConsistency](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#getmobilenumberconsistency)接口中的[ConsistencyRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#consistencyrequest)正确传入或用户已对应用取消授权。

             idValue未按照[getHuaweiIDState](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#gethuaweiidstate)接口中的[StateRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#staterequest)正确传入或用户已对应用取消授权。

     
     **处理步骤**

     
             如需引导用户授权，可调用[AuthorizationWithHuaweiIDRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#authorizationwithhuaweiidrequest)，通过将forceAuthorization参数设置为true（不设置默认为true）拉起用户授权页面引导用户授权。

             参考[ConsistencyRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#consistencyrequest)中的idValue正确方式获取或重新授权。

             参考[StateRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#staterequest)中的idValue正确方式获取或重新授权。
     ...
     **处理步骤**
             ...
             若您的问题仍无法解决，请通过[在线提单](https://developer.huawei.com/consumer/cn/support/feedback/#/add/101704353566310877?level2=101704353626565886&level3=101704354306240980&keyWord=Account Kit)提交问题，华为支持人员会及时处理。
     ...
     **处理步骤**
             ...
             检查AGC上应用的指纹证书，详情请见[添加公钥指纹](https://developer.huawei.com/consumer/cn/doc/app/agc-help-cert-fingerprint-0000002278002933#section7398154810570)。
     ...
     **处理步骤**
             ...
             检查module type为entry的模块下的module.json5配置文件中的Client ID是否正确，详情请见[配置Client ID](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-client-id)。
     ...
     **处理步骤**
             ...
             请检查当前设备是否支持调用该API，建议先使用[canIUse](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/syscap#判断-api-是否可以使用)判断当前设备是否支持该API，再进行调用。
     ...
     **处理步骤**
             ...
             请通过[在线提单](https://developer.huawei.com/consumer/cn/support/feedback/#/add/101704353566310877?level2=101704353626565886&level3=101704354306240980&keyWord=Account Kit)提交问题，华为支持人员会及时处理。
     ...
     **可能原因**
             ...
             用户可能最近变更了户口，或挂失身份证并补发了新卡，但该身份信息尚未与公安部同步。

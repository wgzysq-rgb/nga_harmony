...
**DocID**: `bd4f5fa4b56a46118a40e4dd8b6349fb`

**NodeID**: `00020177579594626726275aa16fcbfc`
...
应用使用华为账号一键登录功能之前，需要完成华为账号一键登录权限申请，详见
申请账号权限
。
...
通过
华为账号一键登录（获取手机号和UnionID/OpenID）
场景获取的Authorization Code，详情可参考一键登录
客户端开发
。
...
在创建应用后，由AppGallery Connect（简称AGC）为应用分配的唯一标识。参数取值详见
查看应用基本信息
中的
...
该参数与获取code参数时的Client ID必须一致，否则会报错（响应消息中resultCode=60180003），如出现此报错，请参考
配置Client ID
排查处理。
...
在创建应用后，由AppGallery Connect（简称AGC）为应用分配的密钥（Client Secret）。参数取值详见
查看应用基本信息
中的
OAuth 2.0客户端ID（凭据）-Client Secret
参数。
...
若发起一键登录时
LoginPanelParams
的verifyPhoneNumber参数值传递为true，表示华为代为验证手机号有效性，开发者无需关注此返回值；
...
code参数获取时的clientId与当前接口参数clientId不一致导致，请检查入参client_id是否与
配置Client ID
中的值一致。

60180004

code过期，code只有5分钟有效期，超过有效期后将无法继续使用。

请引导用户重新授权，获取新的code再重试。
...
code不是通过调用华为账号的一键登录组件获取到的，请参考
客户端开发
的步骤3（展示一键登录页面并获取Authorization Code），获取华为账号一键登录场景所需的code参数。

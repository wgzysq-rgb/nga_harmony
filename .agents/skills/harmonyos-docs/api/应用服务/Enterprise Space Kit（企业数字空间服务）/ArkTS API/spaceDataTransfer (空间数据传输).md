...
**DocID**: `6fdaa17aebec4f6b81a60489027276d7`

**NodeID**: `000201775795946267040548a70902c9`
...
需要权限：
ohos.permission.ENTERPRISE_FILE_TRANSFER_AUDIT_POLICY_MANAGEMENT or ohos.permission.FILE_TRANSFER_OPERATION
...
。
每个节点负责不同的功能，共同构成了完整的策略配置。详细内容参考下表。

inEnable
：控制企业空间接收文件的权限。

incoming_check
：定义个人空间向企业空间发送文件时的检测规则。参数格式为字符串，内容会在后续详细解释。

outEnable 
：控制企业空间外发文件的权限。

outgoing_check
：定义企业空间向个人空间发送文件时的检测规则。参数格式为字符串，内容会在后续详细解释。

checkpoint_config
：配置检测应用的信息，包括应用的包名、组件名、参数、检测函数编码。参数格式为字符串，内容会在后续详细解释。

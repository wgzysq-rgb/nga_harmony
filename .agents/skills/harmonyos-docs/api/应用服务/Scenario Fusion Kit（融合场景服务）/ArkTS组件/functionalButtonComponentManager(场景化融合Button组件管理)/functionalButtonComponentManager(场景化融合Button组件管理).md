...
**Slug**: `scenario-fusion-functionalbuttoncomponentmanager`

**DocID**: `8357f7e1cd1942a0b85b784c9e6e56e7`

**NodeID**: `000201775795946267119719c633ed10`
...
该枚举定义了FunctionalButton组件的功能类型，通过���置该参数来指定FunctionalButton的功能。
...
使用loadingProgressModifier时，使用styleOption设置loading为true时才能生效。代码如下：
...
设备行为差异：
对于4.1.0(11)版本，该接口在Phone、Tablet、2in1中可正常调用，在其他设备类型中返回801错误码。对于5.1.0(18)及之后版本，该接口在Phone、Tablet、2in1、TV中可正常调用，在其他设备类型中返回801错误码。
...
可通过临时登录凭证获取真实手机号，临时登录凭证时效5分钟，具体操作可参考“
服务端开发
”章节。
...
设备行为差异：
该接口在Phone、Tablet中可正常调用，在2in1中返回1001500003错误码，在TV中返回801错误码。
...
设备行为差异：
对于5.0.0(12)版本，该接口在Phone、Tablet中可正常调用，在其他设备类型中返回801错误码。对于5.0.1(13)及之后版本，该接口在Phone、Tablet、2in1中可正常调用，在其他设备类型中返回801错误码。
...
0<=startTime-alertTime。

startTime和alertTime晚于当前时间。

startTime-alertTime <= 3600000(可在push侧配置)。
...
设备行为差异：
对于5.0.2(14)版本，该接口在Phone、Tablet、2in1中可正常调用，在其他设备类型中返回801错误码。对于5.1.0(18)及之后版本，该接口在Phone、Tablet、2in1、TV中可正常调用，在其他设备类型中返回801错误码。
...
回调函数。当打开授权设置页成功，err为undefined，data为打开设置成功时返回的响应；否则为错误对象。
...
回调函数。当选择收货地址点击成功，err为undefined，data为选择收货地址的返回结果；否则为错误对象。
...
回调函数。当选择发票抬头点击成功，err为undefined，data为选择发票抬头的返回结果；否则为错误对象。
...
回调函数。当实名信息校验成功，err为undefined，data为获取到的实名信息校验鉴权对应的返回体：authCode、openID；否则为错误对象。
...
回调函数。当人脸认证成功，err为undefined，data为获取到的人脸认证对应的返回体：authCode、openID；否则为错误对象。
...
验证成功的idToken，三方应用拉起华为账号客户端人脸验证页面时传入此Token，供华为账号客户端校验验证结果，参见
...
回调函数。当人脸验证点击成功，err为undefined，data为人脸验证的返回结果；否则为错误对象。
...
回调函数。当打开地图选点成功，err为undefined，data为地图选点的返回结果；否则为错误对象。
...
回调函数。当实况窗订阅点击成功，err为undefined，data为实况订阅的返回结果；否则为错误对象。
...
回调函数。当权限设置点击成功，err为undefined，data为获取到的权限设置返回结果；否则为错误对象。
...
回调函数。当服务动态授权码按钮点击成功，err为undefined，data为获取到的code返回结果；否则为错误对象。
...
回调函数。当分享按钮点击成功，err为undefined，否则为错误对象。
...
回调函数。当反馈按钮点击成功，err为undefined，否则为错误对象。
...
设置字体列表。默认字体'HarmonyOS Sans'，当前支持'
HarmonyOS Sans
'字体和
注册自定义字体
。

...
**Slug**: `scenario-fusion-atomicservice`

**DocID**: `81941c3d51c949bb922dcefcfcdad344`

**NodeID**: `000201775795946267064600d5430083`
...
蓝牙系统开关。“true”表示开关已打开，“false”表示开关已关闭。
...
地理位置的系统开关。“true”表示开关已打开，“false”表示开关已关闭。
...
Wi-Fi 的系统开关。“true”表示开关已打开，“false”表示开关已关闭。
...
设备当前显示的方向。“portrait”表示竖屏，“landscape”表示横屏。
...
该参数为浮点数，受
densityDPI
范围限制，取值范围在[0.5，4.0]。一般取值1.0、3.0等，实际取值取决于不同设备提供的
densityDPI
。
...
地理位置的系统开关。“true”表示开关已打开，“false”表��开关已关闭。
...
服务号ID。从华为开发者联盟服务号管理页面获取，长度限制1~64位字符。
...
渠道ID。长度限制0~32位字符，只能是数字或字母。用于识别组件关注的来源渠道。
...
回调函数。当关注操作成功，err为undefined，data为获取到的FollowCompleteResult；否则为错误对象。
...
关注组件返回结果。关注成功：FollowResult.SUCCESS，关注失败：FollowResult.FAILED。
...
设备行为差异：
对于5.0.0(12)版本，该接口在Phone、Tablet、2in1中可正常调用，在其他设备类型中返回801错误码。对于5.1.0(18)及之后版本，该接口在Phone、Tablet、2in1、Wearable、TV中可正常调用，在其他设备类型中返回801错误码。
...
获取Wi-Fi信息需要配置权限ohos.permission.GET_WIFI_INFO。

获取蓝牙开启状态需要配置权限ohos.permission.ACCESS_BLUETOOTH。
...
getSystemInfoSync接口不支持获取windowWidth、windowHeight、statusBarHeight和screenSafeArea属性，如需获取可使用
getSystemInfo
接口。

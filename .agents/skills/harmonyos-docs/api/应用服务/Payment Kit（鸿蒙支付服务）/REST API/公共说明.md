...
**DocID**: `12f2970b78894b5fb643ac7299e78536`

**NodeID**: `000201775795946267072784a89c8c57`
...
认证(Authentication)
：认证操作确认调用者身份，防抵赖和数据篡改。所有对商户开放接口均需要商户在请求头中传递
PayMercAuth
...
筛选：排除请求JSON串中sign字段，只保留sign以外的其他字段。

排序拼接：按key值的ASCII码顺序排序，并使用&拼接，列表场景使用符号“=”连接。 
 具体可参考
示例代码
引入华为支付提供
Maven依赖（pay-java）
...
[h2]常用语言SHA256WithRSA/PSS加签示例代码参考
...
javascript可通过jsrsasign模块实现，如已
安装node环境
，可执行如下命令安装jsrsasign模块。
...
获取参与签名的字段，转换为map并将map的“key-value”排序后以符号“&”拼接。

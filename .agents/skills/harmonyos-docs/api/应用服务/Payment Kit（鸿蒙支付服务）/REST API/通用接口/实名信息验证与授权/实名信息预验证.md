...
**Slug**: `payment-api-common-verification-preverify`

**DocID**: `c657760635f6408e8022c88fae4fb94c`

**NodeID**: `000201775795946267374485eaa694e7`
...
开发者可以调用该接口发起实名信息预验证，获取预授权ID（preVerifyId）。

场景描述

在接入实名信息验证服务前，开发者需要调用该接口获取到preVerifyId，然后返回给客户端调用验证接口拉起华为支付实名信息验证页面。
...
https://petalpay-developer.cloud.huawei.com.cn/api/v1/realname/verification/preverify
...
入参中的实名信息，必须和 openId用户下的实名信息保持一致。

请求示例

POST /api/v1/realname/verification/preverify HTTP/1.1
...
CLIENT_ID_PARAM_ERROR

CLIENT_ID错误

检查请求头参数clientId参数是否正确。
...
检查请求入参是否正确，符合相关枚举定义类型。

400000

RARE_CHARACTERS_NOT_SUPPORTED

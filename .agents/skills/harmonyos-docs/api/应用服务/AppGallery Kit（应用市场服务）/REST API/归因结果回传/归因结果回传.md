...
**DocID**: `881c4a3e519549c588e02c089dd11a88`

**NodeID**: `000201775795946267077c725a37eacb`
...
归因成功的分发平台id，分发平台向应用归因云侧注册角色时，由应用归因服务分配，长度固定为8个字符如果接收方为分发平台，该id即分发平台自身的id。

如果接收方为归因监测平台/开发者，该id为归因监测平台/开发者注册转化时指定分发平台列表中归因成功的分发平台id。
...
回传分发平台时，为分发平台关注的业务信息，如创意、素材等；

回传开发者时，为开发者关注的业务信息，如激活、付费相关营销链接信息。

 说明： 

通过白名单方式向分发平台、开发者开放（白名单开放方式请联系华为运营）。
...
通过白名单方式向应用生态伙伴（分发平台、开发者、归因监测平台）开放（白名单开放方式请联系华为运营）。
...
用于计算签名的随机数，不带"-"，由应用归因服务生成，每次归因结果回传，nonce唯一。
...
unix时间戳，毫秒，回传发生的时间戳，建议校验时判断与当前时间相差不超过5min。
...
应用归因侧解析application/json类型响应，“0”表示成功，其他值失败。

...
**DocID**: `3f367ca4ecbe405c9b668e5499c5c066`

**NodeID**: `0002017757962197387788ce98ae358c`
...
关闭传入的上下文环境，关闭后，所有在其中声明的引用都将被关闭。
...
创建出一个可逃逸的handle scope，可将范围内声明的值返回到父作用域。
...
获取给定js value对应的ISO-8859-1编码的字符串。
...
object: 该导出接口支持TypedArray或Sendable TypedArray（
Int8Array
、
Uint8Array
、
...
value: 该导出接口额外支持Sendable TypedArray（
Int8Array
、
Uint8Array
、
...
[h2]napi_delete_async_work

参数：

该导出接口暂时不支持async_hooks资源管理机制。

[h2]napi_queue_async_work

参数：

该导出接口暂时不支持async_hooks资源管理机制。

[h2]napi_cancel_async_work
...
[h2]napi_call_threadsafe_function
...
[h2]napi_release_threadsafe_function
...
获取ArkTS String的UTF-16编码内存缓冲区数据。
...
需要通过外部UTF-16编码的字符串缓冲区创建ArkTS字符串值且避免内存拷贝时使用此函数。
...
需要通过外部ASCII编码的字符串缓冲区创建ArkTS字符串值且避免内存拷贝时使用此函数。
...
创建指向Sendable ArkTS对象的Sendable强引用。
...
[h2]napi_queue_async_work_with_qos
...
[h2]napi_create_object_with_properties
...
[h2]napi_create_object_with_named_properties
...
[h2]napi_coerce_to_native_binding_object
...
[h2]napi_create_ark_runtime

napi_status napi_create_ark_runtime(napi_env *env)

描述：

创建基础运行时环境，一个进程最多创建64个，并满足与
Worker
创建的子线程总数不超过80个。
...
[h2]napi_delete_serialization_data
...
[h2]napi_call_threadsafe_function_with_priority
...
[h2]napi_create_sendable_object_with_properties
...
[h2]napi_create_sendable_array_with_length
...
[h2]napi_create_sendable_arraybuffer
...
[h2]napi_create_sendable_typedarray
...
[h2]napi_wrap_sendable_with_size
...
[in] finalize_cb：[可选]ArkTS对象被销毁时调用的回调函数，详情请参见
napi_finalize回调函数说明
。
...
只支持通过最初的上下文环境创建新的上下文环境，禁止通过该接口创建的上下文环境去创建新的上下文环境。

当前该接口不支持在非主线程的ArkTS线程中调用。

调用该接口前，调用者需要保证当前上下文环境不存在异常，否则会导致该接口调用失败。

该接口创建的上下文环境暂时只支持加载部分ArkUI的native so文件，对于加载应用自带的native so和加载公共基础库的native so暂时不支持。

多上下文运行时环境不支持sendable特性。

通过napi_create_ark_context接口创建的运行时上下文环境暂时不支持console、timer等模块能力。
...
该接口只能销毁通过napi_create_ark_context接口创建的运行时上下文环境。

不能通过该接口去销毁正在运行的上下文环境。
...
不能重复打开临界区作用域，必须在关闭当前作用域后才能再次打开。
...
[h2]napi_get_buffer_string_utf16_in_critical_scope
...
当ArkTS String以UTF-16编码存储时，napi_get_buffer_string_utf16_in_critical_scope才能正确获取其内存缓冲区，否则该函数返回错误。
...
[h2]napi_create_strong_reference
...
[h2]napi_delete_strong_reference
...
[h2]napi_get_strong_reference_value
...
不能使用已删除的强引用去获取ArkTS对象值，否则可能预期外的错误。
...
[h2]napi_finalize_callback回调函数说明
...
[h2]napi_create_external_string_utf16
...
传入的字符串数据必须是UTF-16编码格式，否则可能导致字符串内容异常。

传入的字符串数据在ArkTS字符串对象生命周期内必须保持有效，否则可能导致不可预期的行为。

如果提供了finalize_callback回调函数，当ArkTS字符串对象被销毁时，该回调函数将被调用。finalize_hint参数可以用于传递上下文信息给回调函数。

如果传入的length参数为NAPI_AUTO_LENGTH，接口内部自动查找到'\0'处计算字符串实际长度。
...
[in] finalize_callback：[可选]字符串对象被销毁时调用的回调函数，详情请参见
...
[h2]napi_create_external_string_ascii
...
传入的字符串数据必须是ASCII编码格式，否则可能导致字符串内容异常。
...
传入的字符串在指定的length长度范围内不得包含'\0'字符，否则可能导致异常行为。
...
[h2]napi_create_strong_sendable_reference
...
调用者需要保证传入的env参数是当前调用接口的ArkTS线程环境对象，避免将其他ArkTS线程的env作为参数传入导致出现
多线程安全问题
。
...
[h2]napi_delete_strong_sendable_reference
...
不可将napi_ref、napi_strong_ref等其他引用强转成napi_sendable_ref作为本接口入参。napi_delete_strong_sendable_reference接口仅允许接收由napi_create_strong_sendable_reference创建的napi_sendable_ref。
...
[h2]napi_get_strong_sendable_reference_value
...
不可将napi_ref、napi_strong_ref等其他引用强转成napi_sendable_ref作为本接口入参。napi_get_strong_sendable_reference_value接口仅允许接收由napi_create_strong_sendable_reference创建的napi_sendable_ref。

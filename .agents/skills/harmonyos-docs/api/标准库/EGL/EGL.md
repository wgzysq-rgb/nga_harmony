...
> **分区**: API参考  |  **Slug**: `egl`  |  **DocID**: `caef606ec3274a48a22d06ab3e473881`
...
# EGL
     ...
     #include <EGL/egl.h>     其次在CMakeLists.txt中添加以下动态链接库：

     libEGL.so     如果需要调用EGL扩展接口，需要额外添加头文件并且在CMakeLists.txt中添加宏定义：
...
```
     ...
     **eglGetNativeClientBufferANDROID**
     ...
     从API version 23开始，支持使用EGL扩展接口eglGetNativeClientBufferANDROID，将[OH_NativeBuffer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-buffer-h)转换为EGLClientBuffer类型，以实现纹理的绑定并进行GPU采样。具体使用方法请参考如下示例代码：

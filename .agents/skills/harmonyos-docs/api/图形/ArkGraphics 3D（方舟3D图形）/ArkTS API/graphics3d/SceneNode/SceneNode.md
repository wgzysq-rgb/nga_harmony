...
**DocID**: `0b5b86570f2a4d9e987a96e110254305`

**NodeID**: `00020177579587492007614132aaec71`

---

SceneNode

本模块提供3D图形中场景资源节点的类型及操作方法。
...
返回特定下标的图层是否使能。true表示使用图层掩码，false表示不使用。
...
要设置的使能状态。true表示使用图层掩码，false表示不使用。
...
3D场景由树状层次结构的节点组成，其中每个节点都实现了Node接口。继承自
SceneResource
。
...
节点的子节点，不存在则为空值。为只读属性，表示不能替换整个children容器，但可以通过容器方法操作子节点（如
append()
、
insertAfter()
、
...
几何节点类型，用于承载可渲染的网格数据，并支持可选的形变功能，继承自
Node
。
...
使用屏幕归一化坐标，取值范围为[0, 1]。其中(0,0)表示Component3D控件的左上角，(1,1)表示Component3D控件的右下角。
...
返回命中的结果数组（按距离从近到远排序），若无命中则返回空数组。

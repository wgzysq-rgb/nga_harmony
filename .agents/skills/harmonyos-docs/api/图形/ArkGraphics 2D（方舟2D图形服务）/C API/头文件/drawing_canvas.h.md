...
**DocID**: `80a1e657771a4eeda015543566811e87`

**NodeID**: `00020177579587492011071b38a76029`
...
画布自带一个黑色，开启抗锯齿，��具备其他任何样式的默认画刷，当且仅当画布中主动设置的画刷和画笔都不存在时生效。
...
用于保存当前画布的状态（画布矩阵）到一个栈顶。需要与恢复接口
...
[h2]OH_Drawing_SrcRectConstraint
...
[h2]OH_Drawing_CanvasShadowFlags
...
[h2]OH_Drawing_CanvasDrawSingleCharacterWithFeatures()
...
待绘制的单个字符。可以传入字符串，但只会以UTF-8编码解析并绘制字符串中的首个字符。
...
字符对象基线左端点（靠近字符左下角）的横坐标。

float y

字符对象基线左端点（靠近字符左下角）的纵坐标。
...
[h2]OH_Drawing_CanvasDrawPixelMapRectConstraint()
...
const OH_Drawing_SamplingOptions
* samplingOptions
...
[h2]OH_Drawing_CanvasDrawRecordCmdNesting()
...
[h2]OH_Drawing_CanvasCreateWithPixelMap()
...
[h2]OH_Drawing_CanvasAttachBrush()
...
[h2]OH_Drawing_CanvasDetachBrush()
...
[h2]OH_Drawing_CanvasGetSaveCount()
...
[h2]OH_Drawing_CanvasRestoreToCount()
...
要恢复的画布状态深度。小于等于1时，恢复为初始状态；大于已保存的画布状态数量时，不执行任何操作。

[h2]OH_Drawing_CanvasDrawLine()
...
[h2]OH_Drawing_CanvasDrawPixelMapNine()
...
[h2]OH_Drawing_CanvasDrawPixelMapRect()
...
[h2]OH_Drawing_CanvasDrawBackground()
...
[h2]OH_Drawing_CanvasDrawRegion()
...
[h2]OH_Drawing_CanvasDrawPoints()
...
[h2]OH_Drawing_CanvasDrawBitmap()
...
[h2]OH_Drawing_CanvasDrawBitmapRect()
...
[h2]OH_Drawing_CanvasDrawCircle()
...
弧的起始角度，0度时起始点位于椭圆的右端点，正数时以顺时针方向放置起始点，负数时以逆时针方向放置起始点。

float sweepAngle

弧的扫描角度，正数时顺时针扫描，负数时逆时针扫描。它的有效范围在-360度到360度之间，当绝对值大于360度时，该函数绘制的是一个椭圆。

[h2]OH_Drawing_CanvasDrawArcWithCenter()
...
弧的起始角度，单位为度，该参数为浮点数。0度时起始点位于椭圆的右端点，为正数时以顺时针方向放置起始点，为负数时以逆时针方向放置起始点。

float sweepAngle

弧的扫描角度，单位为度，该参数为浮点数。为正数时顺时针扫描，为负数时逆时针扫描。扫描角度可以超过360度，将绘制一个完整的椭圆。

bool useCenter

表示绘制时弧形的起点和终点是否连接弧形的中心点。true表示连接，false表示不连接。
...
[h2]OH_Drawing_CanvasDrawRoundRect()
...
[h2]OH_Drawing_CanvasDrawNestedRoundRect()
...
[h2]OH_Drawing_CanvasDrawSingleCharacter()
...
[h2]OH_Drawing_CanvasDrawTextBlob()
...
文本对象基线左端点（靠近文本左下角）的横坐标。

float y

文本对象基线左端点（靠近文本左下角）的纵坐标。

[h2]OH_Drawing_CanvasClipRect()
...
值为true则做抗锯齿处理，值为false不做抗锯齿处理。

[h2]OH_Drawing_CanvasClipRoundRect()
...
裁剪方式。支持可选的具体裁剪方式可见@{link OH_Drawing_CanvasClipOp}枚举。

bool doAntiAlias

表示是否需要做抗锯齿处理，值为true时为需要，为false时为不需要。

[h2]OH_Drawing_CanvasClipPath()
...
[h2]OH_Drawing_CanvasClipRegion()
...
表示裁剪类型。支持可选的具体裁剪方式可见@{link OH_Drawing_CanvasClipOp}枚举。
...
沿x轴的倾斜量。正值会使绘制沿y轴增量方向向右倾斜；负值会使绘制沿y轴增量方向向左倾斜。

float sy

沿y轴的倾斜量。正值会使绘制沿x轴增量方向向下倾斜；负值会使绘制沿x轴增量方向向上倾斜。

[h2]OH_Drawing_CanvasGetWidth()
...
[h2]OH_Drawing_CanvasGetLocalClipBounds()
...
[h2]OH_Drawing_CanvasGetTotalMatrix()
...
[h2]OH_Drawing_CanvasConcatMatrix()
...
[h2]OH_Drawing_CanvasDrawShadow()
...
环境阴影颜色，用一个32位（ARGB）的变量表示。

uint32_t spotColor

点阴影颜色，用一个32位（ARGB）的变量表示。
...
[h2]OH_Drawing_CanvasResetMatrix()
...
[h2]OH_Drawing_CanvasDrawImageRectWithSrc()
...
[h2]OH_Drawing_CanvasDrawImageRect()
...
[h2]OH_Drawing_CanvasDrawVertices()
...
描述顶点位置的数组指针，不能为空，其长度必须等于vertexCount。

const 
OH_Drawing_Point2D
* texs

描述顶点对应纹理空间坐标的数组指针，可以为空，若不为空其长度必须等于vertexCount。

const uint32_t* colors

描述顶点对应颜色的数组指针，用于在三角形中进行插值，可以为空，若不为空其长度必须等于vertexCount。

int32_t indexCount

索引的数量，可以为0，若不为0则值必须大于等于3。

const uint16_t* indices

描述顶点对应索引的数组指针，可以为空，若不为空其长度必须等于indexCount。
...
[h2]OH_Drawing_CanvasReadPixels()
...
[h2]OH_Drawing_CanvasReadPixelsToBitmap()
...
[h2]OH_Drawing_CanvasIsClipEmpty()
...
表示裁剪后可绘制区域是否为空。true表示为空，false表示不为空。
...
[h2]OH_Drawing_CanvasGetImageInfo()
...
[h2]OH_Drawing_CanvasDrawRecordCmd()
...
[h2]OH_Drawing_CanvasQuickRejectPath()
...
表示路径与画布区域是否不相交，true表示路径与画布区域不相交，false表示路径与画布区域相交。
...
[h2]OH_Drawing_CanvasQuickRejectRect()
...
表示矩形与画布区域是否不相交，true表示矩形与画布区域不相交，false表示矩形与画布区域相交。

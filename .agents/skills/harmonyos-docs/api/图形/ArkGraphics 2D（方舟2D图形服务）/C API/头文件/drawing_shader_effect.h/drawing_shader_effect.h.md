...
**DocID**: `24bc74c3fea34bb68fd60223cbfe3bfd`

**NodeID**: `0002017757958749201831bc97ba68be`
...
[h2]OH_Drawing_ShaderEffectCreatePixelMapShader()
...
[h2]OH_Drawing_ShaderEffectCreateColorShader()
...
函数会返回一个指针，指针指向创建的着色器对象
OH_Drawing_ShaderEffect
。如果对象返回NULL，表示创建失败；可能的原因是可用内存为空。

[h2]OH_Drawing_ShaderEffectCreateLinearGradient()
...
表示colors中每个对应颜色的相对位置，数组长度需和colors保持一致。如果pos为NULL，颜色均匀分布在起点和终点之间。

uint32_t size

表示颜色数组的长度（即元素个数），范围为[0, 256]，长度为0时，表示无渐变效果。

OH_Drawing_TileMode
 tileMode

着色器效果平铺模式类型，支持可选的具体模式可见
OH_Drawing_TileMode
枚举。
...
[h2]OH_Drawing_ShaderEffectCreateLinearGradientWithLocalMatrix()
...
表示作用于着色器上的矩阵变换，如果matrix是NULL, 默认是一个单位矩阵。
...
[h2]OH_Drawing_ShaderEffectCreateRadialGradient()
...
表示colors中每个对应颜色的相对位置，数组长度需和colors保持一致。如果pos为NULL，颜色均匀分布在径向上。
...
[h2]OH_Drawing_ShaderEffectCreateRadialGradientWithLocalMatrix()
...
[h2]OH_Drawing_ShaderEffectCreateSweepGradientWithLocalMatrix()
...
表示colors中每个对应颜色的相对位置，数组长度需和colors保持一致。如果pos为NULL，颜色均匀分布在0°和360°之间。
...
[h2]OH_Drawing_ShaderEffectCreateSweepGradient()
...
[h2]OH_Drawing_ShaderEffectCreateImageShader()
...
[h2]OH_Drawing_ShaderEffectCreateTwoPointConicalGradient()
...
表示colors中每个对应颜色的相对位置，数组长度需和colors保持一致。如果pos为NULL，颜色均匀分布在两个圆之间。
...
[h2]OH_Drawing_ShaderEffectCreateCompose()
...
[h2]OH_Drawing_ShaderEffectDestroy()

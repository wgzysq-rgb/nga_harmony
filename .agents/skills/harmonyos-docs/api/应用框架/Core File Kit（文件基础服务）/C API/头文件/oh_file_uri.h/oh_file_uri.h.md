...
**DocID**: `a939e4038b0c4374b51e793877516d00`

**NodeID**: `0002017757962161967972402c9295a4`
...
通过传入的路径path生成应用自己的uri；
 ...
 将path转uri时，路径中的中文及非数字字母的特殊字符将会被编码成对应的ASCII码，拼接在uri中。
...
表示转换后的uri, 需要使用standard library标准库的free()方法释放申请的资源。
...
表示转换后的路径path。需要使用standard library标准库的free()方法释放申请的资源。
...
[h2]OH_FileUri_GetFullDirectoryUri()
...
表示获取到的目录uri，需要使用standard library标准库的free()方法释放申请的资源。
...
返回 true: 传入uri是有效的uri；false: 传入的uri是无效的uri。

[h2]OH_FileUri_GetFileName()
...
表示转换后的名称。需要使用standard library标准库的free()方法释放申请的资源。

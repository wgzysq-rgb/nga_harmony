...
**Slug**: `js-apis-application-backupextensionability`

**DocID**: `ca06576484214e52b494914f11389fda`

**NodeID**: `00020177579621619625357c14bcc52d`
...
应用接入数据备份恢复需要通过BackupExtensionAbility实现，开发者可以通过
onBackup
，
onRestore
来实现自定义的备份恢复操作。
...
BackupExtensionAbility的上下文环境，继承自
ExtensionContext
。
...
扩展恢复数据的特殊处理接口中三方应用需要传递的包信息。

backupInfo可能为空字符串，需要开发者针对空字符串场景做判断处理。
...
返回应用执行自定义备份操作的信息，包含备份结果和报错信息，返回值为Json格式。
...
onRestoreEx与onRestore互斥，如��重写onRestoreEx，则优先调用onRestoreEx。
...
restoreInfo可能为空字符串，需要开发者针对空字符串场景做判断处理。
...
返回应用执行自定义恢复操作的信息，包含恢复结果和报错信息，返回值为Json格式。
...
返回应用onBackup或者onRestore执行过程中处理数据的进度信息（返回值为Json格式）。

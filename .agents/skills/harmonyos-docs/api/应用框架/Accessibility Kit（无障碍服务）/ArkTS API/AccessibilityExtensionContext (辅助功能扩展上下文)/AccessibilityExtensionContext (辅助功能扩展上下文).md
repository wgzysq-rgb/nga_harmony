...
**Slug**: `is-inner-application-accessibilityextensioncontext`

**DocID**: `d0f1713257e74af6946a8898f4829172`

**NodeID**: `000201775796216196390114fe797a32`
...
对于可滚动类控件，如List、Grid，内容区相对控件的顶部坐标滚动的像素偏移量。默认值为0。
...
设置关注应用的包名，服务接收关注应用的无障碍事件，默认接收所有应用的无障碍事件，取消关注应用则传空数组。
...
获取的是否是无障碍焦点元素，true表示是，false表示否，默认为否。
...
获取的是否是无障碍焦点元素，True表示是，False表示否。
...
无障碍节点元素, 在调用AccessibilityElement的方法前，需要先通过
AccessibilityExtensionContext.getFocusElement() 
或者
AccessibilityExtensionContext.getWindowRootElement() 

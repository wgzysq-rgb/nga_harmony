...
> **分区**: API参考  |  **Slug**: `errorcode-node`  |  **DocID**: `82df26fa1bc242eca04d08619bbbeb5e`
...
# 自定义节点错误码
     ...
     **可能原因**
      ...
      - 传入的节点参数的组件类型不正确。
      - 传入的节点参数为null或undefined。
      - 传入的控制器参数为null或undefined。
     ...
     **处理步骤**
      ...
      - 当报错信息显示传入参数为null，改为传入一个非空的FrameNode对象。
      - 当报错信息显示找不到公共父节点，传入之前判断目标节点是否为离屏节点，修改目标节点。
      - 其他原因的报错可参考错误信息进行修改。
     ...
     **可能原因**
     ...
     开发者在之前的某个地方使用该实例对象调用了[disposeNode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-arkui-nativenodeapi-1#disposenode)接口，例如：item.dispose()。

     **处理步骤**

     
      - 如果还需要使用该实例对象，请勿对其执行dispose操作。
      - 如果不确定当前实例对象是否可用，可以调用isDisposed接口进行判断。
     ...
     **错误描述**
     ...
     BuilderNode中，[状态管理V2](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-state-management-overview#状态管理v2)暂不支持[组件复用](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-buildernode#reuse12)。
     ...
     **可能原因**
      ...
      - 接口只支持在UI线程调用。
      - 接口支持多线程调用，但是接口操作的节点处于Attached状态。
     
     **处理步骤**

     
      - 调整函数调用时机，确保接口在UI线程调用。

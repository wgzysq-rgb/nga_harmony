...
**DocID**: `695bbe09a3da4393af9f96c7abf4898e`

**NodeID**: `000201775796216197823ea918de979b`
...
对自定义弹框进行封装，用于在保持当前的上下文环境时，临时展示用户需关注的信息或待处理的操作。使用示例参见
示例
。
...
弹框标题文本颜色。默认为$r('sys.color.ohos_id_color_text_primary_contrary')。
...
点击弹框任意位置后触发的用户自定义动作。默认为“执行关闭弹框的函数”，即仅关闭弹框。
...
点击关闭按钮后触发的用户自定义动作。默认为“执行关闭弹框的函数”，即仅关闭弹框。

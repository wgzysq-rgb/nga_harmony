...
**Slug**: `ts-container-embedded-component`

**DocID**: `abb35002fadc4f1b8d8512ae9d378a47`

**NodeID**: `000201775796216196660baad6a12ea8`

---

EmbeddedComponent

EmbeddedComponent用于支持在当前页面嵌入本应用内其他
...
被拉起的EmbeddedUIExtensionAbility通过调用
...
回调函数，入参用于接收EmbeddedUIExtensionAbility的返回结果，类型为
TerminationInfo
。
...
被拉起的EmbeddedUIExtensionAbility在运行过程中发生异常时触发本回调。可通过回调参数中的code、name和message获取错误信息并做处理，业务错误码详细介绍请参见
UIExtension错误码
。
...
回调函数，入参用于接收异常信息，类型为
BusinessError
，可通过参数中的code、name和message获取错误信息并做处理。
...
被拉起EmbeddedUIExtensionAbility退出时返回的结果码，返回的结果码由terminateSelfWithResult或者terminateSelf被调用时传入的数据决定。

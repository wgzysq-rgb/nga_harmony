...
**Slug**: `ts-basic-components-component3d`

**DocID**: `4eb1c2c2db264154a46f4e2027ce9938`

**NodeID**: `00020177579621619661418e09c93b86`
...
当传入glTF（.gltf文件和.glb文件）模型时，组件以自动场景模式运行，框架会自动创建基础相机、光源和默认手势交互（旋转、缩放），其相关参数由框架内部管理，不支持外部修改，开发者仅可通过Component3D的属性对展示效果进行配置。

当传入Scene对象时，组件以自定义场景模式运行，相机、光源及交互由开发者通过ArkGraphics 3D API自行创建和管理。

当不填写此参数时，组件仅作为自定义渲染管线（shader/customRender）的输出容器使用。

说明：

自定义场景模式下未内置相机控制器，因此不会自动响应拖拽或缩放手势；如需交互，请开发者接入手势并更新相机的位置与旋转。
...
自定义渲染的shader文件资源。详细.shader文件格式请参考
.shader资源文件格式要求
。

[h2]shaderImageTexture
...
自定义渲染用到的动效参数，数组长度范围为[0, 1048576]。
...
3D渲染分辨率的宽度，当前仅支持设置Dimension.Percentage，取值范围是[0, 100%]。
...
3D渲染分辨率的长度，当前仅支持设置Dimension.Percentage，取值范围是[0, 100%]。

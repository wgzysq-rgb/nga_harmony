...
**DocID**: `efe558e3e39a4a8588de36f635963033`

**NodeID**: `00020177579621619661216ac203c433`
...
字体大小。fontSize为number类型时，使用fp单位。字体默认大小16fp，Wearable设备上默认值为：18fp。不支持设置百分比字符串。
...
设置最小行数。组件的高度将根据lines自动调整，确保显示高度不低于lines对应的高度。如果设置了
constraintSize
，那么组件最后显示高度会在
constraintSize
约束内。
...
文本的文本行高。需要显式指定
像素单位
，如'10px'，也可设置百分比字符串，如'100%'。

说明
：不指定像素单位时，默认单位fp，如'10'，等同于10。
...
更多Font Feature能力介绍可参考
font-feature-settings 属性
和
OpenType 字体功能的完整 CSS 演示
。
...
内联模式，主动配置textOverflow才会生效按
maxLines
截断效果，不配置时，默认不截断。

文本截断是按字截断。例如，英文以单词为最小单位进行截断，若需要以字母为单位进行截断，
wordBreak
属性可设置为WordBreak.BREAK_ALL。

当textOverflow设置为TextOverflow.None、TextOverflow.Clip、TextOverflow.Ellipsis时，需配合
maxLines
...
设置省略位置。ellipsisMode属性需要配合
textOverflow
设置为TextOverflow.Ellipsis以及
maxLines
使用，单独设置ellipsisMode属性不生效。

EllipsisMode.START和EllipsisMode.CENTER仅在
maxLines
设置为1生效。
...
输入法回车键类型，类型为EnterKeyType.NEW_LINE时不触发onSubmit。
...
的
setExtraConfig
方法可以设置输入法扩展信息。在绑定输入法���功后，输入法会收到扩展信息，输入法可以依据此信息实现自定义功能。
...
类型为EnterKeyType.NEW_LINE时不触发onSubmit。
...
该示例通过
customKeyboard
（从API version 10开始）属性分别将value中的入参类型设置为
CustomBuilder
和
ComponentContent
，实现了自定义键盘的功能。
...
、
onPaste
展示如何监听文本选择菜单的复制、剪切、粘贴按钮，以及如何屏蔽系统粘贴功能并实现自定义的粘贴能力，同时，可以通过
maxFontScale
、
minFontScale
属性设置文本最大和最小的字体缩放倍数。

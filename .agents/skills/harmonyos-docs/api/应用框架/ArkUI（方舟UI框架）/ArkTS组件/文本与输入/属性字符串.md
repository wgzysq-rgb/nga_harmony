...
**DocID**: `67b4eada4d7b4d318d3b79f3b1a5e0b2`

**NodeID**: `000201775796216197057e2a426986fb`
...
当属性字符串和
Text
子组件冲突时，属性字符串优先级高，即当Text组件中绑定了属性字符串，忽略Text组件下包含
Span
等子组件的情况。
...
当value的类型为ImageAttachment或CustomSpan时，styles参数不生效。
...
当length为异常值时，length等于属性字符串在start后的实际长度；

当StyledStringKey与StyledStringValue不匹配时，styles不生效。
...
当属性字符串的文本及样式均一致，视为相等。

不比较
GestureStyle
，当属性字符串配置了不同事件，文本和其他样式相同时，亦视为相等。
...
当start为合法入参时，length的默认值是被查询属性字符串对象的长度与start的值的差。
...
当指定范围属性字符串未设置任何样式，则返回空数组。

当start和length越界或者必填传入undefined时，会抛出异常；

当styledKey传入异常值或undefined时，会抛出异常。

当styledKey为CustomSpan时，返回的是创建CustomSpan时传入的样式对象，即修改该样式对象也会影响实际的显示效果。
...
行内文本，支持样式设置。API version 17及之前，<span>设置的background-color属性转换不生效。
...
当SpanStyle的styledKey为IMAGE或CUSTOM_SPAN时，只有当start的位置当前是image或CustomSpan且长度为1，才会生效，其余情况无效果。
...
默认不清空原有样式，叠加新样式。如果StyledStringValue类型相同，则新样式将覆盖旧样式。
...
FontWeight.Bolder (or 'bolder')

11

FontWeight.Lighter (or 'lighter')

12

FontWeight.Medium (or 'medium')

13

FontWeight.Regular (or 'regular')
...
文本基线偏移量设置项。如果LengthMetrics的unit值是percent，该设置不生效。
...
文本字符间距设置项。如果LengthMetrics的unit值是percent，该设置不生效。
...
文本行高设置项。如果LengthMetrics的value值不大于0时，不限制文本行高，自适应字体大小。
...
图片对象的构造函数。与value类型入参构造函数相比，attachment参数增加了对undefined类型和
ResourceStr
类型图片的支持。
...
ColorFilter类型图片颜色滤镜设置项。

DrawingColorFilter

DrawingColorFilter类型图片颜色滤镜设置项。
...
DrawContext的canvas方法获取的是组件的画布，绘制时不会超出组件的范围。
...
（从API version 12开始）、
toHtml
（从API version 14开始）接口，将HTML中strong、b
20+
、em
20+
、i
...
、sup
20+
标签及其style属性中的background-color转换为属性字符串并转回HTML。

...
**DocID**: `70347b87884a406aa316bad782e02378`

**NodeID**: `0002017757962161966736dcf8e991c2`
...
从API version 11开始，设置inputFilter且输入的字符不为空字符，会导致
type
接口附带的文本过滤效果失效。
...
参数value为true时，才能设置options，文本框开启计数下标功能，需要配合
maxLength
...
内联模式
非编辑态下默认值：TextOverflow.Ellipsis

内联模式编辑态下默认值：TextOverflow.Clip

TextInput组件不支持设置TextOverflow.MARQUEE模式，当设置为TextOverflow.MARQUEE模式时，
内联模式
...
密码模式
时，由于输入框后端的状态和前端应用侧的状态管理变量会不一致，可能导致末尾图标的状态异常。建议在
...
输入状态变化回调，返回值为true表示输入框处于编辑态，返回值为false表示输入框处于非编辑态。
...
文本选择区域结束位置。当selectionEnd<0时，按照0处理；当selectionEnd大于文本长度时，按照文本长度处理。
...
、
onContentScroll
（从API version 10开始）接口实现了输入框监测输入状态变化、复制、剪切、粘贴、文本内容滚动回调的效果，同时，可以通过设置
selectAll

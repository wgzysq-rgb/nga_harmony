...
**Slug**: `ts-basic-components-toggle`

**DocID**: `266ef624740e46308c561ef37c6170fa`

**NodeID**: `000201775796216196599e9d3b44300a`
...
Toggle的样式继承对应组件样式的默认值，且不支持设置。例如，如果ToggleType为Button，则该组件样式继承
ButtonType
的默认值。由于Button.type从API version 18开始，默认类型从胶囊型变更为圆角矩形，胶囊型按钮不支持设置
borderRadius
，此时使用Toggle组件设置borderRadius也不生效。
...
优化深浅色模式切换开销
能力，浅色模式下默认值为0x19000000，表现效果为10%透明度的黑色；深色模式下默认值为0x19FFFFFF，表现效果为10%透明度的白色。

...
**Slug**: `ts-basic-components-rating`

**DocID**: `793bc26608114e3b9625acbfc9d4b5bd`

**NodeID**: `00020177579621619700919437a08b6d`
...
当Rating的父节点有指定宽高时，需为Rating组件指定宽高，或为父节点设置值为true的
clip
属性。
...
未设置时，则按照RatingOptions中各参数的默认值配置。
...
设置评分总数。设置为小于等于0的值时，按默认值显示。与
stars
相比，starCount参数新增了对undefined类型的支持。
...
设置操作评级的步长。设置为小于0.1的值时，按默认值显示。与
stepSize
相比，size参数新增了对undefined类型的支持。
...
此示例的资源不在src > main > resource目录下，从DevEco Studio 6.0.0 Beta2开始，新建工程或者模块时，默认创建的模块不会对非resources目录下的资源进行打包，需使能相关开关：模块的build-profile.json5中buildOptions > resOptions > copyCodeResource > enable设置为true，详见
resOptions
中相关介绍。

...
**DocID**: `685252f81e064fcbab24706589538ce9`

**NodeID**: `000201775796216197446470edd17f06`
...
自定义手势判定回调。当手势即将成功时，触发用户定义的回调获取结果。
...
卡片能力：
 从API version 17开始，该接口支持在ArkTS卡片中使用。
...
返回功能键按压状态。当功能键均处于按压状态时返回true，否则返回false。
...
该示例通过配置
onGestureJudgeBegin
实现了对长按、快滑和滑动手势的自定义判定。从API version 21开始，支持通过
BaseEvent

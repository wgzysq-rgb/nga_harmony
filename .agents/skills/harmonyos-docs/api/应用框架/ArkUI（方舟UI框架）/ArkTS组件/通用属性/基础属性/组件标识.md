...
**Slug**: `ts-universal-attributes-component-id`

**DocID**: `158bdcd3ce2243378990f52ca6dfc3e5`

**NodeID**: `000201775796216197432bee1978d3b9`
...
组件标识的拓展能力仅用于对应用的测试，以下各个接口示例，需要在应用工程的ohosTest/ets/test目录中调试，具体可以参考
自动化测试框架使用指导
。

[h2]getInspectorByKey
9+
...
：
 ...
 字符串信息包含组件的tag、id、位置信息(相对于窗口左上角的坐标)以及用于测试检查的组件所包含的相关属性信息。组件中每个字段的含义请参考
getInspectorInfo
的返回值说明。
...
组件树及组件属性列表的JSON对象。组件中每个字段的含义请参考
getInspectorInfo
的返回值说明。

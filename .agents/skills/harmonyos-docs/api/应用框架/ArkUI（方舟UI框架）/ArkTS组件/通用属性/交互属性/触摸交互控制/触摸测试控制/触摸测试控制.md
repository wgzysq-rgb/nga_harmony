...
**DocID**: `cf61eeeba5f140669dec978d3aa91941`

**NodeID**: `0002017757962161977633717a65360e`
...
设置组件的
触摸测试
类型。在ArkUI开发框架中，处理触屏事件和鼠标事件时，会在事件触发前进行按压点与组件响应热区的触摸测试，以收集需响应事件的组件。基于测试结果，框架会分发相应的事件。hitTestBehavior属性用于设置不同的触摸测试响应模式，影响触摸测试收集结果及后续事件分发。具体影响参考
HitTestMode
枚举说明。影响
点击事件
、
...
[h2]示例1（触摸测试类型为Block和Transparent的触摸测试效果）
...
[h2]示例2（触摸测试类型为BLOCK_HIERARCHY时的触摸测试效果）
...
[h2]示例3（触摸测试类型为BLOCK_DESCENDANTS时的触摸测试效果）
...
[h2]示例4（Stack组件中多节点重合时的触摸测试效果）

该示例演示了在Stack组��中存在多节点触摸区域重叠时的触摸测试效果。此时设置
HitTestMode

...
**Slug**: `ts-state-management-watch-monitor`

**DocID**: `0f096608caf34c9f8ba87f40425768f1`

**NodeID**: `000201775796216196680882e59085ba`
...
@Watch装饰器用于状态管理V1中对状态变量变化的监听。@Watch的详细使用方式见
@Watch装饰器：状态变量更改通知
。
...
@Monitor装饰器用于状态管理V2中对状态变量变化的监听。@Monitor相关内容的详细使用方式见
@Monitor装饰器：状态变量修改监听
。
...
用于监听的变量名路径，内容由开发者指定。当开发者仅传入一个字符串时，入参为该类型。
...
用于监听的变量名路径数组，内容由开发者指定。当开发者传入多个字符串时，入参为该类型。
...
可选，被监听的变量路径名。未指定时默认使用变化路径数组dirty中的第一个路径。
...
@Monitor监听变量的路径以及变化前后值信息。

T为监听变量的类型。

当监听的路径不存在时，返回undefined。

当未指定路径时，默认返回变化路径数组dirty中第一个路径对应的信息。

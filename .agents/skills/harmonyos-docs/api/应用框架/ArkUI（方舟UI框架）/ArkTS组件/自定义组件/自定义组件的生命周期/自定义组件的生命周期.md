...
**DocID**: `874ae533503d486caee9ae65f57fe00c`

**NodeID**: `0002017757962161965195ffbbff9be6`
...
状态变量
，更改将在后续执行build()函数中生效。实现
自定义布局
的自定义组件的aboutToAppear生命周期在布局过程中触发。具体使用说明，详见
自定义组件生命周期指南
。
...
aboutToDisappear函数在自定义组件析构销毁时执行。不允许在aboutToDisappear函数中改变状态变量，特别是@Link变量的修改可能会导致应用程序行为不稳定。具体使用说明，详见
自定义组件生命周期指南
。不建议在aboutToDisappear函数调用后再触发例如
自定义弹窗的创建
等逻辑，这可能会因为组件树信息丢失导致应用行为异常，例如
@Consume
找不到对应的
@Provide
、弹窗内白屏不显示组件等。
...
返回按钮动作。返回true表示页面自己处理返回逻辑，不进行页面路由；返回false表示使用默认的路由返回逻辑，不设置返回值按照false处理。
...
装饰的、作为
router路由
页面存在的自定义组件。当之前存在于路由栈中的页面，通过单实例模式
RouterMode
移动到栈顶时触发该回调。

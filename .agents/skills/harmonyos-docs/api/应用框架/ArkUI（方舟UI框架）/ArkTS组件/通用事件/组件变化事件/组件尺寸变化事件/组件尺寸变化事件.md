...
**Slug**: `ts-universal-component-size-change-event`

**DocID**: `a404e406d10c44e5a82b358f51703643`

**NodeID**: `0002017757962161975304ae6ce6da7b`
...
onSizeChange是布局过程中触发的同步回调，直接在其中更改状态变量存在被纳入动画闭包的风险。具体而言，动画会对比动画前的布局与动画闭包后的布局，若onSizeChange的回调在动画前的布局中同步触发，那么onSizeChange回调中所做的变更将与动画闭包中的变更一同纳入动画过程。为了避免此类问题，可在onSizeChange中使用延迟时间为0的
setTimeout
或
postFrameCallback
，将UI处理逻辑延后至异步执行。

...
**Slug**: `ts-container-listitem`

**DocID**: `180a93c84529464e8cd090d34b79958f`

**NodeID**: `000201775796216196578f65c7859db3`
...
当ListItem配合
LazyForEach
使用时，ListItem子组件在ListItem创建时创建。配合
if/else
、
ForEach
使用时，或父组件为List/ListItemGroup时，ListItem子组件在ListItem布局时创建。
...
从API version 7开始支持，从API version 9开始废弃，建议使用List组件的
stickyStyle枚举
替代。
...
向上或向下滑动（当列表方向为“水平”时）时操作项处于���藏状态。
...
进入鼠标框选范围即被选中返回true， 移出鼠标框选范围即未被选中返回false。
...
The component type of the node is incorrect.

...
**DocID**: `756376e6b9704486874076640c421452`

**NodeID**: `000201775796216197988df0bcab8647`
...
当前显示元素的索引，动画开始前的index值（不是最终结束动画的index值）。
...
动画相关信息，包括主轴方向上当前显示元素和目标元素相对ArcSwiper起始位置的位移，以及离手速度。
...
动画相关信息，只返回主轴方向上当前显示元素相对于ArcSwiper起始位置的位移。
...
当ArcSwiper切换动效结束时触发，包括动画过程中手势中断，通过
SwiperController
...
在页面跟手滑动和离手后执行切换动画的过程中，会对视窗内所���页面逐帧触发

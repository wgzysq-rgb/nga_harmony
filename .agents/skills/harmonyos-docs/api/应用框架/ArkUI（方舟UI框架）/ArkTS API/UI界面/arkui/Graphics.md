...
**DocID**: `9d57c8b3f42840ffb31e367e1f2b17ae`

**NodeID**: `0002017757962161975596714378209a`
...
轴心的x和y轴坐标。该参数为浮点数，默认值为0.5， 取值范围为[0.0, 1.0]。
...
颜色空间，用于指定颜色的色彩空间。使用ColorSpace.DISPLAY_P3，需要对应窗口调用
setWindowColorSpace
接口，将当前窗口设置为广色域模式。
...
Parameter error. Possible cause:1.The type of the input color parameter is not ResourceColor;2.The format of the input color string is not RGB or RGBA.
...
要叠加在上方的颜色对象。alpha属性决定叠加强度。1.0表示完全覆盖，0.0表示完全透明，混合结果为原色。
...
新的颜色对象，其red、green、blue和alpha通道均为当前颜色与叠加颜色混合后的结果值。
...
Parameter error. The type of the input parameter is not ColorMetrics.
...
rgba字符串格式的颜色。 示例：'rgba(255, 100, 255, 0.5)'
...
通过fillColor的透明度和亮度生成一个仅含透明度的颜色。亮度越高，颜色越透明。然后，使用
BlendMode.SRC_IN
...
通过strokeColor的透明度和亮度生成一个仅含透明度的颜色。亮度越高，颜色越透明。然后，使用
BlendMode.SRC_IN
方式与RenderNode本身的颜色混合，生成最终颜色。

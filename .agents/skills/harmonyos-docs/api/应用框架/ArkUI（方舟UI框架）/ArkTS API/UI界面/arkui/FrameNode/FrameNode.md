...
**DocID**: `cc8691e12f48466098ecd78cd3b46f35`

**NodeID**: `0002017757962161975202bfb6b4ee8d`
...
表示不展开当前FrameNode的子节点。如果FrameNode包含
LazyForEach
...
表示展开当前FrameNode的子节点。如果FrameNode包含
LazyForEach
...
表示按需展开当前FrameNode的子节点。如果FrameNode包含
LazyForEach
...
一个RenderNode对象。若该FrameNode不包含RenderNode，则返回空对象null。如果当前FrameNode为声明式组件创建的节点，则返回null。
...
typeNode
在appendChild时会校验子组件类型或个数，不满足时抛出异常信息，限制情况请查看
typeNode
描述。
...
The parameter is invalid. Details about the invalid parameter and the reason are included in the error message. For example: "The parameter 'node' is invalid: it cannot be adopted."
...
新节点将插入到该节点之后。若该参数设置为空，则新节点将插入到首个子节点之前。
...
The parameter is invalid. Details about the invalid parameter and the reason are included in the error message. For example: "The parameter 'child' is invalid: it cannot be adopted."
...
子节点。若该FrameNode不包含所查询的子节点，则返回空对象null。
...
[h2]getFirstChildIndexWithoutExpand
15+
...
[h2]getLastChildIndexWithoutExpand
15+
...
首个子节点。若该FrameNode不包含子节点，则返回空对象null。
...
当前FrameNode的下一个同级节点。若该FrameNode不包含下一个同级节点，则返回空对象null。
...
当前FrameNode的上一个同级节点。若该FrameNode不包含上一个同级节点，则返回空对象null。
...
当前FrameNode的父节点。若该FrameNode不包含父节点，则返回空对象null。
...
将当前FrameNode移动到目标FrameNode的指定位置。当前FrameNode如果不可修改，抛出异常信息。targetParent为
typeNode
时会校验子组件类型或个数，不满足时抛出异常信息，限制情况请查看
typeNode
描述。
...
[h2]getPositionToParentWithTransform
12+

getPositionToParentWithTransform(): Position

获取FrameNode相对于父组件带有绘制属性的位置偏移，单位为VP，绘制属性比如
transform
, 
translate
...
节点相对于父组件的位置偏移，单位为VP。 当设置了其他（比如：transform, translate等）绘制属性，由于浮点数精度的影响，返回值会有微小偏差。
...
[h2]getPositionToWindowWithTransform
12+

getPositionToWindowWithTransform(): Position

获取FrameNode相对于窗口带有绘制属性的位置偏移，单位为VP，绘制属性比如
transform
, 
translate
等，返回的坐标是组件布局时左上角变换后的坐标。
...
节点相对于窗口的位置偏移，单位为VP。 当设置了其他（比如：transform, translate等）绘制属性，由于浮点数精度的影响，返回值会有微小偏差。
...
[h2]getPositionToScreenWithTransform
12+

getPositionToScreenWithTransform(): Position

获取FrameNode相对于屏幕带有绘制属性的位置偏移，单位为VP，绘制属性比如
transform
, 
translate
等，返回的坐标是组件布局时左上角变换后的坐标。
...
节点相对于屏幕的位置偏移，单位为VP。 当设置了其他（比如：transform, translate等）绘制属性，由于浮点数精度的影响，返回值会有微小偏差。
...
后端实体节点是否解除引用。true为节点已与后端实体节点解除引用，false为节点未与后端实体节点解除引用。
...
FrameNode对象调用dispose后，由于不对应任何实体FrameNode节点，在调用部分查询接口(
getMeasuredSize
、
getLayoutPosition
)的时候会导致应用出现jscrash。
...
获取FrameNode中持有的CommonAttribute接口，用于设置
通用属性
和
通用事件
。
...
调用FrameNode的测量方法，根据父容器的布局约束，对FrameNode进行测量，计算出尺寸，如果测量方法被重写，则调用重写的方法。建议在
onMeasure
方法中调用。
...
调用FrameNode的布局方法，为FrameNode及其子节点指定布局位置，如果布局方法被重写，则调用重写的方法。建议在
onLayout
方法中调用。
...
// 自定义挂载事件的自定义组件，作为TestComponent1��子组件与TestComponent3、TestComponent4的父组件
@Component
...
[h2]getInteractionEventBindingInfo
19+
...
如果当前节点上绑定了任意交互事件，则返回一个InteractionEventBindingInfo对象，指示事件绑定详细信息，如果没有绑定任何交互事件则返回undefined。
...
待取消的动画属性枚举数组。可以一次取消一个节点上的多个属性的动画。
...
The current FrameNode is not modifiable.

100025

The parameter is invalid. Details about the invalid parameter and the reason are included in the error message. For example: "The parameter 'child' is invalid: it cannot be disposed."
...
The parameter is invalid. Details about the invalid parameter and the reason are included in the error message. For example: "The parameter 'child' is invalid: it cannot be null."
...
本次坐标转换的目标节点，转换得到的点坐标就是该节点坐标系中的相对坐标。
...
commonAttribute
仅在CustomFrameNode上生效，TypedFrameNode上commonAttribute行为未定义。建议使用
attribute
接口而非
commonAttribute
...
的
initialize
接口的入参，入参为Text组件的构造函数类型。

TextAttribute用于TypedFrameNode的
attribute
...
Text节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为Column组件的构造函数类型。

ColumnAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Column组件的属性设置对象。

[h2]createNode('Column')
12+
...
Column节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为Stack组件的构造函数类型。

StackAttribute用于TypedFrameNode的
attribute
...
Stack节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为GridCol组件的构造函数类型。

GridColAttribute用于TypedFrameNode的
attribute
接口的返回值，返回GridCol组件的属性设置对象。

[h2]createNode('GridCol')
12+
...
的
initialize
接口的入参，入参为Flex组件的构造函数类型。

FlexAttribute用于TypedFrameNode的
attribute
...
Flex节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为Swiper组件的构造函数类型。

SwiperAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Swiper组件的属性设置对象。

[h2]createNode('Swiper')
12+
...
Swiper节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为Progress组件的构造函数类型。

ProgressAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Progress组件的属性设置对象。

[h2]createNode('Progress')
12+
...
Progress节点类型的属性，若获取失败，则返回undefined。
...
Scroll节点类型的属性，若获取失败，则返回undefined。
...
Scroll节点类型的滚动事件，若获取失败，则返回undefined。
...
Parameter error. Possible causes: 1. the type of the node is error. 2. the node is null or undefined.
...
RelativeContainerInterface用于
TypedFrameNode
的
initialize
接口的入参，入参为RelativeContainer组件的构造函数类型。

RelativeContainerAttribute用于TypedFrameNode的
attribute
接口的返回值，返回RelativeContainer组件的属性设置对象。

[h2]createNode('RelativeContainer')
12+
...
RelativeContainer类型的FrameNode节点。
...
[h2]getAttribute('RelativeContainer')
20+
...
RelativeContainer节点类型的属性，若获取失败，则返回undefined。
...
DividerAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Divider组件的属性设置对象。

[h2]createNode('Divider')
12+
...
LoadingProgressInterface用于
TypedFrameNode
的
initialize
接口的入参，入参为LoadingProgress组件的构造函数类型。

LoadingProgressAttribute用于TypedFrameNode的
attribute
接口的返回值，返回LoadingProgress组件的属性设置对象。

[h2]createNode('LoadingProgress')
12+
...
[h2]getAttribute('LoadingProgress')
20+
...
LoadingProgress节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为Search组件的构造函数类型。

SearchAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Search组件的属性设置对象。

[h2]createNode('Search')
12+
...
Image节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为List组件的构造函数类型。

ListAttribute用于TypedFrameNode的
attribute
...
List节点类型的滚动事件，若获取失败，则返回undefined。
...
List节点类型的属性，若获取失败，则返回undefined。
...
ListItem节点类型的属性，若获取失败，则返回undefined。
...
TextInputInterface用于
TypedFrameNode
的
initialize
接口的入参，入参为TextInput组件的构造函数类型。

TextInputAttribute用于TypedFrameNode的
attribute
接口的返回值，返回TextInput组件的属性设置对象。

[h2]createNode('TextInput')
12+
...
TextInput节点类型的属性，若获取失败，则返回undefined。
...
ButtonAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Button组件的属性设置对象。
...
Button节点类型的属性，若获取失败，则返回undefined。
...
ListItemGroup类型的FrameNode节点类型。只允许添加
ListItem
类型子组件。
...
ListItemGroupInterface用于
TypedFrameNode
的
initialize
接口的入参，入参为ListItemGroup组件的构造函数类型。

ListItemGroupAttribute用于TypedFrameNode的
attribute
接口的返回值，返回ListItemGroup组件的属性设置对象。

[h2]createNode('ListItemGroup')
12+
...
ListItemGroup节点类型的属性，若获取失败，则返回undefined。
...
WaterFlow类型的FrameNode节点类型。只允许添加
FlowItem
类型子组件。
...
WaterFlowInterface用于
TypedFrameNode
的
initialize
接口的入参，入参为WaterFlow组件的构造函数类型。

WaterFlowAttribute用于TypedFrameNode的
attribute
接口的返回值，返回WaterFlow组件的属性设置对象。

[h2]createNode('WaterFlow')
12+
...
WaterFlow节点类型的滚动事件，若获取失败，则返回undefined。
...
WaterFlow节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为FlowItem组件的构造函数类型。

FlowItemAttribute用于TypedFrameNode的
attribute
接口的返回值，返回FlowItem组件的属性设置对象。

[h2]createNode('FlowItem')
12+
...
FlowItem节点类型的属性，若获取失败，则返回undefined。
...
XComponentInterface用于
TypedFrameNode
的
initialize
接口的入参，入参为XComponent组件的构造函数类型。

XComponentAttribute用于TypedFrameNode的
attribute
接口的返回值，返回XComponent组件的属性设置对象。

[h2]createNode('XComponent')
12+
...
XComponent节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为QRCode组件的构造函数类型。

QRCodeAttribute用于TypedFrameNode的
attribute
接口的返回值，返回QRCode组件的属性设置对象。

[h2]createNode('QRCode')
14+
...
的
initialize
接口的入参，入参为Badge组件的构造函数类型。

BadgeAttribute用于TypedFrameNode的
attribute
...
的
initialize
接口的入参，入参为Grid组件的构造函数类型。

GridAttribute用于TypedFrameNode的
attribute
...
Grid节点类型的滚动事件，若获取失败，则返回undefined。
...
Grid节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为GridItem组件的构造函数类型。

GridItemAttribute用于TypedFrameNode的
attribute
接口的返回值，返回GridItem组件的属性设置对象。

[h2]createNode('GridItem')
14+
...
GridItem节点类型的属性，若获取失败，则返回undefined。
...
TextClockInterface用于
TypedFrameNode
的
initialize
接口的入参，入参为TextClock组件的构造函数类型。

TextClockAttribute用于TypedFrameNode的
attribute
接口的返回值，返回TextClock组件的属性设置对象。

[h2]createNode('TextClock')
14+
...
TextTimerInterface用于
TypedFrameNode
的
initialize
接口的入参，入参为TextTimer组件的构造函数类型。

TextTimerAttribute用于TypedFrameNode的
attribute
接口的返回值，返回TextTimer组件的属性设置对象。

[h2]createNode('TextTimer')
14+
...
的
initialize
接口的入参，入参为Marquee组件的构造函数类型。

MarqueeAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Marquee组件的属性设置对象。

[h2]createNode('Marquee')
14+
...
的
initialize
接口的入参，入参为TextArea组件的构造函数类型。

TextAreaAttribute用于TypedFrameNode的
attribute
接口的返回值，返回TextArea组件的属性设置对象。

[h2]createNode('TextArea')
14+
...
TextArea节点类型的属性，若获取失败，则返回undefined。
...
SymbolGlyphInterface用于
TypedFrameNode
的
initialize
接口的入参，入参为SymbolGlyph组件的构造函数类型。

SymbolGlyphAttribute用于TypedFrameNode的
attribute
接口的返回值，返回SymbolGlyph组件的属性设置对象。

[h2]createNode('SymbolGlyph')
14+
...
的
initialize
接口的入参，入参为Checkbox组件的构造函数类型。

CheckboxAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Checkbox组件的属性设置对象。

[h2]createNode('Checkbox')
18+
...
Checkbox节点类型的属性，若获取失败，则返回undefined。
...
元服务API：
 从API version 18开始，该接口支持在元���务中使用。
...
CheckboxGroupInterface用于
TypedFrameNode
的
initialize
接口的入参，入参为CheckboxGroup组件的构造函数类型。

CheckboxGroupAttribute用于TypedFrameNode的
attribute
接口的返回值，返回CheckboxGroup组件的属性设置对象。

[h2]createNode('CheckboxGroup')
18+
...
的
initialize
接口的入参，入参为Rating组件的构造函数类型。

RatingAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Rating组件的属性设置对象。

[h2]createNode('Rating')
18+
...
Radio节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为Slider组件的构造函数类型。

SliderAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Slider组件的属性设置对象。

[h2]createNode('Slider')
18+
...
Slider节点类型的属性，若获取失败，则返回undefined。
...
的
initialize
接口的入参，入参为Toggle组件的构造函数类型。

ToggleAttribute用于TypedFrameNode的
attribute
接口的返回值，返回Toggle组件的属性设置对象。

[h2]createNode('Toggle')
18+
...
创建Toggle节点的接口参数，仅可通过ToggleOptions中的type属性设置开关样式。
...
Toggle节点类型的属性，若获取失败，则返回undefined。
...
NodeAdapter提供FrameNode的数据懒加载能力，通过
LazyForEach
实现接口功能。
...
绑定结果，返回true绑定成功，false绑定失败。

[h2]detachNodeAdapter
12+
...
从API version 22开始，该示例演示了如何通过FrameNode的
adoptChild
和
removeAdoptedChild
接口进行接纳为附属节点的相关操作。

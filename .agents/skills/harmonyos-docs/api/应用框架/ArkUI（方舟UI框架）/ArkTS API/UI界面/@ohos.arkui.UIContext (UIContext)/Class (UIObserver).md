...
**DocID**: `b9cfe6ca3a6445ffa5defc64d46e981d`

**NodeID**: `00020177579621619774180d7af9eef2`
...
监听事件，固定为'navDestinationUpdate'，即
NavDestination
组件的状态变化。
...
监听事件，固定为'navDestinationUpdateByUniqueId'，即
NavDestination
组件的状态变化。
...
监听事件，固定为'scrollEvent'，即滚动事件的开始和结束。
...
回调函数。返回滚动事件的信息。不传参数时，取消所有滚动事件的监听回调。
...
取消监听指定id的滚动组件滚动事件的开始和结束。滚动组件包括
List
、
Grid
、
...
监听事件，固定为'densityUpdate'，即屏幕像素密度变化。
...
需要被注销的回调函数。不传参数时，取消所有绘制指令下发事件的监听回调。
...
需要被注销的回调函数。不传参数时，取消所有布局完成的监听回调。
...
监听事件，固定为'navDestinationSwitch'，即
Navigation
的页面切换事件。
...
监听点击事件指令下发情况，所注册回调将于点击事件触发前触发。回调类型为
...
监听事件，固定为'willClick'，用于监听点击事件指令下发情况，所注册回调将于点击事件触发前触发。
...
监听事件，固定为'willClick'，即点击事件指令下发情况。
...
需要被注销的回调函数。不传参数时，取消所有的点击事件指令下发监听回调。
...
监听点击事件指令下发情况，所注册回调将于点击事件触发后触发。回调类型为
GestureEventListenerCallback
。从API version 20开始支持屏幕朗读触控模式。
...
监听事件，固定为'didClick'，用于监听点击事件指令下发情况，所注册回调将于点击事件触发后触发。
...
需要被注销的回调函数。若不指定具体的回调函数，则注销所有通过
on('tabChange')
接口注册的回调函数。
...
需要被注销的回调函数。若不指定具体的回调函数，则注销config指定的
Tabs
组件下注册的所有的回调函数。
...
需要被注销的回调函数。不传参数时，取消输入框文本变���的所有全局监听。
...
需要被注销的回调函数。不传参数时，取消指定ID输入框文本变化的所有局部监听。
...
监听事件，固定为'beforePanStart'，用于监听Pan手势
onActionStart
事件执行前的指令下发情况，所注册回调将于Pan手势
onActionStart
事件触发前触发。
...
监听事件，固定为'beforePanStart'，即Pan手势
onActionStart
事件执行前的指令下发情况。
...
监听事件，固定为'afterPanStart'，用于监听Pan手势
onActionStart
事件执行后的指令下发情况，所注册回调将于Pan手势
onActionStart
事件触发后触发。
...
监听事件，固定为'beforePanEnd'，用于监听Pan手势
onActionEnd
事件执行前的指令下发情况，所注册回调将于Pan手势
onActionEnd
事件触发前触发。
...
注册一个回调函数，以便在特定节点的��染状态发生变化时调用，当注册成功时，此回调将立即执行一次。
...
监听事件，固定为'nodeRenderState'，用于监听节点渲染状态发生改变。
...
监听事件，固定为'nodeRenderState'，即节点渲染状态变化指令下发情况。
...
需要被注销的回调函数。不传参数时，取消该节点所有的渲染状态变化指令下发监听回调。
...
监听事件，固定为'windowSizeLayoutBreakpointChange'，用于监听窗口尺寸布局断点发生改变。
...
回调函数。携带WindowSizeLayoutBreakpointinfo，包含窗口宽度和高度所在的布局断点枚举。
...
回调函数。携带SwiperContentInfo，返回Swiper内容切换的信息。
...
需要被注销的回调函数。不传参数时，取消该Swiper上所有的监听回调。

...
**Slug**: `arkts-apis-uicontext-router`

**DocID**: `ab04d4ec51f14059b4dd14ba0262c27b`

**NodeID**: `000201775796216197723d3910523bc4`
...
跳转到应用内的指定页面，使用Promise异步回调。与
pushUrl
相比，新增了mode参数，即支持设置跳转页面使用的模式。
...
The UI execution context is not found. This error code is thrown only in the standard system.
...
用应用内的某个页面替换当前页面，并销毁被替换的页面，使用Promise异步回调。与
replaceUrl
相比，新增了mode参数，即支持设置跳转页面使用的模式。
...
Failed to get the delegate. This error code is thrown only in the standard system.
...
用应用内的某个页面替换当前页面，并销毁被替换的页面。使用callback异步回调。与
replaceUrl
相比，新增了mode参数，即支持设置跳转页面使用的模式。
...
if the number of parameters is less than 1 or the type of the url parameter is not string.
...
用指定的命名路由页面替换当前页面，并销毁被替换的页面，使用Promise异步回调。与
replaceNamedRoute
相比，新增了mode参数，即支持设置跳转页面使用的模式。
...
用指定的命名路由页面替换当前页面，并销毁被替换的页面。使用callback异步回调。与
replaceNamedRoute
相比，新增了mode参数，即支持设置跳转页面使用的模式。
...
getStateByUrl(url: string): Array<router.
RouterState
>

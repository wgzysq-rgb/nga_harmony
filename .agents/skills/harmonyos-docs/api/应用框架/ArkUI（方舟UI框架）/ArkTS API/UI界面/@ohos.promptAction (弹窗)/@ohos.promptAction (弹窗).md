...
**DocID**: `a3767a52074b4841903056fcc2d7dc39`

**NodeID**: `00020177579621619716163c7e23ca79`
...
直接使用openToast可能导致
UI上下文不明确
的问题，建议使用UIContext中的getPromptAction方法获取到PromptAction对象，再通过该对象调用
openToast
实现。
...
- 当且仅当levelMode属性设置为LevelMode.EMBEDDED时���效。
...
弹窗显示顺序。取值范围为[-100000.0, 100000.0]，如果值小于-100000.0则设置为-100000.0，如果值大于100000.0则设置为100000.0。
...
表示值类型为颜色类型，用于描述资源颜色类型。

EdgeColors

表示值类型为边框颜色，用于描述组件边框四条边的颜色。
...
表示值类型为边框类型，用于描述组件边框的类型。

EdgeStyles

表示值类型为边框样式，用于描述组件边框四条边的样式。
...
表示值类型为阴影属性集合，用于设置阴影的模糊半径、阴影的颜色、X轴和Y轴的偏移量。

ShadowStyle

表示值类型为阴影类型，用于描述阴影的类型。
...
1. 目前只支持挂载在Page或者
NavDestination
节点上，优先挂载在Page节点下，只支持在这两种页面内顶层显示。
...
方法获取
PromptAction
对象，然后通过该对象进行调用。且直接使用showToast可能导致
UI上下文不明确
的问题。
...
当返回100001错误码时，可能出现了UI上下文不明确的问题，对此可以使用UIContext中的接口进行替换，详细说明可参考
使用UI上下文接口操作界面
。
...
方法获取
PromptAction
对象，然后通过该对象进行调用。且直接使用showDialog可能导致
UI上下文不明确
的问题。
...
回调函数。弹出对话框成功，err为undefined，data为获取到的对话框响应结果，否则为错误对象。
...
方法获取
PromptAction
对象，然后通过该对象进行调用。且直接使用showActionMenu可能导致
UI上下文不明确
的问题。
...
回调函数。弹出操作菜单成功，err为undefined，data为获取到的操作菜单响应结果，否则为错误对象。
...
openCustomDialog
替代。openCustomDialog需先通过
UIContext
中的
getPromptAction
方法获取
PromptAction
对象，然后通过该对象进行调用。且直接使用openCustomDialog可能导致
UI上下文不明确
的问题。
...
直接使用openCustomDialog可能导致实例不明确的问题，建议使用
UIContext
中的
getPromptAction
获取
...
从API version 11开始支持，从API version 18开始废弃，建议使用
closeCustomDialog
替代。closeCustomDialog需先通过
UIContext
中的
getPromptAction
方法获取
PromptAction
对象，然后通过该对象进行调用。且直接使用closeCustomDialog可能导致
UI上下文不明确
的问题。

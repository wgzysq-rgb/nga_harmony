...
**Slug**: `js-apis-arkui-observer`

**DocID**: `ff68f2bc0a9c43eaa41a065b255f6587`

**NodeID**: `000201775796216197944d2231983390`

---

@ohos.arkui.observer (无感监听)

提供UI组件行为变化的无感监听能力。推荐使用
UIObserver
进行组件监听。
...
routerPage生命周期触发时对应的状态。RouterPageState用于
RouterPageInfo
中，作为
routerPageUpdate
无感监听的返回值。
...
最近一次聚焦的TabsContent组件的下标索引。索引从0开始。仅在
on('tabChange')
的回调函数中存在。 
...
监听事件，固定为'navDestinationUpdate'，即NavDestination组件的状态变化。
...
uiObserver.on
相比，新增了options参数，即支持指定监听的Navigation的id。
...
监听事件，固定为'routerPageUpdate'，即router中page页面的状态变化。
...
回调函数。携带DensityInfo，返回变化后的屏幕像素密度。
...
需要被注销的回调函数。若不指定具体的回调函数，则注销指定UIContext下所有densityUpdate事件监听。
...
监听事件，固定为'navDestinationSwitch'，即Navigation的页面切换事件。
...
回调函数。携带NavDestinationSwitchInfo，返回页面切换事件的信息。
...
uiObserver.on('tabContentUpdate')
12+
...
监听事件，固定为'tabContentUpdate'，即TabContent页面的切换事件。
...
回调函数。携带TabContentInfo，返回TabContent页面切换事件的信息。

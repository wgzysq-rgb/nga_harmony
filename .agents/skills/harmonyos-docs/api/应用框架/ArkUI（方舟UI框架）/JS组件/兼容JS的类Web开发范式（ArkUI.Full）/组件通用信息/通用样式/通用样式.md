...
**Slug**: `js-components-common-styles`

**DocID**: `0f060ea0deec440aba74ffa8f01567a5`

**NodeID**: `00020177579621619747434fdbe448d0`
...
语法：window-filter: blur(percent), style
5+
...
- grid：网格布局（仅div支持display样式设置为grid）。
...
仅父容器为<div>、<list-item>、<tabs>、<refresh>、<stepper-item>
5+
时生效。
...
- absolute：相对于父元素进行定位，仅在父容器为<div>、<stack>时生效。
...
百分比参数方式：第一个值是��平位置，第二个值是垂直位置。左上角是 0% 0%。右下角是 100% 100%。如果仅规定了一个值，另外一个值为50%。

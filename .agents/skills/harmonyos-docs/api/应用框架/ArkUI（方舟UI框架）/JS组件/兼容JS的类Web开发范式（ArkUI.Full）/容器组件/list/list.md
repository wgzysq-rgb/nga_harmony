...
**Slug**: `js-components-container-list`

**DocID**: `ffbcf998a9514ecfbe22efd8c2514ad6`

**NodeID**: `000201775796216197604eb3ff784b63`
...
点击索引条进行列表项索引需要list-item子组件配合设置相应的
section属性
。
...
多语言索引条切换事件，仅当indexer属性为true，indexermulti为true时生效。booleanValue取值为true或false：
...
stateValue为0时表示列表滑动已停止。

stateValue为1时表示列表在用户触摸状态下处于滑动中。

stateValue为2时表示列表在用户松手状态下处于滑动中。
...
返回当前滑动的偏移量。返回值类型是Object，返回值说明请参见表1 currentOffset返回对象属性说明。

表1
 currentOffset返回对象属性说明

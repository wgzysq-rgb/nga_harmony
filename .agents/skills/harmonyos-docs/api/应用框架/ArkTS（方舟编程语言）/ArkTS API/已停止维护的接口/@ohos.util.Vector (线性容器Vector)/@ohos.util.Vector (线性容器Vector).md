...
> **分区**: API参考  |  **Slug**: `js-apis-vector`  |  **DocID**: `8e43b19f138d468d9a7654437729b856`
...
# @ohos.util.Vector (线性容器Vector)
 ...
 Vector和[ArrayList](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arraylist)相似，都是基于数组实现，但Vector提供了更多操作数组的接口。它们都可以动态调整容量，但Vector每次扩容增加1倍，ArrayList只扩容0.5倍。
...
- T：Type，类
  ...
  API version 9开始，该接口不再维护，推荐使用接口['@ohos.util.ArrayList'](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arraylist)。
...
**系统能力：** SystemCapability.Utils.Lang
  ...
  名称 类型 只读 可选 说明   length number 是 否 Vector的元素个数。       #### [h2]constructor
 ...
 **参数：**
  ...
  类型 说明   boolean 插入成功返回true，失败返回false。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   element T 是 被插入的元素。  index number 是 被插入的位置索引。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   element T 是 指定的元素。     **返回值：**

  类型 说明   boolean 是否包含指定元素，true表示包含该元素，false表示不包含。     **示例：**
 ...
 **参数：**
  ...
  类型 说明   number 返回指定元素第一次出现时的下标值，查找失败返回-1。     **示例：**
 ...
 **参数：**
  ...
  类型 说明   number 返回指定元素最后一次出现时的下标值，查找失败返回-1。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   index number 是 指定元素的下标值。     **返回值：**

  类型 说明   T 返回被删除的元素。数组为空时返回undefined，下标越界时抛出异常。     **示例：**
 ...
 **参数：**
  ...
  类型 说明   boolean 删除成功返回true，否则返回false。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   fromIndex number 是 起始下标。  toIndex number 是 终止下标。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   value T 是 当前遍历到的元素。  index number 否 当前遍历到的下标值，默认值为0。  vector Vector<T> 否 当前调用replaceAllElements方法的实例对象，默认值为当前实例对象。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   value T 是 当前遍历到的元素。  index number 否 当前遍历到的下标值，默认值为0。  vector Vector<T> 否 当前调用forEach方法的实例对象，默认值为当前实例对象。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   comparator function 否 回调函数，默认值为当前实例对象。     comparator的参数说明：

  参数名 类型 必填 说明   firstValue T 是 前一项元素。  secondValue T 是 后一项元素。     **示例：**
 ...
 **参数：**
  ...
  类型 说明   Vector<T> 返回Vector对象实例。     **示例：**
 ...
 **返回值：**
  ...
  类型 说明   boolean 为空返回true，否则返回false。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   newCapacity number 是 新容量。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   array Array<T> 是 指定数组。       #### [h2]getFirstElement
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   element T 是 要查找的元素。  index number 是 从指定索引开始搜索。     **返回值：**

  类型 说明   number 返回该元素的下标，如果查找失败，则返回-1。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   newSize number 是 设置的新长度。     **示例：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   index number 是 查找的下标值。     **返回值：**
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   index number 是 查找的下标值。  element T 是 用来替换的元素。     **返回值：**

  类型 说明   T 返回替换后的元素。       #### [h2][Symbol.iterator]
 ...
 **返回值：**
  ...
  类型 说明   IterableIterator<T> 返回一个迭代器。     **示例：**

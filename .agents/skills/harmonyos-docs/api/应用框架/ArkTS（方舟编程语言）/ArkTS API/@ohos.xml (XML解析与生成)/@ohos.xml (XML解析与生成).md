...
**DocID**: `e80927e7d10f411c971ab89da5caac68`

**NodeID**: `00020177579621619647765f21b2b4e7`
...
当前元素及其子元素的前缀。所组成的XML长度不能超过100000，不可为空字符。
...
当前元素及其子元素的命名空间。所组成的XML长度不能超过100000，不可为空字符。
...
CDATA属性的内容。所组成的XML长度不能超过100000。
...
DocType属性的内容。所组成的XML长度不能超过100000。
...
从API version 8开始支持，从API version 14开始废弃，建议使用
parseXml
14+
...
是否继续解析标签名称、属性名称及属性的值。true表示继续解析，false表示停止解析。
...
返回true，表示当前元素为空元素。返回false，表示当前元素为非空元素。
...
返回true，表示当前文本事件仅包含空格字符。返回false，表示当前文本事件包含非空格字符。

...
**Slug**: `arkts-apis-arkts-utils-sendablelrucache`

**DocID**: `94a903434c114701bbf583d093bf6294`

**NodeID**: `00020177579621619679995e93f0c075`
...
指示缓冲区的自定义容量。不传时，默认值为64，最大值不能超过2147483647；小于等于0时会抛出异常。
...
指示要为缓冲区自定义的容量，最大值不能超过2147483647；小于等于0时会抛出异常。
...
返回true表示当前缓冲区为空，不包含任何键值对；返回false表示当前缓冲区不为空。
...
如果指定的键存在于缓冲区中，则返回与键关联的值；否则调用内部默认接口，生成一个键与值相同的对象，并返回其值。如果内部默认接口返回undefined，则最终返回undefined。
...
返回当前缓冲区中所有值的列表，按从最近访问到最少访问的顺序排列。
...
返回当前缓冲区中所有键的列表，按从最近访问到最少访问的顺序排列。
...
返回与key关联的值；若key不存在，则返回undefined。

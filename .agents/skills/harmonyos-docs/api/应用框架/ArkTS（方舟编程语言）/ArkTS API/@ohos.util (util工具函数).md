...
**DocID**: `5063a25ddd3146cdaf31660ef957c520`

**NodeID**: `000201775796216196433ee80e841927`

---

@ohos.util (util工具函数)

该模块主要提供常用的工具函数，实现字符串编解码（
TextEncoder
，
TextDecoder
）、有理数运算（
...
格式化字符串，可以包含零个或多个占位符，用于指定要插入的参数的位置和格式。
...
替换format参数中占位符的数据，此参数缺失时，默认返回第一个参数。
...
将参数转换为字符串，用于除Object，BigInt和-0之外的所有值。

%d

将参数作为十进制整数进行格式化输出，用于除Symbol和BigInt之外的所有值。

%i

将字符串转换为十进制整数，用于除BigInt和Symbol之外的所有值。

%f

将字符串转换为浮点数，用于除BigInt和Symbol之外的所有值。

%j

将JavaScript对象转换为JSON字符串进行格式化输出。

%o

用于将JavaScript对象进行格式化输出，将对象转换为字符串表示，但不包含对象的原型链信息。

%O

用于将JavaScript对象进行格式化输出，将对象转换为字符串表示。

%c

只在浏览器环境中有效。其余环境不会产生样式效果。
...
返回一个回调函数，该函数第一个参数err是拒绝原因（如果 Promise 已解决，则为 null），第二个参数value是已解决的值。
...
回调函数中第一个参数err是拒绝原因（如果 Promise 已解决，则为 null），第二个参数value是已解决的值。
...
是否使用已缓存的UUID，true表示使用缓存的UUID，false表示不使用缓存的UUID，默认true。
...
替换式样化字符串通配符的数据，此参数缺失时，默认返回第一个参数。
...
从API version 7开始支持，从API version 9开始废弃，此接口不可用，建议使用
util.promisify
9+
...
返回promise函数，采用遵循常见的错误优先的回调风格的函数（也就是将(err, value) => ...回调作为最后一个参数）。
...
主线程的栈追踪信息。若主线程未处于执行JS代码状态，则返回空字符串。
...
是否显示致命错误，true表示显示致命错误，false表示不显示致命错误，默认值是false。
...
是否忽略BOM标记，true表示忽略待解码数据的BOM标记，false表示会对BOM标记解码，默认值是false。
...
输入末尾出现的不完整字节序列是否需要追加在下次调用decodeToString的参数中处理。设置为true，则不完整的字节序列会存储在内部缓存区直到下次调用该函数，false则会在当前调用时直接解码。默认为false。
...
在随后的decodeWithStream()调用中是否跟随附加数据块。如果以块的形式处理数据，则设置为true；如果处理最后的数据未分块，则设置为false。默认为false。
...
在指定的类对象的原方法执行前插入一个函���。执行addBefore接口后，先运行插入的函数逻辑，再执行指定类对象的原方法。
...
解码相关选项参数，存在两个属性fatal和ignoreBOM。此参数不填时，对应各属性取其默认值。
...
在随后的decode()调用中是否跟随附加数据块。如果以块的形式处理数据，则设置为true；如果处理数据未分块，则设置为false。默认为false。
...
需要编码的字符串，默认值是空字符串。当入参是空字符串时，返回undefined。
...
返回一个对象，read表示已编码的字符数，written表示编码字符所占用的字节数。
...
分子，整数类型。取值范围：-Number.MAX_VALUE <= numerator <= Number.MAX_VALUE。
...
分母，整数类型。取值范围：-Number.MAX_VALUE <= denominator <= Number.MAX_VALUE。
...
The type of rationalString must be string.
...
两个对象相等时返回0；给定对象小于当前对象时返回1；给定对象大于当前对象时返回-1。
...
整数类型。-Number.MAX_VALUE <= number1 <= Number.MAX_VALUE。
...
整数类型。-Number.MAX_VALUE <= number2 <= Number.MAX_VALUE。
...
返回Numerator/Denominator格式的字符串，例如3/5，如果分子为0，则返回0/1。如果分母为0，则返回Infinity。如果分子和分母都为0，则返回NaN。
...
指示要为缓冲区自定义的容量，不传默认值为64，最大值不能超过2147483647。
...
指示要为缓冲区自定义的容量，最大值不能超过2147483647。
...
返回键对应的值。当键不在缓冲区中时，通过
createDefault
9+

接口创建，若createDefault创建的值不为undefined时，此时会调用
afterRemoval
9+

接口，返回createDefault创建的值。
...
如果指定的键存在于缓冲区中，则返回与键关联的值；否则返回createDefault创建的值。
...
返回当前缓冲区中所有值的列表，顺序为从最近最少访问（Least Recent）到最近访问（Most Recent）。
...
返回当前缓冲区中所有键的列表，顺序为从最近最少访问（Least Recent）到最近访问（Most Recent）。
...
返回一个包含已删除键值对的Optional对象；如果key不存在，则返回undefined，如果key为null，则抛出异常。
...
删除值后执行后续操作，这些操作由开发者自行实现。本接口会在删除操作时被调用，如
get
9+
...
接口调用之后触发执行，传入的key和value参数类型为MapIterator，可参照示例二进行后续操作。
...
当因容量不足而调用该方法时，参数值设置为true，其他情况设置为false。
...
如果传入的value小于下限，返回lowerObj；如果大于上限值，返回upperObj；如果在当前范围内，返回value。
...
Promise对象，返回异步编码后新分配的Uint8Array对象。
...
需要解码的字符串。会根据输入的编码类型进行解码，参数为Uint8Array时正常解码，参数为string时会将参数直接返回。
...
如果是ArrayBuffer或SharedArrayBuffer类型则返回true，否则返回false。
...
如果是ArrayBufferView类型则返回true，否则返回false。
...
如果是arguments对象则返回true，否则返回false。
...
如果是ArrayBuffer类型则返回true，否则返回false。
...
如果是Boolean、Number、String或Symbol对象则返回true，否则返回false。
...
如果是DataView类型则返回true，否则返回false。
...
如果是native External类型则返回true，否则返回false。
...
如果是Float32Array数组类型则返回true，否则返回false。
...
如果是Float64Array数组类型则返回true，否则返回false。
...
如果是generator函数类型则返回true，否则返回false。
...
如果是generator对象则返回true，否则返回false。
...
如果是Int8Array数组类型则返回true，否则返回false。
...
如果是Int16Array数组类型则返回true，否则返回false。
...
如果是Int32Array数组类型则返回true，否则返回false。
...
如果是Map的Iterator类型则返回true，否则返回false。
...
如果是Number对象类型则返回true，否则返回false。
...
如果是Set的Iterator类型则返回true，否则返回false。
...
如果是String对象类型则返回true，否则返回false。
...
如果是Symbol对象类型为则返回true，否则返回false。
...
如果是TypedArray包含的类型则返回true，否则返回false。
...
如果是Uint8Array数组类型则返回true，否则返回false。
...
如果是Uint8ClampedArray数组类型则返回true，否则返回false。
...
如果是Uint16Array数组类型则返回true，否则返回false。
...
如果是Uint32Array数组类型则返回true，否则返回false。
...
如果是BigInt64Array类型则返回true，否则返回false。
...
如果是BigUint64Array类型则返回true，否则返回false。
...
如果是Module Namespace Object类型则返回true，否则返回false。
...
如果是SharedArrayBuffer类型则返回true，否则返回false。
...
当监听的对象被回收时，该值会被传递给onFinalization回调方法。

AutoFinalizerCleaner<T>
22+
...
实现了AutoFinalizer接口的对象，其onFinalization方法会在该对象被回收时调用。
...
当监听的对象被回收时，该值会被传递给obj.onFinalization方法。

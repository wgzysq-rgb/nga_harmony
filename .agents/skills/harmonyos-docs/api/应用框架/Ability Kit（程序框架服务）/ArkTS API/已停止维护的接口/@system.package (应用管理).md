...
> **分区**: API参考  |  **Slug**: `js-apis-system-package`  |  **DocID**: `e7738f74ab13440aaacdbc42fb8b1217`
 ...
 **参数：**
  ...
  参数名 类型 必填 说明   options [CheckPackageHasInstalledOptions](#checkpackagehasinstalledoptions) 是 选项参数。     **示例：**
 ...
 **系统能力：** SystemCapability.BundleManager.BundleFramework
  ...
  名称 类型 必填 说明   result boolean 是 指示应用是否已安装。       #### CheckPackageHasInstalledOptions
 ...
 **系统能力：** SystemCapability.BundleManager.BundleFramework
  ...
  名称 类型 必填 说明   bundleName string 是 应用Bundle名称。  success Function 否 接口调用成功的回调函数。  fail Function 否 接口调用失败的回调函数。  complete Function 否 接口调用结束的回调函数。

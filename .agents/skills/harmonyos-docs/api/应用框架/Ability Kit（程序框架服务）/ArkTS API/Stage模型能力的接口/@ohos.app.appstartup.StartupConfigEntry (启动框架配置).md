...
**DocID**: `cecbc857d7474f92afcf44c06345a0ea`

**NodeID**: `0002017757962161972671dd4ae9e89a`
...
前，若该AbilityStage对应的HAP中启动框架配置文件中
定义了启动框架配置
，则会触发该回调。

开发者可以在该回调中设置启动框架配置信息，详细使用方法可参考
设置启动参数
章节。
...
开发者可以在该回调中，可以根据调用方传入启动UIAbility的Want中的不同参数来返回不同的自定义匹配规则。启动框架会将其与启动任务配置的matchRules中customization字段进行匹配。若匹配成功，任务将在自动模式执行。详细匹配规则请参考
添加任务匹配规则
章节。

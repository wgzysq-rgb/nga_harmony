...
**Slug**: `js-apis-continuation-continuationmanager`

**DocID**: `6710466271614d11a1ca34cab2b9abe6`

**NodeID**: `0002017757962161972810315719b1d3`
...
本模块首批接口从API version 8开始支持，从API version 22开始废弃，建议使用
分布式设备管理
替代。
...
AsyncCallback形式返回流转管理服务连接后生成的token。
...
从API version 9开始支持，从API version 22开始废弃，建议使用
onDeviceStateChange
替代。
...
当用户从设备选择模块中选择设备时调用，返回设备ID、设备类型和设备名称供开发者使用。
...
当用户从设备选择模块中断开设备时调用，返回设备ID供开发者使用。
...
取消监听的事件类型，固定值"deviceDisconnect"。
...
continuationManager.on('deviceUnselected')
(deprecated)
...
当用户从设备选择模块中断开设备时调用，返回设备ID、设备类型和设备名称供开发者使用。
...
continuationManager.off('deviceUnselected')
(deprecated)
...
取消监听的事件类型，固定值"deviceUnselected"。
...
回调函数。当模块选择完成，err为undefined，否则返回错误对象。
...
拉起���备选择模块，可显示组网内可选择设备列表信息，使用Promise方式作为异步方法。
...
回调函数。当通知设备成功，err为undefined，否则返回错误对象。
...
回调函数。当解注册成功，err为undefined，否则返回错误对象。
...
从API version 10开始支持，从API version 22开始废弃，建议使用
DeviceBasicInfo
替代。
...
_ContinuationExtraParams

表示流转管理入口中设备选择模块所需的过滤参数。

---
*2026-04-22T15:55:01.351Z*
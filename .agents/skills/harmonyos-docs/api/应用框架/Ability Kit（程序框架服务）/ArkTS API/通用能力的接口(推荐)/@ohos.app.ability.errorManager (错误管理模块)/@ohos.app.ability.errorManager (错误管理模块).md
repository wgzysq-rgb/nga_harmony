...
**Slug**: `js-apis-app-ability-errormanager`

**DocID**: `41376b0c2799483486e5dcc92c6ac0d9`

**NodeID**: `00020177579621619701096d91675b05`
...
观测器的索引值，与观测器一一对应。可用于errorManager.off函数中的observerId参数。
...
errorManager.on('globalErrorOccurred')
18+
...
填写'globalErrorOccurred'，表示错误观测器。
...
由on方法注册的callback。建议使用该参数，缺省时默认清除所有通过on注册的相同env的callback，否则删除指定callback。
...
填写'loopObserver'，表示注册主线程消息处理耗时监听器。
...
errorManager.on('globalUnhandledRejectionDetected')
18+
...
填写'globalUnhandledRejectionDetected'，表示注册被拒绝promise监听器。
...
填写'unhandledRejection'，表示注册被拒绝promise监听器。
...
如果该回调函数执行时间超过1s，可能导致
AppRecovery
功能不可用。通过解析hilog日志中的begin与Freeze callback execution completed两者的时间差可以计算回调函数执行时长，如果超过1秒，可以尝试采用异步处理、减少阻塞操作、优化数据结构等方法优化回调逻辑，降低执行时长。
...
由on接口注册的被拒绝promise的callback。建议使用该参数，缺省时默认清除所有通过on注册的相同env的callback，否则删除指定callback。
...
注册了被拒绝promise监听器。建议使用该参数，缺省时默认清除所有通过on注册的相同env的observer，否则删除指定observer。
...
由on接口注册的freeze监听的callback。建议使用该参数，如果参数不填会直接清空callback，否则删除指定的callback。
...
errorManager.setDefaultErrorHandler
21+
...
有关异常事件名字、消息、错误堆栈信息的对象。

---
*2026-04-22T15:53:57.900Z*
...
**Slug**: `js-apis-abilityaccessctrl`

**DocID**: `75150c4cbbc94b88b54c58c524bb0e0e`

**NodeID**: `00020177579621619648481c04e5f79a`
...
Invalid parameter. The tokenID is 0, or the permissionName exceeds 256 characters.
...
订阅事件类型，固定为'selfPermissionStateChange'，自身权限状态变更事件。
...
订阅的权限名列表，如果为空，则表示订阅所有的权限状态变化，合法的权限名取值可在
应用权限列表
中查询。
...
Invalid parameter. Possible causes: 1. The permissionList exceeds the size limit; 2. The permissionNames in the list are all invalid.

12100004

The API is used repeatedly with the same input.

12100005

The registration time has exceeded the limit.
...
订阅事件类型，固定为'selfPermissionStateChange'，权限状态变更事件。
...
取消订阅的权限名列表，为空时表示取消订阅所有的权限状态变化，必须与on的输入一致，合法的权限名取值可在
应用权限列表
中查询。
...
回调函数，返回取消订阅指定tokenID与指定权限名状态变更事件的结果。
...
Invalid parameter. The permissionNames in the list are all invalid.

12100004

The API is not used in pair with 'on'.
...
[h2]requestPermissionsFromUser
9+

requestPermissionsFromUser(context: Context, permissionList: Array<Permissions>, requestCallback: AsyncCallback<PermissionRequestResult>): void

用于UIAbility/UIExtensionAbility拉起弹框请求
用户授权
。使用callback异步回调。

如果用户拒绝授权，将无法再次拉起弹框，需要用户在系统应用“设置”的界面中，手动授予权限，或是调用
...
请求权限的UIAbility/UIExtensionAbility的Context。
...
回调函数。当拉起权限请求弹框成功，err为undefined，data为获取到的PermissionRequestResult；否则err为错误对象。
...
Invalid parameter. The context is invalid when it does not belong to the application itself.

示例：

下述示例中context的获取方式请参见
获取UIAbility的上下文信息
。

关于向用户申请授权的完整流程及示例，请参见
向用户申请授权
。
...
Unexpected permission. You cannot request this type of permission from users via a pop-up window.
...
Promise对象。返回true，表示全局开关状态为开启。返回false，表示全局开关状态为关闭。
...
Invalid parameter. Possible causes: 1. The context is invalid because it does not belong to the application itself; 2. The type of global switch is not support.
...
Invalid parameter. The permissionName is empty or exceeds 256 characters.
...
Common inner error. An error occurs when creating the pop-up window or obtaining user operation result.

12100014

Unexpected permission. The permission is not a manual_settings permission.
...
，暂未调用
requestPermissionsFromUser
接口请求用户授权时，或用户在设置中将权限状态修改为每次询问时，查询权限状态将返回此值。
...
表示无效。应用未
声明权限
或当前无法处理。例如：当模糊位置权限的状态为NOT_DETERMINED时，查询精确位置权限状态，返回此值。

RESTRICTED

3

表示受限。用户未同意隐私声明（仅系统应用会返回此状态）。

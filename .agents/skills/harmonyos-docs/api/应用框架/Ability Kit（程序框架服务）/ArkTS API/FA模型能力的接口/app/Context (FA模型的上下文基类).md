...
**Slug**: `js-apis-inner-app-context`

**DocID**: `769516aa65d743fe87f4a63eadee776b`

**NodeID**: `000201775796216197507e35f4bdf3f0`
...
Promise对象，如果pid和uid具有权限，则使用0进行异步回调；否则使用-1回调。
...
Promise对象，返回当前ability的Bundle名称。
...
回调函数。当设置当前Ability的显示方向成功，err为undefined，否则为错误对象。
...
指定是否在锁屏顶部显示此功能。值true表示在锁屏上显示，值false表示不显示。
...
回调函数。当设置每当显示锁屏时是否在锁屏顶部显示此功能并使该功能保持激活状态的操作成功，err为undefined，否则为错误对象。
...
指定是否唤醒屏幕。值true表示唤醒它，值false表示不唤醒它。
...
回调函数。当设置恢复此功能时是否唤醒屏幕成功，err为undefined，否则为错误对象。
...
回调函数，返回当前ability的ohos.bundleManager.ElementName对象。
...
Promise对象，返回当前ability的ohos.bundleManager.ElementName对象。
...
Promise对象，返回ability调用方的Bundle名称。
...
Promise对象，返回Ability或应用的分布式文件路径。若为首次调用，则将创建目录。
...
Context.isUpdatingConfigurations
7+
...
回调函数，返回true表示该Ability的配置正在更改，否则返回false。
...
Promise对象，返回true表示该Ability的配置正在更改，否则返回false。
...
回调函数。当通知系统绘制此页面功能所需的时间成功，err为undefined，否则为错误对象。

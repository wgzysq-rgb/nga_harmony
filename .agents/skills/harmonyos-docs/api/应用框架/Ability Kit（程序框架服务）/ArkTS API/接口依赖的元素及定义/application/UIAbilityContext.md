...
**Slug**: `js-apis-inner-application-uiabilitycontext`

**DocID**: `0f7eeb34d0754ee491228c8e791077d7`

**NodeID**: `0002017757962161977375d988c27dd8`
...
回调函数，当接口调用成功，err中code为0，message为空字符串；否则err会返回对应的错误码和错误信息。
...
如果被启动的UIAbility是
单实例模式
，且这个UIAbility被不同应用多次调用该接口启动，当这个UIAbility调用
terminateSelfWithResult
...
回调函数，当接口调用成功，err中code为0，data为被拉起方退出时的结果码和数据；否则err会返回对应的错误码和错误信息。
...
回调函数，当销毁UIAbility自身成功，err中code为0；否则err会返回对应的错误码和错误信息。
...
（可选）如果需要在停止UIAbility时，清理任务中心的相关任务（即不保留最近任务列表中的快照），需要在
module.json5
配置文件中将removeMissionAfterTerminate字段取值配置为true。
...
返回给startAbilityForResult 接口调用方的相关信息。
...
返回给startAbilityForResult 接口调用方的信息。
...
，通过返回的proxy与ServiceExtensionAbility进行通信，以使用ServiceExtensionAbility对外提供的能力。仅支持在主线程调用。
...
连接ServiceExtensionAbility的Want信息。
...
断开与
ServiceExtensionAbility
的连接，断开连接之后开发者需要将连接成功时返回的remote对象置空。使用Promise异步回调。仅支持在主线程调用。
...
连接的ServiceExtensionAbility的标识id，即
...
断开与
ServiceExtensionAbility
的连接，断开连接之后开发者需要将连接成功时返回的remote对象置空。使用callback异步回调。仅支持在主线程调用。
...
回调函数。当断开与ServiceExtensionAbility的连接成功，err为undefined；否则为错误对象。
...
通信对象，以便于与
Callee
进行通信。如果指定UIAbility未启动，则会将UIAbility启动至前台或后台。使用Promise异步回调。仅支持在主线程调用。
...
传入需要启动的UIAbility信息，包含abilityName、moduleName、bundleName、deviceId、parameters（可选）。将parameters中的'ohos.aafwk.param.callAbilityToForeground'配置为true可将UIAbility拉起到前台；否则表示将UIAbility拉起到后台。
...
Internal error. Possible causes: 1.Connect to system service failed. 2.Sending restart message to system service failed. 3.System service failed to communicate with dependency module. 4.Non-system applications are only allowed to call this interface across devices, not on the current device.
...
回调函数，当设置UIAbility在多任务界面中显示的名称成功，err中code为0；否则err会返回对应的错误码和错误信息。
...
回调函数，当设置UIAbility任务的流转状态成功，err中code为0；否则err会返回对应的错误码和错误信息。
...
表示是否处于消亡中状态。true表示处于消亡中状态，false表示不处于消亡中状态。
...
setRequestResult
接口返回结果给调用者。使用callback异步回调。仅支持在主线程调用。
...
启动ServiceExtensionAbility的Want信息。
...
回调函数，当启动一个支持模态弹框的ServiceExtensionAbility成功，err中code为0，data为模态弹框请求结果；否则err会返回对应的错误码和错误信息。
...
启动一个支持模态弹框的ServiceExtensionAbility。ServiceExtensionAbility被启动后，应用弹出模态弹框，通过调用
setRequestResult
接口返回结果给调用者。使用Promise异步回调。仅支持在主线程调用。
...
回调函数，当通知系统UIAbility对应的窗口内容已经绘制完成的操作成功，err中code为0；否则err会返回对应的错误码和错误信息。
...
通过type隐式启动
UIExtensionAbility
。使用callback异步回调。仅支持在主线程调用，仅支持处于前台的应用调用。
...
回调函数，当接口调用成功，err为undefined；否则为错误对象。
...
通过type隐式启动
UIExtensionAbility
。使用Promise异步回调。仅支持在主线程调用，仅支持处于前台的应用调用。
...
设备行为差异
：该接口仅在PC/2in1和Tablet设备中可正常调用，在其他设备中返回801错误码。
...
如果不同应用多次调用该接口启动同一个元服务，当这个元服务调用
terminateSelfWithResult
接口销毁自身时，只将正常结果返回给最后一个调用方, 其它调用方返回异常结果，异常结果中resultCode为-1。
...
或
Deep Linking
方式启动UIAbility，并通过回调函数接收被拉起的UIAbility退出时的返回结果。使用Promise异步回调。仅支持在主线程调用。
...
[h2]backToCallerAbilityWithResult
12+
...
或
openLink
拉起目标方UIAbility，且需要目标方返回结果时，目标方可以通过该接口将结果返回并拉起调用方。与
terminateSelfWithResult
...
或
openLink
拉起目标方Ability且需要目标方返回结果时，系统生成的用于标识本次调用的requestCode。该值可以通过want中的
CALLER_REQUEST_CODE
字段获取。
...
[h2]startUIServiceExtensionAbility
14+
...
启动UIServiceExtensionAbility的必要信息。
...
[h2]connectUIServiceExtensionAbility
14+
...
连接UIServiceExtensionAbility的必要信息。
...
Promise<UIServiceProxy>

Promise对象，包含connectUIServiceExtensionAbility执行后返回的
UIServiceProxy
对象。
...
[h2]disconnectUIServiceExtensionAbility
14+
...
在
module.json5
配置文件标签中配置abilitySrcEntryDelegator和abilityStageSrcEntryDelegator。当Module下首个UIAbility冷启动时，系统优先启动abilitySrcEntryDelegator指向的UIAbility。

当UIAbility是通过
startAbilityByCall
启动时，系统会忽略在
module.json5
配置文件标签中配置的abilitySrcEntryDelegator和abilityStageSrcEntryDelegator。
...
- COLOR_MODE_NOT_SET：不设置（跟随系统或应用）
...
[h2]startAppServiceExtensionAbility
20+
...
[h2]stopAppServiceExtensionAbility
20+
...
[h2]connectAppServiceExtensionAbility
20+
...
ConnectOptions类型的回调函数，返回服务连接成功、连接失败、断开的信息。
...
[h2]disconnectAppServiceExtensionAbility
20+
...
[h2]setOnNewWantSkipScenarios
20+

setOnNewWantSkipScenarios(scenarios: number): Promise<void>

在特定场景下拉起UIAbility时，如果不需要触发
onNewWant
生命周期回调，可以通过该接口设置。仅支持在主线程调用。使用Promise异步回调。
...
Internal error. Possible causes: Connection to service failed.
...
[h2]startSelfUIAbilityInCurrentProcess
22+
...
module.json5配置文件
中通过isolationProcess或isolationMode字段来指定进程），均不会生效。
...
Internal error. Connect to system service failed.
...
Want类型参数，传入需要启动的UIAbility的信息，校验bundleName、abilityName。
...
Connect to system server error.

16000063

The target to restart does not belong to the caller or is not a UIAbility.

16000064

Restart too frequently.

16000065

The API can be called only when the ability is focused.
...
setMissionWindowIcon和
setAbilityInstanceInfo
之间不存在调用优先级关系。当多个接口被依次调用时，后一次调用的接口所设置的图标信息将覆盖之前调用接口所设置的内容，最终生效的图标以最后一次调用的接口为准。
...
在应用窗口、任务中心应用卡片、快捷栏窗口快照显示的Ability图标。图标必须为正方形，且大小不能超过128M，否则返回401参数错误。
...
Internal error. 1. Connect to system service failed; 2.System service failed to communicate with dependency module.

16000135

The main window of this ability not exist.

...
**Slug**: `js-apis-inner-ability-dataabilityhelper`

**DocID**: `479007ad8a694b81928bfd382e8428c3`

**NodeID**: `000201775796216197543de0ec5216de`
...
表示文件打开模式，可以设置为‘r’表示只读访问，‘w’表示只写访问，‘rw’表示读写访问等。
...
回调函数。当注册观察者以监听uri指定数据的数据变化通知成功，err为undefined，否则为错误对象。
...
回调函数。当注销观察者以停止监听uri指定数据的数据变化通知成功，err为undefined，否则为错误对象。
...
回调函数，如果数据功能支持uri规范化，则返回规范化uri对象；否则返回null。
...
Promise对象，如果数据功能支持uri规范化，则返回规范化uri对象；否则返回null。
...
回调函数。当通知注册的观察者成功，err为undefined，否则为错误对象。
...
insert(uri: string, valuesBucket: 
rdb.ValuesBucket
...
表示筛选条件。当此参数为null时，自定义查询数据库中数据的处理逻辑。
...
表示待处理的DataAbility。例：'dataability:///com.example.xxx.xxxx'
...
回调函数，在DataAbilityResult数组中返回每个操作的结果。
...
Promise对象，在DataAbilityResult数组中返回每个操作的结果。
...
number | string | boolean | Array<string | number | boolean> | null

...
**Slug**: `js-apis-inner-application-eventhub`

**DocID**: `9e404fd7433c484a8ccdfed8c8c85aa7`

**NodeID**: `0002017757962161977152e61b9c4faf`
...
由于Worker、Taskpool通过Actor模型实现
多线程并发
，不同虚拟机实例之间拥有独占的内存，因此EventHub对象不能用于线程间的数据通信。
...
不支持在Worker、TaskPool线程间通过EventHub对象进行数据通信。如需进行跨线程通信，参考
使用Emitter进行线程间通信
。
...
转换后的Context对象与原先的Context对象属于不同Context对象，不支持其EventHub对象间的数据通信。

...
**Slug**: `errorcode-datashare`

**DocID**: `2d70312611a94b839e4e9bc34775aa5c`

**NodeID**: `0002017757962161962965a3ef1d7324`
...
proxyData的value长度超过4096字节。

接口传入的URI数组长度超过32。

接口传入的proxyData数组长度超过32。

URI格式校验失败。

处理步骤

检查接口传入的URI数组或者proxyData数组中是否有URI的长度超过256字节。

检查接口传入的proxyData数组中是否有proxyData的value长度超过4096字节。

检查接口传入的URI数组的长度是否超过32。

检查接口传入的proxyData数组的长度是否超过32。

检查接口传入的URI数组或者proxyData数组中的URI是否满足格式校验：

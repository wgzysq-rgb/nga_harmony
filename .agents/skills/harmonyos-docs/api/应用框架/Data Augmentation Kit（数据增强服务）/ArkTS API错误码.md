...
> **分区**: API参考  |  **Slug**: `dataaugmentation-error-code`  |  **DocID**: `1f8fbdaa96e5448a9a80e4e586bf1b3e`
...
# ArkTS API错误码
     ...
     **处理步骤**
             ...
             如果依然无法解决，可以提示用户重启应用、升级应用或升级设备版本。
     ...
     **处理步骤**
             ...
             请开发者重新调用[streamRun](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#streamrun)。

             多次重试均失败则需请开发者优化[streamChat](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#streamchat)中使用的大语言模型及相应实现。
     ...
     **可能原因**
     ...
     开发者在[streamChat](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#streamchat)中的某一次大语言模型请求时，累计转发给rag的响应数据长度超出范围8192字节。
     ...
     **可能原因**
     ...
     [RagSession](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#ragsession)未关闭，重复创建[RagSession](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#ragsession)。
     ...
     **可能原因**
     ...
     前一次[streamRun](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#streamrun)回答期间重新调用了[streamRun](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#streamrun)开启新一轮问答。
     ...
     **可能原因**
     ...
     调用[RagSession](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#ragsession)的接口时，[RagSession](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#ragsession)已关闭。
     ...
     **错误描述**
     ...
     用户已取消[streamRun](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#streamrun)。

     **可能原因**

     调用[streamRun](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#streamrun)接口过程中，用户主动取消了本次[streamRun](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#streamrun)。
     ...
     **可能原因**
     ...
     开发者调用[createRagSession](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#createragsession)时传入的[Config](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#config)不满足约束条件。
     ...
     **处理步骤**
             ...
             使用rag能力前请确认已配置[知识加工](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/data-augmentation-knowledge-processing)schema。

             检查schema配置文件格式及内容，确保符合相应约束。
     ...
     **可能原因**
             ...
             在检索模块的Recall阶段发生错误，比如服务内部状态异常，或者输入参数有问题。

             在检索模块的Re-ranking阶段发生错误，比如服务端状态异常或排序逻辑错误。
     ...
     **处理步骤**
             ...
             确保服务端状态正常并且已经完成相关初始化工作。

             检查服务的内部逻辑，确保排序方法符合预期。
     ...
     **处理步骤**
             ...
             修正[Config](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-rag-api#config)后重试。
     ...
     **可能原因**
             ...
             对于“between”类型，没有定义“range”字段。

             对于非“between”类型，没有定义“value”字段。
     ...
     **处理步骤**
             ...
             检查每个RecallCondition中的recallName字段名称，确保其合法性。
     ...
     **可能原因**
             ...
             执行当前操作时，已调用过close接口关闭数据库或者开库未成功。

             执行当前操作时，已调用过commit接口提交事务或者调用过rollback接口回滚事务。
     ...
     **可能原因**
     ...
     执行sql语句过程中出现错误，如：
             ...
             查询时指定了未曾有的列，或指定了不存在的表。

             调用未定义的函数等，参见SQLITE_ERROR的相关错误场景。
     ...
     **可能原因**
             ...
             操作系统级别的权限问题，意味着SQLite试图访问或修改一个文件，但是没有足够的权限去执行这个操作。

             参见SQLITE_PERM的相关错误场景。

     
     **处理步骤**

     
             确认文件没有只读属性，如果有，去掉只读属性。

             检查文件和文件夹的权限，确保当前用户有足够的权限来读写文件。

             检查文件系统是否为只读，如果是，改为可写状态。

             确认没有其他进程锁定数据库文件，如果有，关闭占用文件的进程。

             在处理权限问题时，确保有足够的权限去更改相关的文件或文件夹权限。
     ...
     **可能原因**
             ...
             同一应用两个进程，例如UIability和datashareability同时打开了同一个数据库，进行增删改操作，或者不同应用的同一个group组内的进程通过group组打开同一个数据库，进行增删改操作。

             参见SQLITE_BUSY的相关错误场景。
     ...
     **处理步骤**
             ...
             检查磁盘空间是否足够，并清理不必要的文件释放空间。

             检查文件的权限，确保应用程序有足够的权限去读写文件。
     ...
     **可能原因**
             ...
             参见SQLITE_CANTOPEN的相关错误场景

     
     **处理步骤**

     
             确认数据库文件路径是否正确，检查文件权限，确保应用程序有足够的权限去读写文件。
     ...
     **可能原因**
             ...
             在多个Recall过程中，recall的参数中使用的主键个数不一致。

             recall接口的参数中，主键的个数为空。

             recall接口的参数中，主键为空字符串。
     ...
     **可能原因**
             ...
             缺失必要字段（没有field&没有fields；没有value且没有range；没有op）。

             对于between，没有定义range。

             对于非between，没有定义value。
     ...
     **错误描述**
     ...
     RecallCondition中的recallName字段相同。
     ...
     **可能原因**
             ...
             当前版本不支持自动生成query的向量，需要直接导入向量数组。
     ...
     **处理步骤**
      ...
      - 参照[知识源配置](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/dataaugmentation-knowledgeprocessor-api#knowledgesourceconfig)类型，正确配置rdbSource字段。
     ...
     **处理步骤**
      ...
      - 参考开发步骤中的[知识schema配置示例](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/data-augmentation-knowledge-processing#开发步骤)，将knowledge_schema.json放到指定的entry/src/main/resources/rawfile/arkdata/knowledge目录下。
     ...
     **可能原因**
      ...
      - 知识schema中的内容不符合当前的规格及约束限制。
     
     **处理步骤**

     
      - 参#fig照[知识schema规格及约束限制](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/data-augmentation-knowledge-processing#约束限制)修改其中内容。
     ...
     **可能原因**
      ...
      - RDB数据源异常导致操作RDB数据源时发生了错误。
     
     **处理步骤**

     
      - 尝试重试，如果依然无法解决，可以提示用户重启应用、升级应用或升级设备版
     
                  #### 1021400005 重复启动知识加工

     **错误信息**

     Feature already active. Function called repeatedly.
     ...
     **可能原因**
             ...
             输入的问题内容过长导致端侧问答模型处理过久。

             系统繁忙，硬件资源紧张，端侧问答模型处理过慢。

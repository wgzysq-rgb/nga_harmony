...
**DocID**: `becfdad5eba74f00ae6289ad9b6eb052`

**NodeID**: `000201775796216196273cd730427166`
...
表示LLM流式输出过程中出现的错误。
code
取值范围为[1021011000, 1021012000)，超过范围则会报错
1021000000
...
表示用于接受回答的回调。
注意：
streamRun的完整生命周期包含完整的callback执行。结束的callback回调执行过程中，streamRun的生命周期仍然在持续，在此时进行新的streamRun或者cancel/close操作，都可能产生针对streamRun仍然在进行中的对应错误。

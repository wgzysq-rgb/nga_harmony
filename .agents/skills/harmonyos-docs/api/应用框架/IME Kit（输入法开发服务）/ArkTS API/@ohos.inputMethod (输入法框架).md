...
**Slug**: `js-apis-inputmethod`

**DocID**: `c618160f12ad42c291a492228add5315`

**NodeID**: `000201775796216196381f0ba74ab619`
...
- 当InputMethodProperty作为查询接口的返回值时（如
getCurrentInputMethod
），此字段表示输入法扩展对外显示的名称，优先使用InputMethodExtensionAbility中配置的label，若未配置，自动使用应用入口ability的label；当应用入口ability未配置label时，自动使用应用AppScope中配置的label。
...
Promise对象。返回true表示当前输入法切换子类型成功，返回false表示当前输入法切换子类型成功失败。
...
inputMethod.switchCurrentInputMethodAndSubtype
9+
...
Promise对象。返回true表示切换至指定输入法的指定子类型成功，返回false表示切换至指定输入法的指定子类型失败。
...
inputMethod.setSimpleKeyboardEnabled
20+
...
inputMethod.onAttachmentDidFail
22+
...
回调函数，返回绑定失败的原因，仅当注册者进程触发的绑定失败时，调用该回调函数。
...
inputMethod.offAttachmentDidFail
22+
...
取消订阅的回调函数，需要与订阅接口传入的保持一致。参数不填写时，取消订阅该事件的所有回调函数。
...
回调函数。当绑定输入法成功后，err为undefined；否则为错误对象。
...
回调函数。若成功进入编辑状态，err为undefined；否则为错误对象。
...
回调函数。当成功退出编辑状态时，err为undefined；否则为错误对象。
...
回调函数。当解绑定输入法成功时，err为undefined；否则为错误对象。
...
回调函数。当设置成功时，err为undefined；否则为错误对象。
...
回调函数。当光标信息更新成功时，err为undefined；否则为错误对象。
...
回调函数。当文本信息更新成功时，err为undefined；否则为错误对象。
...
回调函数。当编辑框属性信息更新成功时，err为undefined；否则为错误对象。
...
需要权限：
 ohos.permission.CONNECT_IME_ABILITY，��系统应用可用。
...
回调函数。当会话结束成功，err为undefined，data为true；否则为错误对象。
...
Promise对象。返回true表示会话结束成功；返回false表示会话结束失败。
...
设置监听类型，固定取值为'sendKeyboardStatus'。
...
设置监听类型，固定取值为'handleExtendAction'。
...
设置监听类型，固定取值为'getLeftTextOfCursor'。
...
回调函数，获取编辑框最新状态下光标左侧指定长度的文本内容并返回。
...
设置监听类型，固定取值为'getRightTextOfCursor'。
...
回调函数，获取编辑框最新状态下光标右侧指定长度的文本内容并返回。
...
设置监听类型，固定取值为'getTextIndexAtCursor'。
...
设置监听类型，固定取值为'finishTextPreview'。
...
取消订阅输入法及��类型变化监听事件。使用callback异步回调。
...
回调函数，返回取消订阅的输入法属性对象及子类型对象。

示例：

inputMethod.getSetting().off('imeChange');

[h2]listInputMethodSubtype
9+
...
[h2]listCurrentInputMethodSubtype
9+
...
Promise对象。当输入法选择对话框显示成功，err为undefined，data为true；否则为错误对象。
...
Promise对象，返回EnabledState.DISABLED表示未启用; 返回EnabledState.BASIC_MODE表示基础模式; 返回EnabledState.FULL_EXPERIENCE_MODE表示完整体验模式。

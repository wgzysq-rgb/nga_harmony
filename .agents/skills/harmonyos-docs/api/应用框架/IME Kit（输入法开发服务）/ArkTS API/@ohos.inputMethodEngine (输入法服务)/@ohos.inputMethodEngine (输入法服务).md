...
**Slug**: `js-apis-inputmethodengine`

**DocID**: `96a8cfaebe294eef9a5cb227d4def5ef`

**NodeID**: `00020177579621619640062a43e9ea2d`
...
从API version 8开始支持，API version 9开始废弃，建议使用
...
[h2]on('keyboardShow'|'keyboardHide')
...
[h2]off('keyboardShow'|'keyboardHide')
...
设置监听类型，固定取值为'securityModeChange'。
...
设置监听类型，固定取值为'callingDisplayDidChange'。
...
[h2]off('callingDisplayDidChange')
18+
...
输入法面板不支持创建子窗口。例如：不支持使用
window.createWindow
、
bindContextMenu
、
CustomDialog
等接口创建子窗口弹窗。建议开发者采用非子窗的替代方案，如
弹出框
、
bindMenu
...
回调函数。当输入法面板创建成功，返回当前创建的输入法面板对象。
...
回调函数。当输入法面板销毁成功，err为undefined，否则为错误对象。
...
回调函数，返回按键信息。 若按键事件被事件订阅者消费，则callback应返回true，否则返回false。
...
光标变化事件，固定取值为'cursorContextChange'。
...
文本选择变化事件，固定取值为'selectionChange'。
...
文本变化事件，固定取值为'editorAttributeChanged'。
...
所要取消订阅的回调处理函数。参数不填写时，默认取消订阅type对应的所有回调事件。
...
回调函数。当面板页面内容加载成功，err为undefined，否则err为错误对象。
...
为当前的输入法面板加载具体页面内容，使用Promise异步��调。
...
目标面板的宽度，单位为px。该参数应为大于或等于0的整数，不超出屏幕宽度。
...
目标面板的高度，单位为px。该参数应为大于或等于0的整数，不高于屏幕高度的0.7倍。
...
回调函数。当面板大小改变成功，err为undefined，否则err为错误对象。
...
横轴方向移动的值，值大于0表示右移，单位为px。该参数应为整数。
...
纵轴方向移动的值，值大于0表示下移，单位为px。该参数应为整数。
...
回调函数。当面板位置移动成功，err为undefined，否则err为错误对象。
...
input method engine error. Possible causes: 1.input method panel not created. 2.the input method application does not subscribe to related events.
...
回调函数。当面板显示成功，err为undefined，否则err为错误对象。
...
回调函数。当面板隐藏成功，err为undefined，否则err为错误对象。
...
目标面板状态类型。类型为FLG_FIXED或FLG_FLOATING。
...
目标面板横屏状态及竖屏状态的横坐标，纵坐标，宽度以及高度。固定态：高度不能超过屏幕高度的70%，宽度不能超过屏幕宽度；悬浮态：高度不能超过屏幕高度，宽度不能超过屏幕宽度。
...
adjustPanelRect
的调用方法，若入参rect仅填写属性landscapeRect和portraitRect，则默认调用
adjustPanelRect
。
...
从API version 12-14开始支持，此接口回调函数中仅包含
window.Size
类型的必选参数。
...
监听当前面板的大小是否产生变化，固定值为'sizeChange'。
...
监听当前面板的大小是否产生变化，固定取值为'sizeChange'。
...
invalid immersive effect. 1.The gradient mode and the fluid light mode can only be used when the immersive mode is enabled. 2.The fluid light mode can only be used when the gradient mode is enabled. 3.When the gradient mode is not enabled, the gradient height can only be 0.
...
是否设置屏幕常亮。true表示打开屏幕常亮，false表示关闭屏幕常亮。
...
[h2]getSystemPanelCurrentInsets
21+
...
invalid panel type or panel flag. Possible causes: 1. Current panel's type is not SOFT_KEYBOARD. 2. Panel's flag is not FLG_FIXED or FLG_FLOATING.
...
功能键的颜色，取值范围为[#01000000, #FFFFFFFF] 或 [#000000, #FFFFFF]，不支持具有完全透明Alpha通道（#00xxxxxx）的值。
...
功能键的背景颜色，取值范围为[#01000000, #FFFFFFFF] 或 [#000000, #FFFFFF]，不支持具有完全透明Alpha通道（#00xxxxxx）的值。
...
input method client error. Possible causes: 1.the edit box is not focused. 2.no edit box is bound to current input method application. 3.ipc failed due to the large amount of data transferred or other reasons.
...
回调函数。当退出当前输入类型成功，err为undefined，否则为错误对象。
...
此对象全局唯一，多次注册仅保留最后一次注册的对象及有效性，并触发上一个已注册对象的
onTerminated
回调函数。

若取消注册全局已注册的对象时，会触发被取消对象中
onTerminated
回调函数。
...
当已注册的
MessageHandler
接收到来自已绑定当前输入法应用的编辑框应用所发送的自定义通信数据时，会触发该回调函数。
...
回调函数。当成功发送选中事件后，err为undefined，否则为错误对象。
...
回调函数。当文本索引获取成功，err为undefined，否则为错误对象。
...
回调函数。发送成功，err为undefined，否则为错误对象。
...
需要发送至已绑定当前输入法应用的编辑框应用的自定义数据的标识符。
...
需要发送至已绑定当前输入法应用的编辑框应用的自定义数据的消息体。
...
parameter error. Possible causes: 1. Incorrect parameter types. 2. Incorrect parameter length.
...
MessageHandler
对象全局唯一，多次注册仅保留最后一次注册的对象及有效性，并触发上一个已注册对象的
onTerminated
回调函数。
...
该对象将通过
onMessage
接收来自已绑定当前输入法应用的编辑框应用所发送的自定义通信数据，并通过
onTerminated
接收终止此对象订阅的消息。
...
绑定输入法时的附加选项变更事件，固定取值为'attachOptionsDidChange'。
...
取消订阅的回调函数。参数不填写时，默认取消订阅type对应的所有回调事件。

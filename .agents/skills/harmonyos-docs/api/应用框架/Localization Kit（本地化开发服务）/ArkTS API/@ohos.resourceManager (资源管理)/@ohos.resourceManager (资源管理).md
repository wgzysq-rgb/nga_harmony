...
**DocID**: `1255db4b95214ce9825639b3d311ddc5`

**NodeID**: `000201775796216196330ea1e02f1c15`
...
回调函数，返回应用包名对应的资源管理ResourceManager对象。
...
Promise对象，返回资源管理ResourceManager对象。
...
Promise对象，返回应用包名对应的资源管理ResourceManager对象。
...
resourceManager.getSysResourceManager
20+
...
Failed to access the system resource. which is not mapped to application sandbox, This error code will be thrown.
...
If the input parameter invalid. Possible causes: Incorrect parameter types.
...
[h2]getIntPluralStringValueSync
18+
...
获取指定资源ID对应的
单复数
字符串，��根据args参数对字符串进行格式化，使用同步方式返回。
...
[h2]getIntPluralStringByNameSync
18+
...
获取指定资源名称对应的
单复数
字符串，并根据args参数对字符串进行格式化，使用同步方式返回。
...
[h2]getDoublePluralStringValueSync
18+
...
[h2]getDoublePluralStringByNameSync
18+
...
Promise对象，返回资源ID值对应的图片资源Base64编码。
...
Promise对象，返回资源名称对应的图片资源Base64编码。
...
[h2]getDrawableDescriptorByName
10+
...
Promise对象，返回rawfile文件目录下的文件夹及文件列表。
...
回调函数，返回的rawfile文件所在HAP的文件描述符（fd）。
...
Promise对象，返回rawfile文件所在HAP的文件描述符（fd）。
...
回调函数。当关闭rawfile所在HAP的文件描述符（fd）成功，err为undefined，否则为错误对象。
...
Promise对象，返回设备的DeviceCapability。
...
返回获取的语言列表，列表中的字符串由语言、脚本（可选）、地区（可选），按照顺序使用中划线“-”连接组成。
...
[h2]updateOverrideConfiguration
12+
...
getOverrideResourceManager
接口获取的差异化资源管理对象调用该方法均可更新差异化资源管理对象的配置。
...
Promise对象，返回resource对象对应的字符串数组。
...
Promise对象，返回resource对象对应的媒体文件内容。
...
回调函数，返回resource对象对应的图片资源Base64编码。
...
Promise对象，返回resource对象对应的图片资源Base64编码。
...
从API version 18开始支持，从API version 20开始废弃，建议使用
...
数量值。根据当前语言的复数规则获取该数量值对应的字符串数字，语言的复数规则参见
语言单复数规则
。
...
Promise对象，返回资源ID值对应的指定数量的单复数字符串。
...
回调函数，返回resource对象对应的指定数量的单复数字符串。
...
Promise对象，返回resource对象对应的指定数量的单复数字符串。
...
Promise对象，返回resource对象对应的颜色值（十进制）。
...
resource对象对应的Symbol字符Unicode码（十进制）。
...
Promise对象，返回rawfile文件的文件描述符（fd）。
...
回调函数。当关闭rawfile文件的文件描述符（fd）成功，err为undefined，否则为错误对象。

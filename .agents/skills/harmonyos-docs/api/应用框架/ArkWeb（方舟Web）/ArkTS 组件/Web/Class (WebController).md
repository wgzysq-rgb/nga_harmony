...
**Slug**: `arkts-basic-components-web-webcontroller`

**DocID**: `c412fe753305485985c80972e4d82bc0`

**NodeID**: `000201775796216197223dd734659bb8`
...
返回true表示当前页面可以前进，返回false表示当前页面不可以前进。
...
指定的一个URL路径（“http”/“https”/"data"协议），并由Web组件赋值给window.origin。默认值为空字符串。
...
历史记录URL。默认值为空字符串。非空时，可被历史记录管理，实现前进后退功能。当baseUrl为空时，此属性无效。
...
基于当前网页所需调整的相对缩放比例，当入参为1时为默认加载网页的缩放比例，小于1为缩小，大于1为放大。取值范围(0, 100]。
...
回调执行JavaScript脚本结果。JavaScript脚本若执行失败或无返回值时，返回null。不传入时不进行回调。

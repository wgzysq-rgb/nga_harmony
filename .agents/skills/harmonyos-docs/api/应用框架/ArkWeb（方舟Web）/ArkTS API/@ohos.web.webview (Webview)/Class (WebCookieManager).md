...
**Slug**: `arkts-apis-webview-webcookiemanager`

**DocID**: `9e9e59c7ef044c78bc7cf7541e619ecf`

**NodeID**: `0002017757962161971005feb4441c3f`

---

Class (WebCookieManager)

通过WebCookie可以控制Web组件中的cookie的各种行为，其中每个应用中的所有Web组件共享一个WebCookieManager实例。cookie的格式遵循
RFC6265
标准。当前WebCookieManager的获取cookie接口不支持partitioned cookie。使用隐私模式浏览网页时，Cookie、缓存等数据不会写入本地持久化存储；隐私模式的Web组件销毁后，这些数据将被清除，不会保留。
...
URL error. No valid cookie found for the specified URL.
...
true表示允许覆盖含有http-only的cookies，false表示不允许覆盖含有http-only的cookies。
...
Promise实例，用于获取指定url设置单个cookie值是否成功。
...
设置是否拥有发送和接收cookie的权限，默认为true，表示拥有发送和接收cookie的权限。false表示没有发送和接收cookie的权限。
...
true表示存在cookie，false表示不存在cookie。
...
callback回调，用于获取清除所有会话cookie是否成功。
...
Promise实例，用于获取清除所有会话cookie是否成功。

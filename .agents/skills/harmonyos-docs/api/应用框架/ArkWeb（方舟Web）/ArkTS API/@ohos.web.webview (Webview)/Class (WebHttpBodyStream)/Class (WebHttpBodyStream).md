...
**Slug**: `arkts-apis-webview-webhttpbodystream`

**DocID**: `31c44c525ec24cf2a855abff78bab94c`

**NodeID**: `000201775796216197160b67ff29e5b0`

---

Class (WebHttpBodyStream)

POST、PUT请求的数据体，支持BYTES、FILE、BLOB、CHUNKED类型的数据。注意本类中其他接口需要在
initialize
成功后才能调用。
...
Promise实例，用于获取WebHttpBodyStream是否初始化成功。
...
读取WebHttpBodyStream中的字节数。单位：字节。
...
Promise实例，用于获取WebHttpBodyStream中读取的数据。
...
获取WebHttpBodyStream中的数据大小。单位：字节。
...
WebHttpBodyStream中当前的读取位置。单位：字节。
...
WebHttpBodyStream是否采用分块传输，如果采用分块传输则返回true，否则返回false。

...
**Slug**: `errorcode-webview`

**DocID**: `07c23b5820af426ca87fa3fa12774d8c`

**NodeID**: `000201775796216196279dcb62dd2cac`
...
请检查WebviewController对象是否已与Web组件关联，可以通过
onControllerAttached()
接口进行检查。详情可以参考
Web组件调用WebviewController相关接口报17100001错误定位指南
。
...
通过runJavaScript调用H5侧的方法，这个方法返回了空的ArrayBuffer。
...
使用runJavaScriptExt接口替代runJavaScript接口。
...
请确认本端是否设置了port的onMessageEvent回调。
...
如果已经使用，检查调用失败原因，如databaseAccess开关是否打开。
...
使用的错误码不在
WebNetErrorList
范围内，或者调用didFail接口时使用了NET_OK错误码。

处理步骤

检查使用的错误码是否在
WebNetErrorList
范围内，或者调用didFail时是否使用了NET_OK错误码。

---
*2026-04-22T15:55:02.438Z*
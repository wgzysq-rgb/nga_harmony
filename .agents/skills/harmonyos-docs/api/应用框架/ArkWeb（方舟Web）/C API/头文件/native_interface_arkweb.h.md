...
**DocID**: `eb15ef64cbb74fa0956a9f07cda1c582`

**NodeID**: `000201775796216197845fab992dd513`
...
设置ArkWeb内核版本。若系统不支持指定版本，则设置无效。该接口为全局静态方法，须在调用initializeWebEngine前执行，若已加载任何Web组件，则该设置无效。
...
当前系统的最新Web内核，系统基于此内核进行完整的功能实现，推荐应用使用。

遗留内核

LEGACY WebCore

复用上一版本的内核，只做安全补丁及舆情问题修复，仅作为兼容性回滚使用，且遗留内核的支持有时间限制。
...
系统默认内核，HarmonyOS 6.0版本默认为M132。

ARKWEB_M114 = 1

HarmonyOS 6.0版本的遗留内核。开发者可选择此遗留内核，若系统版本上不存在此内核则设置无效。

ARKWEB_M132 = 2

HarmonyOS 6.0版本的常青内核，M132为此版本的默认内核。若系统版本上不存在此内核则设置无效。

函数说明

[h2]NativeArkWeb_OnJavaScriptCallback()
...
[h2]NativeArkWeb_OnJavaScriptProxyCallback()
...
[h2]NativeArkWeb_OnValidCallback()
...
[h2]NativeArkWeb_OnDestroyCallback()
...
[h2]OH_ArkWeb_OnCookieSaveCallback()
...
ARKWEB_COOKIE_SAVE_FAILED
 保存cookie失败。

ARKWEB_COOKIE_MANAGER_INITIALIZE_FAILED
 CookieManager初始化失败。

[h2]OH_NativeArkWeb_RunJavaScript()
...
是否需要刷新页面。true：刷新页面，false：不刷新页面。

[h2]OH_NativeArkWeb_UnregisterJavaScriptProxy()
...
[h2]OH_NativeArkWeb_SetJavaScriptProxyValidCallback()
...
[h2]OH_NativeArkWeb_GetJavaScriptProxyValidCallback()
...
已注册的对象可注册时的回调函数。如果未设置由参数webTag指定的有效回调函数，则将返回空指针。

[h2]OH_NativeArkWeb_SetDestroyCallback()
...
[h2]OH_NativeArkWeb_GetDestroyCallback()
...
return 已注册的组件销毁时的回调函数。如果未设置由参数webTag指定的销毁回调函数，则将返回空指针。
...
"Base64"或"URL"编码的字符串，不能为空。

const char* mimeType

媒体类型，例如"text/html"，不能为空。
...
指定的URL路径("http"/"https"/"data"协议),由Web组件分配给window.origin。

const char* historyUrl

历史URL，当它不为空时，可以通过历史记录来管理，实现前进和后退功能。
...
ARKWEB_INVALID_PARAM
 必填参数未指定或参数类型不正确或参数校验失败。

ARKWEB_INIT_ERROR
 初始化失败，根据传入的"webTag"找不到有效的Web组件。

ARKWEB_LIBRARY_OPEN_FAILURE
 打开动态链接库失败。

ARKWEB_LIBRARY_SYMBOL_NOT_FOUND
 动态链接库中未找到所需的符号。

[h2]OH_NativeArkWeb_RegisterAsyncThreadJavaScriptProxy()
...
json格式字符串，默认值为空。该字符串用来配置JSBridge的权限限制，可以配置对象和方法级别。

[h2]OH_ArkWebCookieManager_SaveCookieSync()
...
[h2]OH_NativeArkWeb_SetBlanklessLoadingWithKey()
...
非法值设置行为：key长度超过2048时该key不生效；长度超过100时，取前100个；当为NULL时，使用默认值。
...
合法取值范围：0~100。取值超过100时，keys数组取前100个。

非法值设置行为：0。

[h2]OH_NativeArkWeb_SetBlanklessLoadingCacheCapacity()
...
ArkWebEngineVersion webEngineVersion

ArkWeb内核版本（详细说明见
ArkWebEngineVersion
）。

[h2]OH_NativeArkWeb_GetActiveWebEngineVersion()
...
[h2]OH_NativeArkWeb_LazyInitializeWebEngineInCookieManager()
...
该接口仅适用于调用后会初始化CookieManager的接口，比如
...
是否延后初始化ArkWeb内核，true：延后，false：不延后。

---
*2026-04-22T15:55:02.420Z*
...
**Slug**: `capi-arkweb-scheme-handler-h`

**DocID**: `ce5a153567694eeab493a8cb05ffcaf8`

**NodeID**: `000201775796216196718fc53290449a`
...
int32_t OH_ArkWebHttpBodyStream_SetAsyncReadCallback(ArkWeb_HttpBodyStream* httpBodyStream,ArkWeb_HttpBodyStreamReadCallback readCallback)
...
将请求的上传数据读取到buffer。buffer的大小必须大于bufLen。数据将从工作线程读取数据到buffer，因此在回调函数返回之前，不应在其他线程中使用buffer，以避免并发问题。
...
通知ArkWeb内核，被拦截的请求应该失败。对比OH_ArkWebResourceHandler_DidFailWithError接口，新增参数completeIfNoResponse，当值为true时，若之前未调用过OH_ArkWebResourceHandler_DidReceiveResponse，则会自动生成一个response以完成此次网络请求，网络错误码为-104；值为false时，将等待应用调用OH_ArkWebResourceHandler_DidReceiveResponse并传入response，不会直接完成此次网络请求。
...
请求的ArkWeb_ResourceHandler。如果intercept设置为false，则不应使用它。

bool* intercept

如果为true，则会拦截请求；如果为false，则不会拦截。

[h2]ArkWeb_OnRequestStop()
...
[h2]ArkWeb_HttpBodyStreamReadCallback()
...
OH_ArkWebHttpBodyStream_Read后的回调函数。如果bytesRead大于0，则表示buffer已填充了bytesRead大小的数据。调用者可以从buffer中读取数据，如果OH_ArkWebHttpBodyStream_IsEOF为false，则调用者可以继续读取剩余的数据。
...
标识异步读取操作执行结果的字节计数值。如果bytesRead大于0，则表示buffer已填充了bytesRead大小的数据。开发者可以从buffer中读取数据，如果OH_ArkWebHttpBodyStream_IsEOF为false，则开发者可以继续读取剩余的数据。
...
成功时返回ARKWEB_NET_OK，否则请参考
arkweb_net_error_list.h
。

[h2]OH_ArkWebRequestHeaderList_Destroy()
...
将被销毁的ArkWeb_RequestHeaderList。

[h2]OH_ArkWebRequestHeaderList_GetSize()
...
[h2]OH_ArkWebRequestHeaderList_GetHeader()
...
const ArkWeb_RequestHeaderList* requestHeaderList
...
请求头的键（key）。调用者必须使用OH_ArkWeb_ReleaseString函数来释放这个字符串。

char** value

请求头的值（value）。调用者必须使用OH_ArkWeb_ReleaseString函数来释放这个字符串。

[h2]OH_ArkWebResourceRequest_SetUserData()
...
[h2]OH_ArkWebResourceRequest_GetUserData()
...
[h2]OH_ArkWebResourceRequest_GetMethod()
...
HTTP请求方法。此函数将为method字符串分配内存，调用者必须使用OH_ArkWeb_ReleaseString释放字符串。

[h2]OH_ArkWebResourceRequest_GetUrl()
...
请求的URL。此函数将为URL字符串分配内存，调用者必须通过OH_ArkWeb_ReleaseString释放该字符串。

[h2]OH_ArkWebResourceRequest_GetHttpBodyStream()
...
请求的上传数据。此函数将为httpBodyStream分配内存，调用者必须使用OH_ArkWebResourceRequest_DestroyHttpBodyStream释放httpBodyStream。

[h2]OH_ArkWebResourceRequest_DestroyHttpBodyStream()
...
[h2]OH_ArkWebResourceRequest_GetResourceType()
...
[h2]OH_ArkWebResourceRequest_GetFrameUrl()
...
触发此请求的Frame的URL。此函数将为URL字符串分配内存，并且调用者必须通过OH_ArkWeb_ReleaseString来释放该字符串。

[h2]OH_ArkWebHttpBodyStream_SetUserData()
...
[h2]OH_ArkWebHttpBodyStream_GetUserData()
...
[h2]OH_ArkWebHttpBodyStream_SetReadCallback()
...
OH_ArkWebHttpBodyStream_Read的回调函数。
...
[h2]OH_ArkWebHttpBodyStream_SetAsyncReadCallback()
...
OH_ArkWebHttpBodyStream_AsyncRead的回调函数。
...
返回0，表示成功；返回17100101，表示参数无效。

[h2]OH_ArkWebHttpBodyStream_Init()
...
[h2]OH_ArkWebHttpBodyStream_Read()
...
[h2]OH_ArkWebHttpBodyStream_AsyncRead()
...
[h2]OH_ArkWebHttpBodyStream_GetPosition()
...
[h2]OH_ArkWebHttpBodyStream_IsChunked()
...
[h2]OH_ArkWebHttpBodyStream_IsInMemory()
...
[h2]OH_ArkWebResourceRequest_Destroy()
...
[h2]OH_ArkWebResourceRequest_GetReferrer()
...
请求的Referrer。此函数将为referrer字符串分配内存，调用者必须使用 OH_ArkWeb_ReleaseString 释放该字符串。

[h2]OH_ArkWebResourceRequest_GetRequestHeaders()
...
[h2]OH_ArkWebResourceRequest_IsRedirect()
...
[h2]OH_ArkWebResourceRequest_IsMainFrame()
...
[h2]OH_ArkWebResourceRequest_HasGesture()
...
[h2]OH_ArkWeb_RegisterCustomSchemes()
...
[h2]OH_ArkWebServiceWorker_SetSchemeHandler()
...
该scheme的拦截器ArkWeb_SchemeHandler。只有通过ServiceWorker触发的请求才会通过这个schemeHandler进行通知。
...
Web组件的标签名称，用于标识某个唯一组件，由开发者来保证名称唯一性。

ArkWeb_SchemeHandler
* schemeHandler

该scheme的拦截器ArkWeb_SchemeHandler。只有从指定web触发的请求才会通过这个schemeHandler进行通知。
...
[h2]OH_ArkWebServiceWorker_ClearSchemeHandlers()
...
[h2]OH_ArkWeb_ClearSchemeHandlers()
...
[h2]OH_ArkWeb_CreateSchemeHandler()
...
返回创建的ArkWeb_SchemeHandler。在不需要时使用OH_ArkWeb_DestroyschemeHandler销毁它。

[h2]OH_ArkWeb_DestroySchemeHandler()
...
待销毁的ArkWeb_SchemeHandler。

[h2]OH_ArkWebSchemeHandler_SetUserData()
...
[h2]OH_ArkWebSchemeHandler_GetUserData()
...
[h2]OH_ArkWebSchemeHandler_SetOnRequestStart()
...
[h2]OH_ArkWebSchemeHandler_SetOnRequestStop()
...
返回创建的ArkWeb_Response。在不需要时使用OH_ArkWeb_DestroyResponse进行销毁。

[h2]OH_ArkWeb_DestroyResponse()
...
[h2]OH_ArkWebResponse_SetStatus()
...
[h2]OH_ArkWebResponse_GetStatus()
...
[h2]OH_ArkWebResponse_SetStatusText()
...
[h2]OH_ArkWebResponse_GetStatusText()
...
返回ArkWeb_Response的状态文本。此函数将为statusText字符串分配内存，调用方必须通过OH_ArkWeb_ReleaseString释放该字符串。

[h2]OH_ArkWebResponse_SetMimeType()
...
[h2]OH_ArkWebResponse_GetMimeType()
...
返回ArkWeb_Response的媒体类型。此函数将为mimeType字符串分配内存，调用方必须通过OH_ArkWeb_ReleaseString释放该字符串。

[h2]OH_ArkWebResponse_SetCharset()
...
[h2]OH_ArkWebResponse_GetCharset()
...
返回ArkWeb_Response的字符集。此函数将为charset字符串分配内存，调用方必须通过OH_ArkWeb_ReleaseString释放字符串。

[h2]OH_ArkWebResponse_SetHeaderByName()
...
[h2]OH_ArkWebResponse_GetHeaderByName()
...
返回header的值。此函数将为value字符串分配内存，调用方必须通过OH_ArkWeb_ReleaseString释放该字符串。

[h2]OH_ArkWebResourceHandler_Destroy()
...
[h2]OH_ArkWebResourceHandler_DidReceiveResponse()
...
该请求的ArkWeb_ResourceHandler。

const 
ArkWeb_Response
* response

该拦截请求的ArkWeb_Response。
...
[h2]OH_ArkWebResourceHandler_DidReceiveData()
...
[h2]OH_ArkWebResourceHandler_DidFinish()
...
[h2]OH_ArkWebResourceHandler_DidFailWithError()
...
[h2]OH_ArkWebResourceHandler_DidFailWithErrorV2()

int32_t OH_ArkWebResourceHandler_DidFailWithErrorV2(const ArkWeb_ResourceHandler* resourceHandler,ArkWeb_NetError errorCode,bool completeIfNoResponse)

描述：

通知ArkWeb内核，被拦截的请求应该失败。对比
OH_ArkWebResourceHandler_DidFailWithError
接口，新增参数completeIfNoResponse，值为true时，若之前未调用过

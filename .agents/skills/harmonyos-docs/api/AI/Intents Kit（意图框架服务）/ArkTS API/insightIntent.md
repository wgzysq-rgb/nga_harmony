...
**Slug**: `intents-arkts-api-insightintent`

**DocID**: `21966810e7004b09856af243db9ce970`

**NodeID**: `0002017757958576407086ac6ef999ea`
...
表示意图的执行信息，例如：已执行或预测将要执行意图的时间。示例请参考
shareIntent
的示例代码。
...
从5.0.0(12)版本开始，参数类型为
IntentActionInfo
。该类型定义与5.0.0(12)前版本兼容。
...
表示值的类型为包含一个或多个键值对的对象。其中键的类型为字符，值的类型为Object，键和值的取值范围因意图名称而异，具体请参考
各垂域意图Schema
。
...
意图实体的其他信息，表示为一个或多个键值对。其中键的类型为字符，值的类型为Object，键和值的取值范围因意图名称而异，具体请参考
各垂域意图Schema
。
...
`deleteIntent failed, error.code: 
...
'a5c9e9e3-3d0a-4e6a-9f3d-6b7c6b8d6b9f'
...
`deleteEntity failed, error.code: 

...
**Slug**: `js-apis-certmanager`

**DocID**: `0c84e9a70f754769bca1c2b5183b15d9`

**NodeID**: `000201775796341684454fe78ef42745`
...
表示用户输入的凭据别名，当前仅支持传入数字、字母或下划线，长度建议32字节以内。
...
回调函数。当安装私有凭据成功时，err为null，data为
CMResult
对象中的uri属性；否则为错误对象。
...
Internal error. Possible causes: 1. IPC communication failed; 2. Memory operation error; 3. File operation error. Please try again.
...
回调函数。当获取私有凭据的详细信息成功时，err为null，data为
CMResult
对象中的credential属性；否则为错误对象。
...
Promise对象。表示获取私有凭据详细信息的结果，返回值为
CMResult
对象中的credential属性。
...
回调函数。当卸载私有凭据成功时，err为null，否则为错误对象。
...
certificateManager.installUserTrustedCertificateSync
18+
...
表示CA证书的安装结果，返回值CMResult对象中的uri属性。
...
Indicates that the certificate is in an invalid format.

17500004

Indicates that the number of certificates reaches the maximum allowed.

17500007

Indicates that the device enters advanced security mode. In this mode, the user CA certificate cannot be installed.
...
certificateManager.uninstallUserTrustedCertificateSync
18+
...
回调函数。当签名、验签的初始化操作成功时，err为null，data为获取到的CMHandle；否则为错误对象。
...
Promise对象。表示签名、验签的初始化操作结果，返回CMHandle对象。
...
回调函数。当签名、验签的数据更新操作成功时，err为null，否则为错误对象。
...
回调函数。当签名成功时，err为null，data为
CMResult
对象中的outData属性，表示签名数据；否则为错误对象。
...
Promise对象。执行签名操作时，表示签名的结果，返回值为
CMResult
对象中的outData属性；执行验签操作时，无返回结果的Promise对象。
...
回调函数。当中止签名、验签成功时，err为null，否则为错误对象。
...
certificateManager.getPublicCertificate
12+
...
Promise对象。表示获取用户公共凭据详细信息的结果，返回值为
CMResult
对象中的credential属性。
...
certificateManager.isAuthorizedApp
12+
...
表示用户授权给应用使用的凭据的唯一标识符，长度限制256字节以内。
...
Promise对象。表示查询应用是否被授权的结果，true为已授权，false为未授权。
...
Promise对象。表示获取用户根CA证书列表的结果，返回值
CMResult
对象中的certList属性。
...
certificateManager.getUserTrustedCertificate
12+
...
Promise对象。表示获取用户根CA证书详细信息的结果，返回值为
CMResult
对象中的certInfo属性。
...
certificateManager.getPrivateCertificates
13+
...
Promise对象。表示获取应用安装的凭据列表的结果，返回值
CMResult
对象中的credentialList属性。
...
certificateManager.getCertificateStorePath
18+
...
Parameter error. Possible causes: 1. Mandatory parameters are left unspecified; 2. Incorrect parameter types; 3. Parameter verification failed. For example, CertStoreProperty.certType is set to CA_CERT_USER, but CertStoreProperty.certScope is not specified.
...
The device does not support the specified certificate storage path, For example, the device outside China does not support the certificate that uses SM algorithm.
...
certificateManager.getUkeyCertificate
22+
...
Capability not supported. The application does not have the permission required to call the API.
...
Indicates that the input parameters validation failed. For example, the parameter format is incorrect or the value range is invalid.

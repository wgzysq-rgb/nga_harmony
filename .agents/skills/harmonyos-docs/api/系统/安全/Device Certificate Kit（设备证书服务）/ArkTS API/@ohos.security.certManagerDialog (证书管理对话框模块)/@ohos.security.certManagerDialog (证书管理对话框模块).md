...
**Slug**: `js-apis-certmanagerdialog`

**DocID**: `82d3b62229fd4047926232ba98f138bd`

**NodeID**: `0002017757963416845602b7f5643720`
...
certificateManager.CertificatePurpose
...
设备行为差异：
 该接口在PC/2in1设备可正常调用，在其他设备中返回29700004错误码。
...
Promise对象。表示返回证书uri的结果，最大长度为256字节。
...
The user uninstall certificate failed in the certificate manager dialog, such as the certificate uri is not exist.
...
Show the certificate detail dialog failed, such as the certificate is in an invalid format.
...
certificateManagerDialog.openAuthorizeDialog
20+
...
Promise对象。表示返回授权证书uri的结果，最大长度为256字节。
...
Invalid parameter. Possible causes: 1. A mandatory parameter is left unspecified. 2. Incorrect parameter type. 3. Parameter verification failed.
...
Internal error. Possible causes: 1. IPC communication failed; 2. Memory operation error; 3. File operation error; 4. Call other service failed. Please try again.
...
certificateManagerDialog.openUkeyAuthDialog
22+
...
The user cancels the authentication operation.

29700003

The authentication operation failed, such as the USB key certificate does not exist, the USB key status is abnormal.

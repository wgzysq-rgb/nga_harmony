...
**Slug**: `js-apis-dlppermission`

**DocID**: `28a46147b7fd41308ca42c7661d9f179`

**NodeID**: `0002017757963416843886844ccf52bf`
...
Promise对象。返回true表示是DLP文件，返回false表示非DLP文件。
...
回调函数。返回true表示是DLP文件，返回false表示非DLP文件。
...
Promise对象。返回查询的DLP文件的权限信息，无异常则表明查询成功。
...
回调函数。err为undefined时表示查询成功；否则为错误对象。
...
返回DLP文件的原始文件名。例如：DLP文件名为test.txt.dlp，则返回的原始文件名为test.txt。
...
返回DLP文件扩展名。例如：原文件"text.txt"，加密后的DLP文件名为"test.txt.dlp"，返回拓展名为".dlp"。
...
监听事件类型。固定值为'openDLPFile'：打开DLP文件事件。
...
DLP文件打开事件的回调。在当前应用的沙箱应用打开DLP文件时，通知当前应用。
...
DLP文件被打开的事件的回调。在当前应用的沙箱应用打开DLP文件时，取消通知当前应用。默认为空，表示取消该类型事件的所有回调。
...
Promise对象。返回true表示当前应用运行在沙箱中，返回false表示当前应用不是运行在沙箱中。
...
回调函数。err为undefined时表示查询成功；否则为错误对象。返回true表示当前应用运行在沙箱中，返回false表示当前应用不是运行在沙箱中。
...
Promise对象。返回当前可支持权限设置和校验的文件扩展名类型列表。
...
表示需要设置保留状态的文件uri列表。Array不限长度，每个string不超过4095字节。
...
回调函数。err为undefined时表示设置成功；否则为错误对象。
...
指定应用包名。默认为空，查询当前应用的保留沙箱信息列表。最小7字节，最大128字节。
...
dlpPermission.setSandboxAppConfig
11+
...
dlpPermission.cleanSandboxAppConfig
11+
...
dlpPermission.getSandboxAppConfig
11+
...
dlpPermission.isDLPFeatureProvided
12+
...
该接口由
MDM
配置使能，且使能场景为企业设备。其他设备（如消费者终端设备）无需关注该接口，如若调用该接口，则返回值为false。
...
Promise对象。返回true表示当前系统提供加密保护特性，返回false表示不提供加密保护特性。
...
dlpPermission.setEnterprisePolicy
21+
...
表示企业定制策略的json字符串。长度不超过4M（单位：兆）。

dlpPermission.generateDlpFileForEnterprise
21+
...
表示到期后文件是否允许打开（打开后拥有编辑权限），仅在expireTime不为空时生效。
...
接口的参数需要继承该接口，
connectServer
由SA（System Ability）侧调用，通过callback进行回传参数。

...
**DocID**: `c80ffe37edd34d8abdf62b68e76eef46`

**NodeID**: `000201775796341684627048275530dd`
...
当使用生物特征进行认证时，返回结果为临时authToken，在经过生物认证通过后，需使用
...
当操作类型为OPERATE_TYPE_BIOMETRIC_AUTH时，tuiAuthToken表示通过密码认证（即
trustedAuthentication
）获取的authToken信息。

当操作类型为OPERATE_TYPE_CONTENT_AUTH时，tuiAuthToken表示通过交易信息临时确认（即
procContentAuthentication
）获取的authToken信息。

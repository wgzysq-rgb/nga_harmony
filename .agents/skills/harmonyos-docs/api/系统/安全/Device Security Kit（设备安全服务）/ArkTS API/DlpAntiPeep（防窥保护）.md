...
> **分区**: API参考  |  **Slug**: `devicesecurity-dlpantipeep-api`  |  **DocID**: `3ee3a51a7d4d4e0bba2b6a36988f4336`
...
```
     ...
     **起始版本：** 6.0.0(20)
                                   ...
                                   名称         值         说明                                       PASS         0         表示当前设备屏幕无人在窥视。                         HIDE         1         表示有除机主以外的人在窥视设备屏幕。                                            #### isDlpAntiPeepSwitchOn
     ...
     **返回值：**
                                   ...
                                   类型         说明                                       Promise<boolean>         Promise对象，返回当前应用是否开启了防窥保护，true代表开启，false代表未开启，默认值为false。                               **错误码：**
     ...
     以下错误码的详细介绍请参见[ArkTS API错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/devicesecurity-arktsapi-errcode-dlpantipeep) **。**
                                   ...
                                   错误码ID         错误信息                                       201         Permission denied.                         801         Capability not supported. Function isDlpAntiPeepSwitchOn can not work correctly due to limited device capabilities.                         1020600001         Internal error.                               **示例：**
...
```
     ...
     **参数：**
                                   ...
                                   参数名         类型         必填         说明                                       type         string         是         表示需要订阅的事件'dlpAntiPeep'，取值为固定字符串'dlpAntiPeep'。                         callback         Callback<[DlpAntiPeepStatus](#dlpantipeepstatus)>         是         回调函数，返回调用结果。                               **错误码：**

     以下错误码的详细介绍请参见[ArkTS API错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/devicesecurity-arktsapi-errcode-dlpantipeep) **。**

                                   错误码ID         错误信息                                       201         Permission denied.                         801         Capability not supported.                         1020600001         Internal error.                               **示例：**
...
```
     ...
     **参数：**
                                   ...
                                   参数名         类型         必填         说明                                       type         string         是         表示需要解订阅的事件'dlpAntiPeep'，取值为固定字符串'dlpAntiPeep'。                         callback         Callback<[DlpAntiPeepStatus](#dlpantipeepstatus)>         否         用于接收防窥保护通知的回调函数。如果传入了callback，则取消callback的订阅，否则取消所有callback的订阅。                               **错误码：**
...
```
     ...
     **返回值：**
                                   ...
                                   类型         说明                                       [DlpAntiPeepStatus](#dlpantipeepstatus)         表示窥视状态。                               **错误码：**

     以下错误码的详细介绍请参见[ArkTS API错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/devicesecurity-arktsapi-errcode-dlpantipeep) **。**

                                   错误码ID         错误信息                                       201         Permission denied.                         801         Capability not supported. Function getDlpAntiPeepInfo can not work correctly due to limited device capabilities.                         1020600001         Internal error.                               **示例：**
...
```
     ...
     **错误码：**
     ...
     以下错误码的详细介绍请参见[ArkTS API错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/devicesecurity-arktsapi-errcode-dlpantipeep) **。**
                                   ...
                                   错误码ID         错误信息                                       201         Permission denied.                         801         Capability not supported. Function passDlpAntiPeepInfo can not work correctly due to limited device capabilities.                         1020600001         Internal error.                               **示例：**
...
```
     ...
     **参数：**
                                   ...
                                   参数名         类型         必填         说明                                       windowId         number         是         应用需要覆盖保护的窗口ID，该参数取值范围为大于0的整数。                               **返回值：**

                                   类型         说明                                       Promise<void>         Promise对象，无返回结果。                               **错误码：**

     以下错误码的详细介绍请参见[ArkTS API错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/devicesecurity-arktsapi-errcode-dlpantipeep) **。**

                                   错误码ID         错误信息                                       201         Permission denied.                         801         Capability not supported. Function setAntiPeepMaskLayer can not work correctly due to limited device capabilities.                         1020600001         Internal error.                         1020600002         The antipeep function is not enabled.                         1020600003         The protected application is not displayed on the screen.                               **示例：**
...
```
     ...
     **参数：**
                                   ...
                                   参数名         类型         必填         说明                                       context         Context         是         请求权限的UIAbility/UIExtensionAbility的Context。                               **返回值：**

                                   类型         说明                                       Promise<[AntiPeepOptionsResult](#antipeepoptionsresult)>         Promise对象，返回防窥保护开关设置的结果。                               **错误码：**

     以下错误码的详细介绍请参见[ArkTS API错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/devicesecurity-arktsapi-errcode-dlpantipeep) **。**

                                   错误码ID         错误信息                                       201         Permission denied.                         801         Capability not supported. Function requestAntiPeepOptions can not work correctly due to limited device capabilities.                         1020600001         Internal error.                         1020600004         Facial recognition is not set.                               **示例：**
...
```
     ...
     **起始版本：** 6.1.0(23)
                                   ...
                                   名称         值         说明                                       SUCCESS         0         表示防窥保护开关开启成功。                         FAIL         1         表示防窥保护开关开启失败。                         ALREADY_ON         2         表示防窥保护开关已处于开启状态。                                            #### publishAntiPeepInformation
     ...
     **返回值：**
     ...
     以下错误码的详细介绍请参见[ArkTS API错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/devicesecurity-arktsapi-errcode-dlpantipeep) **。**
                                   ...
                                   错误码ID         错误信息                                       201         Permission denied.                         801         Capability not supported. Function publishAntiPeepInformation can not work correctly due to limited device capabilities.                         1020600001         Internal error.                         1020600002         The anti-peep function is not enabled.                         1020600003         The protected application is not displayed on the screen.                         1020600005         The anti-peep information has been published.                         1020600006         No peeping risk detected.                               **示例：**

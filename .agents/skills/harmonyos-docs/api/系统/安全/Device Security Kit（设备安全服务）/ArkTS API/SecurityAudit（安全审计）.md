...
**DocID**: `d73d73d646c647189b24d90ca0d97a7b`

**NodeID**: `0002017757963416845965fb23a1f47e`
...
审计事件内容。不同审计事件具备不同的内容，内容为json格式字符串。例如：{"type": 1}
...
APP_INSTALL_INTERCEPTED

0x18000100

应用程序安装拦截事件。

APP_UNINSTALL_INTERCEPTED

0x18000101

应用程序卸载拦截事件。

APP_UPDATE_INTERCEPTED

0x18000102

应用程序更新拦截事件。

APP_RECOVER_INTERCEPTED

0x18000103

应用程序恢复拦截事件。

APP_START_INTERCEPTED

0x18000104

应用程序启动拦截事件。

USB_ACCESS_INTERCEPTED
...
on(type: 'auditEventOccur', auditEventInfo: 
AuditEventInfo
, callback: Callback<
AuditEvent
>): void
...
订阅的事件类型名称，固定值 'auditEventOccur'，表示审计事件。
...
off(type: 'auditEventOccur', auditEventInfo: 
AuditEventInfo
, callback?: Callback<
AuditEvent
>): void
...
取消订阅的事件类型名称，固定值 ‘auditEventOccur’，表示审计事件。
...
用于接收审计数据的回调函数。如果传入了callback，则取消该callback的订阅，否则取消所有callback的订阅。
...
删除阻断事件的过滤条件，满足该过滤���件的事件将不会再根据过滤器中的过滤标签进行处理。
...
newAuthClient(callback: Callback<
AuditEvent
>): AuthClient

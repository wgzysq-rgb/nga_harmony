...
**DocID**: `9670a7f10e4f4404a84bc5b05f2a1d43`

**NodeID**: `0002017757963416844508db1b9e08ac`
...
可选参数列表，用于设置传感器上报频率，默认值为200000000ns。
...
传感器类型，该值固定为SensorId.FUSION_PRESSURE
...
回调函数，异步上报的传感器数据固定为FusionPressureResponse。
...
可选参数列表，默认值为200000000ns。当霍尔事件被触发的很频繁时，该参数用于限定事件上报的频率。
...
[h2]MAGNETIC_FIELD_UNCALIBRATED
9+
...
回调函数，异步上报的传���器数据固定为OrientationResponse。
...
可选参数列表，默认值为200000000ns。当接近光事件被触发的很频繁时，该参数用于限定事件上报的频率。
...
固定传入'sensorStatusChange',状态监听固定参数。
...
回调函数，异步上报的传感器事件数据SensorStatusEvent。
...
取消订阅的回调函数，若无此参数，则取消订阅当前类型的所有回调函数。
...
sensor.getSensorListByDeviceSync
19+
...
设备ID，默认为查询本地设备，默认值为-1，表示本地设备，设备ID需通过
getSensorList
查询或者监听设备上下线接口
sensorStatusChange
获取。
...
sensor.getSingleSensorByDeviceSync
19+
...
Promise对象，使用异步方式返回指定的气压值对应的海拔高度，单位为米。
...
Promise对象，使用异步方式返回绕z、x、y轴方向的旋转角度，单位度（°）。
...
Promise对象，使用异步方式返回围绕z、x、y轴方向的旋转角度，单位度（°）。
...
传感器索引：默认值为0，为设备上的默认传感器，其它传感器ID需通过
getSensorList
查询或者监听设备上下线接口
sensorStatusChange
获取。
...
传感器数据上报的时间戳。从设备开机开始计时到上报数据的时间，单位 : ns。
...
是否本地传感器，true为本地传感器，false为非本地传感器。
...
表示剧烈运动程度。测量三个物理轴（x、y 和 z）上，设备是否存在大幅度运动；若存在大幅度运动则数据上报为1。
...
色温（单位：开尔文），可选参数，如果该参数不支持则返回固定值（固定值由传感器自定义），支持则返回正常数值。
...
红外亮度（单位：cd/m²），可选参数，如果该参数不支持则返回固定值（固定值由传感器自定义），支持则返回正常数值。
...
表示传感器的上报频率，默认值为200000000ns。该属性有最小值和最大值的限制，由硬件支持的上报频率决定，当设置频率大于最大值时以最大值上报数据，小于最小值时以最小值上报数据。
...
用于指定传感器上报频率，频率值为20000000ns，该频率被设置在硬件支持的频率范围内时会生效，值固定为'game'字符串。

'ui'

用于指定传感器上报频率，频率值为60000000ns，该频率被设置在硬件支持的频率范围内时会生效，值固定为'ui'字符串。

'normal'

用于指定传感器上报频率，频率值为200000000ns，该频率被设置在硬件支持的频率范围内时会生效，值固定为'normal'字符串。
...
注册心率传感器的回调函数，上报的数据类型为HeartRateResponse。
...
要取消��阅的陀螺仪传感器类型为SENSOR_TYPE_ID_GYROSCOPE。
...
要取消订阅的有效运动传感器类型为SENSOR_TYPE_ID_SIGNIFICANT_MOTION。

...
**Slug**: `js-apis-hidebug`

**DocID**: `4c95befe0db949a28c050aa5eb4e5dc6`

**NodeID**: `00020177579634168451235e0e8caa17`
...
内存分配器统计的进程持有的普通块所占用内存的大小（含分配器元数据），单位为Byte。
...
返回内存分配器���计的进程持有的已使用的普通块所占用内存大小，单位为Byte。
...
返回内存分配器统计的进程持有的空闲的普通块所占用内存大小，单位为Byte。
...
由于/proc/{pid}/smaps_rollup的读取耗时较长，建议不要在主线程中使用该接口，可通过
@ohos.taskpool
或
@ohos.worker
开启异步线程以避免应用出现卡顿。
...
系统服务的dump接口参数列表。string长度的最大值为254。超出部分将会被截断。
...
the parameter check failed,Possible causes:1.the parameter type error 2.the args parameter is not string array.
...
用户自定义的采样结果输出的文件名，将在应用的files目录下生成以该参数命名的json文件。string长度的最大值为128。
...
the parameter check failed,Parameter type error.
...
用户自定义的虚拟机堆数据转储输出的文件名，将在应用的files目录下生成以该参数命名的heapsnapshot文件。string长度的最大值为128。
...
从API version 8支持，从API version 9开始废弃，建议使用
hidebug.startJsCpuProfiling
替代。
...
用户自定义的虚拟机堆转储文件名，将在应用的files目录下生成以该参数命名的heapsnapshot文件。string长度的最大值为128。
...
该接口补充了
hitrace
功能，开发者可通过该接口完成指定范围的trace自动化采集。由于该接口中trace采集过程中消耗的性能与需要采集的范围成正相关，建议开发者在使用该接口前，通过hitrace命令抓取应用的trace日志，从中筛选出所需trace采集的关键范围，以提高该接口性能。

startAppTraceCapture()方法的调用需要与'
stopAppTraceCapture()
'方法的调用一一对应，重复开启trace采集将导致接口调用异常，由于trace采集过程中会消耗较多性能，开发者应在完成采集后及时关闭。
...
开启trace文件大小限制，单位为Byte，单个文件大小上限为500MB。
...
返回trace文件名路径（接口返回真实物理路径，若应用内需要访问，请参考
应用沙箱路径和真实物理路径的对应关系
进行路径转换）。
...
Invalid argument, Possible causes:1.The limit parameter is too small 2.The parameter is not within the enumeration type 3.The parameter type error or parameter order error.
...
停止应用trace采集。调用前，需先调用'
startAppTraceCapture()
'方法开始采集。关闭前未开启或重复关闭会导致接口异常。
...
Invalid argument, Possible causes:1.The limit parameter is too small 2.The parameter is not in the specified type 3.The parameter type error or parameter order error.

11400104

Set limit failed due to remote exception.
...
由于读取 /proc/{pid}/smaps_rollup 比较耗时，建议不在主线程中使用该接口。可以通过
@ohos.taskpool
或
@ohos.worker
开启异步线程，以避免应用卡顿。
...
Possible causes:1. Invalid parameter, a string parameter required. 2. Invalid parameter, unknown property.
...
应用进程的Ark层或Native层是否处于调试状态。true：处于调试状态。false：未处于调试状态。
...
Failed to get the application memory due to a remote exception.
...
为当前线程转储虚拟机的原始堆快照，并生成的rawheap文件，该文件可通过
rawheap-translator工具
将所生成文件转化为heapsnapshot文件进行解析。生成的文件路径使用Promise进行异步回调。
...
转储堆快照前是否需要GC。true：需要GC。false：不需GC。默认值：true。
...
若设备运行期间通过本接口设置的GWP-ASan应用数量超过配额限制，调用该接口将会失败并抛出错误码。请使用try-catch捕获异常，以避免应用异常退出。

设备重启后，本接口设置的GWP-ASan参数将会失效。
...
GWP-ASan持续时间，单位为天，默认值为7。需传入大于0的正整数。
...
停止使能GWP-ASan。调用该接口将取消自定义配置，恢复默认参数
GwpAsanOptions
。
...
获取当前GWP-ASan剩余使能天数。若当前未使能，返回值0。
...
rawheap-translator
工具才能将.rawheap文件转换为.heapsnapshot文件，否则可能导致转换失败。

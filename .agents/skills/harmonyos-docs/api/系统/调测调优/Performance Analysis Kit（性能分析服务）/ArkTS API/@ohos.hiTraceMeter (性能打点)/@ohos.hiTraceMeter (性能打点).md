...
**DocID**: `3bf76fcfc10c49358ade6d63345681e8`

**NodeID**: `000201775796341684601e7fd3912533`
...
建议使用API version 19的性能打点接口，后续性能打点接口
startTrace()
、
finishTrace()
、
...
如果具有相同name的任务是串行执行的，则taskId可以相同。具体示例可参考
finishTrace()
中的示例。
...
finishTrace的name和taskId必须与流程开始的
startTrace()
对应参数值一致。
...
finishAsyncTrace的level、name和taskId必须与流程开始的
startAsyncTrace()
对应参数值一致。
...
标记一个同步跟踪耗时任务的开始，分级控制跟踪输出。具体示例可参考
finishSyncTrace()
中的示例。
...
hiTraceMeter.registerTraceListener
22+
...
hiTraceMeter.unregisterTraceListener
22+

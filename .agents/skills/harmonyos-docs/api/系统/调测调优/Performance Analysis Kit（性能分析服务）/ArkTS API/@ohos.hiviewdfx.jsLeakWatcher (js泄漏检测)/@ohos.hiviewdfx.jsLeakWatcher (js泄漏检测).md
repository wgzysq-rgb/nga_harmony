...
**Slug**: `js-apis-jsleakwatcher`

**DocID**: `a5d157cf9f674916bf9bc6d8160edfcd`

**NodeID**: `000201775796341684642006f90d5c2e`
...
是否使能jsLeakWatcher。true：使能jsLeakWatcher；false：不使能jsLeakWatcher。
...
说明
：check成功，返回JSON格式的泄漏对象列表；check失败，返回空字符串。
...
导出结果。分别为文件名后缀为.jsleaklist的泄漏列表和文件名后缀为.heapsnapshot虚拟机内存快照文件。

说明
：dump成功，返回泄漏列表文件路径和虚拟机内存快照路径；dump失败，返回空数组。
...
Parameter error. The filepath is invalid.
...
jsLeakWatcher.enableLeakWatcher
20+
...
是否使能js对象内存泄漏检测功能。true：开启js内存泄漏检测功能；false：关闭js内存泄漏检测功能。
...
回调函数中传入一个数组对象，索引0为泄漏列表文件名，后缀为.jsleaklist；索引1为虚拟机内存快照文件名，后缀为.rawheap。
...
The parameter callback is invalid. Input parameter error. Possible causes: 1.Mandatory parameters are left unspecified; 2.Incorrect parameter types; 3.Parameter verification failed.

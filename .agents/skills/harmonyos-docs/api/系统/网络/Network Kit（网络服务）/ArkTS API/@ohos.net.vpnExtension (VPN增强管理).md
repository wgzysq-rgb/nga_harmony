...
**Slug**: `js-apis-net-vpnextension`

**DocID**: `8ea89653408f41339b41126be94a01e5`

**NodeID**: `00020177579634168464689e6aa17c58`
...
以下模块不支持在VpnExtensionAbility中不支持引用，可能会导致程序异常退出。
...
说明：
 从API version 22开始，支持在VPN首次启动时传递want中的parameters字段。
...
destroy()
或
destroy(vpnId: string)
接口销毁启动的VPN网络，并执行资源清理等操作。
...
以 Promise 形式返回获取结果，返回指定虚拟网卡的文件描述符 fd。
...
注意
：若支持IPv4功能，需要在addresses中配置IPv4类型的IP地址。
...
注意
：若支持IPv6功能，需要在addresses中配置IPv6类型的IP地址。
...
注意
：trustedApplications和blockedApplications列表不能同时配置。

...
**DocID**: `a922a26c0e68495ebee9173b65ce90b0`

**NodeID**: `000201775796341684613b2b163a5045`
...
发送数据前，需要先调用
UDPSocket.bind()
绑定IP地址和端口。该接口为耗时操作，请在Worker线程或taskpool线程调用该接口。
...
回调函数。成功返回UDPSocket状态信息，失败返回错误码、错误信息。
...
回调函数。返回订阅某类事件后UDPSocket连接成功的状态信息。
...
回调函数。可以指定传入on中的callback取消对应的订阅，也可以不指定callback清空所有订阅。
...
回调函数。UDPSocket连接的某类数据包消息事件或关闭事件发生变化后触发回调函数。
...
回调函数。UDPSocket连接发生error事件后触发回调函数。
...
- 3：Domain
18+
。地址为Domain类型，该字段必须被显式指定为3。当前仅支持
TCPSocket.connect
和
TLSSocket.connect
。
...
socket.constructMulticastSocketInstance
11+
...
以Promise形式返回MulticastSocket加入多播组的行为结果。
...
以Promise形式返回MulticastSocket加入多播组的执行结果。
...
以Promise形式返回MulticastSocket设置TTL数值的结果。
...
是否开启环回模式。true表示环回模式开启，false表示环回模式关闭。
...
以Promise形式返回MulticastSocket设置环回模式的结果。
...
回调函数。返回值为环回模式状态，true表示环回模式开启，false表示环回模式关闭。
...
Promise对象。返回true表示环回模式开启，返回false表示环回模式关闭。
...
回调函数。成功时返回对端Socket地址，失败时返回错误码、错误信息。
...
回调函数。成功时获取TCPSocket状态，失败时返回错误码、错误信息。
...
回调函数，当成功时，返回socket的文件描述符，失败时，返回undefined。
...
回调函数。TCPSocket的连接事件或关闭事件触发时调用回调函数。
...
回调函数。TCPSocket连接订阅的某类error事件触发时调用回调函数。
...
方法绑定的端口。若多次调用
listen
方法，再调用此方法时会释放TCPSocketServer的所有监听端口。使用Promise异步回调。

该方法不会关闭已有连接。如需关闭，请调用
TCPSocketConnection
的
close
方法。
...
socket.constructLocalSocketInstance
11+
...
在本文档的示例中，通过this.context来获取UIAbilityContext，其中this代表继承自UIAbility的UIAbility实例。如需在页面中使用UIAbilityContext提供的能力，请参见
获取UIAbility的上下文信息
。
...
以Promise形式返回LocalSocket连接服务端的结果。
...
以Promise形式返回获取LocalSocket状态的结果。
...
以Promise形式返回设置LocalSocket套接字属性的结果。
...
以Promise形式返回设置LocalSocket套接字的属性。
...
取消订阅的事件类型。'connect'：LocalSocket的connect事件。
...
以callback的形式异步返回关闭localsocket的结果。
...
取消订阅的事件类型。'close'：LocalSocket的关闭事件。
...
取消订阅的事件类型。'error'：LocalSocket的error事件。
...
以Promise形式返回执行结果， 成功返回空，失败返回错误码错误信息。
...
以Promise形式返回获取LocalSocketServer状态的结果。
...
取消订阅的事件类型。'connect'：LocalSocketServer的连接事件。
...
LocalSocketServer停止监听并释放通过
listen
方法绑定的监听端口。使用Promise异步回调。
...
回调函数。TLSSocket连接订阅某类接受消息事件触发的调用函数，返回TLSSocket连接信息。
...
回调函数。TLSSocket连接取消订阅某类接受消息事件触发的调用函数，返回TLSSocket连接信息。
...
回调函数。TLSSocket连接订阅某类error事件触发的调用函数。
...
回调函数。TLSSocket连接取消订阅某类error事件触发的调用函数。
...
服务端的ca证书，用于认证校验服务端的数字证书。默认为系统预置CA证书
12+
。
...
socket.constructTLSSocketServerInstance
10+
...
以Promise形式返回，成功返回空，失败返回错误码，错误信息。
...
回调函数。成功返回TLSSocketServer状态，失败返回错误码、错误信息。
...
以Promise形式返回获取TLSSocketServer状态的结果。失败返回错误码，错误信息。
...
TLSSocketServer停止监听并释放通过
listen
方法绑定的端口。使用Promise异步回调。
...
在TLSSocketServer通信连接成功之后，获取对端Socket地址。使用Promise��步回调。
...
以Promise形式返回对端的数字证书的结果。失败返回错误码，错误信息。
...
回调函数。成功时返回TLSSocketConnection连接信息，失败时返回错误码、错误信息。

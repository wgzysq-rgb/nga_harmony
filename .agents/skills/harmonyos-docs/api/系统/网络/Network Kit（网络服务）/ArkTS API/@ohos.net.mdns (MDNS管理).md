...
**Slug**: `js-apis-net-mdns`

**DocID**: `d7d2fcd3cce640709e18be64a667108d`

**NodeID**: `000201775796341684581661788a4976`
...
回调函数。成功添加时error为undefined，data为添加到本地的MDNS服务信息。
...
回调函数。成功移除error为undefined，data为移除本地的MDNS服务信息。
...
基于指定服务类型（serviceType）和Context的发现服务对象。
...
回调函数。成功移除error为undefined，data为解析的MDNS服务信息。
...
MDNS服务的信息和事件错误信息。可以指定传入on中的callback取消对应的订阅，也可以不指定callback清空所有订阅。
...
MDNS服务的信息，需调用resolveLocalService解析这个MDNS服务信息。
...
MDNS服务的信息。可以指定传入on中的callback取消对应的订阅，也可以不指定callback清空所有订阅。

...
**DocID**: `9596c9cf69ff4acfab05d4f1dd7b0fc1`

**NodeID**: `0002017757963416845647d960e4663b`
...
连接的目标设备的deviceId，即对端设备的BLE MAC地址。BLE MAC的获取方法，请参考
查找设备
。
...
事件回调类型，支持的事件为'connectionAccepted'，收到对端连接，触发该事件。
...
事件回调类型，支持的事件为'serverStopped'，底层服务异常时，触发该事件。
...
事件回调类型，支持的事件为'serverStopped'，底层服务异常时触发。
...
对端设备的deviceId，即对端设备的BLE MAC地址。如果获取失败返回空字符串。
...
事件回调类型，支持的事件为'connectResult'，完成connect()调用，触发该事件。
...
事件回调类型，支持的事件为'disconnected'，连接被动断开或底层异常断开时，触发该事件。
...
事件回调类型，支持的事件为'dataReceived'，收到数据时，触发该事件。

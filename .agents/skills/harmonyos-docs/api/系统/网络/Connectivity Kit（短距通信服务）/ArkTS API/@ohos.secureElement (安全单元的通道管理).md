...
**Slug**: `js-apis-secureelement`

**DocID**: `0b784f9cbe884d3d922812fa9260672a`

**NodeID**: `00020177579634168468446d7a8bf34b`
...
此类的实例表示该设备支持的SE，如果支持eSE、SIM和SIM2，则返回3个实例，其中SIM2从API version 22开始支持。

Session

此类的实例表示在某个SE Reader实例上创建连接会话。

Channel

此类的实例表示在某个Session实例上创建通道，可能为基础通道或逻辑通道。
...
从 API version 10 开始支持，从 API version 12 开始废弃，建议使用
createService
替代。
...
SEService表示可用于连接到系统中所有可用SE的连接（服务），通过
createService
获取SEService实例。

[h2]SEService.getReaders
...
true: SE服务状态已连接，false: SE服务状态已断开。
...
OMA版本号（例如，“3.3”表示Open Mobile API规范版本3.3）
...
[h2]Reader.isSecureElementPresent
...
IllegalStateError, service state exception.
...
Session的实例表示在某个SE Reader实例上创建连接会话。通过
Reader.openSession
获取Session实例。

[h2]Session.getReader
...
true：Session状态已关闭，false：Session是打开的。
...
在此Channel上选择的Applet的AID或如果没有Applet被选择时空的数组。
...
以Promise形式异步返回可用的基础Channel对象实例。
...
以callback形式异步返回可用的基础Channel对象实例。
...
以Promise形式异步返回可用的逻辑Channel对象实例。
...
NoSuchElementError, the AID on the SE is not available or cannot be selected or a logical channel is already open to a non-multi-selectable applet.
...
以callback形式异步返回可用的逻辑Channel对象实例。
...
以Promise形式异步返回可用的逻辑Channel实例对象。
...
true: 该Channel是基础Channel, false：该Channel逻辑Channel 。
...
以Promise形式异步返回接收到的响应APDU数据，number数组。
...
IllegalStateError, an attempt is made to use an SE session or channel that has been closed.

3300103

SecurityError, the command is filtered by the security policy.

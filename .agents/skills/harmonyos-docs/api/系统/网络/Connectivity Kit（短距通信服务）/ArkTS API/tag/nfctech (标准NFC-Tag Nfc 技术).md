...
**DocID**: `d41f6f5d7a4c42a490f7c0d98e59220c`

**NodeID**: `00020177579634168472589794b9b7e8`
...
NfcA 标签的SAK值，十六进制表示，范围是0x00~0xFF。
...
NfcA 标签的Atqa值，每个number十六进制表示，范围是0x00~0xFF。
...
NfcB 标签的应用程序数据，每个number十六进制表示，范围是0x00~0xFF。
...
NfcB 标签的协议信息，每个number十六进制表示，范围是0x00~0xFF。
...
NfcF 标签的系统代码，每个number十六进制表示，范围是0x00~0xFF。
...
NfcF 标签的PMm信息，每个number十六进制表示，范围是0x00~0xFF。
...
NfcV 标签的响应标志，十六进制表示，范围是0x00~0xFF。
...
NfcV 标签的数据存储格式标识符，十六进制表示，范围是0x00~0xFF。
...
[h2]IsoDepTag.getHistoricalBytes
9+
...
IsoDepTag 标签的历史字节，每个number十六进制表示，范围是0x00~0xFF。如果该IsoDep类型Tag是基于NfcB技术的，则该返回值为空。
...
[h2]IsoDepTag.getHiLayerResponse
9+
...
IsoDepTag 标签的更高层响应字节，每个number十六进制表示，范围是0x00~0xFF。如果该IsoDep类型Tag是基于NfcA技术的，则该返回值为空。
...
[h2]IsoDepTag.isExtendedApduSupported
9+
...
Promise对象。返回从NDEF标签中读取到的Message数据对象。
...
回调函数。当NDEF Message数据对象写入成功时，err为undefined，否则为错误对象。
...
true: NDEF标签可设置为只读， false: NDEF标签不可设置为只读。
...
回调函数。当NDEF标签设置为只读成功时，err为undefined，否则为错误对象。
...
[h2]NdefTag.getNdefTagTypeString
9+
...
MifareClassicTag获取方式请参考
nfc-tag开发指南
。

以下是MifareClassicTag的独有接口。

[h2]MifareClassicTag.authenticateSector
9+
...
isKeyA标志。true 表示KeyA，false 表示KeyB。
...
回调函数。当身份验证成功时，err为undefined，否则为错误对象。
...
[h2]MifareClassicTag.readSingleBlock
9+
...
[h2]MifareClassicTag.writeSingleBlock
9+
...
回调函数。当向块存储写入内容成功时，err为undefined，否则为错误对象。
...
[h2]MifareClassicTag.incrementBlock
9+
...
对���定块的内容，增加指定的数值，并将结果存储在内部传输缓冲器中。使用callback异步回调。
...
回调函数。当对块增加指定数值成功时，err为undefined，否则为错误对象。
...
[h2]MifareClassicTag.decrementBlock
9+
...
回调函数。当对块减少指定数值成功时，err为undefined，否则为错误对象。
...
[h2]MifareClassicTag.transferToBlock
9+
...
回调函数。当临时寄存器的值转移到指定块成功时，err为undefined，否则为错误对象。
...
[h2]MifareClassicTag.restoreFromBlock
9+
...
回调函数。当复制指定块内容到临时寄存器成功时，err为undefined，否则为错误对象。
...
[h2]MifareClassicTag.getSectorCount
9+
...
[h2]MifareClassicTag.getBlockCountInSector
9+
...
[h2]MifareClassicTag.getTagSize
9+
...
[h2]MifareClassicTag.isEmulatedTag
9+
...
[h2]MifareClassicTag.getBlockIndex
9+
...
[h2]MifareClassicTag.getSectorIndex
9+
...
MifareUltralightTag获取方式请参考
nfc-tag开发指南
。

以下是MifareUltralightTag的独有接口。

[h2]MifareUltralightTag.readMultiplePages
9+
...
Promise对象。以Promise形式返回读取的4页的数据，共16字节。
...
以callback形式异步返回页操作结果。返回读取到的数据，共16字节。
...
[h2]MifareUltralightTag.writeSinglePage
9+
...
回调函数。当写入数据成功时，err为undefined，否则为错误对象。
...
[h2]MifareUltralightTag.getType
9+
...
TagSession是所有NFC Tag 技术类型的基类， 提供建立连接和发送数据等共同接口。具体请参见
TagSession
。

NdefFormatableTag获取方式请参考
nfc-tag开发指南
。

以下是NdefFormatableTag的独有接口。

[h2]NdefFormatableTag.format
9+
...
格式化成功时要写入的NDEF消息。可以为null，为null时仅格式化标签，不写入内容。
...
回调函数。当NDEF消息写入标签成功时，err为undefined，否则为错误对象。
...
[h2]NdefFormatableTag.formatReadOnly
9+

formatReadOnly(message: 
NdefMessage
): Promise<void>
...
回调函数。当NDEF消息写入NDEF标签成功时，err为undefined，否则为错误对象。

...
**Slug**: `js-apis-bluetooth-ble`

**DocID**: `77020c48486d4bc1974ebae139d1c295`

**NodeID**: `000201775796341684575693adde91bc`

---

@ohos.bluetooth.ble (蓝牙ble模块)

本模块提供了基于低功耗蓝牙（Bluetooth Low Energy，
BLE
）技术的蓝牙能力，支持发起BLE扫描、发送BLE广播报文、以及基于通用属性协议（Generic Attribute Profile，
GATT
）的连接和传输数据。
...
constant.ProfileConnectionState
...
创建
GattServer
实例，表示GATT连接中的server端。

通过该实例可以操作server端的行为，如添加服务
addService
、通知特征值变化
...
创建
GattClientDevice
实例，表示GATT连接中的client端。

通过该实例可以操作client端行为，如调用
connect
向对端设备发起连接，调用
getServices
获取对端设备支持的所有服务能力。

创建该实例所需要的设备地址表示server端设备。可以通过
ble.startBLEScan
或
BleScanner
的
startScan
接口获取server端设备地址，且需保证server端设备的BLE广播是可连接的。
...
的回调函数获取到。只能扫描BLE设备，调用
ble.stopBLEScan
可以停止该方法开启的扫描流程。

该接口只支持单路扫描，即应用同时只能调用一次，下一次调用前，需要先调用
ble.stopBLEScan
停止上一次的扫描流程。
...
当应用不再需要发送BLE广播报文时，需主动调用
ble.stopAdvertising
停止发送。

同步接口，不要和API version 11的
ble.stopAdvertising
搭配使用。
...
若携带了发送广播持续时间，则一定时间后，广播会停止发送，但分配的广播资源还存在，可以通过
...
ble.stopAdvertising
完全停止该广播，不要与API version 10开始支持的
ble.stopAdvertising
混用。
...
中advertisingId对应的广播资源已在
ble.startAdvertising
首次启动广播时分配。

若
ble.startAdvertising
首次启动广播时指定了广播持续时间，超时后广播自动停止，调用此接口可重新启动同一路BLE广播。

通过
ble.disableAdvertising
停止的广播，调用此接口可重新启动同一路BLE广播。
...
Invalid parameter. Possible causes: 1. Mandatory parameters are left unspecified. 2. Incorrect parameter types.
...
即将删除的服务的UUID。例如：00001810-0000-1000-8000-00805F9B34FB。
...
方法创建多个
GattServer
实例。本方法仅支持获取当前实例添加过的服务，无法获取当前应用创建的其他实例或由其他应用创建的实例添加过的服务。
...
需要获取的服务的UUID。例如：00001810-0000-1000-8000-00805F9B34FB。
...
[h2]notifyCharacteristicChanged
...
入参的characteristicValue数据长度默认限制为（MTU-3）字节，MTU大小可从订阅的回调
on('BLEMtuChange')
获取。
...
接收通知的client设备地址。例如：“XX:XX:XX:XX:XX:XX”。
...
回调函数。当通知成功，err为undefined，否则为错误对象。
...
[h2]on('characteristicRead')

on(type: 'characteristicRead', callback: Callback<CharacteristicReadRequest>): void

server端订阅client的特征值读请求事件，server端收到该事件后需要调用
sendResponse
接口回复client。使用Callback异步回调。
...
[h2]on('characteristicWrite')

on(type: 'characteristicWrite', callback: Callback<CharacteristicWriteRequest>): void

server端订阅client的特征值写请求事件，server端收到该事件后需要根据
CharacteristicWriteRequest
中的needRsp决定是否调用
sendResponse
接口回复client。
...
[h2]on('descriptorRead')

on(type: 'descriptorRead', callback: Callback<DescriptorReadRequest>): void

server端订阅client的描述符读请求事件，server端收到该事件后需要调用
sendResponse
接口回复client。
...
[h2]on('descriptorWrite')

on(type: 'descriptorWrite', callback: Callback<DescriptorWriteRequest>): void

server端订阅client的描述符写请求事件，server端收到该事件后需要根据
DescriptorWriteRequest
里的needRsp决定是否调用
sendResponse
接口回复client。
...
指定订阅的回调函数，会携带协商后的MTU大小。单位：Byte。
...
要查询连接状态的对端蓝牙设备地址。例如："XX:XX:XX:XX:XX:XX"。
...
回调函数。当读取成功，err为undefined，data为server端设备名称。
...
回调函数。当读取成功，err为undefined，data为server端的服务列表。
...
需要先调用
getServices
，获取到server端所有支持的能力，且包含指定的入参特征值UUID；否则会读取失败。
...
读取特征值过程中，需确保
BLECharacteristic
入参特征值的serviceUuid、characteristicUuid准确。characteristicValue表示的数据内容长度可由用户任意指定，不会影响实际读取到的特征值数据内容。
...
回调函数。当读取成功，err为undefined，data为获取到的特征值对象，包含读取到的数据内容；否则为错误对象。
...
Promise对象，返回获取到的特征值对象，包含读取到的数据内容。
...
需要先调用
getServices
，获取到server端所有支持的能力，且包含指定的入参描述符UUID；否则会读取失败。
...
读取描述符过程中，需确保
BLEDescriptor
入参描述符的serviceUuid、characteristicUuid、descriptorUuid准确。descriptorValue表示的数据内容长度可由用户任意指定，不会影响实际读取到的描述符数据内容。
...
回调函数。当读取成功，err为undefined，data为获取到的描述符对象，包含读取到的数据内容；否则为错误对象。
...
Promise对象，返回获取到的描述符对象，包含读取到的数据内容。
...
需要先调用
getServices
，获取到server端所有支持的能力，且包含指定的入参特征值UUID；否则会写入失败。
...
应用单次可写入的特征值数据长度默认限制为（MTU-3）字节，MTU大小可由
setBLEMtuSize
接口指定。
...
需要先调用
getServices
，获取到server端所有支持的能力，且包含指定的入参描述符UUID；否则会写入失败。
...
应用单次可写入的描述符数据长度默认限制为（MTU-3）字节，MTU大小可由
setBLEMtuSize
接口指定。
...
回调函数。获取链路信号强度成功，err为undefined，data为获取到的信号强度值，单位：dBm；否则为错误对象。
...
需要协商的mtu大小，取值范围：[23, 517]，单位：Byte。
...
[h2]setCharacteristicChangeNotification
...
需要先调用
getServices
，获取到server端所有支持的能力，且需包含指定的入参特征值UUID。
...
回调函数。当调用成功，err为undefined，否则为错误对象。
...
[h2]setCharacteristicChangeIndication
...
[h2]off('BLECharacteristicChange')
...
[h2]on('BLEConnectionStateChange')
...
[h2]off('BLEConnectionStateChange')
...
指定取消订阅服务变化的回调函数通知。若传参，则需与
on('serviceChange')
中传入的回调函数一致；若无传参，则取消订阅该type对应的所有回调函数通知。
...
- 围栏模式下（
ScanReportMode
设置为FENCE_SENSITIVITY_LOW或FENCE_SENSITIVITY_HIGH时），该参数不可设置为null，需传入非空过滤器。
...
特征值的唯一标识句柄。当server端BLE蓝牙设备提供了多个相同UUID特征值时，可以通过此句柄区分不同的特征值。
...
描述符的唯一标识句柄。当server端BLE蓝牙设备提供了多个相同UUID描述符时，可以通过此句柄区分不同的描述符。
...
收到client端请求的标识符，与订阅client端读或者写请求事件携带的transId保持一致。
...
扫描到的设备广播发送功率，从原始数据data字段中解析而来，在蓝牙协议中广播数据类型为0x0A。若广播报文中携带设备广播发送功率，则该字段有值，否则内容为undefined。
...
是否携带蓝牙设备名称。true表示携带，false表示不携带，默认值为false。
...
过滤该BLE设备地址的广播报文。例如："XX:XX:XX:XX:XX:XX"。
...
true表示支持，server端可将特征值内容以
ServiceData
类型在广播报文中携带，false表示不支持。默认值为false。
...
true表示支持，且该特征值权限
GattPermissions
中的writeSigned或writeSignedMitm需设置为true，否则该属性不生效，false表示不支持。默认值为false。
...
true表示需要加密后，方可写入内容，false表示不需要���通方式加密。默认值为false。
...
当不再扫描到符合过滤条件的BLE广播报文时，触发���报，只搭配围栏上报模式使用。
...
- 该模式下，扫描到符合过滤条件的BLE广播报文后不会立刻上报，需要缓存一段时间（
ScanOptions
中的interval字段）后上报。

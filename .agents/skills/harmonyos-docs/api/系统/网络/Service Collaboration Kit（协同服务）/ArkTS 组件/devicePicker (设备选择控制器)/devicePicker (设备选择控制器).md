...
**DocID**: `4aefcf47b40e499793bb26918c499e01`

**NodeID**: `0002017757963416845673cd2a6a9358`

---

devicePicker (设备选择控制器)

该模块提供设备选择控制器和设备选择面板的能力。设备选择面板没有对外提供直接显示的接口，需要结合
CollaborationDevicePicker (流转控件)
组件一起使用，点击
CollaborationDevicePicker (流转控件)
组件后该面板会自动显示。通过设备选择控制器可以和设备选择面板进行交互，设备选择面板中包括：应用的描述信息和发现的已组网设备列表（当前设备列表中只支持显示已组网的可信设备）。

用户点击设备后，会触发设备选择控制器对应的接口回调，并返回已选择的设备信息；应用可以通过获取到的设备信息处理分布式业务，比如通过startAbility启动远端UIAbility；待启动远端ability成功后，可以通过
updateState
...
设备相关的事件，目前支持：设备选择、设备取消选择、已选择设备的下线事件。
...
返回发生对应事件的设备信息，可以在callback中使用返回的设备信息进行跨设备应用ability组件启动。
...
图1 
设备空闲状态效果图（下图中my phone处于空闲状态，点击该设备后会触发deviceSelected事件）

图2 
设备成功状态效果图（下图中my phone处于成功状态，点击该设备后会触发deviceUnselected事件）
...
接口对应，可以在不需要监听设备事件的位置进行取消。比如在组件生命周期的aboutToAppear()调用了on注册接口，则可以在页面aboutToDisappear()调用off取消注册。
...
业务状态，当前支持IDLE、SUCCESSFUL、FAILED三种状态类型。
...
导致业务失败的具体描述信息。在业务失败情况下，可以传入该参数，会将该信息显示在指定设备名字下方。长度限制1-128。
...
图3 
设备流转过程效果图（下图中my phone处于流转中，设备名称右侧会有流转过程动画）：

图4 
设备流转成功效果图（下图中my phone流转成功）：

图5 
设备流转失败效果图（下图中my phone流转失败，失败原因是对端设备忙）：

图6 
应用描述信息效果图（在设备选择面板的最上方显示应用的相关信息，包括应用图标、应用名称、应用描述信息）
...
在应用不需要设备选择控制器的时候需要调用release()接口进行释放，比如在应用onCreate()生命周期回调中通过
createDevicePickerController

创建了设备选择控制器，可以在
onDestroy()生命周期回调中通过release()进行释放。
...
设备选择器属性类，在设备选择面板的应用描述部分会显示该属性配置的信息；如果未设置则使用调用者对应的ability配置文件中的标签信息。该属性与流转组件接口
CollaborationDevicePicker
配合使用。
...
；
如果不设置该属性，
默认使用module.json5中的description标签配置。
...
分布式业务描述信息，比如流转业务：“流转到”

，投屏业务

：“投屏到”
；
如果不设置该属性，对于流转业务默认使用“流转到”
。

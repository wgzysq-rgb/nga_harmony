...
**DocID**: `6607b9e42c154667bd4ed0301254d1be`

**NodeID**: `0002017757963416845882310e1ff207`
...
远端服务端设备地址。地址格式参考："11:22:33:AA:BB:FF"。
...
let client: ssap.Client;
try { 
 client = ssap.createClient(addr); // 一个应用���对一个远端设备只需要创建一次实例
 client.close();
} catch (err) {
 console.error('errCode: ' + (err as BusinessError).code + ', errMessage: ' + (err as BusinessError).message);
}
...
固定填写"propertyChange"字符串，表示Property变化事件。
...
表示星闪Property变化事件回调函数的入参。回调函数由用户创建通过该接口注册。
...
可选参数，需要取消注册的回调函数，需与订阅时传入的回调函数是同一个。若无此参数，则取消注册所有的回调函数。
...
固定填写"connectionStateChange"字符串，表示连接状态变化事件。
...
表示连接状态变化事件回调函数的入参。回调函数由用户创建通过该接口注册。
...
表示MTU变化事件回调函数的入参。回调函数由用户创建通过该接口注册。
...
服务端提供的服务信息，支持添加多个服务，根据不同的UUID区分。
...
服务UUID，用户添加服务时的UUID号，例如：37bea880-fc70-11ea-b720-000000004386。UUID格式参考
星闪标准服务UUID
。
...
客户端设备地址。地址格式参考："11:22:33:AA:BB:FF"。
...
表示读请求事件回调函数的入参。回调函数由用户创建通过该接口注册。
...
固定填写"propertyWrite"字符串，表示写请求事件。
...
表示写请求事件回调函数的入参。回调函数由用户创建通过该接口注册。
...
表示服务UUID例如：37bea880-fc70-11ea-b720-000000004386。UUID格式参考
星闪标准服务UUID
。
...
表示Property支持的操作方式，默认值为0，即不支持操作。如要使属性支持相应的操作，需要对该字段赋值，例如赋值为：READABLE|WRITE_NO_RESPONSE。取值范围[0, 15]，各比特位对应的操作方式详见
Operation
。
...
表示服务UUID，例如：37bea880-fc70-11ea-b720-000000004386。UUID格式参考
星闪标准服务UUID
。

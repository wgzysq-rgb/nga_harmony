...
**Slug**: `js-apis-inputconsumer`

**DocID**: `846377ce7b3249268f94882bfe847737`

**NodeID**: `0002017757963416846623192500ed95`
...
被修饰键，除修饰键和Meta键以外的按键，详细按键介绍请参见
键值
。
...
设备行为差异
：该接口仅在Phone和Tablet设备上生效，在其他设备上返回801错误码。
...
说明：
 从API version 21开始，支持取值为1和2，取值为1表示订阅按键按下事件，取值为2表示同时订阅按键按下事件和按键抬起事件。
...
Callback<HotkeyOptions>

是

回调函数，获取满足条件的组合按键输入事件。
...
需要取消订阅的回调函数。若缺省，则取消当前应用快捷键选项已订阅的所有回调函数。
...
设备行为差异
：该接口仅在Phone和Tablet设备中可正常调用，在其他设备上返回801错误码。
...
回调函数，用于返回按键事件。订阅不同的按键事件需要使用不同的callback，否则订阅不生效。
...
需要取消订阅的回调函数。若缺省，则取消当前已订阅的所有回调函数。

...
**Slug**: `js-apis-inputdevice`

**DocID**: `df408e53a43c46658e294b3e2ed6deb4`

**NodeID**: `0002017757963416843821bb71da8a4e`
...
回调函数，返回所有输入设备的ID列表。ID是输入设备的唯一标识。
...
Promise对象，返回所有输入设备的ID列表。ID是输入设备的唯一标识。
...
回调函数。返回输入设备信息，包括输入设备ID、名称、支持的输入能力、物理地址、版本信息及产品信息等。
...
Promise对象，返回输入设备信息，包括输入设备ID、名称、支持的输入能力、物理地址、版本信息及产品信息等。
...
返回输入设备信息，包括输入设备ID、名称、支持的输入能力、物理地址、版本信息及产品信息等。
...
Promise对象，返回查询结果。true 表示支持，false表示不支持。
...
inputDevice.getKeyboardTypeSync
10+
...
inputDevice.isFunctionKeyEnabled
15+
...
Promise对象。返回查询结果，true表示功能键使能，false表示功能键未使能。
...
inputDevice.setFunctionKeyEnabled
15+
...
功能键使能状态。取值为true表示使能功能键，取值为false表示不使能功能键。
...
Promise对象，返回距离上次系统输入事件的时间间隔，单位：μs。

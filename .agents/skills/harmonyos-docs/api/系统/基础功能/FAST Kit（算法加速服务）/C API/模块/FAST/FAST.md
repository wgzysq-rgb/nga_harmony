...
**DocID**: `66b9c8c55718442eb48aaaa0c099781e`

**NodeID**: `000201775796341684763fa339298bab`
...
FAST_SegmentMapQueryType
 { FAST_SEGMENTMAP_QUERY_TYPE_SUM = 0, FAST_SEGMENTMAP_QUERY_TYPE_MIN = 1, FAST_SEGMENTMAP_QUERY_TYPE_MAX = 2 }

线段表支持的查询操作类型。

FAST_SegmentMapUpdateType
 { FAST_SEGMENTMAP_UPDATE_TYPE_SET = 0, FAST_SEGMENTMAP_UPDATE_TYPE_ADD = 1, FAST_SEGMENTMAP_UPDATE_TYPE_SUB = 2 }
...
HMS_FAST_SegmentMap_Create
 (
FAST_SegmentMapHandle
 *handle, size_t size, const int32_t *array, 
FAST_SegmentMapConfig
 *config)
...
），否则函数返回FAST_ERROR_CODE_ILLEGAL_INPUT。
...
当成功时，返回
FAST_ERROR_CODE_SUCCESS
。

当config为NULL时，返回
FAST_ERROR_CODE_INVALID_PTR
。
...
FAST_EXPORT void HMS_FAST_RectPartition_DestroyConfig (
FAST_RectPartitionConfig
 * config)
...
当config或name为NULL时，返回
FAST_ERROR_CODE_INVALID_PTR
。

当算法不支持时，返回
FAST_ERROR_CODE_ILLEGAL_INPUT
。
...
由矩形划分求解器得到的
FAST_Rect
结果，在调用函数之前需要初始化该结果数组，大小需要和源数组相等，否则可能导致溢出。
...
当算法求解失败时，返回
FAST_ERROR_CODE_FAIL
。

注解：

当选择“SweepLineAlgo”时，不应该返回
FAST_ERROR_CODE_FAIL
，此处仅作为预防性设置。
...
可选；用于初始化线段表的底层数组。如果为NULL，则线段表中的元素均初始化为0，否则数组大小必须与参数size保持一致。

config

线段表的不透明配置
FAST_SegmentMapConfig
，若该参数为NULL或未配置，默认查询类型为
FAST_SEGMENTMAP_QUERY_TYPE_SUM
、更新类型为
FAST_SEGMENTMAP_UPDATE_TYPE_SET
。
...
FAST_EXPORT void HMS_FAST_SegmentMap_Destroy (
FAST_SegmentMapHandle
 handle)
...
FAST_EXPORT void HMS_FAST_SegmentMap_DestroyConfig (
FAST_SegmentMapConfig
 * config)

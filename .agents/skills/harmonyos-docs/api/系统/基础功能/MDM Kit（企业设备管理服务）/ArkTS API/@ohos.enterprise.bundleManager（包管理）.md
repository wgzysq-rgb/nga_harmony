...
**Slug**: `js-apis-enterprise-bundlemanager`

**DocID**: `c1420e607b5d4d7db9252dfd230c652c`

**NodeID**: `00020177579634168460902fd36bd365`
...
取值范围：单个用户下该名单总数不能超过200。例如100用户下已经设置了50个、101用户未设置，则100用户还能再设置150个，101用户还能再设置200个。不建议一次性设置个数大于50个，可能引入性能问题。
...
待安装应用包路径数组。应用包路径为应用沙箱路径(应用沙箱路径和真实路径的对应关系可参见：
应用沙箱路径和真实物理路径的对应关系
)等应用有权限访问的路径。
...
无返回结果的Promise对象。当应用程序包安装失败时，抛出错误对象。
...
bundleManager.addInstallationAllowedAppDistributionTypes
20+

addInstallationAllowedAppDistributionTypes(admin: Want, appDistributionTypes: Array<AppDistributionType>): void

添加可安装应用的分发类型。添加成功后，当前设备可以安装对应分发类型的应用，但无法安装
AppDistributionType
中未添加的分发类型的应用。

应用程序签名证书的分发类型详细介绍请参见
ApplicationInfo
的appDistributionType属性。
...
bundleManager.removeInstallationAllowedAppDistributionTypes
20+
...
bundleManager.installMarketApps
22+
...
应用包名列表，一次最多传入10个。包名需与应用市场中包名一致，否则无法创建下载任务，并抛出错误码9201002。
...
分布式场景下的应用包兼容的最低版本，对应
app.json5
中配置的minCompatibleVersionCode字段。
...
标识应用的描述信息，对应
app.json5
中配置的description字段。关于description的详细信息详见本表中的descriptionResource字段说明。
...
应用程序的图标，对应
app.json5
中配置的icon字段。关于icon的详细信息详见本表中的iconResource字段说明。
...
标识应用打包时使用的SDK的发布类型。当前SDK的发布类型可能为Canary、Beta、Release，其中Canary和Beta可能通过序号进一步细分，例如Canary1、Canary2、Beta1、Beta2等。开发者可通过对比应用打包依赖的SDK发布类型和OS的发布类型（
deviceInfo.distributionOSReleaseType
）来判断兼容性。

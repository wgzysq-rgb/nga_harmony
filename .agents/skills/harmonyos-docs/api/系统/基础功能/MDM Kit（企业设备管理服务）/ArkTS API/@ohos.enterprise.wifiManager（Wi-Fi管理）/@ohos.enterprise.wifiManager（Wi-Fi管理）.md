...
**DocID**: `0e5b41711cf3474287bc7bb0c44b5e1e`

**NodeID**: `000201775796341684683dfe663955dd`
...
返回Wi-Fi开启状态，true表示Wi-Fi开启，false表示Wi-Fi关闭。
...
wifiManager.addAllowedWifiList
19+
...
Wi-Fi允许名单数组。数组总长度不能超过200。例如，若当前允许名单数组中已有100个Wi-Fi，则最多支持通过该接口再添加100个。
...
wifiManager.removeAllowedWifiList
19+
...
wifiManager.addDisallowedWifiList
19+
...
Wi-Fi禁用名单数组。数组总长度不能超过200。例如，若当前禁用名单数组中已有100个Wi-Fi，则最多支持通过该接口再添加100个。
...
wifiManager.removeDisallowedWifiList
19+
...
true表示强制开启Wi-Fi，强制开启后不支持用户在设备上手动关闭Wi-Fi开关，必须采用
turnOffWifi
...
Wi-Fi热点的MAC地址。获取方式如下：打开设置应用-点击系统选项-点击开发者选项-开启WLAN详细日志记录开关，然后进入设置应用中的WLAN列表，查看显示的MAC地址。若一个Wi-Fi对应多个MAC地址，需添加所有MAC地址。
...
IP地址，十进制表示，正常点分十进制写法为192.168.1.1，对应的十进制为3232235777。
...
DNS服务器，数组内最多包含首选DNS服务器和备用DNS服务器两个地址。

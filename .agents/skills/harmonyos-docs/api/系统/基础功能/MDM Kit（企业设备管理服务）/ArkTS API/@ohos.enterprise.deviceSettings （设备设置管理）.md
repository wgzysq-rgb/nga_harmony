...
**Slug**: `js-apis-enterprise-devicesettings`

**DocID**: `bfc5b2281896473a8a9e6cfc3a59f264`

**NodeID**: `0002017757963416846567020ab26b44`
...
deviceSettings.setHomeWallpaper
20+
...
需要设置为桌面壁纸图片的文件描述符，可以通过file.fs的
openSync
接口获取应用沙箱目录下的图片文件描述符。壁纸图片大小不能超过100MB。
...
无返回结果的Promise对象。当设置桌面壁纸失败后会抛出错误对象。
...
deviceSettings.setUnlockWallpaper
20+
...
需要设置为锁屏壁纸图片的文件描述符，可以通过file.fs的
openSync
接口获取应用沙箱目录下的图片文件描述符。壁纸图片大小不能超过100MB。
...
无返回结果的Promise对象。当设置锁屏壁纸失败后会抛出错误对象。

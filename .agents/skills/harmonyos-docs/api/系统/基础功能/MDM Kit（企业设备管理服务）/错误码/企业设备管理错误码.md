...
> **分区**: API参考  |  **Slug**: `errorcode-enterprisedevicemanager`  |  **DocID**: `35c6635b546f44608ff7eba3c532565b`
...
# 企业设备管理错误码
     ...
     **处理步骤**
      ...
      - 检查指定的设备管理器是否已激活了，且重复激活时管理器类型发生变化，此时需要先解除激活，再重新激活。
     ...
     **处理步骤**
      ...
      - 检查解除激活时设备管理应用是否激活过。
      - 检查解除激活时设备管理应用中指定设备管理元能力组件是否激活过。
      - 检查调用者是否解除激活自身，设备管理器应用不能解除激活其他设备管理器应用。
     ...
     **可能原因**
     ...
     该错误码表示指定的用户ID非法，可能原因如下。
      ...
      - 调用接口设置用户策略，指定的user ID不存在。
      - DA管理器不允许跨用户设置，指定的user ID和调用者user ID不同。
     
     **处理步骤**

     
      - 检查调用接口中指定的user ID是否是有效的user ID。
      - 检查调用者user ID和指定设置的user ID是否是同一user ID，DA模式下不允许跨用户设置策略。
     ...
     **可能原因**
     ...
     参数校验失败时，可能的原因如下。
      ...
      - 传入的企业设备管理扩展组件不属于当前应用。
      - 传入的参数长度超长。
      - 传入的数组长度超出限制。
      - 参数值不在取值范围内。
      - 对尚未设置的禁用或允许名单进行了移除操作。
     
     **处理步骤**

     
      - 检查传入的企业设备管理扩展组件是否属于当前应用。
      - 检查传入的参数长度是否超过接口限制。
      - 检测传入的数组长度是否超过接口限制。
      - 检查传入参数值是否在接口支持取值范围内。
      - 移除前先查询是否有设置相应的名单。
     ...
     **可能原因**
      ...
      - 当设置了禁止运行应用名单（例如调用[addDisallowedRunningBundlesSync](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-enterprise-applicationmanager#applicationmanageradddisallowedrunningbundlessync)接口），且应用在禁止运行应用名单中时，启动组件会失败。
      - 当设置了允许运行应用名单（例如调用[addAllowedRunningBundles](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-enterprise-applicationmanager#applicationmanageraddallowedrunningbundles21)接口），且应用不在允许运行应用名单中时，启动组件会失败。
     
     **处理步骤**

     
      - 将应用从禁止运行名单中移除，例如调用[removeDisallowedRunningBundlesSync](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-enterprise-applicationmanager#applicationmanagerremovedisallowedrunningbundlessync)接口。
      - 将应用添加到允许运行名单，例如调用[addAllowedRunningBundles](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-enterprise-applicationmanager#applicationmanageraddallowedrunningbundles21)接口。
     ...
     **可能原因**
      ...
      - 传入了不存在bundleName和abilityName。
      - 传入的Ability对外不可见。
     
     **处理步骤**

     
      - 检查want中的bundleName和abilityName是否正确。
      - 检查传入的Ability对外是否可见。
     ...
     **可能原因**
     ...
     当已有一个日志收集任务正在执行时，调用了[systemManager.startCollectLog](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-enterprise-systemmanager#systemmanagerstartcollectlog23)接口创建日志收集任务。

     **处理步骤**

     等前一个日志收集任务完成（即收到[EnterpriseAdminExtensionAbility.onLogCollected](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-enterpriseadminextensionability#onlogcollected23)回调）后，再调用[systemManager.startCollectLog](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-enterprise-systemmanager#systemmanagerstartcollectlog23)接口创建日志收集任务。

...
**Slug**: `js-apis-serialmanager`

**DocID**: `573d0dbd5bbb4b049c6418e2080d3d6c`

**NodeID**: `000201775796341684961ddf6788ef69`
...
Promise对象，true表示请求权限成功，false表示请求权限失败。
...
超时时间（单位：ms），API在目标端口缓冲区无数据时，等待指定时间后返回。默认值0表示不等待直接返回。
...
超时时间（单位：ms），指定时间内等待API在目标端口的缓冲区是否可写，若可写则正常处理，若不可写等待超过指定时间后返回超时。默认值0表示不可写时不等待直接返回。

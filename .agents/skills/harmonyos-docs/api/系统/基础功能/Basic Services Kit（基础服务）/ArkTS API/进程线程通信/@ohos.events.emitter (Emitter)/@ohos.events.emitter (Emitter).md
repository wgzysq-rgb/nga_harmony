...
**DocID**: `ac08ec5f48c04c6088bf781d8867fcda`

**NodeID**: `000201775796341684823bda2b87ffa8`
...
持续订阅的事件。取值为长度不超过10240字节的自定义字符串，且不可为空字符。
...
单次订阅的事件。取值为长度不超过10240字节的自定义字符串，且不可为空字符。
...
事件ID。取值为长度不超过10240字节的自定义字符串，且不可为空字符。
...
该接口支持跨线程传输数据对象，需要遵循数据跨线程传输的规格约束，详见
线程间通信对象
。目前不支持使用
@State装饰器
、
...
发送的事件ID。取值为长度不超过10240字节的自定义字符串，且不可为空字符。
...
事件ID，string类型的eventId取值为长度不超过10240字节的自定义字符串，且不可为空字符。

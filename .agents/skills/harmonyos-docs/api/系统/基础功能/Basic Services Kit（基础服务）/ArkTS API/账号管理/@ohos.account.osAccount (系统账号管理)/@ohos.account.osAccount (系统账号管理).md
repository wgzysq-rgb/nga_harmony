...
**Slug**: `js-apis-osaccount`

**DocID**: `db0e6362e37944908c99d356765cdc1c`

**NodeID**: `000201775796341684848029195a46ed`
...
回调函数。返回true表示支持多系统账号；返回false表示不支持。
...
Promise对象。返回true表示支持多系统账号；返回false表示不支持。
...
需要权限：
 ohos.permission.MANAGE_LOCAL_ACCOUNTS或ohos.permission.INTERACT_ACROSS_LOCAL_ACCOUNTS，以上权限仅系统应用可申请。
...
回调函数。返回true表示账号已激活；返回false表示账号未激活。
...
Promise对象。返回true表示账号已激活；返回false表示账号未激活。
...
[h2]isOsAccountConstraintEnabled
11+
...
[h2]checkOsAccountConstraintEnabled
(deprecated)
...
回调函数。返回true表示当前账号为测试账号；返回false表示当前账号非测试账号。
...
Promise对象。返回true表示当前账号为测试账号；返回false表示当前账号非测试账号。
...
Promise对象。返回true表示当前账号已认证解锁；返回false表示当前账号未认证解锁。
...
回调函数。返回true表示当前账号已认证解锁；返回false表示当前账号未认证解锁。
...
需要权限：
 ohos.permission.MANAGE_LOCAL_ACCOUNTS，以上权限仅系统应用可申请。
...
回调函数。如果获取成功，err为null，data为已创建的系统账号的数量；否则为错误对象。
...
回调函数。如果获取成功，err为null，data为当前进程所属的系统账号ID；否则为错误对象。
...
回调函数。如果查询成功，err为null，data为对应的系统账号ID；否则为错误对象。
...
[h2]getOsAccountLocalIdForUidSync
10+
...
[h2]getOsAccountLocalIdForDomain
9+
...
回调函数。如果查询成功，err为null，data为域账号关联的系统账号ID；否则为错误对象。
...
回调函数，如果获取成功，err为null，data为该系统账号的全部
约束
；否则为错误对象。
...
[h2]getActivatedOsAccountLocalIds
9+
...
回调函数。如果查询成功，err为null，data为当前处于激活状态的系统账号的ID列表；否则为错误对象。
...
Promise对象，返回当前处于激活状态的系统账号的ID列表。
...
需要权限：
 ohos.permission.MANAGE_LOCAL_ACCOUNTS或ohos.permission.GET_LOCAL_ACCOUNTS
10+
，以上权限仅系统应用可申请。
...
回调函数。如果查询成功，err为null，data为当前进程所属的系统账号信息；否则为错误对象。
...
回调函数。如果查询成功，err为null，data为当前进程所属的系统账号的账号类型；否则为错误对象。
...
[h2]queryDistributedVirtualDeviceId
9+
...
需要权限：
 ohos.permission.MANAGE_LOCAL_ACCOUNTS（仅系统应用可申请）或 ohos.permission.DISTRIBUTED_DATASYNC
...
[h2]getOsAccountLocalIdForSerialNumber
9+
...
回调函数。如果成功，err为null，data为与SN码关联的系统账号的账号ID；否则为错误对象。
...
The account indicated by serialNumber does not exist.
...
[h2]getSerialNumberForOsAccountLocalId
9+
...
回调函数。如果获取成功，err为null，data为与该系统账号关联的SN码；否则为错误对象。
...
通过系统账号ID获取与该系统账号关联的SN码。使��Promise异步回调。
...
[h2]isOsAccountConstraintEnable
(deprecated)
...
需要权限：
 ohos.permission.MANAGE_LOCAL_ACCOUNTS，该权限仅系统应用可申请。
...
回调函数。返回true表示指定账号已验证；返回false表示指定账号未验证。
...
Promise对象。返回true表示指定账号已验证；返回false表示指定账号未验证。
...
[h2]getOsAccountLocalIdFromProcess
(deprecated)
...
[h2]getOsAccountLocalIdFromDomain
(deprecated)
...
回调函数，如果获取成功，err为null，data为域账号关联的系统账号ID；否则为错误对象。
...
回调函数。如果获取成功，err为null，data为指定系统账号的全部
约束
；否则为错误对象。
...
[h2]getOsAccountTypeFromProcess
(deprecated)
...
[h2]getDistributedVirtualDeviceId
(deprecated)
...
需要权限：
 ohos.permission.MANAGE_LOCAL_ACCOUNTS（仅系统应用可申请）或ohos.permission.DISTRIBUTED_DATASYNC
...
[h2]getOsAccountLocalIdBySerialNumber
(deprecated)
...
回调函数。如果查询成功，err为null，data为与SN码关联的系统账号的账号ID；否则为错误对象。
...
[h2]getSerialNumberByOsAccountLocalId
(deprecated)
...
[h2]getForegroundOsAccountLocalId
15+
...
需要权限：
 ohos.permission.GET_DOMAIN_ACCOUNTS 和 ohos.permission.INTERACT_ACROSS_LOCAL_ACCOUNTS，以上权限允许系统应用和企业应用进行申请。
...
需要权限：
 ohos.permission.MANAGE_LOCAL_ACCOUNTS或ohos.permission.MANAGE_DOMAIN_ACCOUNTS
...
The new account info is invalid.

12300003

The old account not found.

12300004

The new account already exists.
...
Invalid server config parameters.
...
The number of server config reaches the upper limit.
...
Server config has been associated with an account.
...
constraint.os.account.activate

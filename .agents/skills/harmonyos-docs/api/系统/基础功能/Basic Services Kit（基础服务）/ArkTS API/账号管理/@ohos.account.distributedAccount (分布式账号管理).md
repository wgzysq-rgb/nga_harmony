...
**Slug**: `js-apis-distributed-account`

**DocID**: `7342b007af264298b866a27ecf3c96d3`

**NodeID**: `00020177579634168480085d7f053ecc`
...
[h2]getOsAccountDistributedInfo
9+
...
需要权限：
 ohos.permission.MANAGE_DISTRIBUTED_ACCOUNTS（仅系统应用可申请）或ohos.permission.GET_DISTRIBUTED_ACCOUNTS（仅系统应用可申请）或ohos.permission.DISTRIBUTED_DATASYNC
...
回调参数。当获取分布式账号信息成功，err为undefined，data为获取到的分布式账号信息对象；否则为错误对象。
...
[h2]queryOsAccountDistributedInfo
(deprecated)
...
回调函数。当获取分布式账号信息成功，err为undefined，data为获取到的分布式账号信息对象；否则为错误对象。
...
[h2]setOsAccountDistributedInfo
9+
...
需要权限：
 ohos.permission.MANAGE_DISTRIBUTED_ACCOUNTS，该权限仅系统应用可申请。
...
回调函数。当更新分布式账号信息成功时，err为undefined，否则为错误对象。
...
[h2]updateOsAccountDistributedInfo
(deprecated)

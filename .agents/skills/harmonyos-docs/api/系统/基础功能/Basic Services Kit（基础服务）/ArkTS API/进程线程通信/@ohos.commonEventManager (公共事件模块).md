...
**DocID**: `fe5ebebef0114eafa57061a3478ef78d`

**NodeID**: `00020177579634168480480716dd7fd5`
...
系统公共事件是指由系统服务或系统应用发布的事件，订阅这些公共事件需要特定的权限、使用相应的值，详见
系统定义的公共事件
。
...
回调函数。当公共事件发布成功时，err为undefined，否则为错误对象。
...
回调函数。当公共事件订阅者创建成功时，err为undefined，否则为错误对象。
...
回调函数。当公共事件订阅成功后，事件触发时执行的回调函数；否则订阅失败时，err为错误对象。
...
回调函数。当取消公共事件订阅成功时，err为undefined，否则为错误对象。
...
_CommonEventPublishData

描述公共事件内容和属性。

---
*2026-04-22T15:55:03.076Z*
...
**DocID**: `00ef23f69c76482cb26c6fe203f07438`

**NodeID**: `0002017757963416845572ea6b209597`
...
检查待压缩的文件路径是否存在，并且路径是否在正确的沙箱路径下。
...
当调用decompressFile接口时，传入的源文件压缩格式有误。

当调用decompressFile接口时，传入的源文件不完整或已损坏。

处理步骤

检查源文件压缩格式是否为ZIP格式。

检查源文件是否完整，如果是网络下载的，需要确保文件下载完成后再调用decompress接口。
...
当调用gzopen接口时，传入的文件路径或文件访问模式错误。

当调用gzdopen接口时，传入的文件描述符或文件访问模式错误。

处理步骤

检查gz文件路径或文件描述符是否正确，如果是新建gz文件的操作，需要确保文件访问模式为压缩模式(w或wb等)。

检查文件访问模式，是否为"w"、"wb"、"r"、"rb"、"a"、"ab"等。
...
当调用deflate或inflate等接口时，未先使用deflateInit或inflateInit接口对压缩或解压流进行初始化。

当调用gzsetparams、gzclose或gzflush等接口时，传入的刷新模式错误或接口调用前，调用打开gz文件的相关接口时传入的文件访问模式错误。

处理步骤

当调用deflate或inflate等接口前，先使用deflateInit或inflateInit接口对压缩或解压流进行初始化。

检查接口调用前，打开gz文件的相关接口是否未被调用或调用失败。

检查接口调用前，调用打开gz文件的相关接口时传入的访问模式是否与该接口匹配。例如gzprintf属于压缩操作接口，调用打开gz文件的相关接口时传入的访问模式需要为压缩模式(w或wb等)。
...
当调用uncompress或uncompress2接口时，传入的输入buffer大小大于解压后输出buffer的大小。

当调用compress或compress2接口时，传入的输入buffer大小大于压缩后输出buffer的大小。

当调用compress或compress2或uncompress或uncompress2接口，传入的buffer大小为0。
...
当调用接口前，打开gz文件的相关接口未被调用或调用失败。

当调用接口前，调用打开gz文件的相关接口时传入的文件访问模式错误。

当调用gzwrite接口时，传入的未压缩字节长度为0。

当调用gzfwrite或gzfread接口时，传入的数据块大小或数量为0。

当调用gzprintf接口时，传入的格式化描述符和纯文本为空字符串。

当调用gzgets接口时，传入空的ArrayBuffer。

当调用gzgetc接口时，gz文件为空文件。

处理步骤

检查接口调用前，打开gz文件的相关接口是否未被调用或调用失败。

检查接口调用前，调用打开gz文件的相关接口时传入的访问模式是否与该接口匹配。例如gzgetc属于解压缩操作接口，调用打开gz文件的相关接口时传入的访问模式需要为解压缩模式(r或rb等)。

检查传入的参数是否正确。

---
*2026-04-22T15:53:59.371Z*
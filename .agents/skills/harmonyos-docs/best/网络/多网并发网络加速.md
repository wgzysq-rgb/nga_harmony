...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-multi-path-network-turbo`  |  **DocID**: `85120e27785c43ce81deb3ac7b1466ad`  |  **NodeID**: `000201775546867127702010d82b5252`
...
开发前准备多网并发能力在设备类型、网络状态及开发方案的选择上存在一定限制，具体接入要求可参考[多网发起和释放](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/networkboost-netmultipath-request-release)，开发者也可以在调试过程中通过[多网请求错误码](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-nethandover#section8508131819279)分析。
 ...
 Socket发起网络传输任务绑定网络后，TCP/UDP Socket进行[数据传输](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/socket-connection#应用通过tcp-socket-server进行数据传输)时将会使用对应的网络通路，多个Socket使用不同的网络通路同时发起网络传输任务就可以起到多网并发的效果。
...
- [Network Boost Kit](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/network-boost-kit-guide)：多网能力的发起、监听、状态管理与绑定。
- [@ohos.net.connection](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-net-connection)：网络连接管理，网络通路获取。
- [Socket数据传输](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/socket-connection)：创建网络连接，执行网络传输任务。
- [TaskPool](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/taskpool-introduction)：管理多线程网络并发任务。
- [@ohos.file.fs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-file-fs)：负责文件管理，包括文件分片、大小查询等。

  #### [h2]开发步骤

  #### [h2]代码实现

多网发起与接入根据当前网络传输任务通过[netBoost.setSceneDesc()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-netboost#section13106021163)设置多网场景，帮助系统进行多网并发管控和业务时长分析，当前对于文件分片上传任务选择'upload'场景。```
/**
...
```
 多网通路绑定Socket封装网络查询函数，通过[connection.getAllNetsSync()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-net-connection#connectiongetallnetssync10)获取当前所有网络信息。由于网络传输任务一般放在子线程实现，而当前无法跨线程传递Socket对象和NetHandle对象，为了在子线程实现网络与Socket绑定需要获取所有网络信息，再通过之前请求多网后记录的NetHandle对象的ID找到指定网络在执行绑定操作；如果应用业务不涉及多线程问题可直接绑定上一步多网状态监听中获取到的NetHandle。
...
```
 文件分片处理封装文件处理类，设置分片文件数据结构，包括当前文件片段数量、开始位置、分片大小、片段重试次数等信息。
...
export function splitFile(file: fileIo.File): FileUpChunk[] {
  try {
    ...
    hilog.error(DOMAIN, TAG, `[splitFile] error:${err.code},${err.name}`);
    return [];
  }
}
```
 多Socket并发上传不同文件片段封装分片文件上传类，提供初始化分片上传接口，通知服务端待上传文件片段对应的存储路径。此处代码片段仅供参考，开发者可以根据自己业务的实际实现方式调整通信格式、请求参数等。
  ...
  if (value) {
    try {
      ...
      if (eTagMatch && eTagMatch[1]) {
        ...
        hilog.error(0x0000, 'FilePartsUploadManager',
          `[uploadPart] onMessage--> fileChunkId:${fileChunk.chunkNumber} upload failed`);
        return undefined;
      }
    } catch (error) {
      const err: BusinessError = error as BusinessError;
...
```
 通知服务端整合分片文件全部文件片段上传完毕后，通知服务端文件片段顺序，进行文件整合。
...
```
 释放资源通过[netHandover.releaseMultiPath()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-nethandover#section196405414517)释放多网，注销相关的监听方法。
  ...
  if (this.downUrls) {
    ...
    try {
      ...
      if (fileSize > 0) {
        ...
        // download failed,retry again
        this.retryDownload();
        hilog.error(DOMAIN, TAG, `[startDownload] download failed`);
      }
    } catch (error) {
      // download failed,retry again
      this.retryDownload();
...
```
 ...
 #### [h2]在进行多网并发传输时，如何判断当前使用的网络是Wi-Fi还是流量

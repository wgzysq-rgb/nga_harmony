...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-app-privacy-protection`  |  **DocID**: `fa6b73859807474884dfd1ded7c2f005`  |  **NodeID**: `000201775547083344269af964531fc9`
...
- 什么是隐私数据：根据个人数据的敏感程度，个人数据分为敏感个人数据和一般个人数据。健康、性生活、个人基因、宗教信仰、生物体征等属于敏感数据。
- [隐私保护的原则](#section137983974814)。
- 隐私保护的措施：隐私保护的一些建议和方法，以及隐私保护的一些[最佳实践](#section182128512489)。
...
数据收集及使用公开透明应用采集个人数据时，应告知用户个人信息的使用方式。

 数据收集和使用最小化应用收集个人数据应与目的相关，适当、必要。开发者应匿名化或化名个人数据，降低风险。仅收集和处理必需的个人数据，不进行无关处理。

 数据处理选择和控制处理个人数据需征得用户同意，用户有充分的控制权。

 数据安全技术上保证数据处理的安全性，包括加密存储和安全传输，系统默认开启或采取安全保护措施。

 本地化处理应用开发时，数据优先在本地处理。本地无法处理的数据上传至云服务时，应遵循最小化原则，不得默认选择上传。

 未成年人数据保护要求如果应用面向未成年人，或通过用户年龄数据识别出未成年人，开发者应根据目标市场的相关法律，专门分析未成年人个人数据保护问题，并在收集未成年人数据前征得监护人同意。
...
使用隐私声明获取用户同意。应用采集个人数据时，应告知用户个人信息的使用方式。例如，应用启动时使用隐私声明弹窗说明敏感数据的使用和收集，获得用户同意后才能获取数据。
 ...
 减少使用存储权限。过多的存储访问权限可能会导致用户隐私数据的泄露，应该减少使用存储权限，仅请求访问应用程序所需的数据，可以用[Picker](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-file-picker)来减少对用户存储数据的访问权限。
...
- 增强用户控制权：隐私声明弹窗允许用户授权或拒绝应用程序访问敏感权限，从而增强对个人数据的控制。
- 保护用户隐私：隐私声明弹窗确保用户在同意授权前了解应用程序如何使用个人数据，防止滥用，保护隐私权益。
- 增强透明度：隐私声明弹窗为用户提供应用程序的数据使用透明度。弹窗中提供详细说明，用户了解所需权限及其使用目的，从而放心做出授权决策。
...
在module.json5配置文件中声明ohos.permission.APPROXIMATELY_LOCATION权限。
...
```
 在需要使用位置信息的代码块中，动态申请 ohos.permission.APPROXIMATELY_LOCATION权限。
...
```
 获取位置信息的接口相同，但模糊定位和精确定位的返回结果精确度不同。
...
- 合理的权限范围：应用程序应仅请求所需敏感权限，并将权限范围限制在最小必要。这可减少权限滥用和数据泄露风险。
- 明确解释权限用途：应用程序应清楚、明确地说明每个敏感权限的用途，并在隐私政策和其他位置提供此信息。

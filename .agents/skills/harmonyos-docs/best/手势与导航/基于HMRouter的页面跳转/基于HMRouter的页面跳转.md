...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-hmrouter`  |  **DocID**: `3f71255b6bec4a488896a7601b387feb`  |  **NodeID**: `000201775546825968249ea1e84dc83e`
...
定义网络请求函数，可使用[TaskPool](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/taskpool-introduction)在其他线程执行网络请求并返回请求结果。```
@Concurrent
...
使用[BuilderNode](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-user-defined-arktsnode-buildernode)构造出评论区组件，在makeNode函数中，若评论区不存在则创建，存在便直接返回。```
@Builder
...
- 定义一个类PageDurationLifecycle实现IHMLifecycle接口。
- 为该类添加@HMLifecycle注解，并配置global为true，将该生命周期配置到全局，所有页面都会执行该生命周期。
在页面显示时（onShown）记录当前的时间戳，在页面隐藏时（onHidden）计算页面停留时长。```

...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-comment-reply-pop-up-window`  |  **DocID**: `0b1a45b85dfe4c7cbe7d603a3fd941cc`  |  **NodeID**: `000201775546819649169683183bb07b`
...
- 现成封装好的组件，无需开发者实现弹窗相关的交互逻辑。
- 自定义弹窗默认避让软键盘，评论模块无需计算高度，调用时直接被软键盘抬起即可。

 方案劣势：

 
- 由于自定义弹窗完全避让软键盘，且该行为无法配置。在点击表情按钮时，展示表情面板，此过程评论模块高度发生变化，到软键盘完全收起的过程中，表情面板仍然处于软键盘上方，评论模块会被短暂顶起。
- 软键盘动画不能自定义配置或获取，无法配合动画能力抵消顶起操作。
...
- 现成封装好的组件，无需开发者实现弹窗相关的交互逻辑。
- 可解决CustomDialog中评论模块被顶起的问题。

 方案劣势：

 
- 设置高度自适应后，bindSheet内部的Scroll依然生效，在bindSheet内部可滚动。
- 设置dragBar为false时，bindSheet依然可以上下拖动，松手后回到原位，但此过程会暴露软键盘下方的表情面板区域。

 

 [Navigation Dialog](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-navigation-navigation#页面显示类型)方案优势：

 
- 可解决上述两种方案中存在的问题。
- 基于Navigation路由形式，以进出路由栈的方式打开或关闭弹窗，可以实现弹窗与UI界面解耦。

 方案劣势：

 
- 弹窗遮罩层以及点击遮罩关闭弹窗的逻辑需要手动实现。
...
- 使用addTextSpan实现@好友
在textSpan前后通过软键盘输入的文字会自动与之合并成一个textSpan，而@好友具有特殊样式和逻辑，需要对前后文字进行分割，这些细节需要手动处理。
- @好友作为一个整体，光标不能在中间点击，删除需要整体删除，使用textSpan需要手动处理这些细节。
- 获取编辑区域内容时，可以获取到textSpan中的文字（好友昵称），但是如果想获取到该好友的其他信息，仅用好友昵称去关联显然不是一种可靠的方案。

 - 使用addBuilderSpan实现@好友
builderSpan不会和前后文字合并。
- builderSpan默认作为一个整体，对光标点击以及删除逻辑不需要另外处理。
- 获取编辑区域内容时，无法获取到builderSpan中的内容，需要手动对添加或删除的builderSpan信息进行维护，自己维护的同时自然也能与好友的其他信息进行关联。

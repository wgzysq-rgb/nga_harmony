...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-improve-layout-performance`  |  **DocID**: `7397ff0392a7451ba32f66c945cdf706`  |  **NodeID**: `000201775546819649163c6908d883fc`
...
# 布局优化指导
     ...
     - 数据处理过程中主要是对状态数据进行更新，状态数据指的是所定义的@State等相关的数据。数据变化时，会有一定的更新耗时，并且数据关联的组件数量，也影响下一步UI更新的耗时，在开发过程中需要避免无效的数据更新，从而减少冗余的UI更新耗时。关于这部分的优化措施可以参考《[状态管理最佳实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-status-management)》。
...
```
     ...
     在初次加载的情况下的测试结果如下：
               ...
               **表3 ****使用if/else和visibility属性控制显隐的布局时间对比**                                         对比指标
     ...
     在切换显示状态的情况下的结果如下：
               ...
               **表4 ****使用if条件判断和visibility控制显隐的Measure/Layout时间对比**                                         对比指标
     ...
     **懒加载**
     ...
     针对长列表这一场景，在本地模拟了10、100、1000、10000条数据，分别使用ForEach、LazyForEach，来测试关闭和开启懒加载情况下的完全显示所用时间、列表挂载时间、独占内存，并分析了其滑动过程中的丢帧率。其中，列表挂载时间是指创建组件和组件挂载数据的总时长。最终，使用DevEco Studio的Profiler工具检测下述指标，得到的数据如下所示：
               ...
               **表6 ****LazyForEach在不同数据量下的指标对比**                                         **Lazy**ForEach对比指标

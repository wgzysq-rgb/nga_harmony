...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-custom-hvigor-plugin`  |  **DocID**: `919fb41a4c91486ea303555714fbbfea`  |  **NodeID**: `000201775547138833514075fd2e984e`
...
- hvigor-ohos-plugin将默认构建任务和自定义插件任务，都装进hvigor任务流。
...
- [初始化typescript项目](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-plugin#section1243715533156)：配置npm环境，安装typescript依赖，新建一个npm项目做为插件的工程。
[开发插件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-plugin#section627771916612)：对插件项目配置HarmonyOS镜像仓库，安装hvigor开发依赖，然后进行插件开发，以下是该场景的插件逻辑实现。

- 在新建的custom-plugin.ts文件中，编写任务函数renameHarTask()。
- 在函数中注册自定义任务，使用pluginContext.registerTask()方法。
- 通过taskContext得到modulePath（模块路径）、moduleName（模块文件名），拼装得到编译产物的原始文件路径。
- 根据需要，读取oh-package.json5中配置属性，拼接生成目标文件名。
- 使用fs.mkdir()创建指定目录，再调用fs.rename()，将编译产物移动到目标路径，并修改为新文件名。

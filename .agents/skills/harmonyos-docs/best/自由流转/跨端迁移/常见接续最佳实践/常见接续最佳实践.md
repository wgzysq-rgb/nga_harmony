...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-application-continue-progess`  |  **DocID**: `417e6c5f1164437eb534490b04d250dc`  |  **NodeID**: `000201775547034473700ce2011beff5`
...
进度接续的核心在于确保进度数据在不同设备间的精确传输与同步。在实际开发过程中，开发者会遇到各种复杂的接续需求，首要任务是深入分析哪些数据对接续控制至关重要。在源设备启动接续时，应保存数据；在目标设备接续时，需准确恢复数据，以确保进度的连续性和设备间数据的一致性。
 ...
 本章节将介绍如何配置应用以使用接续能力，以及如何保存和恢复数据以实现应用的无缝接续。具体场景包括[长列表进度接续](#section16702516134216)、[媒体播放进度接续](#section12439210434)和[Web浏览进度接续](#section3512987460)。

 
启用接续在module.json5文件的abilities中，将continuable标签配置为“true”，表示该UIAbility可以被迁移。配置为“false”的UIAbility将被系统识别为不可迁移，且该配置的默认值为“false”。
...
```
 源端保存迁移数据当对端点击接续图标时，源端将触发UIAbility中的[onContinue()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-uiability#oncontinue)接口。在此接口中，开发者可以将需要迁移的数据以键值对形式保存至wantParam中，并返回AbilityConstant.OnContinueResult.AGREE，标识应用同意迁移，从而将数据迁移至对端。
...
```
 对端恢复数据在源端保存数据并同意迁移后，对端可启动应用，并在UIAbility中的onCreate()或onNewWant()生命周期回调中恢复数据。如果Ability的启动原因为LaunchReason.CONTINUATION，开发者可以从want.parameters中获取保存的键值对数据。
...
.restoreId(1)
```
 使用分布式迁移标识可以快速实现接续。然而，该方法存在局限性，具体支持的场景和版本详见[分布式迁移标识](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-restoreid)的说明。若需在开发中进行更多自定义设置以提升用户体验，可参考以下步骤。

 

 
- [启用接续](#li6149192715494)。
在Scroll组件的onDidScroll()接口中监听长列表的浏览进度变化。```
...
```
 - 在UIAbility的onContinue()回调中，将进度相关数据保存到wantParam中，参考[保存迁移数据](#li1745816354491)。
- 在UIAbility的onNewWant()和onCreate()回调中，从want.parameters中恢复数据，参考[恢复数据](#li631218439498)。
...
- [启用接续](#li6149192715494)。
使用avPlayer.on('timeUpdate')接口来监听媒体播放进度的变化。```
...
```
 - 在UIAbility的onContinue()回调中，将当前播放时间this.time保存到wantParam中，参考[保存迁移数据](#li1745816354491)。
- 在UIAbility中的onNewWant()和onCreate()回调中，从want.parameters中恢复数据，参考[恢复数据](#li631218439498)。
在avPlayer初始化完成后，判断当前为接续状态，调用封装的调整视频进度方法videoSeek()，恢复至接续前的播放状态。```
if (this.continue) {
...
- [启用接续](#li6149192715494)。
- 使用onTouch()事件监听屏幕滑动，并通过runJavaScript()接口获取页面滚动条距离顶部的距离。
- 在onContinue()回调中，将this.scrollDistance保存到wantParam中，参考[保存迁移数据](#li1745816354491)。
- 在onNewWant()和onCreate()回调中，从want.parameters中恢复数据，参考[恢复数据](#li631218439498)。
在onPageEnd()回调中调用runJavaScript()接口以恢复进度。```

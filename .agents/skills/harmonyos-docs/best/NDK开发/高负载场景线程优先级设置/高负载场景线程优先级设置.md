...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-thread-priority-setting`  |  **DocID**: `f0fb372522c24f04af95260a4ac20c01`  |  **NodeID**: `000201775546912183519b5a099daeac`
...
[服务质量（QoS）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/qos-guidelines)一文介绍了QoS的基本概念、原理、各个QoS等级适用的场景及负载特征及相关接口的用法。
 ...
 [FFRT（Function Flow运行时）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ffrt-overview)的QoS提供了ffrt_qos_inherit（-1）到ffrt_qos_user_initiated（3）5个优先等级，它与当前的QoS接口有着同一套底层逻辑。两者的差别在于，当前开发所用的QoS接口是直接开放给应用线程的，而FFRT的QoS则是面向任务的优先级配置，关于线程编程模型和任务编程模型的对比详见 [FFRT 概述](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ffrt-overview)。
...
```
 2、实现高、低QoS等级计算线程（关键线程）所要完成的计算任务（斐波那契数列计算）。先通过 [OH_QoS_SetThreadQoS](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/qos-guidelines#oh_qos_setthreadqos) 接口设置当前线程的QoS等级，再执行 DoFib() 斐波那契数列计算。

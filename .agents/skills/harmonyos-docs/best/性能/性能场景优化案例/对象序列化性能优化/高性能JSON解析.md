...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-high-performance-json-parsing`  |  **DocID**: `28bfc305f3854a1ea40d699f051f80cc`  |  **NodeID**: `00020177554711181382145574fdea07`
...
- 手动创建业务对象并遍历JSON属性赋值，但是代码冗余且易出错；
- 引入[class-transformer](https://gitcode.com/openharmony-tpc/openharmony_tpc_samples/tree/master/class-transformer)等第三方库，通过运行时反射动态映射字段，但反射操作会显著影响性能（如解析耗时增加30%+）。
...
```
 ...
 插件生效还需在工程根目录 hvigorfile.ts 添加相关插件配置，参考如下：```
...
```
 给自定义类添加@Serializable装饰器，标记为自定义序列化类。```
...
```
 在首次配置插件之后，执行下述命令，确保模块配置：hvigorw jsonSync 该命令会扫描项目中使用 @Serializable 注解的模块，自动配置 build-profile.json5 文件并生成序列化器代码。

 获取网络数据后，可使用TJSON提供的fromBuffer()或fromString()方法进行反序列化，生成相应的业务对象。```
...
向TurboTransJSON提供的@Serializable装饰器添加generateSendable: true属性。```
import { Serializable } from '@hadss/turbo-trans-core';
...
```
 定义子线程执行方法，在子线程内创建原始对象并使用toSendable()方法，将原始对象转换为Sendable对象，并返回给主线程。使用引用方式跨线程传输数据，避免序列化和序列化耗时以及破坏对象结构。```
...
```
 ...
 通过实例化对象调用toOrigin()方法将子线程返回的Senable对象转换为开发者自定义对象。```
...
通过toJsonNodeFromBuffer()或toJsonNodeFromString()方法，将ArrayBuffer数据或JSON字符串转换为JsonNode对象。```
// this.json.buffer is read citylots.json buffer
let jsonNode = TJSON.toJsonNodeFromBuffer(buffer);
```
 通过JsonNode提供的JsonPointer局部访问功能，调用JsonNode的at()方法读取当前需解析的部分。```
...
```
 将读取的部分JsonNode转化为业务所需对象，其他部分保持未解析状态。CityLots类型定义：
...
```
 - 针对未定义业务类型的，可以使用toPlainObjectAsync()方法将JsonNode转换为简单对象。在实际业务场景中，可先利用JsonNode提供的JsonPointer局部访问能力读取所需节点，再通过toPlainObjectAsync方法转换，以减少对象创建的耗时。```

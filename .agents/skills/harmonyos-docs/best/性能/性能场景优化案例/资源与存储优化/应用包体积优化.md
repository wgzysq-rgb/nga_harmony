...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-decrease_pakage_size`  |  **DocID**: `ff8d4f3c38d144f589a5461accae5360`  |  **NodeID**: `0002017755471118138231d4833d5535`
...
减小应用包大小可以提升应用下载和安装体验。压缩、精简或复用代码和资源可以有效降低应用包体积。在优化包大小前，先了解HarmonyOS应用的[Stage模型应用程序包结构](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/application-package-structure-stage)，对HAP、HAR、HSP等特点建立一定认知，然后使用扫描工具分析App包，根据报告优化应用。具体优化方法可以参考以下内容：
...
- 使用ohpm的[override](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-oh-package-json5#zh-cn_topic_0000001792256137_overrides)机制或开启 `[resolve_conflict](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-ohpmrc#section368717475562)` 解决依赖冲突，减少依赖包导致的重复编译问题。
- 将不常用的功能作为按需加载的模块。

  #### 使用扫描工具分析App大小问题

[扫描工具](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-check-tool)可用于分析检测应用包，根据不同的参数设定，扫描指定路径的App、HAP、HSP包内容并输出检测结果报告，为开发者优化包结构或排查问题提供数据支撑。
...
- JPG、PNG、GIF等文件，可以考虑压缩图片。
 ...
 so文件，通过[配置so库压缩选项](#section15826132851819)来实现压缩打包。
...
对于ohpm 1.5.0之前的版本，如果hap依赖了不同版本的har（例如下图中的V1版本的harC和V2版本的harC），默认情况下，V1和V2两个版本的harC都会被打包到hap中。开发者可以使用ohpm的[override](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-oh-package-json5#zh-cn_topic_0000001792256137_overrides)机制，指定只打包一个版本。
...
```
 ...
 对于1.5.0版本之后的ohpm，可以通过开启`[resolve_conflict](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-ohpmrc#section368717475562)`，自动解决依赖冲突。依赖冲突的处理策略为：当项目同时依赖某个三方库的不同版本时，ohpm会选择其中的最高版本进行安装。

 #### [h2]按需分发

对于应用中用户不常用的功能，可以考虑通过[产品特性按需分发](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/store-moduleinstall)的方式，将下载时机交由用户选择，使用时从应用市场获取安装，减少用户初次下载的包大小。

  #### [h2]多包场景下使用HSP共享代码和资源

...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-application-latency-optimization-cases`  |  **DocID**: `284e3492fc9b4f608860dc7bea31fad2`  |  **NodeID**: `0002017755471118138316ccd4a225cf`
...
**表6 **不同拖动识别距离下的耗时拖动距离设置
  ...
  最小拖动距离过大可能导致滑动脱手和响应时延慢等问题，从而导致性能劣化
...
- 在两个页面中设置pageTransition()转场动画参数，然后在列表页头像的点击事件中添加router跳转，实现列表页到个人页的转场动画效果。
- 在两个页面的头像组件Image中添加属性sharedTransition，并赋予相同的id以进行唯一匹配。同时，添加共享元素动画的时间等相关参数，实现头像从列表页向个人页移动的动画效果。
- 根据个人页内容的版面样式，实现一个骨架图组件，并使用animateTo()添加反复渐显渐隐的动画行为。在个人页组件onAppear()时呈现该动画，具体内容刷新后隐藏动画元素。
...
**表7 **运行效果图设置animationDuration为100ms
...
**表8 **animationDuration属性值对比animationDuration属性值

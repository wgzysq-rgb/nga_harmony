...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-zhenlv`  |  **DocID**: `c5697ff9af0644188094e62edfdeabbd`  |  **NodeID**: `00020177554711181380486a2b0c0201`
...
# 帧率问题分析
    ...
    《应用性能体检建议》中针对滑动帧率问题的指标有[应用或元服务内滑动操作响应快](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/performance-delay#section1767913186810)、[应用或元服务应用内滑动过程流畅](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/performance-frame-rate#section159268494256)，当前AppAnalyzer工具已支持（或等效支持）上述指标项。
     ...
     HarmonyOS图形系统采用统一渲染模式，遵循流水线模式。90Hz刷新率下，每个Vsync周期是11.1ms。60Hz刷新率下，每个Vsync周期是16.7ms。120Hz刷新率下，每个Vsync周期是8.3ms。
      ...
      - 应用侧处理用户的屏幕点击等输入事件，生成界面描述数据结构。数据包括UI元素的位置、大小、资源、绘制指令及动效属性。
      - Render Service是图形栈中负责界面内容绘制的模块，主要职责是对接ArkUI框架，支撑ArkUI应用的界面显示，包括控件和动效等UI元素。Render Service的RenderThread线程在Vsync信号触发下进行UI绘制，绘制过程包含三个阶段：动效、描画和提交。
      - Display是显示屏幕的抽象概念，既包括实际的物理屏，也包括虚拟屏。
     ...
     下图展示了解决丢帧问题的简要流程：
      ...
      - 识别卡顿：首先使用[AppAnalyzer](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-app-analyzer)检测滑动场景是否存在性能问题，如果检测存在丢帧，会得到相应的体检报告。
      - 查看建议：在体检报告中，可以看到工具定位出的故障位置，优化建议等。
      - 分析原因：根据工具的建议，结合代码逻辑，分析判断产生性能问题的原因。也可以结合代码分析Trace、查看函数调用栈等定位丢帧原因。
      - 选择优化方案：根据分析的丢帧原因，选择适合的优化方案。
      - 验证优化效果：优化完成后需要重新测试验证丢帧问题是否得到解决。
     ...
     **使用AppAnalyzer检测性能问题**
     ...
     首先使用AppAnalyzer工具进行性能问题检测，AppAnalyzer是DevEco Studio中提供的检测评分工具，用于测试并评价HarmonyOS应用或元服务的质量，能快速提供评估结果和改进建议，当前支持的测试类型包括兼容性、性能、UX测试和最佳实践等。因为本文主要是介绍丢帧问题的分析，所以下面重点介绍了使用AppAnalyzer对列表滑动响应和滑动过程中的流畅性能检测，具体使用可参考[《应用与元服务体检》](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-app-analyzer)。

     在进行规则体检或场景化体检之前，先要确保[DevEco Studio与真机设备已连接](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-run-device)，并根据[应用/元服务签名](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-signing)章节进行签名，再编译生成HAP或HSP。

     
      在DevEco Studio中启动AppAnalyzer工具，详细请参考：[AppAnalyzer](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-performance-detection#section135451444171)。       

       

       

      执行滑动场景体检。选择场景化体检，然后点击“手动性能页面滑动体检”，工具会进行准备，自动编译、安装、运行当前工程，需要保持手机解锁状态。       

       
        当提示“准备完成，请操作手机至检测页面”等内容时，需要在手机应用中找到待检测的滑动页面，然后点击开始按钮。         

        等待工具录制准备，当提示“体检中，请操作手机”时，在手机上的待检测页面执行滑动若干次。等待剩余时间结束或点击暂停按钮，再点击停止完成本次检测。         
     ...
     **问题1：UI线程应用自身方法耗时长**
     ...
     具体有以下三种情况：
      ...
      如果方法名aboutToBeDeleted调用次数过多，如下图所示：       
       ...
       解决方案：可以采用并行化，或者缓存的方式来优化业务逻辑。开发者可参考[其他主线程优化思路](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-time-optimization-of-the-main-thread#section4365993361)。

       设置子线程优先级的方法有两种，具体可参考：[Priority](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-taskpool#priority)和[QoS开发指导](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/qos-guidelines)。
     ...
     **问题2：组件未有效复用**
     ...
     在测试结果列表中，工具会明确标识出滑动等高频操作中存在创建行为的组件，这些组件可能存在优化空间，建议结合“[组件复用](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-component-reuse)”机制进行相应调整。组件的复用更新流程与常规状态管理更新方式保持一致，具体可参考[状态管理最佳实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-status-management)。各种复用问题的分析优化可参考[组件复用问题诊断分析](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-component-reuse-issue-diagnosis-and-analysis)。
     ...
     **超出尺寸占比**表示源图超过目标尺寸的程度。
     ...
     图片大纹理检测尺寸大于 256*256 像素且超出组件尺寸 10% 的图片建议开发者优化，针对不同来源与种类的图片可以采取每行对应的优化建议进行修改。例如，针对IDE中存放的本地图片可参考[图片资源加载优化](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-texture-compression-improve-performance)，针对沙箱类型的图片可以设置Image组件的[autoResize属性](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-image#autoresize)为true来让源图尺寸贴近组件尺寸，针对网络图片可以采取webp的压缩手段缩小图片体积，针对Gif图片可以降低每帧分辨率。
     ...
     **录制Frame模板**
     ...
     发现卡顿丢帧问题后，创建Frame模板进行录制。在录制期间，复现卡顿丢帧场景。具体操作步骤请参见[性能问题定位：深度录制](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/deep-recording)。
     ...
     **1 看线程状态和运行核，看是否被其他进程抢占资源，排除系统侧运行异常。**
     ...
     接下来通过Trace查看每一帧的具体耗时，点击图标跳转到卡顿帧应用侧Trace详情，如下图所示。开发者，可以点击泳道信息区的收藏按钮，将应用帧处理的泳道收藏置顶，防止上下文信息丢失。
    ...
    选择优化方案需要一些经验的积累，开发者可以参考一些[性能优化](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-performance-optimization)的最佳实践，来选择相应的优化方法。
...
```
     ...
     避免在主线程中执行冗余或耗时操作，这可能导致UI渲染阻塞，引起界面卡顿或掉帧，尤其在高频回调中。参考[主线程耗时操作优化指导](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-time-optimization-of-the-main-thread)。

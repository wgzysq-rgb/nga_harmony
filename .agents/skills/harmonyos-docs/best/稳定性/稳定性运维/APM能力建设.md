...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-stability-operate-apm`  |  **DocID**: `241921f1dd384d46978940e558ca5e4a`  |  **NodeID**: `00020177554713550938273ad62f5055`
...
# APM能力建设
 ...
 [音频卡顿事件介绍](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-audio-jank-event)
  ...
  external_log返回的路径是沙箱目录，非真实物理路径，应用有权限访问自己的沙箱目录，参考[应用沙箱目录](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-sandbox-directory)。external_log日志空间受限，应用处理完日志文件后请及时删除。
...
- APM代码早于业务代码。
- 应用启动阶段注册ErrorManager，可以捕获JS异常，发生JsError崩溃时进程不会退出。参考[如何使用ErrorManager捕获异常](https://developer.huawei.com/consumer/cn/doc/harmonyos-faqs-V5/faqs-arkts-81-V5)。
- 在ErrorManager回调中执行takeNext接口获取JsError崩溃事件。
- 应用发生CPP Crash异常退出，下次启动获取NativeCrash崩溃事件。
- 获取的崩溃信息可以持久化存储，上传后再删除，避免上传失败导致数据丢失。
- 日志管理，external_log日志处理后（比如上传后）及时清理。

---
*Updated: 2026-04-10 09:44:39*

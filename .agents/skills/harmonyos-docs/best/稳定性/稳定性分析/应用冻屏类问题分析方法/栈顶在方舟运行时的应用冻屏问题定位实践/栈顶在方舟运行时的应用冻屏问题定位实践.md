...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-stability-app-freeze-ark-runtime`  |  **DocID**: `d4e3c53da1b14a71a589172d70667970`  |  **NodeID**: `000201775547135509414fcd3659863f`
...
- 通过解析栈顶代码行，可以知道应用所有线程3S和6S正在执行的代码逻辑。
- 如果3S和6S的栈不一致，则说明3S和6S间程序仍在执行业务代码，线程未卡死，那么就需要考虑是否是应用执行的任务太重导致的线程繁忙，这个可以通过trace和log做进一步确认。
- 如果3S和6S的栈相同，可以看主线程和其他线程是否有相互持锁导致的死锁问题。
- 还可以根据调用栈查看是否有长耗时的函数调用，如DumpHeapSnapshot、RegExpExecutor::Execute等典型耗时函数。
...
- 主线程是否有大段空白，如果有大段空白，则说明主线程在持锁等待，可以进一步看主线程在等待谁唤醒。
- 主线程是否有某个函数调用耗时太长，如果有某个函数执行耗时过长（如超过3S），则可以分析为产生freeze的根因，需要进行优化。
- 主线程是否有某个函数频繁调用，如果有某个函数频繁调用，则可能是不合理的for循环逻辑导致。
- 主线程是否频繁触发GC，如果有频繁GC触发，则可能是应用有内存泄漏，需要排查内存泄漏问题。
- 主线程是否有大段runnable，如果有，则说明有调度问题，需要确认是否是应用高优先级任务过多，或者找调度域协助定位。
- 主线程是否有大段的Uninterruptible sleep，如果有，则说明有D状态问题，需要分析D状态问题发生原因。
...
- freeze前主线程是否仍有频繁的业务日志打印，如果有，则很有可能是业务繁忙导致，需要根据打印的日志信息找到对应代码逻辑进行优化。
- 如果freeze前打印的日志很少，则重点看最后几条日志信息，找到对应代码排查是否有死锁或者函数执行时间长的问题。
...
- 应用代码逻辑复杂，执行时间长导致freeze，问题典型特征如下：
除去jsruntime的栈顶后，freeze栈3S和6S执行的应用JS代码相同，则很可能是应用在执行耗时函数导致超时（详见案例一）
- trace显示应用主线程频繁调用某个应用接口（详见案例二）
- trace显示应用主线程调用某个应用接口耗时长（详见案例三）
- freeze前日志中有关键应用日志打印（详见案例四）

 - 应用内存泄漏导致频繁GC或触发OOM dump，问题典型特征如下：
trace上看到频繁触发GC（详见案例五）
- OOM触发Dump，Freeze栈上显示调用DumpHeapSnapshot接口（详见案例六）

 - 高优先级任务太多，问题典型特征如下：
freeze文件任务队列中VIP Priority Event+Immediate Priority Event+High Priority Event的任务数总量相加超过400（详见案例七）
- 栈顶为2帧napi+4 帧libruntime，前两帧反编译后是IsValidScriptBuffer和RunScriptBuffer（详见案例七）

 - 死锁问题，堆栈显示持锁和wait的都是应用的so库，问题典型特征如下：
3S和6S的栈相同
- 主线程在wait，其他线程持锁不释放
...
- 系统库代码逻辑复杂，执行时间长导致freeze，问题典型特征如下：
除去jsruntime的栈顶后，freeze栈3S和6S执行的系统库代码相同（详见案例十）
- trace显示应用主线程频繁调用某个系统库接口
- trace显示应用主线程调用某个系统库接口耗时长
- freeze前日志中有关键系统库日志打印

 - 死锁问题，堆栈显示持锁和wait的都是系统库so，问题典型特征如下：
3S和6S的栈相同
- 主线程在wait，其他线程持锁不释放（详见案例十一）
 ...
 **案例二：应用主线程频繁调用某个应用接口**
 ...
 该案例主线程的trace如下，从trace可以看出，应用在频繁调用某个接口。打开其中一小段发现是在频繁调用反序列化接口，每段反序列化的时间1us以上。反序列化接口在应用跨线程传输数据时调用，如调用Work的PostMessage接口或TaskPool的execute接口。由此可确认，该freeze问题发生的根因是���用频繁跨线程传输数据导致。
...
- 优化正则匹配的逻辑，避免超长字符串的正则匹配；
- 将耗时操作放到taskpool线程执行，不阻塞主线程。
...
- 应用市场线程从网络下载应用hap包，先保存在tmp文件中，共2.6G。
- 下载完毕后将tmp文件rename为entry.hap，此步无IO。
- 应用市场线程调用BMS的StreamInstall接口，将hap包写入el1的stream_install文件夹中（正常从应用市场目录拷贝到bms的目录），共2.6G。
- foundation的BMS线程将hap从stream_install复制到security_stream_install中（因为安全问题，步骤3的fd返回给应用市场客户端会有被篡改的风险，额外拷贝一次安全目录），共2.6G。
- installs线程从hap包中解压名种so文件，共300M。
- installs线程从security_stream_install文件夹将entry.hap复制到应用沙箱中，共2.6G。
- 安装完成后先后删除el1两文件夹中的hap、应用市场沙箱中的hap。

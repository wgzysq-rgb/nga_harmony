...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-stability-address-sanitizer-opt`  |  **DocID**: `5153afbc734a45568826e9b5f504174c`  |  **NodeID**: `0002017755471355093937a250a1cc74`
...
- 异步或跨线程场景中禁止使用[&]捕获局部变量或this
- 采用智能指针管理对象生命周期结合weak_from_this获取弱指针，保证对象在访问时仍然有效。

 实现步骤：

 
- 对象要从std::enable_shared_from_this继承。```
...
lambda表达式中调用weak_from_this捕获this。```
...
- weak_from_this在C++17开始支持，且对象必须由智能指针管理，否则调用weak.lock()会为空，导致回调不执行。
- 如果无法使用weak_from_this，可用C++11支持的shared_from_this方式。
...
- 在指针销毁时，对指针置空。
- 在子线程对主线程的指针进行操作时，都增加判空处理，以此来避免子线程在主线程销毁时依旧执行后续操作。
建议使用智能指针代替裸指针。```
int* g_ptr2 = nullptr;

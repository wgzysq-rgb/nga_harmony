...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-stability-leak-opt`  |  **DocID**: `13ed108517594cb1be7fd8bc0daf3db0`  |  **NodeID**: `00020177554713550939614799d6c4ba`
...
- 同一函数内，异常分支未释放资源。
- 构造函数中申请资源，析构函数未释放资源。
- 资源申请释放有配对，但配对函数或变量不匹配。
- 指针地址偏移等原因导致申请的内存地址丢失。
...
- 严格控制线程的生命周期，尽量避免大量线程同时存在以及线程生命周期脱离控制的情况；
- 保证异常场景的线程安全，补充异常场景下的线程管理机制，推荐使用线程池来进行管理；
- 创建线程时为线程取名（默认继承父进程名，导致大量同名线程），便于出现线程泄漏后的快速定位；
- pthread_create后需要调用pthread_join或者pthread_detach确保线程资源能回收掉。

...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-component-reuse-issue-diagnosis-and-analysis`  |  **DocID**: `ba59e033912347e0b275c0f769002e26`  |  **NodeID**: `0002017755467853447937fdd2c1186b`
...
组件复用的本质是自定义组件的缓存池。使用@Reusable装饰器标记自定义组件后，当该组件进入销毁流程时，将改为进入缓存池。当需要创建此自定义组件时，优先尝试从缓存池中寻找对应reuseId的自定义组件，若未找到再进行创建。此缓存池的实例位于@Reusable标记的自定义组件的父自定义组件上，会随父自定义组件的销毁一并销毁。关于自定义组件复用的详细使用方法，可参考[组件复用](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-component-reuse)文档。综上所述，组件复用的关键概念如下：
...
- 在DevEco Studio中启动AppAnalyzer工具，详细请参见[AppAnalyzer](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-performance-detection#section135451444171)。
本文展示的[正反例代码](https://gitcode.com/harmonyos_samples/BestPracticeSnippets/tree/master/ComponentReuse)分别位于positive和negative模块中，检测时需选择相应模块。

 点击工具“手动性能页面滑动体检”，按照提示进行检测操作。

 通过分析检测结果，结合滑动过程中列表卡片的创建次数，可以分析出当前列表的组件未使用复用或者复用未生效。
 ...
 **正例二**
 ...
 为了更灵活地复用和更新局部内容，通常需要降低复用粒度，即给组件内部需要复用的子组件做复用，而不是组件自身，避免全量更新。通过降低复用粒度，可以显著提升列表滚动的流畅度，减少内存占用。具体可参考[场景三：列表项内子组件可拆分组合](#li15198164511246)。
 ...
 **正例**
 ...
 以下为简单示例参考。若通过if-else条件渲染不同子组件，例如包含单图、三图和视频三种形式，可参考[未使用组件复用](#section10228161101910)的[场景二：列表项结构类型不同](#li1441155123913)和[场景三：列表项内子组件可拆分组合](#li15198164511246)进行优化。
...
**表1 **快速滑动可能出现的问题问题
  ...
  缓存池无法填充，新项只能通过新建（BuildItem）方式进行创建
 ...
 **异步化方案**
 ...
 [应用并发设计](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-app-concurrency-design)

 #### [h2]复用过程刷新耗时长

复用过程的更新与状态管理的标准更新一致，可参考文档：[状态管理最佳实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-status-management)。

...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-cross-module-resource-access`  |  **DocID**: `3e0881e3410d41dc8079d53e7442a491`  |  **NodeID**: `0002017755468639066521345107b422`
...
# 跨模块资源访问
      ...
      **方法一：通过'$r'或'$rawfile'语法访问资源。**       此方法适合简单的、静态的资源引用场景，比如在UI组件中直接引用。通过$('app.type.name')或$rawfile('name')访问资源，其中，app为resources目录中定义的资源；type为资源类型或资源的存放位置；name为资源名，开发者定义资源时确定。

       ```
Text($r('app.string.string_in_har2'))
Image($r('app.media.image_in_har2'))
```
      **方法二：通过resourceManager访问资源。**       此方法提供的API更为丰富，适用于构建更加复杂的应用逻辑，比如图像效果处理。通过本应用上下文获取ResourceManager后，调用不同[资源管理接口](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-resource-manager)访问不同资源。例如：resourceManager.getStringSync('string_in_har1').id可获取字符串资源，推荐使用资源id作为入参的方式使用，这种方式性能更好，具体可以参考：避免使用耗时接口中关于[ResourceManager](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-time-optimization-of-the-main-thread#section193673511440)的介绍。
...
```
      ...
      **方法一：通过"$r"或"$rawfile"语法访问资源。**       **使用场景**
...
```
      **方法二：通过createModuleContext访问资源。**       **使用场景**
...
```
      ...
      对外暴露的接口，需要在HSP入口文件index.ets中声明：       

       export { ResManager } from './src/main/ets/common/ResManager';       

       

      使用方使用import导入ResManager，访问HSP导出的资源，如下所示：       

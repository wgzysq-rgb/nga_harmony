...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-special-text-recognition`  |  **DocID**: `eef363d8dd584d6ba91e9a565367ab56`  |  **NodeID**: `000201775546863906662b788d11b8e2`
...
# 特殊文本识别跳转
     ...
     常用的文本与输入组件是否支持设置[enableDataDetector](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-text#enabledatadetector11)属性如下表所示。
      ...
      自定义带头像和文本信息的组件。       封装如TextLink组件，参数包含头像资源（img）和文本内容（text）。
...
```
      组件样式设置。       为头像、文本等元素分配合适UI样式，提升聊天或内容流的展示效果。
...
```
      ...
      启用特殊文本识别。       在Text组件上启用[enableDataDetector](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-text#enabledatadetector11)属性，即可实现对一段文字中的链接、日期、电话号码、地址、邮箱等特殊文本进行自动识别并标记。

       ```
Text(this.text)
  .enableDataDetector(this.enableDataDetector)
```
      识别效果验证。       分别传入包含标准HTTP/HTTPS链接、App Linking链接、日期、电话号码、地址、邮箱等文字内容到TextLink组件中，均以可交互形式显示。

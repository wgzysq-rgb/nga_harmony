...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-social-share`  |  **DocID**: `8d3c202d11ad4a1a82f2d4fce73a99e9`  |  **NodeID**: `00020177554686390665063831a580e2`
...
# 社交分享跳转
     ...
     安装完成后，首次打开应用将通过[延迟链接](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/applinking-deferredlink)功能自动导航至原始分享内容，无需用户重新搜索或操作。
                  ...
                  #### [h2]场景三：目标应用未安装，未配置直达应用市场（有Web页面）
    ...
    本文将详细介绍社交分享应用使用App Linking实现跳转的开发步骤，可根据业务需求决定是否配置直达应用市场的跳转路径。社交应用主要开发步骤如下：
     ...
     - [接入分享面板](#section2364187182115)
           [应用内处理并跳转指定页面](#section17342105516215)

           [配置直达应用市场与延迟链接（可选）](#section11682827182215)
     ...
     - [Web页面开发与部署（可选）](#section157709544229)
      ...
      - [在AGC开通App Linking服务](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-linking-startup#zh-cn_topic_0000001862787784_section189581229144811)。
      - [在开发者网站上关联应用](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-linking-startup#section6903241628)。
      - [在AGC控制台关联网址域名](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-linking-startup#section1101111611317)。
      [在DevEco Studio中配置关联的网址域名](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-linking-startup#section13808113610362)。       ```
...
```
      - 验证应用被拉起效果。       
        ...
        - 编译打包，并安装应用至调试设备。
        可以通过如下命令查询应用域名校验结果。         hdc shell hidumper -s AppDomainVerifyManager         运行hidumper命令后，即可在控制台上看到success消息。
...
```
     ...
     社交应用可通过集成[Share Kit（分享服务）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/share-introduction)拉起分享面板，用于分享文章详情的App Linking链接。接收方点击链接后可直接跳转至目标应用。详细请参见[分享App Linking直达应用](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/share-utd-link#section195231314185618)。
...
```
     ...
     当用户点击分享链接并拉起应用时，跳转到文章详情页有以下两种情况：
      ...
      应用未在后台运行：如果应用从冷启动开始（即不在后台），需要在onCreate()方法中获取链接中的文章唯一标识符aid。       ```
...
```
      应用已在后台运行：如果应用已经在后台，它将从后台切换到前台。此时，需要在onNewWant()方法中获取链接中的文章唯一标识符aid，保存到AppStorage。       ```
export default class EntryAbility extends UIAbility {
  private mAid: string = '';
...
```
     ...
     通过[deferredLink.popDeferredLink()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/applinking-deferredlink-api#section15555111210233)接口获取原始App Linking链接，并根据解析该链接直接跳转至对应的内容详情页。
...
```
     **延迟链接与Web页面结合**
     ...
     Web页面与延迟链接的结合是创建完整用户体验的关键。当用户通过Web页面安装应用后，可通过[延迟链接](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/applinking-deferredlink)实现首次打开应用时自动跳转到原始内容。
      ...
      - 在左侧导航栏中选择“增长 > App Linking”菜单栏查看App Linking链接地址的配置状态，若为成功则表明AGC上App Linking配置成功。
     
     

     

                  #### [h2]AGC中App Linking显示配置失败如何解决
     ...
     **解决措施**
      ...
      - applinking.json配置文件配置错误，请参考[在开发者网站上关联应用](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-linking-startup#section6903241628)进行配置。
      - App Linking地址仅支持https链接、若为http链接，请自行申请SSL证书。
      - 检查配置文件所处的文件目录与文件权限确保外部可以访问。
     
        若上述检查无误，建议求助网站运维侧人员，共同解决。

              #### [h2]AGC上显示了配置成功，但是配置到目标方后无法拉起如何解决
     ...
     **可能原因**
     ...
     - AGC配置App Linking链接时以“/”结尾。
     - 目标方中module.json5配置的链接与AGC配置链接不一致导致无法拉起。
     - 应用未手动从AGC上申请证书签名进行应用签名。
     - 开发者所处的网络有访问限制。
    ...
    **解决措施**
     ...
     - 检查AGC上配置AppLinking链接地址，当前该链接不支持末尾以“/”结尾，检查AGC配置的链接地址末尾是否多了“/”。
     - 检查module.json5中配置的链接地址与AGC中配置的链接地址是否一致，如AGC中配置了“https://www.huawei.com”而在代码中使用了“https://developer.huawei.com”。
     - 开发者请参考[验证应用被拉起效果](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-linking-startup#section1862871612126)在AGC上手动申请证书与Profile进行手动签名。
     - 重新更换其他网络。
...
```
     ...
     **可能原因**
      ...
      - Web页面的User-Agent（后续简称为UA）标识未适配HarmonyOS设备或浏览器环境，导致兼容性问题。
      - 对应的Web页面可能未完全开发或存在错误（如资源加载失败、后端服务异常等）。
      - 链接地址错误，导致无法正确加载目标页面。
     ...
     **解决措施**
      ...
      - 检查页面开发完整性。确认页面资源是否正常加载，后端接口是否返回正确数据。
      - 验证链接地址。确保社交应用的聊天界面中发送的链接地址正确无误。
     ...
     **可能原因**
      ...
      - 目标Web B页面的UA标识未适配HarmonyOS设备或浏览器环境，导致兼容性问题。
      - 目标Web B页面资源加载失败。
     
     **解决措施**

     
      - 适配UA标识。确保Web B页面适配了HarmonyOS设备和配套浏览器的UA标识，以正确识别设备并跳转到对应页面或平台。详细请参见[User-Agent开发指导](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/web-default-useragent)。
      - 检查Web B页面的资源加载情况。
     ...
     **问题现象**
     ...
     用户从社交应用的聊天界面中的链接跳转到Web页面后，尝试通过页面操作跳转应用时发生异常，例如：
      ...
      - 点击“打开”按钮后无反应。
      - 点击“下载”按钮后跳转到错误平台，或提示直接下载APK包。
     
     **可能原因**

     
      - 点击“打开”按钮无反应，可能是因为目标应用未正确配置跳转逻辑（例如App Linking），或者配置过程中存在错误。
      - 点击“打开”按钮无反应，Web页面使用链接不正确。
      - 目标应用未安装，点击“下载”按钮，触发应用下载逻辑跳转到应用下载的Web页面，但如果Web页面未正确适配HarmonyOS设备的UA标识，导致跳转到错误平台，或提示直接下载APK包。

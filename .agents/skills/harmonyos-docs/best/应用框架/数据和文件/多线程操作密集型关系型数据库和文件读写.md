...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-local-file-and-data-multithreaded-io`  |  **DocID**: `1ba6c01c43ce4d50a67a9b19c4a60175`  |  **NodeID**: `000201775546863906643a763b54dac8`
...
- 自动任务分发：当调用TaskPool执行密集型读写时，其线程池会自动将任务分发到子线程完成，无需人工干预，实现了高效的任务分配流程。
- 不阻塞主线程：通过在子线程完成任务，有效避免了对主线程的阻塞，确保主线程能够继续执行其他操作，维持系统整体的流畅运行。
- 资源节约：TaskPool本身通过系统统一线程管理，结合动态调度及负载均衡算法可节约系统资源，这也为执行密集型读写提供了更优的系统资源环境，保障操作的顺利进行。
...
封装write()函数，使用@Concurrent进行装饰，执行的并发函数需要使用该装饰器修饰，否则无法通过相关校验。```
@Concurrent
...
```
 封装read()函数，同样地，也使用@Concurrent进行装饰。在读文件时使用循环读取，这也是大文件读取时常用的方法，使用Array<ArrayBuffer>存储一个大文件中的信息。```
@Concurrent
...
```
 使用taskpool.execute()执行任务时，传入的第一个参数是调用的函数名，其余参数则是该函数的参数。```
...
封装关系型数据库的写数据库的函数，使用@Concurrent进行装饰。```
@Concurrent
...
```
 封装数据库读出的函数，使用getRow()和goToNextRow()循环读出符合查询条件的数据，这里读出所有数据。```
@Concurrent
...
为了实现[Sendable数据](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-sendable#sendable支持的数据类型)在不同并发实例间的引用传递，Sendable共享对象会分配在共享堆中，以实现跨并发实例的内存共享。
...
- 跨并发实例传递带方法的class实例对象。
  ...
  #### [h2]文件读写大数据使用@Sendable传输

**开发步骤**

 
在使用@Sendable进行文件写入操作时，首先需要定义Sendable对象存放写入数据，然后封装TaskPool函数传入。```
@Sendable
class Content {
  content: string;
...
```
 接下来进行读取操作，封装TaskPool函数，需要使用collections.Array代替Array，collections.ArrayBuffer代替ArrayBuffer接收结果值，在使用Sendable时，需要注意该数据类型Sendable是否支持，详情请见[sendable支持的数据类型](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-sendable#sendable支持的数据类型)。```
@Concurrent
...
```
  ...
  #### [h2]关系型数据库读写使用@Sendable

**开发步骤**

 
在关系型数据库写入操作时，同样需要封装写入数据使用@Sendable进行装饰，传入TaskPool函数中。```
@Sendable
...
```
 在读取时，关系型数据库模块提供了getSendableRow()可以直接获取当前行数据的Sendable形式。```
@Concurrent
async function read(context: common.UIAbilityContext, config: relationalStore.StoreConfig) {
  try {
    const store = await relationalStore.getRdbStore(context, config);

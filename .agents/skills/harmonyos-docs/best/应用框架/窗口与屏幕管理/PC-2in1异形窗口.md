...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-2in1-window-shape`  |  **DocID**: `cd28f1bd7bcd442f910445f8442995fc`  |  **NodeID**: `00020177554686390665491a1982e48d`
...
- 创建异形子窗口或悬浮窗，设置窗口的位置、大小等属性，设置窗口加载的page页面。
- 根据待实现的异形形状，计算出与其对应的二维数组掩码，对目标窗口设置该掩码。
- 根据逻辑需要，显示目标异形窗口。
...
为使用setWindowMask()等接口，需要额外[配置syscap.json文件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/syscap#加入自定义syscap)，在模块/src/main目录下，创建syscap.json文件，填入以下内容：
...
根据要实现的异形窗口形状，计算二维数组掩码windowMask。

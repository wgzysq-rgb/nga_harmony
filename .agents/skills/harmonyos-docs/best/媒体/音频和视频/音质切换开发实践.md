...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-sound-quality-switching`  |  **DocID**: `af35f4139f164b809017986af18da590`  |  **NodeID**: `000201775546903668374bb8cea20ba5`
...
- 使用AudioRenderer播放PCM文件，即MP3音质（标清）切换到无损未压缩的PCM音质时，通过跳转的时间戳计算出新的offset偏移位置，待下次读取音频数据时，从该offset偏移位置开始读取，AudioRenderer即可实现跳转至指定位置播放。
- 使用AVPlayer播放MP3文件，即无损未压缩的PCM音质切换到MP3音质（标清）时，通过AVPlayer的[seek()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avplayer#seek9)接口可直接跳转到对应时间戳位置播放。

  #### [h2]开发步骤

- 在MediaTools类中，定义如下方法：
...
```
 - 在AudioRendererController类中，定义如下方法：
...
```
 在页面中自定义changeType()事件，通过[getRawfd()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-resource-manager#getrawfd9-1)获取resources/rawfile目录下的音频资源，将获取到的数据传入PlayerController的changeType()方法，调用playerController的seek()方法传入当前Slider进度的值，用于跳转播放。```
...
在MenuItem组件的点击事件中调用自定义changeType()方法，传入音频资源文件路径和对应音频格式audioType。```
Menu() {
  MenuItem({
    content: $r('app.string.standard_quality'),
  }).onClick(() => {
...
// ...
```
 同样的，在页面自定义changeType()事件中，通过getRawfd()获取音频资源，将获取到的数据传入PlayerController的changeType()方法，调用playerController的seek()方法传入当前Slider进度值，用于跳转播放。```
private changeType(songSrc: string, audioType: AudioType): void {
  let current: number = this.value;
  this.getUIContext().getHostContext()?.resourceManager.getRawFd(songSrc).then((rawFileDescriptor) => {
    this.playerController.changeType(rawFileDescriptor, audioType);
...
```
 - 在PlayerController的changeType()方法中：
通过传入的audioType的值，判断音频是否为MP3或FLAC格式。
- 调用audioRendererController的pause()方法，暂停AudioRenderer音频渲染。
- 通过avPlayerController的reset()方法重置音频资源的描述符、偏移量和文件长度，实现音频资源FLAC格式与MP3格式的切换播放，即AVPlayer与AVPlayer切换播放。

...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-managing-audio-output-devices`  |  **DocID**: `41fe8fe0649a423783aa6d764a7bfbb3`  |  **NodeID**: `000201775546903668379465b2521b5f`
...
**模块**
  ...
  **应用场景**
  ...
  管理全局音频输出设备，提供系统输出设备查询及状态变化的监听接口

  
 [AudioSessionManager](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager)

  管理应用音频输出设备，当应用不持有AudioRenderer对象时，管理应用的音频输出设备

  
 [AudioRenderer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiorenderer)

  管理音频流输出设备，提供音频流输出设备的切换接口及设备变化的监听接口

  
 [AVCastPicker](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ohos-multimedia-avcastpicker#avcastpicker)
...
```
 使用[AudioRoutingManager.on('deviceChange')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audioroutingmanager#ondevicechange9)监听输出设备连接状态的变化。```
...
```
 使用[AudioRoutingManager.on('availableDeviceChange')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audioroutingmanager#onavailabledevicechange12)监听可用输出设备的变化，并在设备变化时更新设备列表。```
...
```
 使用[AudioRoutingManager.on('preferOutputDeviceChangeForRendererInfo')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audioroutingmanager#onpreferoutputdevicechangeforrendererinfo10)监听播放设备的变化，并在变化时弹框提示用户。```
...
使用[AVSession.createAVSession(context: Context, tag: string, type: AVSessionType)](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-avsession-f#avsessioncreateavsession10)创建AVSession。通过[AVSessionType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-avsession-t#avsessiontype10)区分不同的使用场景，audio表示音乐播放，voice_call表示语音通话。```
// Create AVSession.
try {
...
```
 在需要切换设备的界面创建[AVCastPicker](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ohos-multimedia-avcastpicker#avcastpicker)组件。```
AVCastPicker({
...
```
 - 手动点击AVCastPicker组件，如果输出设备只有听筒和扬声器，可直接切换；否则，需在弹框中选择目标设备，以完成输出设备的切换。
...
使用[AudioRenderer.on('outputDeviceChangeWithInfo')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiorenderer#onoutputdevicechangewithinfo11)监听音频流输出设备变化及原因。```
...
```
 当监听到AudioStreamDeviceChangeReason.REASON_OLD_DEVICE_UNAVAILABLE时，如果新设备是扬声器，并且在媒体播放场景，应考虑暂停音频播放。```
...
使用[AudioSessionManager](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager)的管理应用音频输出设备的相关能力，包括切换输出设备、监听输出设��的变化，实现具体场景下输出设备间的切换。与AudioRenderer相同，仅当音频流类型为语音消息、VoIP语音通话或VoIP视频通话时，支持听筒、扬声器和系统默认设备间的切换。

  #### [h2]开发步骤

创建AudioSessionManager实例。```
private audioManager = audio.getAudioManager();
private audioSessionManager: audio.AudioSessionManager = this.audioManager.getSessionManager();
```
 使用[AudioSessionManager.activateAudioSession()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#activateaudiosession12)激活AudioSession。```
...
```
 使用[AudioSessionManager.setDefaultOutputDevice()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#setdefaultoutputdevice20)将用户选择的设备设置为输出设备。```
// Set output device.
async setOutputDevice(deviceType: audio.DeviceType) {
...
```
 使用[AudioSessionManager.on('currentOutputDeviceChanged')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiosessionmanager#oncurrentoutputdevicechanged20)监听输出设备变化，当输出设备切换成功后会触发该回调。```

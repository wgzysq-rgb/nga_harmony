...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-image-to-video-synthesis`  |  **DocID**: `df27eb3fca8140b5abd3a9f192607118`  |  **NodeID**: `00020177554690366837553bb1a05931`
...
使用[PhotoViewPicker](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-photoaccesshelper-photoviewpicker)从图库中选择图片，数量不少于两张。当选择数量不足两张时，将弹出提示框。```
...
```
 遍历处理从图库选择的图片。（1）读取图片数据并创建ImageSource。
...
```
  对图片进行编码和封装。（1）启动编码器和封装器，启动编码输入输出处理线程。```
...
```
  所有图片都处理完成后，使用[Video](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-media-components-video)组件播放合成后的视频。```
...
```
 使用[SaveButton](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-security-components-savebutton)安全控件，将视频保存到图库。（1）创建安全控件按钮。```

...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-playing-pcm-audio-based-ohaudio`  |  **DocID**: `675fcbabfca0411db19ad9f01656e6ef`  |  **NodeID**: `0002017755469036683881655886a3f9`
...
场景名称
 ...
 [切换歌曲播放](#section590418431566)
  ...
  调用[OH_AudioStreamBuilder_SetRendererWriteDataCallback()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-audiostreambuilder-h#oh_audiostreambuilder_setrendererwritedatacallback)接口，设置写入音频数据的回调。在回调函数中，
 ...
 [倍速设置](#section189460361122)
  ...
  使用[OH_AudioRenderer_SetSpeed()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-audiorenderer-h#oh_audiorenderer_setspeed)设置播放倍速。
 ...
 [音量设置](#section88718617116)
  ...
  使用[OH_AudioRenderer_SetVolume()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-audiorenderer-h#oh_audiorenderer_setvolume)设置播放音量。
...
```
 3. 调用[OH_AudioStreamBuilder_SetRendererWriteDataCallback()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-audiostreambuilder-h#oh_audiostreambuilder_setrendererwritedatacallback)接口设置写入音频数据的回调函数，将获取的歌曲资源写入回调函数中，实现资源加载。
...
```
 4. 在[OH_AudioStreamBuilder_SetRendererWriteDataCallback()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-audiostreambuilder-h#oh_audiostreambuilder_setrendererwritedatacallback)设置的回调函数中，从对应的起始位置开始获取歌曲资源并写入回调中，从而实现跳转播放。
...
```
 3. 调用[OH_AudioStreamBuilder_SetRendererWriteDataCallback()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-audiostreambuilder-h#oh_audiostreambuilder_setrendererwritedatacallback)接口，设置写入音频数据的回调。在回调函数中，将获取到的不同的歌曲资源写入数据缓冲区，实现播放不同歌曲的功能。
...
```
 2. 根据支持的倍数范围，通过[OH_AudioRenderer_SetSpeed()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-audiorenderer-h#oh_audiorenderer_setspeed)接口设置播放的倍数值。
...
```
 2. 调用[OH_AudioRenderer_SetVolume()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-audiorenderer-h#oh_audiorenderer_setvolume)接口，第二个参数中传入设置的音量值，设置当前音频流音量。

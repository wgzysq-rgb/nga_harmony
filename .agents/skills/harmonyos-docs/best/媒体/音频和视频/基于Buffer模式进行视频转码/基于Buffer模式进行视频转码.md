...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-buffer-mode-transcoding`  |  **DocID**: `e2c56d3871924cdb8d9d59f39b970524`  |  **NodeID**: `00020177554690366836449b62585a5d`
...
- 解码器提供空的Buffer地址，该地址用于填充需要解码的视频数据。
- 将解封装后的视频数据（如H.264）填充到解码器提供的Buffer地址中。
- 通知解码器，当前Buffer地址已完成视频数据填充。
- 当解码器收到通知后，会对数据进行解码操作。

 输出数据流转的步骤如下所示。

 
- 在完成输出数据流转后，解码器会提供已解码的数据。
- 获取数据后，开发者可以根据实际业务使用视频数据，如送显到屏幕。
- 使用完毕后，开发者需要将Buffer的使用权移交给解码器。
- 解码器会循环再利用Buffer地址。Buffer内的视频数据不会被释放或者清零，而是在下一次循环中，将解码好的数据直接覆盖写入到已经空闲的Buffer地址里。

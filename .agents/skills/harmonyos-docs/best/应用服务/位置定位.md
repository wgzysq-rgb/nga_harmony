...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-positioning`  |  **DocID**: `0ac96a63b9a24380a7e163d2d10ccc7b`  |  **NodeID**: `000201775546910357478bf0e9c1a432`
...
- 申请定位权限，具体内容可参考[申请位置权限开发指导](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/location-permission-guidelines)。
...
```
 调用[getAddressesFromLocation()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-geolocationmanager#geolocationmanagergetaddressesfromlocation)接口进行逆地理编码转化，将位置坐标信息转换为对应的地理位置描述。```
...
在on('locationChange')的回调函数中，调用[getAddressesFromLocation()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-geolocationmanager#geolocationmanagergetaddressesfromlocation)接口进行逆地理编码转化，将坐标信息转换为对应的地理位置描述。```
geoLocationManager.getAddressesFromLocation(reverseGeocodeRequest, async (err, data) => {
  if (data) {
    this.address = data[0]?.placeName || '';
    // ...
...
- 申请后台定位权限，具体内容可参考[申请位置权限���发指导](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/location-permission-guidelines)。
...
```
 开启任务模式为定位导航类型的长时任务，在其回调接口中开启位置变化订阅，并发起定位请求，在应用后台持续获取当前位置信息。开启长时任务
...
```
 在位置变化的回调中，调用[getAddressesFromLocation()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-geolocationmanager#geolocationmanagergetaddressesfromlocation)接口进行逆地理编码转化，将坐标信息转换为对应的地理位置描述。```
geoLocationManager.getAddressesFromLocation(reverseGeocodeRequest, async (err, data) => {
  if (data) {
    this.address = data[0]?.placeName || '';
    // ...
...
```
 当不需要在应用后台获取位置信息时，及时关闭长时任务和位置变化订阅，并删除对应的定位请求，减少设备功耗。关闭长时任务
...
- 申请定位权限，具体内容可参考[申请位置权限开发指导](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/location-permission-guidelines)。
- 调用getLastLocation()接口获取系统缓存的最新位置信息。```
let location = geoLocationManager.getLastLocation();
```
 
调用[getAddressesFromLocation()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-geolocationmanager#geolocationmanagergetaddressesfromlocation)接口进行逆地理编码转化，将坐标信息转换为对应的地理位置描述。```
geoLocationManager.getAddressesFromLocation(reverseGeocodeRequest, async (err, data) => {
  if (data) {
    this.address = data[0]?.placeName || '';
    // ...

...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-smartwatch`  |  **DocID**: `a8ca6558111d4afc93ecc1ae82546728`  |  **NodeID**: `0002017755470691180960a43cce588e`
...
```
 ...
 this.getUIContext().getFocusController().activate(true, false); 激活焦点激活态后，可调用[requestFocus()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-focuscontroller#requestfocus12)方法使目标组件获得焦点，随后手势操作方可生效。示例代码如下，页面渲染后默认使得id为"btn1"的按钮获焦：
...
通过[wearEngine.getNotifyClient()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#section552416431386)获取Notify客户端类。
...
通过NotifyClient.notify()实现手机向智能穿戴设备发送模板化通知。
...
通过wearEngine.getDeviceClient()获取Device模块的客户端用于获取连接设备。
...
通过Device客户端获取当前已连接且支持wearEngine能力集的设备。
...
默认启用深色主题模式降低显示能耗。
 ...
 - 仅允许视频、游戏、导航等必要场景保持屏幕常亮。

 - 后台应用管理
关键服务类应用（闹钟、日程提醒、邮件、IM类应用）可依据实际需求启动。
- 除被前台应用拉起的情况外，闹钟、日程提醒、邮件、IM类应用按需自启，禁止非必要应用以任何形式自启动。

 - 定位服务规范
后台GPS仅对导航、运动健康等核心功能开放。
- 严禁非用户交互场景在后台调用定位服务。
...
设定应用屏幕常亮首先需要在EntryAbility获取window实例。
...
调用window的[setWindowKeepScreenOn()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#setwindowkeepscreenon9)接口，设置屏幕常亮。详细的接口信息请参考[窗口管理](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-window-window#setwindowkeepscreenon9-1)。
 ...
 **解决方案**
 ...
 huks.generateKeyItem(alias, options, callback); 迁移项目后，改写为：
...
手表应用在发布时，仅可选择“手表”设备，无法对智能穿戴与轻量级智能穿戴做区分。

 - 应用涉及两种穿戴设备时，应用发布是否需要上传两个对应的发布包。

 **解决方案**

 
点击确认后跳转至发布页，可在发布页具体选择设备类型。

 - 智能穿戴支持ArkTS与JS，轻量级智能穿戴仅支持JS，若应用发布包支持在智能穿戴与轻量级智能穿戴上运行，则无需对应用发布包做区分，若应用发布包仅支持某一种穿戴设备，则需要分别为智能穿戴设备与轻量级智能穿戴设备创建AppID，并上传其对应的发布包。
- APP创建流程可参考[创建HarmonyOS应用](https://developer.huawei.com/consumer/cn/doc/app/agc-help-create-app-0000002247955506)。
...
将原轻量级智能穿戴项目/entry/src/main/js目录下的代码文件及资源文件按对应位置迁移至新工程。
...
代码适配。由于轻量级智能穿戴和智能穿戴的同步/异步机制不同，因此在迁移时时，需要对相关接口进行修改。以Huks接口为例，说明如何进行这些修改。

huks.generateKeyItem(alias, options, callback); 在轻量级智能穿戴设备上，回调将同步执行。但在智能穿戴设备上是异步的，因此需要更改书写样式增加await，保证代码同轻量级智能穿戴设备一样同步执行，代码如下所示：

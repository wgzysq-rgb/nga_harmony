...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-drawing-capability-improve-performance`  |  **DocID**: `cdc8bdddc9454a0888c549c173c4582a`  |  **NodeID**: `0002017755469081964596537ffe12ae`
...
前端定义一个[RenderNode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-arkui-rendernode)自绘制渲染节点，将背景图this.pMap及图形绘制上下文context传入Native，调用Native侧的nativeOnDraw接口进行绘制。```
...
```
 在页面中将自绘制节点挂载到[NodeContainer](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-basic-components-nodecontainer)上。```
// entry\src\main\ets\pages\Index.ets
@Entry
@Component
struct Index {
...
```
 Native侧暴露绘制接口nativeOnDraw()供前端调用，该接口绑定Native侧的OnDraw()函数，ArkTS传入的参数在该函数中处理。```
// entry\src\main\cpp\native_bridge.cpp
EXTERN_C_START
...
EXTERN_C_END
```
 在OnDraw()函数中接收前端传入的参数，主要是图形绘制上下文与背景图。```
// entry\src\main\cpp\native_bridge.cpp
...
```
 在NativeOnDrawPixelMap函数中实现透明圆圈绘制（主要使用[OH_Drawing_CanvasSaveLayer()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-drawing-canvas-h#oh_drawing_canvassavelayer)分层接口及 [OH_Drawing_BrushSetBlendMode()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-drawing-brush-h#oh_drawing_brushsetblendmode)融合接口得到图形融合效果）。```
// entry\src\main\cpp\native_bridge.cpp

...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-maleoon-gpu-best-practices`  |  **DocID**: `984125a15a174804b9f298af38885617`  |  **NodeID**: `000201775546908196458d24ff5da50e`
...
**1. Device Memory分配与释放** 仅适用于[Vulkan](https://registry.khronos.org/vulkan/#apispecs)。
...
gvec4 texture(gsampler2D sampler, vec2 P, [float bias]);
 ...
 bias is imm value or some other uniform variable

  
 gvec4 textureLod(gsampler2D sampler, vec2 P, float lod);

 lod is imm value or some other uniform variable

  
 gvec4 textureProj(gsampler2D sampler, vec4(or vec3) P, [float bias]);

 bias = 0.0 (or no bias parameter)
...
Blend mode为add、sub。 仅适用于Maleoon 910 GPU。
...
Blend mode为min、max。 仅适用于Maleoon 910 GPU。
...
- 如果精度允许，可以使用ushort或者uchar等替换int等类型。
 ...
 Vload3的数据操���需要进行vector4对齐通过vload3加载global memory时，内存需要按照vector4的格式进行数据对齐。
...
Input attachment的format不能是srgb格式。 仅适用于Maleoon 910 GPU。

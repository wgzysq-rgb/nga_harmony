...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-multi-device-responsive-layout`  |  **DocID**: `f5146b8937cf4e1c8c423b47a17ef322`  |  **NodeID**: `000201775547012322495842008f5272`
...
- 部分手机、小折叠屏机型横屏/反向横屏时横向断点会落入lg，包括Pura70 Pro和Pocket2。
- 开发者可以根据实际使用场景决定适配哪些断点。如xs断点对应的一般是智能穿戴类设备，如果确定某页面不会在智能穿戴设备上显示，则可以不适配xs断点。
- 可以根据实际业务场景需要在lg断点后面新增xl、xxl等断点。
...
产品类型
  ...
  Mate60、Mate60 Pro、Mate70、Mate70 Pro、Pura60、Pura60 Pro、Pura70、Pura70 Pro
...
在EntryAbility的onWindowStageCreate()生命周期中，获取应用窗口实例，对WindowUtil进行初始化操作，并保存至AppStorage中。在loadContent加载完页面后，完成断点监听注册、断点更新等操作。完成上述步骤后，当窗口尺寸发生变化时，更新横纵断点。
...
使用@StorageLink装饰器获取窗口管理类WindowUtil对象，子组件初始化时传递窗口信息类对象，使子组件可以获取当前窗口的断点信息。
...
使用@ObjectLink装饰器接收窗口信息类对象，根据获取的窗口信息，对不同断点区间进行针对性布局。如下示例代码完成了在不同断点下，Grid组件显示不同数量列的功能。
 ...
 **独特的小窗口布局**
 ...
 为了提供独特的用户体验，类方形小窗口设计为独特布局。常见场景为手机上下1:1分屏或Pura X外屏，可使用横向断点为sm，纵向断点为md进行区分，示意图如下。更多详情和示例代码请参考[Pura X阔折叠应用开发](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-purax-guide)。
...
- 应用设置窗口旋转方案需要结合多种条件一起判断，详情可参考[为多设备配置旋转策略](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-multi-device-window-direction#section189311691213)。
- Pura X外屏默认不支持窗口旋转。
...
- 确保已完成[通过断点刷新UI](#section175001836203617)中步骤5和步骤6的初始化操作。
使用@Watch装饰器监听状态变量isFullScreen的变化，以判断视频是否全屏播放，并在显示或隐藏时同步修改窗口方向。全屏播放时，未使用断点的窗口设置逻辑如下：需要将窗口设置为AUTO_ROTATION_LANDSCAPE属性的情况包括手机、双折叠屏（X 系列）的折叠态与半折态。
...
- 手机与双折叠屏（X系列）折叠态竖屏时的横向断点为sm，纵向断点为lg，全屏播放时支持横向旋转。将窗口显示方向设置为AUTO_ROTATION_LANDSCAPE_RESTRICTED。
手机与双折叠屏（X系列）折叠态横屏时的横向断点为md，纵向断点为sm，此时如果退出全屏播放，则竖屏展示布局。将窗口显示方向设置为PORTRAIT。 本案例未覆盖部分手机（如Pocket 2、Pura 70Pro）横屏时横向断点落入lg的情况。
...
对通过媒体查询监听[断点](#section1532120147301)的功能做简单的封装，方便后续使用。```
...
- ArkUI在API version 9对栅格组件做了重构，推出了新的栅格组件[GridRow](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-gridrow)和[GridCol](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-gridcol)，同时原有的[GridContainer组件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-container-gridcontainer)及[栅格设置](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-grid)已经废弃。
...
栅格组件支持开发者修改断点的取值范围，除了默认的四个断点，还支持开发者启用xl和xxl两个额外的断点。 断点并非越多越好，通常每个断点都需要开发者“精心适配”以达到最佳显示效果。
...
参数名
  ...
  在栅格中占据的列数。span为0，意味着该元素既不参与布局计算，也不会被渲染。

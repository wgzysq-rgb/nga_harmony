...
> **分区**: 最佳实践  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bpta-web-adaptation`  |  **DocID**: `b865000d31fa4b3d884e4e0775e629e5`  |  **NodeID**: `000201775547012322496ab97e56aeab`
...
- 设置容器属性：首先要将容器的display属性设置为grid。
- 确定元素的排列方式：
确定列宽与行高：列宽通过grid-template-columns定义，参数个数为列数，参数大小为列宽；行高通过grid-template-rows定义，方式类似。
- 确定行列间距：行间距用row-gap控制，列间距用column-gap控制。若间距相等，用gap属性控制。

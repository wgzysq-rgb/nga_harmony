...
> **分区**: 指南  |  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  |  **Slug**: `intents-skill-all-rec-function-search`  |  **DocID**: `5df46353e82d46f0915be3ed3f581bc0`  |  **NodeID**: `000201775795449800360915d0ad4441`
...
| **参数类别**  | **参数（中文）**  | **参数（英文）**  | 是否必选  | **描述**  | 类型  | **数据样例**  
 ...
 | **Output**  | 结果码  | code  | 是  | 意图调用的结果码。  | number  0：成功。

 其他：失败（需提前与华为侧协商，不支持自定义）。
...
继承InsightIntentExecutor。
  ...
  通过意图名称，识别跳转功能页面意图（JumpFunctionPage），在对应的方法中传递意图参数（param），并拉起对应落地页。

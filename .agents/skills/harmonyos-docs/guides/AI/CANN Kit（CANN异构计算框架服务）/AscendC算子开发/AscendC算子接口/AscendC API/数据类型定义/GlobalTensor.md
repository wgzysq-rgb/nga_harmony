...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `cannkit-globaltensor`  |  **DocID**: `04fee4f2bdb94f2989ceeee926718c18`  |  **NodeID**: `000201775795449800459bee5387fd7b`
...
| 函数名称  | 入参说明  | 含义  
 ...
 | SetGlobalBuffer  buffer：主机侧传入的全局数据指针

 bufferSize：所包含的类型为T的数据个数，单位为element，需自行保证不会超出实际数据的长度

  | 设置GlobalTensor的存储位置：buffer指向外部存储的起始地址，bufferSize为Tensor所占外部存储的大小，如指向的外部存储有连续256个int32_t，则其bufferSize为256。  
 ...
 | operator()  | index:  下标索引  | 获取本GlobalTensor的第index个变量的引用。与[LocalTensor](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-localtensor)的operator()类似。  
 | SetShapeInfo  | shapeInfo：ShapeInfo结构体  | 设置GlobalTensor的shapeInfo。  
 | GetShapeInfo  | 无  获取GlobalTensor的shapeInfo。

 **说明：** Shape信息没有默认值，只有通过SetShapeInfo设置过Shape信息后，才可以调用该接口获取正确的ShapeInfo。

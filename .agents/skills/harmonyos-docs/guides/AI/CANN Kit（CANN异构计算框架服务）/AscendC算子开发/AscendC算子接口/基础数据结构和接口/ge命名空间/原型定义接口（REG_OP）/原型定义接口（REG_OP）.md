...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `cannkit-prototype-definition-api`  |  **DocID**: `7fb9753916d74f3abf4c770ce5093ee6`  |  **NodeID**: `0002017757954498005877fe95be2890`
...
| **接口名称**  | **接口说明**  | 衍生接口（可用于IR模型构建）  
  | REG_OP(x)  | 定义一个算子原型，算子类型为x。  | [REG_OP](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-prototype-definition-derivative-api#reg_op)  
 | .INPUT(x, type)  定义输入名称（x）和类型(type)。
 ...
 | .OPTIONAL_INPUT(x, type)  定义可选输入的名称（x）和类型（type）。
 ...
 关于TensorType类，请参见[TensorType](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-tensortype)。
  ...
  | [OPTIONAL_INPUT](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-prototype-definition-derivative-api#optional_input)  
 | .DYNAMIC_INPUT(x, type)  定义动态输入的名称（x）和类型（type）。
 ...
 关于TensorType类，请参见[TensorType](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-tensortype)。
  ...
  | [DYNAMIC_INPUT](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-prototype-definition-derivative-api#dynamic_input)  
 | .OUTPUT(x, type)  定义输出的名称（x）和类型（type）。
 ...
 | .DYNAMIC_OUTPUT(x, type)  定义动态输出的名称（x）和类型（type）。
 ...
 关于TensorType类，请参见[TensorType](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-tensortype)。
  ...
  | [DYNAMIC_OUTPUT](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-prototype-definition-derivative-api#dynamic_output)  
 | .REQUIRED_ATTR(x, type)  定义必备属性的名称（x）和类型（type）。
 ...
 - ListNamedAttrs，属性类型为vector<NamedAttrs>，NamedAttrs列表
  ...
  | [REQUIRED_ATTR](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-prototype-definition-derivative-api#required_attr)  
 | .ATTR(x, type, default_value)  定义可选属性的名称、类型以及默认值。
 ...
 | .GRAPH(z1)  注册算子中包含的子图信息，输入z1为子图名称。
 ...
 | .DYNAMIC_GRAPH(z2)  注册动态算子子图信息，输入z2为子图名称。
 ...
 对于同一个算子，注册的算子子图名称需要保持唯一。
  ...
  | [DYNAMIC_GRAPH](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-prototype-definition-derivative-api#dynamic_graph)  
 | .INFER_SHAPE_AND_TYPE()  | 该接口为历史遗留兼容性接口，当前版本开发者无需使用。  | -  
 | .OP_END_FACTORY_REG(x)  与REG_OP配对，结束算子原型定义。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `cannkit-common-data-movement`  |  **DocID**: `7d33538568e545fea90e82e121acefa9`  |  **NodeID**: `000201775795449801087f69181f2127`
...
| 支持型号  | 数据通路（通过[TPosition](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-tposition)章节中表1表达）  | 源操作数和目的操作数的数据类型 (两者保持一致)  
...
| 参数名称  | 含义  
 ...
 | blockLen  指定该指令每个连续传输数据块长度，单位为datablock(32Bytes)。取��范围：blockLen∈[1, 65535]。
 ...
 | dstStride  目的操作数，相邻连续数据块间的间隔（前面一个数据块的尾与后面数据块的头的间隔），单位为datablock(32Bytes)。数据类型为uint16_t，dstStride不要超出该数据类型的取值范围。

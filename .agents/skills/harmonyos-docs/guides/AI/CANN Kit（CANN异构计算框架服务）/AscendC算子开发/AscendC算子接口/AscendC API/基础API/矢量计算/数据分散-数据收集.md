...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `cannkit-data-distribution-collection`  |  **DocID**: `8c916fa5a97a450dbc013e6f9e3a1f11`  |  **NodeID**: `000201775795449800883942428438e3`
...
| 参数名称  | 输入/输出  | 含义  
 ...
 前n个tensor：uint16_t、int16_t、uint32_t、int32_t、half、float32_t
 ...
 | srcOffsetLocal  | 输入  每个元素在src中对应的地址偏移。
 ...
 每个元素在src中对应的地址偏移，地址偏移要大于等于0。该偏移量是相对于src的基地址而言，每个数值的单位为Bytes，数据类型根据srcLocal数据类型决定，支持情况如下。

 srcOffsetLocal支持数据类型：u32。

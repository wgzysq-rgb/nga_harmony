...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `cannkit-vector-calculation-binocular-or`  |  **DocID**: `879437d8c5db40c3bd6c54b69fcaf2a4`  |  **NodeID**: `00020177579544980116471eeaa10c6f`
...
| 参数名  | 输入/输出  | 描述  
 ...
 Kirin9020训练系列产品，支持的数据类型为：uint16_t/int16_t。不支持浮点位运算，lv0 逐bit不支持uint8_t/int8_t。

 KirinX90训练系列产品，支持的数据类型为：uint16_t/int16_t。不支持浮点位运算，lv0 逐bit不支持uint8_t/int8_t。

  
 | src0Local、src1Local  | 输入  源操作数。
...
本样例中只展示Compute流程中的部分代码。如果开发者需要运行样例代码，请将该代码段拷贝并替换双目指令样例模板[更多样例](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkitvectorcalculation-binocularinstructions)中的Compute函数即可。
 ...
 AscendC::Or(dstLocal, src0Local, src1Local, 512); 结果示例如下。

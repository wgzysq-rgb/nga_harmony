...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `cannkit-valuedepend`  |  **DocID**: `da36c0ac83284795a26f72b459158415`  |  **NodeID**: `000201775795449801131e137b6480cd`
...
| 参数  | 输入/输出  | 说明  
  | value_depend  | 输入  value_depend有以下两种取值：

 - REQUIRED：表示算子的输入必须是Const类型。  

   会校验算子的输入是否是Const类型。若校验通过，则将此输入的值下发到算子，否则报错。

 - OPTIONAL：表示算子的输入可以是Const类型，也可以不是Const类型。如果输入是Const类型，则将输入的值下发到算子，否则不下发。

  
 | scope  | 输入  scope类型为枚举DependScope，支持的取值为：

 - ALL：指在Tiling/InferShape等函数实现时都依赖该输入的具体数据，行为与调用单参数的ValueDepend接口一致。

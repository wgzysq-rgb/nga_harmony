...
> **分区**: 指南  |  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  |  **Slug**: `cannkit-cpu-twin-debugging`  |  **DocID**: `441eb5470aa34eeda9e4fceec5909621`  |  **NodeID**: `000201775795449800466d97c6d79961`
...
| 功能名称  | 功能说明  
 ...
 | printf/PRINTF功能  | 支持屏显打印Scalar数据，如常量、字符串等信息，具体参考[printf/PRINTF功能](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-commissioning-function-printf)。  
 | DumpTensor功能  | 支持dump Tensor数据，功能和产物与NPU上板dump功能类似，具体参考[DumpTensor功能](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-commissioning-function-dumptensor)。  
 | DumpAccChkPoint功能  | 支持dump偏移位置的Tensor数据，具体参考[DumpAccChkPoint功能](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-commissioning-function-dumpaccchkpoint)。  
 | assert功能  | 支持屏显打印断言，具体参考[assert功能](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-commissioning-function-assert)。  
 | CCE指令流打印  | 默认开启，CCE指令流文件参见[产物说明](#产物说明)。  
 | gdb调试  | 可使用gdb单步调试算子计算精度。具体参考[gdb调试](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-gdb)。  
...
完成环境搭建，并准备好输入/标杆数据文件。
  ...
  执行如下命令进行CPU调测，这里仅提供关键参数项示例，其他参数请参考[CPU调测参数](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-cli-parameters#cpu调测参数)按需设置。

  ascendebug kernel --backend cpu --chip-version ${chip_version} --repo-type customize --json-file ${op_config_json_file} --core-type ${core_type} --work-dir ${work_dir} ... {其他参数}  CPU调测涉及的所有参数可通过**ascendebug kernel -h**或**ascendebug kernel --help**查看。

  查看结果文件，详细说明参见[产物说明](#产物说明)。

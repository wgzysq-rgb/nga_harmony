...
> **分区**: 指南  |  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  |  **Slug**: `cannkit-onnx-framework`  |  **DocID**: `21664efc77ee4550ade56c0df55fc353`  |  **NodeID**: `000201775795449800473344d2b51381`
...
```
  ...
  使用REGISTER_CUSTOM_OP宏，完成DDK算子和ONNX框架的算子映射关系注册。使用方法如下。
...
- [ParseParamsByOperatorFn](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-parseparamsbyoperatorfn)(ParseParamByOpFunc)：用来注册解析算子参数实现映射关系的回调函数，需要开发者自定义实现[ParseParamsByOperatorFn](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-parseparamsbyoperatorfn)。具体实现方式参考步骤3。
  ...
  实现回调函数ParseParamByOpFunc。其函数声明如下所示：
...
```
  在%{DDK_INSTALL_PATH}/tools/tools_omg执行如下命令生成离线模型。（如下命令中使用的目录以及文件均为样例，请以实际为准）

  ./omg --model ./add_custom.onnx --framework 5 --output out/custom_graph --target=omc  关键参数的解释如下。
...
--platform：omc模型为硬件相关模型，指定omc模型运行的芯片平台。   模型转换命令相关参数参考[离线模型转换](https://developer.huawei.com/consumer/cn/doc/hiai-Guides/offline-model-conversion-0000001053807006)。

    
  若提示有出现如下信息，则说明进入了AscendC自定义算子编译流程且模型转换成功。
...
```
  ...
  参考[生成输入数据](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-graph-compilation-and-execution#生成输入数据)准备符合模型输入要求的*.bin格式的输入数据。

  参考[APP集成代码](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-graph-compilation-and-execution#app集成代码)，完成模型集成。

...
> **分区**: 指南  |  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  |  **Slug**: `cannkit-tiling-implementation-on-the-host`  |  **DocID**: `90748fbecc724d27a6515235c0b7df57`  |  **NodeID**: `00020177579544980070576f0e55a8d8`
...
```
   编译时，可以通过设置[--tiling_keys](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-operator-project-compilation)编译选项指定TilingKey，编译时只编译指定TilingKey相关的kernel代码，用于加速编译过程。
...
```
  ...
  TilingData结构定义，通过BEGIN_TILING_DATA_DEF接口定义一个TilingData的类，通过TILING_DATA_FIELD_DEF接口增加TilingData的两个字段totalLength、tileNum，通过END_TILING_DATA_DEF接口结束TilingData定义。相关接口的详细说明请参考[TilingData结构定义](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-tilingdata-structure-definition)。
...
获取TilingContext的上下文，即Tiling函数的入参gert::TilingContext* context。
  ...
  设置TilingData。上文步骤3中，定义了TilingData的类，此时可以用TilingData定义一个具体的实例，通过调用TilingData类的set_+field_name接口来设置TilingData的字段值，通过调用TilingData类的SaveToBuffer接口完成TilingData的序列化和保存。

  
通过上下文获取输入输出shape信息。本样例中通过TilingContext的[GetInputShape](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-getinputshape)接口获取输入的shape大小。```
// 获取输入shape信息
uint32_t totalLength = context->GetInputShape(0)->GetOriginShape().GetShapeSize();
```
...
tiling.set_tileNum(TILE_NUM);
```
  调用TilingData类的SaveToBuffer接口完成序列化并保存至TilingContext上下文。SaveToBuffer的第一个参数为存储Buffer的首地址，第二个参数为Buffer的长度。通过调用[GetRawTilingData](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-getrawtilingdata)获取无类型的TilingData的地址，再通过[GetData](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-tilingdata-getdata)获取数据指针，作为Buffer的首地址；通过调用[GetRawTilingData](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-getrawtilingdata)获取无类型的TilingData的地址，再通过[GetCapacity](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-getcapacity)获取TilingData的长度，作为Buffer的长度。完成SaveToBuffer操作后需要通过[SetDataSize](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-setdatasize)设置TilingData的长度，该长度通过TilingData类的[GetDataSize](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-getdatasize)接口获取。```
// 序列化并保存
tiling.SaveToBuffer(context->GetRawTilingData()->GetData(), context->GetRawTilingData()->GetCapacity());
context->GetRawTilingData()->SetDataSize(tiling.GetDataSize());
```
  
  通过[SetBlockDim](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-setblockdim)接口设置BlockDim。

  context->SetBlockDim(BLOCK_DIM);  （可选）通过[SetTilingKey](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-settilingkey)设置TilingKey。

  context->SetTilingKey(1);  （可选）通过[GetWorkspaceSizes](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cannkit-getworkspacesizes)获取workspace size指针，并设置size大小。此处仅作为举例，设置workspace的大小为0。

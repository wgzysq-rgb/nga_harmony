...
> **分区**: 指南  |  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  |  **Slug**: `cannkit-plugin-based-quantization`  |  **DocID**: `40318b2cf3db4f3f88f1bebb665eff59`  |  **NodeID**: `000201775795449800382c32f0ea6475`
...
准备TensorFlow环境（[准备TensorFlow环境](#准备tensorflow环境)）。

  调用API生成模型优化策略模板（[获取量化配置模板](#获取量化配置模板)）。

  优化策略文件配置（[优化策略配置](#优化策略配置)）。

  训练模型（[训练或校准模型](#训练模型)）。（重训练模式对量化模型训练，无训练模式对量化模型进行校准）
...
| **参数名称**  | **是否必填**  | **参数描述**  
 ...
 | calibration_mode  | 是  bool变量，开发者是否开启无训练（校准）模式。
...
准备PyTorch环境（[准备PyTorch环境](#准备pytorch环境)）。

  调用API生成量化配置json文件（[生成量化配置json文件](#生成量化配置json文件)）。

  修改策略文件配置（[优化策略配置](#优化策略配置-1)）。

  调用API训练量化模型（[训练或校准模型](#训练模型-1)）。

  调用API生成量化ONNX模型及量化参数文件（[生成量化模型](#生成量化模型)）。
...
| **参数名称**  | **是否必填**  | **参数描述**  
 ...
 torch.save(quant_model.state_dict(),"quant.pth") 如果开发者无需进行重训练量化可以只进行无训练量化，开启无训练只需调用set_calibrate_state API

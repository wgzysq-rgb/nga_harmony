...
> **分区**: 指南  |  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  |  **Slug**: `cannkit-plug-in-quantification`  |  **DocID**: `1d4b76c7011e409593ba04b9f4ee4629`  |  **NodeID**: `0002017757954498004142edeb4acafd`
...
| 配置信息  | 配置信息  | 可选项  | 选项说明  
  | quant_strategy  | N/A  Quant_aigc_ptq

 Quant_aigc_qat

 Quant_act_weight_eco

 Quant_lm_head

 Quant_Embed_MinMax

  PTQ量化策略，默认U8S8。量化器为minmax

 QAT量化策略，默认U8S8。量化器为omni_minmax

 16-4grouplinear 量化策略。默认groupsize128，针对llm大模型，QAT量化器
 ...
 omni_group_min_max
  ...
  perchannel量化，最大最小值统计用于PTQ量化

 pergroup量化，最大最小值统计，用于PTQ量化
 ...
 | input  | input_algo  min_max

 ema

 omni_min_max

  pertensor量化，最大最小值统计用于PTQ量化，clip误差小

 pertensor量化，滑动平均统计用于PTQ量化，round误差小

 pertensor量化，QAT量化器

  
 | input  | unsigned_quant  True

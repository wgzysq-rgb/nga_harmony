...
> **分区**: 指南  |  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  |  **Slug**: `neural-network-runtime-guidelines`  |  **DocID**: `36ed0ee9c074461c83085987f20509bd`  |  **NodeID**: `0002017757954498002654f085bbaeff`
...
# Neural Network Runtime对接AI推理框架开发指导
     ...
     **图1** Add单算子网络示意图
     ...
     由于Neural Network Runtime通过Native API对外开放，需要下载对应的SDK并通过Native开发套件编译Neural Network Runtime应用。可以使用DevEco Studio来搭建环境和编译代码。
      ...
      - 使用Ubuntu编译服务器的终端。
      - 指定native工具链路径来编译代码，可以使用DevEco Studio来下载对应的SDK来进行编译。
      - DevEco Studio安装目录下的SDK路径可以在DevEco Studio工程界面，点击File > Settings... > 在settings中搜索SDK，下载对应的SDK即可。
...
```
             查询Neural Network Runtime已经对接的AI加速芯片。
...
```
       ...
       ls /data/local/tmp       以下为打印结果：

       # 0.nncache 1.nncache 2.nncache cache_info.nncache       如果缓存不再使用，需要手动删除缓存，可以参考以下命令，删除缓存文件。

...
> **分区**: 指南  |  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  |  **Slug**: `texttospeech-guide`  |  **DocID**: `432c731359aa476a9f5bdf3c1bf1d58d`  |  **NodeID**: `000201775795449800272dc36ee9e687`
...
```
             调用createEngine接口，创建[TextToSpeechEngine](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/hms-ai-texttospeech#texttospeechengine)实例。
...
```
             得到[TextToSpeechEngine](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/hms-ai-texttospeech#texttospeechengine)实例对象后，实例化[SpeakParams](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/hms-ai-texttospeech#speakparams)对象、[SpeakListener](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/hms-ai-texttospeech#speaklistener)对象，并传入待合成及播报的文本originalText，调用[speak](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/hms-ai-texttospeech#speak)接口进行播报。
...
```
             （可选）当需要停止合成及播报时，可调用[stop](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/hms-ai-texttospeech#stop)接口。

       ttsEngine.stop();             （可选）当需要查询文本转语音服务是否处于忙碌状态时，可调用[isBusy](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/hms-ai-texttospeech#isbusy)接口。

       ttsEngine.isBusy();             （可选）当需要查询支持的语种音色信息时，可调用[listVoices](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/hms-ai-texttospeech#listvoices)接口。
...
```
     ...
     "hello[h1] world"     hello使用单词发音，world及后续单词将会逐个字母进行发音。
     ...
     "[n2]123[n1]456[n0]"     其中，123将会按照数值播报，456则会按照号码播报，而后的文本中的数字，均会自动判断。
     ...
     "你好[p500]小艺"     该句播报时，将会在“你好”后插入500ms的静音停顿。

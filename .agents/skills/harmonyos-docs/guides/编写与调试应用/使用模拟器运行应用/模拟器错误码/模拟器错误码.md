...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ide-emulator-faqs`  |  **DocID**: `ba808fa0ac3349b2a9ff778648dddd16`  |  **NodeID**: `00020177579012255826392ef42e6a54`
...
若勾选后启动模拟器仍然提示该错误，需要以管理员权限打开命令行窗口执行以下命令，并重启电脑。

bcdedit /set hypervisorlaunchtype auto 

如果按照上述步骤无法解决，打开**任务管理器->性能**，若显示虚拟化已禁用或未开启，则是BIOS中虚拟化没有开启，请根据计算机的主板型号，进入BIOS设置界面，并开启虚拟化功能。
...
- 清理磁盘空间或更换模拟器所在磁盘位置。
- 如果是macOS，且磁盘显示空间大于模拟器要求空间，请打开macOS磁盘工具，查看**可用**数据栏中，是否有可清除空间，导致磁盘实际可用空间远小于显示可用空间。如有请尝试从磁盘工具中清理无用的APFS快照文件，以及清除本地缓存文件。
...
首先打开**任务管理器>详细信息**，在列表表头右键**>选择列**，找到并勾选“**提交大小**”，点击“**提交大小**”列进行排序，关闭部分提交大小占用高的进程。

 

 打开**任务管理器>性能>内存**页面，确保已提交内存的剩余量大于模拟器设置的RAM大小。
...
- 执行如下命令，结束hdc的进程，然后尝试重新连接，更多关于hdc工具的使用指导请参考[hdc](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hdc)。```
hdc kill
```
 
- 若按照步骤1操作后还是不能连接，请重启DevEco Studio和模拟器，然后尝试重新连接。
- 重新下载模拟器镜像，具体请参考[创建模拟器](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-emulator-create)。
- 如果已配置hdc环境变量，请检查环境变量配置是否正确。
...
- 在弹出的崩溃报告页面，点击**发送报告**按钮，将相关的错误日志信息发送给我们。
- 重新启动模拟器。

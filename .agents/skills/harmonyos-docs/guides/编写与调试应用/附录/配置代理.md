...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ide-environment-config`  |  **DocID**: `ef34c78b431e4131ae01e5fdc184521a`  |  **NodeID**: `0002017757901225582262ac16f63277`
...
在欢迎页单击**Customize > All settings… > Appearance & Behavior > System Settings > HTTP Proxy**进入HTTP Proxy设置界面。如果已经打开了工程，可以单击**File > Settings**（macOS为**DevEco Studio > Preferences/Settings**）** > Appearance & Behavior > System Settings > HTTP Proxy**进入HTTP Proxy设置界面。
...
- 配置完成后，单击**Check connection**，输入网络地址（如：https://developer.huawei.com），检查网络连通性。提示“Connection successful”表示代理设置成功。
...
进入*C:\Users\用户名*目录，打开**.npmrc**文件。如果该目录下没有**.npmrc**文件，请新建一个。
...
修改代理信息，在proxy和https-proxy中，将user、password、proxyserver和port按照实际代理服务器进行修改。**如果不清楚代理服务器信息，请咨询您的网络管理人员**。示例如下所示：
...
npm info express 执行结果如下图所示，则说明代理设置成功。
...
修改ohpm代理信息，在http_proxy和https_proxy中，将user、password、proxyserver和port按照实际代理服务器进行修改。示例如下所示：
...
代理配置完成后，执行如下命令验证网络是否正常。

ohpm info @ohos/lottie 执行结果如下图所示，则说明代理设置成功。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ide-ohpm-version-rules`  |  **DocID**: `168f7083ab98496ea37b0491cd1196af`  |  **NodeID**: `0002017757901225582659b7d6d5afcc`
...
数字标识符按数字比较大小。举例：2.0.0-beta.2与2.0.0-beta.11比较，标准版本："2.0.0" 部分一致，大小由先行版本号部分：beta.2和beta.11决定；按 "." 分割后，第一段标识符"beta"一致，进而比较数字部分：2 < 11，故：2.0.0-beta.2 < 2.0.0-beta.11。

 非数字标识符按ASCII字典序比较大小。举例：4.0.0-beta.2与4.0.0-rc.1比较，标准版本："4.0.0" 一致，比较 "beta.2" 与 "rc.1" 部分，因首个字符字典序：b < r，故：4.0.0-beta.2 < 4.0.0-rc.1。

 数字标识符优先级低于非数字标识符。举例：1.0.0-1 < 1.0.0-a。

 标识符数量差异，标识符数量更多优先级更高。举例：3.0.0-alpha < 3.0.0-alpha.1。

 多个连接符（-）连接的标识符作为一个整体参与版本比较。举例：2.1.0-100-1002与2.1.0-1001-001比较，标准版本部分 "2.1.0" 一致，比较 "100-1002" 与 "1001-001" 部分，从左到右逐字符（按ASCII字典序）比较，在第四位字符时："-" < "1"，故：2.1.0-100-1002 < 2.1.0-1001-001。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ide-exception-stack-parsing-principle`  |  **DocID**: `cef6b2f735554fe293fde1ac9a4b7f7d`  |  **NodeID**: `000201775790122558297e42263ad4d0`
...
```
     ...
     llvm-addr2line -f -e File.so     查找指定的地址所对应的代码位置：

     llvm-addr2line 0x00000000004005e7 -e test -f -C -s     例如：
...
```
     ...
     - **ObfName**：key为固定字段，value为每个文件混淆后的名称，与**originalfieldname**配对。      "obfName": "home/src/main/ets/pages/a.ts"

...
> **分区**: Guides  |  **Slug**: `ide-local-test`  |  **DocID**: `f69b654f20aa481da9db84e83d671a36`
...
在弹出的Create Local Test窗口，输入或选择如下参数。
...
在DevEco Studio的右上角，选择一项测试任务的配置，然后单击右侧的按钮，执行Local Test。
...
在**Run/Debug Configurations**界面，单击**+**按钮，在弹出的下拉菜单中，单击**Local Test**。
...
通过命令行方式执行Local Test，在工程根目录下执行命令：hvigorw test -p module={moduleName} -p coverage={true | false} -p scope={suiteName}#{methodName} 

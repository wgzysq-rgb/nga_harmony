...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `hiappevent-watcher-resourceleak-events-ndk`  |  **DocID**: `1a669fafe8a4432ab90f4c7bd4ca9737`  |  **NodeID**: `00020177579585271665057763a1de6b`
...
```
       ...
       export const registerWatcher: () => void;             编辑“EntryAbility.ets”文件，在onCreate()函数中添加接口调用：
...
```
       ...
       export const leakMB: (size: number) => void;             单击DevEco Studio界面中的运行按钮，运行应用工程，单击 pss leak 按钮后，等待15到30分钟，系统将上报应用内存泄漏事件。

       同一个应用，24小时内至多上报一次资源泄漏事件，如果短时间内要二次上报，需要重启设备。

             内存泄漏事件上报后，可以在Log窗口看到对系统事件数据的处理日志：

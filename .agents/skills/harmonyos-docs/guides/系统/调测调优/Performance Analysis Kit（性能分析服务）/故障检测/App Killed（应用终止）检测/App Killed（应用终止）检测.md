...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `appkilled-guidelines`  |  **DocID**: `702b650013fc47469cec5de4fb2946d6`  |  **NodeID**: `0002017757958527163291c1652dee8b`
...
# App Killed（应用终止）检测
     ...
     应用退出通常包含以下几种情况：
             ...
             应用自身发生异常或主动抛出异常，例如因SIGSEGV、SIGABRT触发的CPP_CRASH异常，系统可监控并记录维测日志。

             用户主动终止应用，例如在任务列表中点击清理按钮以清除所有应用，或上划清除单个应用，不会生成栈等维测日志。

             应用开发者主动调用exit系统调用时，不会生成栈等维测日志。

             应用发生主线程堵塞，导致界面冻结，通常会生成APP_FREEZE日志。

             资源使用过度将导致系统对应用进行管控，并提供详细的维测信息。例如，应用发生内存泄漏时，通常会生成资源泄漏类的维测日志。开发者可以通过HiAppEvent订阅RESOURCE_OVERLIMIT获取对应的事件和日志。

             系统对应用进行管控时，部分场景无法提供详细的维测信息，比如LowMemoryKiller、应用的RSS内存超过4G、快速泄漏等。
     ...
     本节主要覆盖在场景5和6中因SIGKILL信号导致的应用终止。
             ...
             系统触发管控，终止应用时会添加系统事件打点。

             打点事件中包含uid、包名、前后台信息、终止原因和维测信息。

     
                  #### 约束和限制

     
             应用需先通过HiAppEvent订阅，才能接收终止事件。

             终止事件以异步的方式传递给应用，应用需在下次启动时才能接收到。

             系统的管控行为会随着版本演进而不断新增，不保证当前的管控机制是系统的全部。
     ...
     系统终止应用，有以下的场景:
             ...
             应用的内存、CPU和IO类负载超过一定限额，文件句柄和线程数量超标。

             整机低内存时，会根据内存使用情况和优先级终止应用。

             功耗类检查，包括应用Binder调用导致频繁唤醒、音频播放或录音导致系统无法冻结、GPS或蓝牙等外设使用异常问题。
     ...
     应用可以通过两种方式感知到被异常终止。
             ...
             从元能力的Ability的onCreate回调参数中获取终止原因。具体为LaunchParam启动参数中的LastExitReason字段，请参考[元能力LastExitReason章节](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-abilityconstant#lastexitreason)。

             通过HiAppEvent订阅APP_KILLED事件。订阅方式请参考[应用终止事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-app-killed-events)。

     
                  #### 分析思路和分析步骤

     
             从Ability的onCreate回调参数中获取终止原因。
       ...
       通过APP_KILLED事件，可以获取终止原因、应用前后台等关键信息，对照下表进行处理：
                     ...
                     | IllegalAudioRendererBySuspend            | 未申请合理的后台任务，但是后台有大量音频播放。            | 应用退至后台时，应避免不必要的后台音频播放，或者合理使用后台任务，具体参考[后台任务开发服务](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/background-task-overview)。            | 是            | 否           

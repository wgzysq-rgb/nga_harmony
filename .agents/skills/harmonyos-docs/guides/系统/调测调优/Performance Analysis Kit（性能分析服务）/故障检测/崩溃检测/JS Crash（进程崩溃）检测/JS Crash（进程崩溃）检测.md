...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `jscrash-guidelines`  |  **DocID**: `1f7e532278cd4353827cf93cc3ecc5bb`  |  **NodeID**: `000201775795852716376f03f452f789`
...
# JS Crash（进程崩溃）检测
     ...
     方舟运行时捕获进程异常。生成故障日志的流程如下：
             ...
             上报崩溃事件，开发者可通过HiAppEvent订阅[崩溃事件](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hiappevent-watcher-crash-events)。如需了解JS Crash问题分析方法，请参见[JS Crash类问题分析方法](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-stability-app-crash-js-way)。
     ...
     **方式三：通过hdc获取日志，需打开开发者选项**
     ...
     hdc file recv /data/log/faultlog/faultlogger 本地路径     故障日志文件名格式为：jscrash-进程名-进程UID-毫秒级时间.log。
                 ...
                 | SubmitterStacktrace          | 提交者线程栈          | 20          | 否                    异步线程栈跟踪维测功能默认仅在ARM 64位系统中开启。

          对于**API version 22**之前版本，**三方和系统应用**[libuv](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/libuv)和[ffrt](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-ffrt)提交异步任务仅debug版本默认开启。
...
```
     ...
     **Release模式**
     ...
     at onPageShow (entry|har1|1.0.0|src/main/ets/pages/Index.ts:7:13)     格式解析：
     ...
     **Debug模式**
     ...
     at onPageShow har1 (har1/src/main/ets/pages/Index.ets:7:13)     格式解析：
...
hdc shell reboot
```
     Promise异步任务中抛出的异常默认不会导致JS Crash，但可以通过[on('unhandledRejection')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-errormanager#errormanageronunhandledrejection12)捕获Rejected Promise后，主动将异常抛出，从而触发JS Crash。

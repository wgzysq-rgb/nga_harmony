...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `address-sanitizer-guidelines`  |  **DocID**: `f8d2d3f31669408b9d32b7c52bab5fb6`  |  **NodeID**: `000201775795852716222b5c34544f83`
...
```
 ...
 ptrBeg was re-written after free 0x000100946540[1], 0x000100946548 5555555500000009:5555555555555555 其中，0x000100946540问题内存块起始地址，[1]为检测出问题的内存基于起始地址的8字节偏移数，0x000100946548为实际被修改的地址，5555555500000009:5555555555555555表示内存中的内容被修改后的实际值和预期值的对比。在该信息之后，日志还会输出对应内存块的释放堆栈和分配堆栈，调用栈的格式与HWASan日志一致，此处不再赘述。
...
| 故障特征  | 聚类特征  
  | 故障特征1-使用栈  /data/storage/el1/bundle/libs/arm64/libsample.so+0x7ba4

 /data/storage/el1/bundle/libs/arm64/libsample.so+0x849c

  
 | 故障特征2-释放栈  /data/storage/el1/bundle/libs/arm64/libsample.so+0x7b9c

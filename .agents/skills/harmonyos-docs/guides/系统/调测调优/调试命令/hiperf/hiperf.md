...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `hiperf`  |  **DocID**: `9fbfb9db5f6e4807bc2f3ab3ec7a5e95`  |  **NodeID**: `000201775795852716027e376b3a828d`
...
对进程ID为1234的进程采样10秒，设置回栈方式为fp（栈指针），采样频率1000次/秒，事件类型为hw-cpu-cycles和hw-instructions，采样文件保存至/data/local/tmp/perf.data。
...
```
  ...
  对应用com.example.insight_test_stage进行采样，采样时长为10s，回栈方式设置为dwarf（调试信息表），采样周期为1000，事件类型为hw-cpu-cycles和hw-instructions，使用默认保存路径。
...
```
  对进程ID为1745和1910的进程进行计数，计数时长为10秒，事件类型为hw-cpu-cycles、hw-instructions、sw-task-clock，指定打印信息的时间间隔为3000ms。
...
| 参数名  | 说明  
 ...
 Usage: hiperf list [event type name] 查询支持的硬件事件类型。
...
| 参数  | 参数说明  
 ...
 | -g  | 采集事件群组，以逗号隔开。  
 | --no-inherit  | 不采集子进程数据。  
 | -p  | 采集进程ID，支持对多个进程进行采样，多个进程号之间以逗号隔开。采集的进程应为“[使用调试证书签名的应用](#hiperf采集没有debug证书签名的应用失败)”。  
 | -t  | 采集线程ID，支持对多个线程进行采样，多个线程号之间以逗号隔开。采集的线程应为“[使用调试证书签名的应用](#hiperf采集没有debug证书签名的应用失败)”。  
 | --exclude-tid  | 不采集线程ID，通过线程ID排除特定线程的采样。多个线程号之间以逗号隔开。  
 | --exclude-thread  | 不采集线程名，通过线程名排除特定线程的采样。多个线程名之间以逗号隔开。  
 ...
 | --app  | 采集的应用程序名，以逗号隔开。应用程序必须是启动状态，应用程序未启动时会等待20s，20s内应用程序未启动，命令会自动结束。采集的应用应为“[使用调试证书签名的应用](#hiperf采集没有debug证书签名的应用失败)”。  
 ...
 | --append-smo-data  开启此参数后增加打包的so中原始so的名称。

 **说明**：从API version 23开始，支持该参数。
 ...
 Usage: hiperf record [options] [command [command-args]] 对指定的pid为267的进程采样10秒，并使用dwarf回栈。
...
| 参数  | 参数说明  
 ...
 | --control [command]  采集操作启停控制参数。命令包括prepare/start/stop。该参数不能和-d一起使用。
 ...
 **说明**：从API version 20开始，支持该参数。
 ...
 默认路径以设备上运行 hiperf stat -h/--help 时显示的 -o 参数说明为准。

 该参数必须和--control prepare一起使用，不能和--control一起使用。
 ...
 **说明**：从API version 20开始，支持该参数。
 ...
 hiperf stat [options] [command [command-args]] 使用stat命令监听进程号为2349的进程在CPU 0上运行3秒的性能数据。
...
| 参数  | 参数说明  
 ...
 Usage: hiperf dump [option] \<filename\> 使用dump命令将/data/local/tmp/perf.data文件读取出来，并输出到/data/local/tmp/perf.dump文件中。
...
| 参数  | 参数说明  
 ...
 Usage: hiperf report [option] \<filename\> 从采样文件（perf.data）中提取对性能影响较大（占比≥1%）的关键数据，并以报告的形式进行展示。
 ...
 **可采取的解决方法**：
 ...
 hdc shell "bm dump -n com.example.myapplication | grep appProvisionType" 如包名对应的应用是可调试应用，预期返回信息如下：

 "appProvisionType": "debug", 构建可调试应用需要使用debug证书进行签名，申请调试证书及签名可参考：[申请调试证书](https://developer.huawei.com/consumer/cn/doc/app/agc-help-add-debugcert-0000001914263178)。

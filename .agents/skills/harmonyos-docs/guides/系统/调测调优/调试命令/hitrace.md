...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `hitrace`  |  **DocID**: `b1cf525b82e5401babc96cb771d1b936`  |  **NodeID**: `00020177579585271602625c860d3cdf`
...
# hitrace
       ...
       hitrace命令行工具提供trace信息采集能力，支持采集系统提供的打点和开发者使用[HitraceMeter接口](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hitracemeter-intro)在应用中设置的打点。该工具支持多种方式采集文本格式或二进制格式的trace信息，帮助开发者观测程序运行情况和定位故障问题。
             ...
             确保设备已正常连接，并执行hdc shell。
                 ...
                 | --trace_clock                    设置向trace添加时间戳的时钟类型，可以是boot（默认）、global、mono、uptime或perf。不同设备支持的时钟类型不同，建议使用默认的boot进行采集，其时间为从本次开机起的时间戳，单位为s。每种时间类型的说明如下：

          - boot：系统启动后的时间戳，休眠或系统挂起时也会计时。

          - global：全局同步的时钟，可能有性能开销。

          - mono：系统启动后的时间戳，休眠或系统挂起时不会计时。

          - uptime：系统启动后的时间戳，类似于mono。
          ...
          **说明**：从API version 19开始，支持该命令。

        
                 | --get_level                    查询trace输出级别阈值。

          **说明**：从API version 20开始，支持该命令。
...
```
     ...
     hitrace -l     各tag具体含义参考下面的tag名称与描述对照表。
                 ...
                 | schedlt                    内核态trace标签，轻量级CPU调度。

          **说明**：从API version 23开始，支持该标签。
...
```
     ...
     hitrace -t 10 -b 204800 app     **使用样例**：
...
```
     ...
     hitrace -t 10 -b 204800 app -o /data/local/tmp/test.ftrace     **使用样例**：
...
```
     ...
     hitrace -t 10 -b 204800 app --raw     **使用样例**：
...
```
     ...
     hitrace --trace_begin -b 204800 app graphic     **使用样例**：
...
```
     ...
     hitrace --trace_dump -o /data/local/tmp/test.ftrace     **使用样例**：
...
```
       ...
       hitrace --trace_finish -o /data/local/tmp/test.ftrace       **使用样例**：
...
```
       ...
       hitrace --trace_finish_nodump       **使用样例**：
...
```
     ...
     "net", "dsched", "graphic", "multimodalinput", "dinput", "ark", "ace", "window","zaudio", "daudio", "zmedia", "dcamera", "zcamera", "dhfwk", "app", "gresource", "ability", "power", "samgr", "ffrt", "nweb", "hdf", "virse", "workq", "ipa", "sched", "freq", "disk", "sync", "binder", "mmc", "membus", "load"     使用下面的命令开启捕获二进制trace。

     hitrace --start_bgsrv     **使用样例**：
...
```
     ...
     hitrace --trace_begin --record -b 204800 --file_size 102400 app graphic     **使用样例**：
...
```
     ...
     hitrace --trace_finish --record     **使用样例**：
...
```
     ...
     hitrace -z -b 102400 -t 10 sched freq idle disk -o /data/local/tmp/test.ftrace     **使用样例**：
...
```
     ...
     开发者可使用带trace级别的打点接口（参考[@ohos.hiTraceMeter](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-hitracemeter)和[trace.h](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-trace-h)中的API version 19的trace打点接口），测试不同阈值下的trace输出是否符合预期。
...
```
     **可能原因&解决方法**
     ...
     错误码1表示hiview进程状态异常，可以尝试重启手机后重新采集。
                  ...
                  #### [h2]执行hitrace命令后报错“not support category on this device”

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `restool`  |  **DocID**: `ad1996aec9f040ed9435371bed000298`  |  **NodeID**: `0002017757958527160059bb4fc53920`
...
```
     ...
     restool dump [-h] [config] filePath     dump命令参数列表：
...
```
     ...
     restool -i entry/src/main -j entry/src/main/module.json -p com.ohos.demo -o out -r out/ResourceTable.txt -f     2、增量资源编译，具体步骤如下：

     步骤一：生成资源中间件，命令如下:

     restool -x entry/src/main/resource -o out     步骤二：编译资源中间件，命令如下:

     restool -i out1 -i out2 -o out -p com.ohos.demo -r out/ResourceTable.txt -j entry/src/main/module.json -f -z     3、叠加资源编译，命令如下：
...
```
     ...
     **处理步骤**
     ...
     检查JSON中是否缺少xxx节点。
                  ...
                  #### [h2]11203005 JSON中存在空object或空数组
     ...
     **处理步骤**
     ...
     检查--target-config选项对应参数格式是否准确，如：Locale[zh_CN,en_US];Device[phone]，具体格式信息可以参考[restool支持的命令选项](#restool支持的命令选项)中关于target-config的说明。
                  ...
                  #### [h2]11210017 无效的系统资源id_defined.json路径
     ...
     **处理步骤**
     ...
     检查系统资源的id_defined.json路径是否准确且有访问权限。
                  ...
                  #### [h2]11210018 重复的系统资源id_defined.json路径
     ...
     **处理步骤**
     ...
     检查--defined-sysids选项指定的系统资源id_defined.json文件路径是否重复。
                  ...
                  #### [h2]11210019 compressed-config存在冲突
     ...
     **可能原因**
     ...
     同时指定了两个互斥的选项，如-x和--target-config。
     ...
     **可能原因**
     ...
     当通过-i/--inputPath仅指定了单个输入路径时，restool默认从输入路径的父目录读取应用配置文件；当指定了多个输入路径时，如restool -i xxx\AppScope -i xxx\entry\main，需要通过-j/--json明确指定应用配置文件的路径。
     ...
     **处理步骤**
     ...
     检查模块类型是否在指定的范围内。
                  ...
                  #### [h2]11211004 资源起始id与id_defined.json存在冲突
     ...
     **可能原因**
     ...
     默认情况下，资源起始id为0x01000000，编译时资源id依次递增，同时restool提供了两种方式自定义资源id：

     
      - 选项-e/--startId：可以在[0x01000000, 0x06FFFFFF) 或 [0x08000000, 0xFFFFFFFF)的范围内指定资源起始id。
      - [固定资源ID](#固定资源id)：通过id_defined.json文件指定资源id。
     ...
     **处理步骤**
     ...
     检查是否同时使用上述两种方式自定义资源id。
                  ...
                  #### [h2]11211007 id_defined.json中的资源类型无效
     ...
     **处理步骤**
     ...
     检查资源类型是否在指定范围内。
                  ...
                  #### [h2]11211008 id_defined.json中的资源id无效
     ...
     **处理步骤**
     ...
     检查资源id是否为十六进制格式且取值在[0x01000000,0x06FFFFFF] 或 [0x08000000,0xFFFFFFFF]范围内。
                  ...
                  #### [h2]11211012 id_defined.json定义了重复的id
     ...
     **可能原因**
      ...
      - 应用配置文件[module.json（Stage模型）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file)或[config.json（FA模型）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/application-configuration-file-overview-fa)中name指定的模块名称错误。
     ...
     **可能原因**
     ...
     资源文件不符合要求，比如：
      ...
      - rawfile应该是一个文件夹，实际是文件。
      - base/element下应该都是JSON文件，实际存在文件夹。
     ...
     **可能原因**
     ...
     color资源只支持以下两种格式：
      ...
      - 引用其他color资源，如$color:xxx引用应用自身color资源，或$ohos:color:xxx引用系统color资源。
      - 以#开头的rgb颜色值格式，如#rgb,#argb,#rrggbb,#aarrggbb。
     ...
     **可能原因**
     ...
     资源引用只适用于同类型资源之间，如string.json中的字符串资源可以用$string:xxx引用应用自身的字符串资源，或使用$ohos:string:xxx引用系统字符串资源，但不能通过$integer:xxx引用整数资源。

     **处理步骤**

     检查资源引用方式是否正确。

                  #### [h2]11211110 theme资源的parent为空
...
```
     ...
     **可能原因**
     ...
     单复数资源中重复声明了相同的类别，如下错误示例中eat_apple重复声明了类别one：
...
```
     ...
     **处理步骤**
      ...
      - 检查报错的资源是否存在。
      - 检查资源所在的resources的父目录是否通过-i/--inputPath指定为输入路径，如报错信息中的资源定义在xxx/entry/src/main/resources/base/element/string.json中，则需指定xxx/entry/src/main为输入路径。
     
                  #### [h2]11211124 解析resources.index文件失败

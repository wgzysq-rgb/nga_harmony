...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `aa-tool`  |  **DocID**: `2135c288750b438580c9e3dede8d44ef`  |  **NodeID**: `00020177579585271689685ba4dcab80`
     ...
     **启动命令参数列表**
          ...
          **约束：**
          ...
          仅当2in1设备处于开发者模式下，且被启动应用采用调试签名时，该字段生效。
                 ...
                 | --wh                    可选参数，windowHeight，窗口高度，单位px。
          ...
          **约束：**
          ...
          仅当2in1设备处于开发者模式下，且被启动应用采用调试签名时，该字段生效。
                 ...
                 | -E                    可选参数，调试时是否显示详细的异常信息。携带该参数代表显示，不携带代表不显示。
          ...
          **说明：** 从API version 14开始，支持该参数。
                 ...
                 | -W                    可选参数，调优命令。用于测量UIAbility从启动到切换至前台状态耗时。
          ...
          **异常情况的返回信息如下：**
          ...
          - "The wait option does not support starting implicit" ：不支持隐式启动。

          - "The wait option does not support starting non-uiability" ：不支持启动非UIAbility组件。
...
```
         ...
         aa start -U myscheme://www.test.com:8080/path                 在上述基础上，如果需要携带参数，可以使用如下命令：

         aa start -U myscheme://www.test.com:8080/path --pi paramNumber 1 --pb paramBoolean true --ps paramString teststring  --psn paramNullString         UIAbility获取传入参数示例如下：
...
```
     ...
     aa stop-service [-d <deviceId>] -a <abilityName> -b <bundleName> [-m <moduleName>]     **停止命令参数列表**
...
```
     ...
     aa dump -a                         aa dump命令从API version 7开始支持，从API version 9废弃，替换命令为[hidumper](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hidumper) -s AbilityManagerService。
                ...
                **打印命令参数列表**
                 ...
                 | -l/--mission-list          | type（缺省打印全部）                    服务侧为了方便管理任务链，内部维护了4种类型的任务链。
...
```
     ...
     aa force-stop <bundle-name> [-p pid] [-r kill-reason]     **强制停止进程命令参数列表**
...
```
     ...
     aa test -b <bundleName> [-m <module-name>] [-p <package-name>] [-s class <test-class>] [-s level <test-level>] [-s size <test-size>] [-s testType <test-testType>] [-s timeout <test-timeout>] [-s <any-key> <any-value>] [-w <wait-time>] -s unittest <testRunner>                         关于class、level、size、testType等参数的详细说明请参见[命令行执行测试脚本](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/unittest-guidelines#命令行执行测试脚本)。
                ...
                **启动测试框架命令参数列表**
                 ...
                 | -p                    可选参数，testRunner的packageName。

          **说明**：该可选参数仅可在FA模型下使用。

        
                 | -m                    可选参数，testRunner的moduleName。
...
```
     ...
     aa attach -b <bundleName>     **进入调试模式命令参数列表**
...
```
     ...
     aa detach -b <bundleName>     **退出调试模式命令参数列表**
...
```
     ...
     aa appdebug -b <bundleName> [-p]     **等待调试命令参数列表**
...
```
     ...
     从API version 13开始，开发者可以通过该命令来调试应用的[onMemoryLevel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-abilitystage#onmemorylevel)生命周期。通过在参数中指定进程的pid和内存使用级别来触发该进程的onMemoryLevel生命周期回调。该命令只提供基本的应用调试能力，不能完全模拟真实的内存加压测试场景。
...
```
     **参数列表**
                        ...
                        | 参数          | 参数说明         
                               | -h/--help          | 帮助信息。         
                 | -p          | 必选参数，进程pid。         
                 | -l          | 必选参数，内存使用级别，具体值参考[AbilityConstant.MemoryLevel](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-abilityconstant#memorylevel)。         
...
```
     ...
     **处理步骤**
      ...
      - 检查aa命令的-a的参数abilityName和-b的参数bundleName是否正确。
...
```
                  ...
                  #### [h2]10100102 aa start命令无法拉起UIExtensionAbility
     ...
     **处理步骤**
      ...
      - 如果为显式启动，需要确保abilityName传参正确。
                  ...
                  #### [h2]10103102 传入的AppCloneIndex是一个无效值
     ...
     **处理步骤**
     ...
     确认AppCloneIndex是否合法。
                  ...
                  #### [h2]10106101 上一个Ability未启动完成，先缓存在队列中等待后续启动
     ...
     **处理步骤**
     ...
     内部错误是系统运行过程中出现的内部错误，开发者无法处理。
                  ...
                  #### [h2]10103201 目标Ability不是ServiceAbility类型
...
```
     ...
     **可能原因**
      ...
      - aa force-stop命令指定的应用不存在。
      - 未成功连接到AppManagerService。
     ...
     **处理步骤**
       ...
       hdc shell bm dump -a             尝试重启设备。

     
                  #### [h2]10106402 常驻进程无法被杀死

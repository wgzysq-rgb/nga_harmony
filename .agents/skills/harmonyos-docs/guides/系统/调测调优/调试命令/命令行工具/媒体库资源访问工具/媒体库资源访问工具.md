...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `mediatool`  |  **DocID**: `850bffe3610b46b294106683531359d6`  |  **NodeID**: `000201775795852716125626aa54c533`
...
- 使用hdc shell进入命令行执行模式。
   ...
   #### [h2]导入命令（mediatool send）

mediatool send <path-to-local-media-file> [-ts] [-tas] [-rf] [-urf] 该命令能够将设备<path-to-local-media-file>路径下的图片视频文件推入媒体库中保存。支持保存图片、视频和音频文件。文件在媒体库中会保留原有的名字。<path-to-local-media-file>可以为文件夹，mediatool会将文件夹里的所有文件置入媒体库中。保存成功后会打印成功置入的资源的uri。
...
```
   #### [h2]打印命令（mediatool list）

mediatool list <resource-uri> 该命令能够将<resource-uri>指定uri对应的媒体库内资源信息以csv格式打印出来。

 例如，媒体库内图片资源A的uri为file://media/Photo/3/IMG_1721381297_001/MyImage.jpg, mediatool list file://media/Photo/3或者mediatool list file://media/Photo/3/IMG_1721381297_001/MyImage.jpg都能成功打印出该资源信息。
...
mediatool recv <media-target> <dest-path> 该命令能够将<media-target>指定的媒体库资源的源文件内容导出到<dest-path>指定的设备路径下。
...
mediatool delete <resource-uri> 该命令能够删除<resource-uri>指定uri的媒体库资源。被删除的资源无法恢复，请谨慎执行。
...
mediatool query <display-name> [-p] [-u] 该命令能够查询出所有图库中显示名字为<display-name>的媒体库资源，返回资源源文件真实路径或媒体资源uri。默认返回源文件真实路径。
...
mediatool ls -l <media-path> 列举出<media-path>所指定的系统媒体路径下的所有文件。效果类似文件系统ls -l。
...
支持导出指定路径下所有未被隐藏的本地文件和目录。
 ...
 hdc file recv DEST SOURCE **使用示例**：
...
支持导入媒体文件（图片、视频等）及目录，但不支持创建目录。当目录名称相同时会将内容合并（保留所有不重名的文件）；当文件名称相同时会覆盖目标文件。
 ...
 hdc file send SOURCE DEST **使用示例**：

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `huks-signing-signature-verification-overview`  |  **DocID**: `383feba2fb9b4a208460a89e6c1b2187`  |  **NodeID**: `000201775795852716572735a0cf8921`
...
# 签名/验签介绍及算法规格
     ...
     **手机、平板、PC/2in1设备、TV、智能穿戴规格**
         ...
         | 对于PSS模式，salt长度支持设置为摘要长度和最大长度（最大长度=密钥长度-摘要长度-2），对应枚举值详见[HuksRsaPssSaltLenType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-huks#huksrsapsssaltlentype10)。          | 8+         
          ...
          ECC算法支持的椭圆曲线函数包括：P-256、P-384、P-521。

         | 8+         
                 | ED25519/NoDigest          | NoDigest需要指定TAG HuksKeyDigest.HUKS_DIGEST_NONE。          | 8+         
                 | SM2/SM3          | 签名是ASN1格式。          | 9+         
                 | SM2/NoDigest                    签名是ASN1格式。

          NoDigest需要指定TAG HuksKeyDigest.HUKS_DIGEST_NONE。由业务对明文做SM3哈希，再将哈希后的数据传入，哈希后的数据长度必须满足SM2签名验签支持的摘要算法规格。

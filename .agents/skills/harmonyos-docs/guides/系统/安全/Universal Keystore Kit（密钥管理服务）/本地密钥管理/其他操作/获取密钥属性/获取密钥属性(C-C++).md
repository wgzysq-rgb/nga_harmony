...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `huks-obtain-key-properties-ndk`  |  **DocID**: `5423145c6878436d9a06b3190fbb0c76`  |  **NodeID**: `00020177579585271658348d65f6d5ff`
...
```
        ...
        - paramSetOut：用于放置获取到的参数集结果，为[OH_Huks_ParamSet](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-hukstypeapi-oh-huks-paramset)类型对象，需要业务提前申请好内存，需申请足够容纳获取到的密钥属性集的内存大小。
       
             调用接口[OH_Huks_GetKeyItemParamSet](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-huks-api-h#oh_huks_getkeyitemparamset)，传入上述参数。

             返回值为成功码/错误码，获取成功后，从参数集中读取需要的参数。

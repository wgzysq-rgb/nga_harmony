...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `crypto-key-derivation-overview`  |  **DocID**: `71ff71559faf4e7b98ccda228a6d7e55`  |  **NodeID**: `0002017757958527160486ddf13cce3d`
...
PBKDF（Password-Based Key Derivation Function）是具有可变计算成本的密钥派生函数，PBKDF2是PKCS系列的标准之一。
 ...
 PBKDF2是将伪随机函数PRF（Pseudo-Random Function，例如基于散列的[HMAC](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-compute-hmac)），输入密码明文和盐值，重复多次运算来进行密钥派生。
...
HKDF（HMAC-based Extract-and-Expand Key Derivation Function，是基于[HMAC](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/crypto-compute-hmac)）消息身份验证代码的简单密钥派生算法。
...
X963KDF算法是一种基于HMAC的密钥派生函数（KDF），通常与椭圆曲线结合使用，用于生成密钥。

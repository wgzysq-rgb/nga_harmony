...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `crypto-generate-random-number-hardware-ndk`  |  **DocID**: `1b39cbd118ea429799956b9d1f10d08f`  |  **NodeID**: `0002017757958527162789b5017a22f9`
...
# 使用硬件熵源生成安全随机数(C/C++)
    ...
    在开发前，开发者应该先对加解密基础知识有一定了解，并熟知以下随机数相关的基本概念：
     ...
     安全随机数生成，设置硬件熵源之后，使用OpenSSL的RAND_priv_bytes接口生成。
             ...
             调用[OH_CryptoRand_Create](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-crypto-rand-h#oh_cryptorand_create)，创建随机数生成器。

             调用[OH_CryptoRand_EnableHardwareEntropy](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-crypto-rand-h#oh_cryptorand_enablehardwareentropy)，开启硬件熵源。

             （可选）调用[OH_CryptoRand_SetSeed](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-crypto-rand-h#oh_cryptorand_setseed)，为随机数生成器设置种子。

             调用[OH_CryptoRand_GenerateRandom](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-crypto-rand-h#oh_cryptorand_generaterandom)，生成指定长度的安全随机数。指定字节长度范围为1~INT_MAX。

             调用[OH_CryptoRand_GetAlgoName](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-crypto-rand-h#oh_cryptorand_getalgoname)，获取随机数生成器使用的算法名称。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `create-verify-cerchainvalidator-object`  |  **DocID**: `1e707162caaa46f8a2999c0d5781f9cf`  |  **NodeID**: `0002017757958527163246555b36bf55`
...
# 证书链校验器对象的创建和校验
    ...
    开发者可以参考示例将已有的多个证书构建出证书链数据。
       ...
       import { cert } from '@kit.DeviceCertificateKit';             [cert.createCertChainValidator](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certcreatecertchainvalidator)创建证书链校验器对象。

             基于已有的证书数据，创建证书链数据对象[CertChainData](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certchaindata)。
       ...
       证书算法库框架提供了证书链校验器对象可用于校验证书链，以验证信任链根源，但待校验的证书链数据对象应符合证书链数据对象的数据结构定义[CertChainData](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certchaindata)。
             ...
             调用[CertChainValidator.validate](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#validate)校验证书链数据。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `create-verify-cerchainvalidator-revocation-object`  |  **DocID**: `41d26c91837f418a9758062d27730490`  |  **NodeID**: `0002017757958527163555b0157f2352`
...
# 证书链在线校验证书吊销状态
     ...
     API 22开始支持本地证书链吊销状态校验时仅校验终端实体证书。
       ...
       import { cert } from '@kit.DeviceCertificateKit';             调用[cert.createX509CertChain](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certcreatex509certchain11)创建证书链对象。

             调用[cert.createX509Cert](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certcreatex509cert)创建X509证书对象。

             调用[cert.createX509CRL](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certcreatex509crl11)创建X509证书吊销列表对象。

             构造[cert.CertChainValidationParameters](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certchainvalidationparameters11)证书链校验参数对象。

             调用[cert.validate](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#validate11)，传入证书链校验参数，进行证书链校验。
...
```
     ...
     从API 22开始，支持在线校验证书链中的中间CA证书的吊销状态。
       ...
       import { cert } from '@kit.DeviceCertificateKit';             调用[cert.createX509CertChain](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certcreatex509certchain11)创建证书链对象。
             ...
             构造[cert.CertChainValidationParameters](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certchainvalidationparameters11)证书链校验参数。
...
```
     ...
     从API 23开始，支持证书链校验时忽略网络不可达的在线证书吊销检查异常。
       ...
       import { cert } from '@kit.DeviceCertificateKit';             调用[cert.createX509CertChain](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certcreatex509certchain11-2)创建证书链对象。

             调用[cert.createX509Cert](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cert#certcreatex509cert)创建X509证书对象。构造 cert.CertChainValidationParameters 证书链校验参数，配置 revocationCheckParam 为 RevocationCheckOptions.REVOCATION_CHECK_OPTION_IGNORE_NETWORK_ERROR，以忽略网络不可达的情况。

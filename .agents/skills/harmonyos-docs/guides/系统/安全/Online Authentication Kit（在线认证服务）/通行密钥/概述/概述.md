...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `onlineauthentication-passkey-intro`  |  **DocID**: `eb3c0009c19e4f50ac59989d019b7bb9`  |  **NodeID**: `0002017757958527160838e75518b5ac`
...
需要提供方便、快速、安全的本地应用或网页登录方式时，可以使用通行密钥服务的通行密钥注册能力为用户创建通行密钥。
   ...
   #### [h2]使用通行密钥登录本设备的应用或网页账号

用户在登录应用或者网页时，需要验证用户的身份，可以使用通行密钥服务的本地免密认证能力。

 

   #### [h2]使用跨设备扫码登录其他设备的应用或网页账号
...
应用程序或浏览器集成Online Authentication Kit中的通行密钥模块，向FIDO服务器发起注册请求。

  FIDO服务器将注册信息（如挑战值等）返回给应用程序，应用程序组装注册报文，使用通行密钥服务进行注册。

  通行密钥服务使用认证器验证通过后，会生成一对凭据密钥对，然后用其预置的私钥对生成的公钥及注册信息（如挑战值等）进行签名。

  通行密钥服务返回签名给应用程序。应用程序发给FIDO服务器进行注册。

  FIDO服务器验证签名，保存公钥，并将处理结果返回给应用程序。

  
 **认证流程介绍**

 
应用程序或浏览器集成Online Authentication Kit中的通行密钥模块，并向FIDO服务器发起认证请求。

  FIDO服务器将认证信息（如挑战值等）返回给应用程序，应用程序组装认证报文，使用通行密钥服务进行认证。

  通行密钥服务使用认证器验证通过后，用其保存的私钥对认证信息（如挑战值等）进行签名。

  通行密钥服务返回签名给应用程序。应用程序发给FIDO服务器进行认证。

  FIDO服务器验证签名，并将处理结果返回给应用程序。

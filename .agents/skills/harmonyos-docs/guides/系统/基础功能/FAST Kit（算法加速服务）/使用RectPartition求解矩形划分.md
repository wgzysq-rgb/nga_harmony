...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `fast-rect-partition`  |  **DocID**: `3ac6bb52c1a94c998c7dd7333f2435a6`  |  **NodeID**: `0002017757958527169080f506c6f373`
...
# 使用RectPartition求解矩形划分
    ...
    其相关定义如下：一个矩形为二维网格内横纵坐标满足的所有单元矩形构成的集合（坐标系说明：X轴从左到右递增，Y轴从上到下递增）。两个矩形相交，当且仅当它们共享至少一个公共的单元矩形。
    ...
    矩形划分求解器运行的时间复杂度为 ，可以高效处理大规模输入数据。在网格数据处理和空间几何计算等优化场景中，可以使用矩形划分求解器提升区域处理效率，减少冗余空间。
     ...
     具体API说明详见[接口文档](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/fast-kit-fast)。
       ...
       target_link_libraries(entry PUBLIC libfast_ads.so)             调用HMS_FAST_RectPartition_CreateConfig生成矩形划分求解器配置实例（FAST_RectPartitionConfig）。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `devicemanager-guidelines`  |  **DocID**: `966423f75ad64b4c93c8a6e0f5389c0e`  |  **NodeID**: `000201775795852716869f4a7f979b1a`
...
```
             导入abilityAccessCtrl模块，用于获取权限申请的能力。

       import { abilityAccessCtrl } from '@kit.AbilityKit';             分布式数据同步权限的授权方式为user_grant，因此需要调用requestPermissionsFromUser接口，以动态弹窗的方式向用户申请授权。
...
```
     ...
     发现周边同局域网或者开启蓝牙的设备。详细信息参见：[startDiscovering](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-distributeddevicemanager#startdiscovering)。
       ...
       import { distributedDeviceManager } from '@kit.DistributedServiceKit';             导入BusinessError模块，用于获取distributedDeviceManager模块相关接口抛出的错误码。

       import { BusinessError } from '@kit.BasicServicesKit';             创建设备管理实例，设备管理实例是分布式设备管理方法的调用入口。
...
```
             注册发现设备的回调，调用发现接口发现周边设备。发现状态持续两分钟，超过两分钟，会停止发现，最大发现数量99个。
...
```
     ...
     设备绑定。详细信息参见：[bindTarget](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-distributeddevicemanager#bindtarget)。
...
```
     ...
     设备信息查询。详细信息参见：[getAvailableDeviceListSync](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-distributeddevicemanager#getavailabledevicelistsync)。
...
```
     ...
     设备上下线监听。详细信息参见：[on('deviceStateChange')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-distributeddevicemanager#ondevicestatechange)。
       ...
       import { BusinessError } from '@kit.BasicServicesKit';             创建设备管理实例，设备管理实例是分布式设备管理方法的调用入口，并注册设备上下线回调。

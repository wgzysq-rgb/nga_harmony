...
> **分区**: Guides  |  **Slug**: `bpta-multi-path-network-turbo`  |  **DocID**: `b81f4d8d805b43ae9880326d6608096b`  |  **NodeID**: `00020177683999622491278b89b58b54`
...
# 多网并发网络加速
     ...
     多网并发是指通过系统接口同时建立并使用多个网络通路，例如同时使用Wi-Fi和流量，或者主卡和副卡的流量通路。通过多网并发，可以突破默认单网传输的带宽和速率限制，达到网络传输加速的效果。应用发起多网请求后，系统依据业务场景决定并发组合和实施相应的并发管控，并对并发做收益度量。使用多网并发功能的原则是应用申请（受限权限）、系统管控、最小化使用。
       ...
       (1) 成功发起多网请求后，可以在[netHandover.on('multiPathStateChange')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-nethandover#nethandoveronmultipathstatechange)回调中监听并获取到可用的[NetHandle](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-net-connection#nethandle)数据网络句柄对象。
     ...
     当发布视频帖子、下载应用资源包时，应用本质上在进行大文件网络传输，通过将大文件分片可以充分发挥多网并发能力，将不同文件片段分配给不同网络通路同时传输，能极大地加快传输速度。
      ...
      - [@ohos.file.fs](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-file-fs)：负责文件管理，包括文件分片、大小查询等。
       ...
       根据当前网络传输任务通过[netBoost.setSceneDesc()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-netboost#netboostsetscenedesc)设置多网场景，帮助系统进行多网并发管控和业务时长分析，当前对于文件分片上传任务选择'upload'场景。
...
```
       ...
       通过[netHandover.releaseMultiPath()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/networkboost-nethandover#nethandoverreleasemultipath)释放多网，注销相关的监听方法。
...
```
     ...
     当进行数据、文件同步，下载器下载资源时，往往会需要处理多个���件的传输任务，使用多网并发拓宽网络带宽，不同文件传输任务分配给不同网络通路并发传输，达到提升传输效率的效果。

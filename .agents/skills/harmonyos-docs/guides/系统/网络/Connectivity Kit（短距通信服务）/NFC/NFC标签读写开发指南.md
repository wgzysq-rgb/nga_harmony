...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `nfc-tag-access-guide`  |  **DocID**: `8bed56fc416f4b06ad8243309a3fa309`  |  **NodeID**: `0002017757958527160435b3fb84f590`
...
# NFC标签读写开发指南
     ...
     NFC标签读写应用开发者根据业务需要，可以选择实现前台读卡或者后台读卡。两种不同的读卡方式，代码实现上会存在一些差异。
      ...
      - 在配置文件module.json5中，不需要静态声明过滤读取NFC标签的技术类型，而是通过[tag.registerForegroundDispatch](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-nfctag#tagregisterforegrounddispatch10)或者[tag.on](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-nfctag#tagon11)来完成动态注册。
      - 通过tag.registerForegroundDispatch或tag.on来动态注册前台读写标签时，入参中必须指定需要读取NFC标签的技术类型。
      - 如果选择tag.registerForegroundDispatch注册，当应用运行在前台并进入该页面，NFC的卡模拟功能在打开时，可以同时完成刷卡。如果选择tag.on注册，当应用运行在前台并进入该页面时，NFC的卡模拟是关闭的，无法同时进行刷卡功能。
      - 当应用页面切换到后台时，需要显式调用[tag.unregisterForegroundDispatch](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-nfctag#tagunregisterforegrounddispatch10)或者[tag.off](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-nfctag#tagoff11)来取消注册，退出前台读卡优先功能。
      ...
      - 技术类型的描述字符，必须完整匹配并区分大小写，需要严格匹配。
        ...
        - NFC标签读写示例代码的提供，全部按照[Stage模型](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ability-terminology#stage模型)来说明。
...
```
     ...
     应用程序需要支持后台读卡时，需要在应用的属性配置文件中，声明与NFC相关的属性值。比如，在module.json5文件中，声明下面属性值。
        ...
        - 声明"actions"字段的内容填写，必须包含"ohos.nfc.tag.action.TAG_FOUND"，不能更改。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `nfc-hce-guide`  |  **DocID**: `b8cb91e81ce64f24b70c690f3b32f9e0`  |  **NodeID**: `000201775795852716195e2b23ac0351`
...
# HCE卡模拟开发指南
     ...
     安全单元模拟NFC卡片，和NFC读卡器完成NFC刷卡业务。要模拟的卡片通过应用配置到安全单元中，当电子设备触碰到NFC读卡器时，数据不经过设备CPU处理，而是直接发送到安全单元，完成刷卡。
             ...
             基于刷卡安全性考虑，不论HCE应用是前台方式还是后台方式刷卡，均不支持电子设备在灭屏或熄屏状态下的HCE刷卡操作。

             电子设备必须具备NFC控制器芯片，才支持HCE刷卡能力。对于是否具有NFC安全单元芯片，没有约束要求。

             HCE应用程序需要声明NFC卡模拟权限，具体见示例。
     ...
     HCE应用开发者根据业务需要，可以选择实现前台刷卡或者后台刷卡。两种不同的刷卡方式，代码实现上会存在一些差异。
      ...
      - 在配置文件module.json5中，不需要静态声明NFC读卡器选择的应用ID（AID，参考ISO/IEC 7816-4规范），而是通过[start](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cardemulation#start9)来动态注册。
      - HCE应用的刷卡页面退出时，需要显式调用[stop](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cardemulation#stop9)来释放动态注册的AID刷卡配置项。
      ...
      - 在配置文件module.json5中，需要静态声明NFC读卡器选择的应用ID（AID）。根据业务选择， 选择声明的AID是属于Payment类型，还是Other类型。
      - 如果选择Payment类型，该HCE应用会在系统设置页面的NFC"默认付款应用"里出现。用户必须选择该HCE应用作为默认支付应用后，才能实现后台刷卡功能。由于提供了默认支付应用的选项， 因此Payment类型的HCE应用，不会出现多个冲突的情况。
      - 如果选择Other类型，该HCE应用不会出现在系统设置页面的NFC"默认付款应用"里，但是多个HCE应用如果都声明了相同的Other类型的AID时，会出现冲突的可能。
      - HCE应用后台刷卡的实现，不需要调用接口start和stop。
      - HCE应用后台刷卡建议使用单独专用的HceAbility实现，以减少[onCreate](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-uiability#oncreate)函数耗时。
      - HceAbility的OnCreate函数中除了[hceService.on](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-cardemulation#on8)外，尽量减少其它操作。
      - HceAbility的OnCreate函数中hceService.on要同步执行，不能异步执行，以免影响HCE通信时序。
        ...
        - HCE示例代码的提供，全部按照[Stage模型](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ability-terminology#stage模型)来说明。
...
```
      ...
      - 在module.json5文件中声明OFFHOST特定的action，声明应用能够处理的AID，声明安全单元。
      - 将OFFHOST应用设置为默认付款应用。

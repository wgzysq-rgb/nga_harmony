...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `servicecollaboration-servicendk-guide`  |  **DocID**: `9ad9fecbd60f4527afbe901ee24e3ae9`  |  **NodeID**: `0002017757958527161331d194a83768`
...
# 跨设备互通NDK开发指导
     ...
     需同时满足以下条件，才能使用该功能：
        ...
        - 跨设备互通API支持根据特定调用策略调用设备。调用策略：TV、2in1设备可以调用平板和手机，平板可以调用手机，并且在6.1.0(23)之后支持TV、手机、平板或2in1设备调用支持拍照、扫描、选择图库中图片与视频能力的手机，支持拍照、扫描、选择图库中图片与视频能力的平板，以及支持选择图库中图片与视频能力的2in1设备。
         ...
         条件允许时，建议双端设备接入同一个局域网，可提升唤醒相机的速度。
             ...
             通过[HMS_ServiceCollaboration_StartCollaboration](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/servicecollaboration-capi-module#hms_servicecollaboration_startcollaboration)接口拉起跨设备互通能力。
     ...
     在开发具体功能前，请先查阅[参考文档](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/servicecollaboration-capi-module)。
       ...
       #include "service_collaboration/service_collaboration_api.h"             编写CMakeLists.txt。
...
```
        ...
        - 通过调用HMS_ServiceCollaboration_GetCollaborationDeviceInfos接口获取设备列表信息，传入需要的ServiceCollaborationFilterType能力数组，接口会返回支持对应能力设备。每个设备中包含所支持的能力类型ServiceCollaborationFilterTypes和设备类型deviceType信息。
        - 创建回调ServiceCollaborationCallback，其中包括事件回调OnEventProc和图片数据回调OnDataCallbackProc；创建ServiceCollaboration_SelectInfo，示例中传入了TAKE_PHOTO能力，并选择了列表的第一个设备。

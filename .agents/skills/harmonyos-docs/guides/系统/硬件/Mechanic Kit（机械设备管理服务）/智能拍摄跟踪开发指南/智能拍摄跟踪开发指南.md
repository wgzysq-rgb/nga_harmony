...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `camera-tracking-guide`  |  **DocID**: `0981feeea1044e01baa85fb62ade819a`  |  **NodeID**: `000201775795852716880d980f21b3a4`
...
# 目标拍摄跟踪开发指南
    ...
    目标拍摄跟踪功能通过机械体设备实现人脸和物体的自动化跟踪，提升拍摄质量和用户体验，助力开发者构建更自动化、高效的拍摄解决方案。
     ...
     机械体设备控制器API的接口使用指导请参见[MechanicManager API参考](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-mechanicmanager)。
      ...
      - 支持Mechanic Kit协议的机械体设备。
      - 若要验证目标跟踪功能，主设备的相机驱动必须支持人脸检测。
      - 请将SDK更新到API 20或以上版本，具体操作参见[更新指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-software-install)。
      - 请确保机械体设备已通过蓝牙与主设备连接。
     ...
     确保机械体设备连接或断开时，应用能及时响应，支持设备连接状态的动态管理。
       ...
       import { mechanicManager } from '@kit.MechanicKit';             获取已连接的机械体列表。
...
```
     ...
     **建立连接**
      ...
      - 确保机械体与开发设备已通过蓝牙配对并连接。
      - 将开发设备放置在机械体设备上。
     
     **功能验证步骤**

     
      - **设备列表查询**：调用 getAttachedMechDevices 接口查询当前已连接的机械体设备列表，验证设备是否正确识别。
      - **目标拍摄跟踪**：调用 setCameraTrackingEnabled 启用摄像头目标跟踪功能，使用 getCameraTrackingEnabled 验证状态，测试设备是否能跟随目标自动旋转。

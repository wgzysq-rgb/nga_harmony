...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `watch_p2p_communication`  |  **DocID**: `6c53158233254648b1a4239f06176d93`  |  **NodeID**: `000201775795852716188d03422340af`
...
# 应用间消息通信
      ...
      - 对端设备侧应用和穿戴设备应用必须同时处于已启动状态。
             ...
             参见[已连接对端设备查询](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/watch_query_connected_devices)章节，从已连接设备列表中选定需要通信的对端设备。

             调用[getP2pClient](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#wearenginegetp2pclient)方法，获取[P2pClient](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#p2pclient)对象。

             调用[isRemoteAppInstalled](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#isremoteappinstalled)方法，查看对端设备是否安装指定应用。
...
```
     ...
     穿戴侧实现发送消息和文件功能后，对端应用需要实现接收消息和文件的功能。
       ...
       import { util } from '@kit.ArkTS';             参见[已连接对端设备查询](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/watch_query_connected_devices)章节，从已连接设备列表中选定需要通信的对端设备。

             构造对端应用参数[P2pAppParam](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#p2pappparam)。

             构造需要发送的消息[P2pMessage](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#p2pmessage)。

             调用[getP2pClient](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#wearenginegetp2pclient)方法，获取[P2pClient](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#p2pclient)对象。

             调用[sendMessage](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#sendmessage)方法，从穿戴侧应用发送简短消息到对端应用。对端应用已注册监听消息接收后，即可收到穿戴侧应用发送的消息。
...
```
       ...
       import { fileIo } from '@kit.CoreFileKit';             参见[已连接对端设备查询](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/watch_query_connected_devices)章节，从已连接设备列表中选定需要通信的对端设备。
             ...
             调用[transferFile](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/wearengine_api#transferfile)方法，从穿戴侧应用发送文件到对端应用。
...
```
             ...
             构造接收到对端传来消息后的回调函数[Callback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-base#callback)。
...
```
             在module.json5中的abilities中配置HiWearMainAbility。更多配置详情参考[abilities标签](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/module-configuration-file#abilities标签)。

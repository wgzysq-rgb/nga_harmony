...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `getting-started-with-arkts-for-swift-programmers`  |  **DocID**: `c7cdfdd4cf6345498cffe8ff3a8097b5`  |  **NodeID**: `0002017757954661235202665032a4b1`
...
| Swift类型  | ArkTS类型  | 示例代码  | 核心差异说明  
 ...
 | Int8  | number  | let count: number = 10;  Swift的Int8为8位整数。

 ArkTS统一用number表示小整数类型。

  
 | Int16  | number  | let count: number = 10;  Swift的Int16为16位整数。

 ArkTS统一用number表示小整数类型。

  
 | Int32  | number  | let count: number = 10;  Swift的Int32为32位整数。

 ArkTS的number是双精度浮点型，可存储整数和浮点数。

  
 | Int64  | number  | let largeNum: number = 9007199254740991;  Swift需处理大整数。

 ArkTS用同一类型表示。

  
 | Float  | number  | let pi: number = 3.14;  Swift需显式指定Float。

 ArkTS直接使用number。

  
 | Double  | number  | let e: number = 2.71828;  Swift区分Float和Double。
...
| Swift类型体系  | ArkTS类型体系  | ArkTS示例代码  | 核心差异说明  
  | **数组**：var arr: [Int] = [1, 2, 3]  | **Array**：let arr: number[] = [1, 2, 3];  // 动态长度语法糖
 ...
 | **集合 - Set**：var mySet: Set<String> = ["a", "b"]  | **Set**：let mySet: Set<string> = new Set(["a", "b"]);  mySet.add('c'); // 向集合内添加元素
 ...
 | **字典 - Dictionary**：var dict: [String: Int] = ["key": 1]  | **Map**：let map: Map<string, number> = new Map();  map.set('key', 1); // 添加键值对
 ...
 | **协议**：protocol Shape { func area() -> Double }  | **interface**：interface Shapes { area(): number; }  class Rectangles implements Shapes {
 ...
 | **类**：class Circle: Shape { /* 类定义 */ }  | **class**：class Circles implements Shape { /* 类定义 */ }  class Circles {
 ...
 | **枚举**：enum Color { case red, green, blue }  | **enum**：enum Colors { Red, Green, Blue }  enum Colors { Red = 1, Green, Blue };

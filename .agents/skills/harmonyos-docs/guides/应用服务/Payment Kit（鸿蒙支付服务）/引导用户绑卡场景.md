...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `payment-partner-bindcard`  |  **DocID**: `b8a7eff886254da4a918c5e294a4d3d4`  |  **NodeID**: `000201775795649086803dabc73b5091`
...
# 引导用户绑卡场景
     ...
     **场景1：用户取消绑卡**
             ...
             Payment Kit客户端展示绑卡页面，商户客户端可根据用户不同处理场景完成后续流程。

             用户返回或取消绑卡。

             Payment Kit客户端返回商户客户端并将绑卡结果一并返回，商户客户端根据绑卡结果完成后续业务处理。
     ...
     **场景2：用户完成绑卡**
             ...
             用户通过绑卡页面完成绑卡操作，Payment Kit客户端会请求Payment Kit服务端处理绑卡。

             Payment Kit服务端处理绑卡。

             Payment Kit服务端将绑卡结果返回给Payment Kit客户端。

             Payment Kit客户端展示绑卡结果页面。

             用户关闭绑卡页后，Payment Kit客户端返回商户客户端并将绑卡结果一并返回。

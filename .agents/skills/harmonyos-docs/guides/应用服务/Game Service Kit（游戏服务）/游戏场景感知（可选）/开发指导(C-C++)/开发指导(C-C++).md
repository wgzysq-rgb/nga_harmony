...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `gameservice-gameperformance-access-procedure-c`  |  **DocID**: `e8df503e5ff34bdc982a9601f58f622b`  |  **NodeID**: `000201775795649086076cb388cc722f`
...
# 开发指导(C/C++)
       ...
       游戏场景感知包括：
           ...
           Game Service Kit通过感知游戏设备的系统状态信息（包括温度变化趋势数据、GPU性能信息和CPU性能信息等），并将其反馈给游戏应用，游戏应用可以基于当前设备状态自行调整游戏设置等内容，在系统资源有限的情况下优化玩家的游戏体验。
             ...
             游戏启动后调用[HMS_GamePerformance_Init](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-game-performance#hms_gameperformance_init)接口对游戏场景感知进行初始化。

             初始化成功后，游戏调用[HMS_GamePerformance_RegisterThermalLevelChangedCallback](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-game-performance#hms_gameperformance_registerthermallevelchangedcallback)接口注册设备状态变化事件监听，订阅设备状态变化通知。

             游戏调用以下接口向游戏场景感知上报各种游戏信息。

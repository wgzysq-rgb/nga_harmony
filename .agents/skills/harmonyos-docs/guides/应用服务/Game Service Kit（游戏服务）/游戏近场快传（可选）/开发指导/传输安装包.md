...
> **分区**: Guides  |  **Slug**: `gameservice-nearbytransfer-installation-package`  |  **DocID**: `a036a537629443f4bfd6b097ce8db17b`  |  **NodeID**: `000201776839809741615e9cc341ba2b`
...
# 传输安装包
       ...
       游戏近场快传支持已安装游戏的玩家通过碰一碰或隔空传送将游戏安装包传输给未安装游戏的玩家，实现游戏传播效率的提升。
             ...
             发送端设备打开游戏后与接收端设备通过[碰一碰](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/knock-share-between-phones-overview)或[隔空传送](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/gestures-share-overview)触发安装包传输。

             发送端调用[create](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-nearbytransfer#gamenearbytransfercreate)创建安装包传输任务。

             创建成功后，游戏客户端调[onRemoteInstallationInfoNotify](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-nearbytransfer#gamenearbytransferonremoteinstallationinfonotify)注册远程安装信息事件监听。

             游戏应用获取到安装游戏所需要的linkingForInstallation地址。

             通过linkingForInstallation地址拉起接收端游戏服务。

             接收端发送游戏安装包是否安装的信息，发送端接收到[onRemoteInstallationInfoNotify](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-nearbytransfer#gamenearbytransferonremoteinstallationinfonotify)远程安装信息事件监听回调。

             根据接收端游戏是否已安装，后续分为两种情况：
          ...
          - 发送端传输游戏安装包。
          - 接收端检查游戏中心是否安装，若未安装将重新自动安装游戏中心。
          - 接收端拉起游戏中心客户端，并打开游戏详情页。
          - 接收端完成安装包的接收，安装并打开游戏。
          - 接收端游戏中心客户端向游戏服务返回安装包安装结果。
          - 接收端游戏服务自动关闭。

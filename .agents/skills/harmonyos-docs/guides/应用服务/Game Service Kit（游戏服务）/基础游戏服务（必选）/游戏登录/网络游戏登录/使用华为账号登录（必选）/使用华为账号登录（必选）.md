...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `gameservice-gameplayer-huawei`  |  **DocID**: `99e29ea865d54352bb51e7f61a857ed0`  |  **NodeID**: `000201775795649086415155c3f17468`
     ...
     由于要求实现HarmonyOS 5.0及以上系统和HarmonyOS 4及以下系统上游戏内资产互通，用户登录新系统游戏时需要找到对应的老系统游戏，以及用户在该游戏下的账号ID，所以要求开发者在上架游戏时配置游戏在新老系统下的APP ID映射关系，并配置游戏在老系统游戏上接入的账号ID类型（playerId/openId）。
            ...
            若无待上架的游戏，请先创建草稿状态的APK游戏。
                    ...
                    在弹出的窗口中继续填写信息，完成后点击“下一步”。
                              ...
                              | 信息项            | 说明           
                                       | 支持数据继承的玩家标识类型                        请选择HarmonyOS 4及以下游戏的玩家标识类型：
       ...
       配置/删除APP ID映射关系的审核结果将通过互动中心或邮件进行通知。
        ...
        游戏调用[createLoginWithHuaweiIDRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#createloginwithhuaweiidrequest)接口，获取用于服务器校验的Authorization Code。                                         Authorization Code只有5分钟有效期，且仅能使用一次，不可复用。

                           - 游戏向开发者服务器上传Authorization Code、gamePlayerId等信息。
        - 开发者服务器携带Authorization Code，请求[获取用户级凭证](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-obtain-user-token)接口，获取Access Token。
        - 开发者服务器携带Access Token，请求[获取玩家标识](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-getplayerinfo)接口，获取gamePlayerId。
        - 开发者服务器把从华为游戏服务器获取的gamePlayerId与客户端获取的gamePlayerId进行一致性核验。若核验不通过，已通过合规校验进入游戏的玩家也需强制退出游戏。
...
```
     目前有如下两个场景可以监听到账号变化：
                        ...
                        | 序号          | 能监听到账号切换的场景          | 监听到账号切换后的措施         
                               | 场景一                    玩家点击“欢迎横幅->管理->切换”或者点击“欢迎横幅->管理->X”。
...
```
     使用华为账号登录的网络游戏，华为账号的实名认证、未成年人防沉迷由基础游戏服务实现，华为账号的支付合规控制（例如未成年人支付限额）由IAP Kit（应用内支付服务）实现。
          ...
          - 已实名认证为未成年人的华为账号在非规定游戏时间内登录游戏，基础游戏服务会弹框提示玩家不允许游戏，强制玩家退出游戏并返回[1002000006](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/gameservice-error-code#section1002000006-玩家未成年并且当前不在可游戏时间)错误码。
                 ...
                 | 未成年人支付限额          | 校验已实名认证为未成年人的华为账号是否限额付费。          | 根据国家新闻出版署的最新规定，网络游戏企业不得为未满8周岁的用户提供游戏付费服务。同一网络游戏企业所提供的游戏付费服务，8周岁以上未满16周岁的用户，单次充值金额不得超过50元人民币，每月充值金额累计不得超过200元人民币；16周岁以上未满18周岁的用户，单次充值金额不得超过100元人民币，每月充值金额累计不得超过400元人民币。                    已实名认证为未成年人的华为账号在游戏内超额付费，IAP Kit会弹窗提示消费金额超出限制。

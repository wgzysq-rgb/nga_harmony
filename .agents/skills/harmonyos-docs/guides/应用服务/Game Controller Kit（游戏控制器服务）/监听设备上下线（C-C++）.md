...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `game-controller-monitor-device`  |  **DocID**: `dc1d11afc38c47f591da5d00e12eea22`  |  **NodeID**: `000201775795649085656c361a40ea85`
...
# 监听设备上下线（C/C++）
     ...
     Game Controller Kit提供设备上下线事件监听和查询在线设备信息的功能。注册监听事件后，设备拔插时可获取实时回调通知，同时支持查询当前所有在线设备的具体信息。
             ...
             游戏调用OH_GameDevice_RegisterDeviceMonitor接口注册设备状态变化事件监听。

             玩家插拔设备。

             终端系统将设备状态变化通知Game Controller Kit。

             Game Controller Kit向游戏反馈设备状态变化。

             游戏调用OH_GameDevice_GetAllDeviceInfos接口向Game Controller Kit查询所有在线的游戏设备信息。

             Game Controller Kit从终端系统获取所有在线的游戏设备信息。

             如果不再需要订阅，游戏可以调用OH_GameDevice_UnregisterDeviceMonitor接口取消设备状态变化事件监听。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `cloudfoundation-storage-upload-file`  |  **DocID**: `38b0f8d1351f45d6bc2f23434d5c261e`  |  **NodeID**: `000201775795649086088de1d474085b`
...
# 上传指定文件至云侧
     ...
     已[初始化存储实例](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/cloudfoundation-storage-initialize-bucket)。
             ...
             将待上传的文件复制到[context.cacheDir](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-application-context#属性)目录下。

                                 由于StorageBucket.uploadFile接口传入参数localPath只能设置为context.cacheDir目录下的文件路径，所以上传前需要先将文件复制到context.cacheDir目录下。

                            调用[StorageBucket.uploadFile](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/cloudfoundation-cloudstorage#uploadfile)接口创建上传任务，监听上传任务的progress、completed、failed等事件。

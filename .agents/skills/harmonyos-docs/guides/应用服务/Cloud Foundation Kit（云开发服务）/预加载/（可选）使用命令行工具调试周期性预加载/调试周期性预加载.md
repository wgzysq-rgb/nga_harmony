...
> **分区**: Guides  |  **Slug**: `cloudfoundation-commandtool-debug`  |  **DocID**: `d6ef5ce536194a8586334f1c1d293e8f`
...
# 调试周期性预加载
 ...
 当前命令行工具支持的命令集如下：
  ...
  命令名 描述   [getcache](#调试命令) 提供获取周期性预加载数据的能力。      #### 调试准备
...
```
 ...
 命令选项 必填(M)/选填(O) 描述 示例   -m M 应用包名。此处的包名需要与您在AppGallery Connect中创建应用时配置的包名保持一致。 cf_prefetch getcache -m  com.huawei.hms.xs.test       #### 调用示例

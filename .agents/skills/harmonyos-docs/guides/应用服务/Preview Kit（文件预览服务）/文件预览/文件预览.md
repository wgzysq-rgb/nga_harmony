...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `preview-filepreview`  |  **DocID**: `f61f2f4d164b4daa9396319038f8ed6c`  |  **NodeID**: `000201775795649085670264df698508`
...
# 文件预览
     ...
     **表1** Preview Kit的接口介绍
                 ...
                 | canPreview(context: Context, uri: string): Promise<boolean>                    根据文件的uri判断文件是否可预览。

          - 当传入支持的文件类型（图片、视频、音频、文本、html）并且文件存在时，会返回true。

          - 当传入不可预览的文件uri时，返回false。

        
                 | hasDisplayed(context: Context): Promise<boolean>                    判断预览窗口是否已经存在。预览窗口是单例的形式。

          - 如果预览窗口已经打开过并且没关闭，那会返回true。

          - 如果没打开或者打开后已关闭，那将返回false。

        
                 | closePreview(context: Context): Promise<void>          | 关闭预览窗口，仅当预览窗口存在时起效。         
                 | loadData(context: Context, file: PreviewInfo): Promise<void>                    加载预览文件信息。仅当预览窗口存在时生效。100毫秒内重复调用无效。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `store-privacy`  |  **DocID**: `1837a00b318f41e883ef08f4d9d7d6e3`  |  **NodeID**: `0002017757956490867837365a6a4d4a`
...
```
             调用[getAppPrivacyMgmtInfo](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/store-privacymanager#privacymanagergetappprivacymgmtinfo)方法查询隐私链接信息。
...
import { hilog } from '@kit.PerformanceAnalysisKit';
```
             调用[getAppPrivacyResult](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/store-privacymanager#privacymanagergetappprivacyresult)方法查询隐私签署状态。
...
import { hilog } from '@kit.PerformanceAnalysisKit';
```
             调用[disableService](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/store-privacymanager#privacymanagerdisableservice)方法撤销同意记录。
...
```
             调用[requestAppPrivacyConsent](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/store-privacymanager#privacymanagerrequestappprivacyconsent)方法拉起标准化隐私弹框请求用户同意。
...
```
     ...
     用户点击同意隐私弹框，应用市场会发送隐私弹框签署结果公共事件。应用可通过监听该事件，感知用户隐私签署结果。
                  ...
                  #### [h2]事件说明
                        ...
                        | 事件名称          | 值          | 描述         
                               | [COMMON_EVENT_PRIVACY_STATE_CHANGED](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/commoneventmanager-definitions#common_event_privacy_state_changed11)          | usual.event.PRIVACY_STATE_CHANGED                    隐私弹框签署结果公共事件，事件携带数据如下：
...
```
     ...
     预置隐私链接信息完成后，打开应用会弹出统一隐私弹框，应用可以使用隐私管理服务提供的查询隐私链接、查询隐私签署状态和撤销同意记录等相关功能。
             ...
             打开代码工程中type为entry类型的模块，修改其中的src/main/module.json5文件，添加module.metadata信息，其中包含四个字段，值均为字符串类型：
                     ...
                     | appgallery_privacy_link_user_agreements                        多个用户协议url（https），在隐私弹框中作为多个用户协议的内容。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `screentimeguard-switch-state-change-callback`  |  **DocID**: `0720651fe7464333b8244d30573f72b8`  |  **NodeID**: `0002017757956490860841c8d890f8d1`
...
# 健康使用设备授权列表页中应用授权开关打开/关闭时触发回调
     ...
     授权开关打开/关闭时的回调关键接口如下表所示：
                 ...
                 TimeGuardExtensionAbility与应用直接无法直接传递数据，如需传递数据可以通过[用户首选项](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-data-preferences)/[数据库](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-data-relationalstore)等数据持久化手段进行传递，或者通过[公共事件模块](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-commoneventmanager)传递。
...
```
             继承TimeGuardExtensionAbility，重写onUserAuthSwitchOn和onUserAuthSwitchOff 回调。

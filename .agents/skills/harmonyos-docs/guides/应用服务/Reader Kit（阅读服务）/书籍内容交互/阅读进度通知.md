...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `reader-progress`  |  **DocID**: `9dfeebcfe7884befab6ef53427ba1fe0`  |  **NodeID**: `00020177579564908694666a3b351f81`
...
# 阅读进度通知
     ...
     阅读进度通知涉及2个接口，具体介绍如下表所示。
                        ...
                        | 接口名          | 描述         
                               | [on('pageShow')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/reader-read-core#onpageshow)          | 注册章节内容分页展示结果回调。         
                 | [off('pageShow')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/reader-read-core#offpageshow)          | 注销章节内容分页展示结果回调，可在页面销毁时调用。         
     ...
     在进行阅读进度通知监听之前，请先确保已经“[构建阅读器](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/reader-read-page)”。
       ...
       开发者可在on('pageShow')接口回调中将阅读进度实时保存到数据库当中，防止用户异常退出阅读器时的进度丢失。当用户下次继续阅读时，可将保存domPos及resourceIndex属性传入到[startPlay](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/reader-read-core#startplay)接口中，用于阅读进度的恢复。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `account-faq-12`  |  **DocID**: `62d33e3b18d243a89ee1697331b00c75`  |  **NodeID**: `00020177579564908621319e7a148ff8`
...
# ID Token的使用场景与使用方法
    ...
    ID Token 是 JWT Token格式，意味着：
           ...
           用户的身份信息直接被编码进了ID Token，不需要额外请求其他的资源来获取用户信息。

           ID Token 可以验证其是华为账号服务颁发的，携带华为账号签名信息，验证签名可证明其没有被篡改过。

    
              #### 使用场景

     
             应用无服务器，只有客户端，该场景下无法使用Authorization Code完成服务器侧的接口调用获取用户信息，需从ID Token中解析出用户信息；

             应用有服务器，希望在服务器侧解析ID Token对应字段，获取用户信息。
     ...
     ID Token是JWT Token格式数据，其中payload包含字段如下：
                 ...
                 | display_name          | string          | 否                    华为账号对应的昵称，没有昵称则取匿名化的邮箱或手机号。该字段返回场景：

          [AuthorizationWithHuaweiIDRequest](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/account-api-authentication#authorizationwithhuaweiidrequest)中的scopes包含profile。

        
                 | nickname          | string          | 否                    华为账号对应的昵称。该字段返回场景：

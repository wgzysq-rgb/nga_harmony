...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `subscribe-process-userinfo-change`  |  **DocID**: `e1fed65b0fa640548c4f3187c181efeb`  |  **NodeID**: `0002017757956490867696054fd8b03b`
...
登录[华为开发者联盟](https://developer.huawei.com/consumer/cn/)，选择“管理中心 > API服务 > API库”。

  在App Services找到RISC。

 

  点击启用按钮（未选择项目时，需先选择项目，再点击启用按钮）。
  ...
  订阅范围：订阅的用户信息变更事件，详见[用户信息变更事件介绍](#用户信息变更事件介绍)。
...
- 校验JWT签名中的aud与[订阅用户信息变更](#订阅用户信息变更)中提供的Client ID一致。
...
| 参数  | 描述  
  | aud  | Client ID（与[订阅用户信息变更](#订阅用户信息变更)中提供的Client ID一致）。  
 ...
 | subject  用户信息变更事件对应的消息体，格式为json，包含字段说明如下：

 - sub：触发事件用户的UnionID（用户在同一个开发者下的所有应用中，此值唯一）。具体格式要求请参考[OpenID和UnionID的格式说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-faq-9)。

 - subject_type：固定为“iss_sub”。

 - extra：触发事件用户的OpenID（用户在同一个应用中，此值唯一）。具体格式要求请参考[OpenID和UnionID的格式说明](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/account-faq-9)。

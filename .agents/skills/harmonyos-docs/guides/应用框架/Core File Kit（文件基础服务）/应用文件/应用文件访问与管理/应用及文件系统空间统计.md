...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `app-fs-space-statistics`  |  **DocID**: `a6ebcec886d8465096779d09bc55e2ab`  |  **NodeID**: `000201775795788884496a8a936bc88e`
...
# 应用及文件系统空间统计
     ...
     **表1** 文件系统空间和应用空间统计
        ...
        - nsenter -t {pid} -m sh。
          ...
          **说明**：${el1-el5}指的是[el1，el2，el3，el4，el5目录](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-sandbox-directory#应用文件目录与应用文件路径)。${moduleName}是模块名。

        
                 | dataSize          | 应用文件存储大小（除应用安装文件）（单位为Byte）                    应用文件通常由本地文件、分布式文件、数据库文件等部分组成。
          ...
          **说明**：${el1-el5}指的是[el1，el2，el3，el4，el5目录](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/app-sandbox-directory#应用文件目录与应用文件路径)。

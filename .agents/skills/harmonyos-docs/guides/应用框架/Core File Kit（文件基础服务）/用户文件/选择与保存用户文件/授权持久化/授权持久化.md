...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `file-persistpermission`  |  **DocID**: `6c74494d68294817b6ed66f56eca2e82`  |  **NodeID**: `0002017757957888845198decf988502`
...
```
        ...
        - 持久化授权文件信息建议应用在本地存储数据，供后续按需激活持久化文件。
        - 持久化授权的数据存储在系统的数据库中，应用或者设备重启后需要激活已持久化的授权才可以正常使用[激活持久化授权](#激活已经持久化的权限访问文件或目录)。
        - 持久化权限接口(可以使用canIUse接口进行校验能力是否可用)，且需要申请对应的权限。
        - 应用在卸载时会将之前的授权数据全部清除，重新安装后需要重新授权。
        - 只能对已获取到的临时权限进行持久化授权操作，否则会报错。
...
```
        ...
        - 示例中的URI来源自应用存储的持久化数据中。
        - 只能对已获取到的持久化权限进行取消持久化授权操作，建议按照使用需求去取消对应的持久化权限。
        - 持久化权限接口(可以使用canIUse接口进行校验能力是否可用)，且需要申请对应的权限。
...
```
        ...
        - 示例中的URI来源自应用存储的持久化数据中。
        - 建议按照使用需求去激活对应的持久化权限，不要盲目的全量激活。
        - 如果激活失败显示未持久化的权限可以按照示例进行持久化。
        - 持久化权限接口可以使用canIUse接口进行校验能力是否可用，且需要申请对应的权限。

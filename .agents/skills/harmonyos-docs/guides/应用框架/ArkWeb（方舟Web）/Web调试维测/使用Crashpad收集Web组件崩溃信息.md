...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `web-crashpad`  |  **DocID**: `66655d62c146447d8f4b9b958552610f`  |  **NodeID**: `000201775795788884160819694ec25e`
...
# 使用Crashpad收集Web组件崩溃信息
    ...
    使用步骤如下：
     ...
     - 在应用使用Web组件导致的进程崩溃出现后，Crashpad收到信号，对应Hilog日志（节选部分）如下
...
```
     ...
     - 参考[Native访问应用沙箱](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-file-native-side)实现访问应用沙箱dmp文件；也可将存放dmp文件的沙箱路径的文件复制到可以查看的路径。示例如下
...
```
    ...
    /data/app/el2/100/base/com.example.myapplication/haps/entry/files/    这个路径可以利用DevEco Studio查看。
        ...
        ./minidump_stackwalk b678e0b5-894b-4794-9ab3-fb5d6dda06a3.dmp > parsed_stacktrace.txt        minidump_stackwalk由Breakpad项目源码编译得到，编译方法见项目仓库：[Breakpad仓库地址](https://chromium.googlesource.com/breakpad/breakpad)。
...
```
        ...
        ./llvm-addr2line -Cfpie libweb_engine.so 0x2e0b340        llvm-addr2line工具链位于SDK中。

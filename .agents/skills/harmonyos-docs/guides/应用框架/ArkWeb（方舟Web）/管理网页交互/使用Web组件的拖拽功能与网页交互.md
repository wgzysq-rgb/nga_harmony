...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `web-drag`  |  **DocID**: `9cfd3201a9fc4f4d90ef79003286a0ce`  |  **NodeID**: `0002017757957888843324f5c833bb93`
...
# 使用Web组件的拖拽功能与网页交互
     ...
     在多数情况下，应用在H5端实现的拖拽功能能够满足需求。如有需要，请参考以下案例，实现在ArkTS端进行拖拽数据读取等操作。
      ...
      - 在onDrop方法中，做简单逻辑，例如暂存一些关键数据。
      - 在ArkTS侧接收消息的方法中，添加应用处理逻辑，可以进行耗时任务。
...
```
     ...
     禁用拖拽方式主要分为两类：
      ...
      - 网页侧通过W3C CSS、JS进行拦截/禁用。
      - 应用侧通过Web组件runJavaScriptExt接口注入JS进行拦截/禁用。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `customize-bytecode-during-compilation`  |  **DocID**: `66405da36a5448c5a33ea12e7d43517f`  |  **NodeID**: `00020177579578888458006dcc495503`
...
```
  使用C语言编译工具（这里使用g++）编译动态库。

  Windows平台：

  g++ --shared -o example.dll example.cpp  Linux平台：

  g++ --shared -o example.so example.cpp  Mac平台：

  g++ --shared -o example.so example.cpp  在DevEco Studio中配置build-profile.json5的transformLib选项（以Windows环境为例）。

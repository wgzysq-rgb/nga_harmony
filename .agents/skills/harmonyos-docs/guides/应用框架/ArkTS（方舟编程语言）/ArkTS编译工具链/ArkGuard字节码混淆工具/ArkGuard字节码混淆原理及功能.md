...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `bytecode-obfuscation`  |  **DocID**: `1f301bd2efa94663b0d919226a0c274d`  |  **NodeID**: `000201775795788884465c00823781ed`
...
# ArkGuard字节码混淆原理及功能
     ...
     ArkGuard提供的是**面向方舟字节码的基础名称混淆**：在可选规则下，对**类、接口、枚举、函数、类成员方法、变量、对象属性名、源文件名**等标识符进行重命名，并可通过保留选项配置**白名单**；各选项的覆盖范围与例外说明见下文[混淆选项](#混淆选项)。函数参数名不参与字节码名称混淆。

     **不支持**以下高级混淆能力：**控制流混淆**（如控制流平坦化、虚假分支插入等）、**数据混淆**（如对常量、字面量或内存数据布局做加密与变换）、**指令替换或插入无关指令**、**虚拟机保护/加壳**等不以标识符重命名为主要手段的保护方式。
...
```
     ...
     **2.安全保证的有限性**
     ...
     在使用混淆功能前，建议开发者先通过文档了解[混淆选项的能力](#混淆选项)与[混淆选项所需要保留白名单的场景](#保留选项)，再根据开发需求选择对应的混淆功能。
                 ...
                 | 属性名称混淆          | [-enable-property-obfuscation](#section-enable-property-obfuscation)         
                 | 字符串属性名称混淆          | [-enable-string-property-obfuscation](#section-enable-string-property-obfuscation)         
                 | 顶层作用域名称混淆          | [-enable-toplevel-obfuscation](#section-enable-toplevel-obfuscation)         
                 | 导入导出名称混淆          | [-enable-export-obfuscation](#section-enable-export-obfuscation)         
                 | 文件名混淆          | [-enable-filename-obfuscation](#section-enable-filename-obfuscation)         
                 | 代码压缩          | [-compact](#section-compact)         
                 | console打印删除          | [-remove-log](#section-remove-log)         
                 ...
                 | 开启字节码混淆          | [-enable-bytecode-obfuscation](#section-enable-bytecode-obfuscation)         
                 | 开启字节码混淆调试能力          | [-enable-bytecode-obfuscation-debugging](#section-enable-bytecode-obfuscation-debugging)         
...
```
             被[保留选项指定的属性名](#section-keep-property-name)不会被混淆。
...
```
     ...
     **1.** 如果字符串属性名包含特殊字符（除了a-z、A-Z、0-9、_之外的字符），例如let obj = {"\n": 123, "": 4, " ": 5}，建议不要开启-enable-string-property-obfuscation选项，因为可能无法通过[保留选项](#section-keep-property-name)来指定保留这些名字。
...
```
     因此在开启了-enable-string-property-obfuscation选项时，如果想保留代码中使用的SDK API字符串常量的属性不被混淆，例如obj['ohos.want.action.home']，那么需要使用[-keep-property-name选项](#section-keep-property-name)保留。
...
```
     ...
     混淆规则合并逻辑参考[混淆规则合并策略](#混淆规则合并策略)。
                  ...
                  #### [h2]-enable-bytecode-obfuscation

     字节码混淆控制开关。默认不开启。

                  #### [h2]-enable-bytecode-obfuscation-debugging
     ...
     指定想保留的属性名，支持使用[名称类通配符](#名称类通配符)。按如下方式进行配置，表示保留名称为age、firstName和lastName的属性：
...
```
     ...
     filepath仅支持相对路径，./与../为相对于混淆配置文件所在目录，支持使用[路径类通配符](#路径类通配符)。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `source-obfuscation`  |  **DocID**: `9572a0d7ffdd4fd488ac7356a1ca4321`  |  **NodeID**: `0002017757957888844380b06e5e17b5`
...
```
     ...
     **2.安全保证的有限性**
     ...
     开发者可以在模块的build-profile.json5配置文件中开启混淆功能，详细参考[ArkGuard混淆开启指南](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/source-obfuscation-guide)，从而在编译打包的过程中自动对源码进行混淆处理。

     混淆过程中，首先读取混淆开关。在开关开启的情况下，解析混淆配置文件，并依据[混淆规则合并策略](#混淆规则合并策略)合并混淆规则。然后按照混淆规则对经过语法转换的中间文件进行混淆，最后将混淆后的中��文件落盘至build目录。开发者可以通过build目录中混淆后的产物，确认混淆效果。

     在使用混淆功能前，建议开发者先通过文档了解[混淆选项的能力](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/source-obfuscation#混淆选项)与[混淆选项所需要保留白名单的场景](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/source-obfuscation#保留选项)，再根据开发需求选择对应的混淆功能。
                  ...
                  #### [h2]已有混淆选项汇总
                        ...
                        | 功能          | 选项         
                               | 默认混淆          | 开启混淆后生效         
                 | 关闭混淆          | [-disable-obfuscation](#section-disable-obfuscation)         
                 | 开启属性名称混淆          | [-enable-property-obfuscation](#section-enable-property-obfuscation)         
                 | 开启字符串属性名称混淆          | [-enable-string-property-obfuscation](#section-enable-string-property-obfuscation)         
                 | 开启顶层作用域名称混淆          | [-enable-toplevel-obfuscation](#section-enable-toplevel-obfuscation)         
                 | 开启导入导出名称混淆          | [-enable-export-obfuscation](#section-enable-export-obfuscation)         
                 | 开启文件名混淆          | [-enable-filename-obfuscation](#section-enable-filename-obfuscation)         
                 | 代码压缩          | [-compact](#section-compact)         
                 | 声明文件注释删除          | [-remove-comments](#section-remove-comments)         
                 | 删除console.*语句          | [-remove-log](#section-remove-log)         
                 | 名称缓存输出          | [-print-namecache](#section-print-namecache)         
                 | 名称缓存复用          | [-apply-namecache](#section-apply-namecache)         
                 | 输出未混淆名单          | [-print-kept-names](#section-print-kept-names)         
                 | 缩减语言预置白名单          | [-extra-options strip-language-default](#section-extra-options-strip-language-default)         
                 | 缩减系统预置白名单          | [-extra-options strip-system-api-args](#section-extra-options-strip-system-api-args)         
                 | 不保留未参与编译模块名称          | [-extra-options strip-not-compiled-module-name](#section-extra-options-strip-not-compiled-module-name)         
                 | 保留声明文件参数          | [-keep-parameter-names](#section-keep-parameter-names)         
                 | 合并依赖模块选项          | [-enable-lib-obfuscation-options](#section-enable-lib-obfuscation-options)         
                 | 通过注释在源码中标记白名单          | [-use-keep-in-source](#section-use-keep-in-source)         
                 | 保留对象字面量属性名称          | [-keep-object-props](#section-keep-object-props)         
                 | 删除指定的方法调用语句          | [-remove-nosideeffects-calls](#section-remove-nosideeffects-calls)         
     ...
     开启混淆后默认生效，仅混淆局部变量名及参数名。
                  ...
                  #### [h2]-disable-obfuscation

     关闭所有混淆。

     配置该选项后，默认混淆（仅混淆局部变量及参数名）和其他已配置的混淆选项、保留选项将全部失效。

                  #### [h2]-enable-property-obfuscation

                         开启该选项后，在需要[手动配置白名单的场景中](#section-keep-property-name)，请将对应的属性名配置到白名单中。
...
```
             被[保留选项](#section-keep-property-name)指定的属性名不会被混淆。
...
```
                  ...
                  #### [h2]-enable-string-property-obfuscation
...
```
     **使用该选项时，需要注意以下事项：**
             ...
             如果代码里面有字符串属性名包含特殊字符（除了a-z、A-Z、0-9、_之外的字符），例如let obj = {"\n": 123, "": 4, " ": 5}，建议不要开启-enable-string-property-obfuscation选项，因为可能无法通过[-keep-property-name](#section-keep-property-name)来保留这些名称。
...
```
       因此，在开启-enable-string-property-obfuscation选项后，如果希望保留代码中使用的SDK API字符串常量的属性不被混淆，例如obj['ohos.want.action.home']，可以使用[-keep-property-name](#section-keep-property-name)选项进行保留。

     
                  #### [h2]-enable-toplevel-obfuscation

                         开启该选项后，在需要[手动配置白名单的场景中](#section-keep-global-name)，请将对应的顶层作用域名称配置到白名单中。
...
```
     配置该选项后，所有顶层作用域的名称都会被混淆，以下场景除外：
      ...
      - 当前文件找不到声明的名称不会被混淆。
      - 被[保留选项](#section-keep-global-name)指定的顶层作用域名称不会被混淆。
      - SDK API列表中的顶层作用域名称不会被混淆。
     
                  #### [h2]-enable-export-obfuscation
...
```
     若仅配置该选项，那么只有非顶层作用域中导入或导出的名称会被混淆。**若想混淆顶层作用域中导入或导出的名称，需要在已配置-enable-toplevel-obfuscation的基础上使用；若想混淆导入或导出的属性名，需要在已配置-enable-property-obfuscation的基础上使用。** 开启此选项时，以下特殊场景不会被混淆：
      ...
      - SDK API列表中的名称不会被混淆。
                  ...
                  #### [h2]-enable-filename-obfuscation

                         开启该选��后，在需要[手动配置白名单的场景中](#section-keep-file-name)，请将对应的文件夹/文件名称配置到白名单中。
...
```
     配置该选项后，所有文件和文件夹名称都将被混淆，以下场景除外：
      ...
      - 被[-keep-file-name](#section-keep-file-name)指定的文件/文件夹名称不会被混淆。
      - 非ECMAScript模块引用方式（例如：const module = require('./module')）。
      - 非路径引用方式，例如import module from 'json5'中的json5不会被混淆。
     
                         由于系统会在应用运行时加载某些指定的文件，针对这类文件，开发者需要手动在[-keep-file-name](#section-keep-file-name)选项中配置相应的白名单，防止指定文件被混淆，导致运行失败。
...
```
     ...
     declare let count: number;     使用[-keep-comments](#section-keep-comments)配置保留声明文件中的JsDoc注释。
...
```
     若配置该选项，以下场景中的console.*语句将被删除。
       ...
       console.info("in tolevel");             代码块中的调用。
...
```
       其中，编译HAP/HSP模块时，enum白名单内容为['outdoor', 'member1']；编译字节码HAR模块时，enum白名单内容为['outdoor', 'member1', 'member2']。
                  ...
                  #### [h2]-extra-options strip-language-default
     ...
     开启-print-kept-names选项，对比开启和关闭-extra-options strip-language-default选项时，全量白名单（whitelist.json）中lang字段的内容差异，该差异即为预置语言白名单的具体减少范围。
                  ...
                  #### [h2]-extra-options strip-system-api-args
     ...
     通过对比开启和关闭-extra-options strip-system-api-args选项时系统API白名单文件（systemApiCache.json）中ReservedLocalNames和ReservedPropertyNames字段的内容差异，该差异即为系统白名单的具体减少范围，ReservedGlobalNames字段的内容不会产生变化。
                  ...
                  #### [h2]-extra-options strip-not-compiled-module-name
...
```
     ...
     **使用该选项时，需要注意以下事项：**
             ...
             源码文件中的参数名称仍然会被混淆，无法通过此选项保留。
                  ...
                  #### [h2]-enable-lib-obfuscation-options
...
```
     ...
     从API version 23开始，支持使用-keep-object-props配置选项保留对象字面量中的属性名称和字符串属性名称。使用方法如下：
        ...
        - 当在开启属性混淆选项或者同时开启属性混淆和字符串属性混淆前提下，使用-keep-object-props选项才生效，否则不生效。
...
```
     ...
     排查场景和配置字段时，推荐使用[混淆助手配置保留选项](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-build-obfuscation#section19439175917123)，快速识别需要配置的保留选项和白名单字段。
                  ...
                  #### [h2]已有保留选项汇总
                        ...
                        | 功能          | 选项         
                               | 指定保留属性名称          | [-keep-property-name](#section-keep-property-name)         
                 | 指定保留顶层作用域或导入导出元素名称          | [-keep-global-name](#section-keep-global-name)         
                 | 指定保留文件/文件夹名称          | [-keep-file-name](#section-keep-file-name)         
                 | 指定保留注释          | [-keep-comments](#section-keep-comments)         
                 | 指定保留声明文件中的所有名称          | [-keep-dts](#section-keep-dts)         
                 | 指定保留源码文件中的所有名称          | [-keep](#section-keep)         
             

                       #### [h2]-keep-property-name

     指定想保留的属性名，支持使用[名称类通配符](#保留选项支持的通配符)。按如下方式进行配置，表示保留名称为firstName和lastName的属性：
...
```
     **使用该选项时，需要注意以下事项：**
             ...
             该选项在开启-enable-toplevel-obfuscation或-enable-export-obfuscation时生效。
...
```
     **使用该选项时，需要注意以下事项：**
             ...
             该选项在开启-enable-filename-obfuscation时生效。
...
```
     3.对于API version 19及之前版本，使用[跨包路由](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-navigation-cross-package)进行路由跳转时，传递给动态路由的路径应被保留。动态路由提供系统路由表和自定义路由表两种方式：
...
```
     ...
     filepath仅支持相对路径，./和../为相对于混淆配置文件所在目录，支持使用[路径类通配符](#保留选项支持的通配符)。
...
```
     ...
     **混淆规则合并逻辑**
     ...
     保留选项：合并时，对于白名单选项，其内容取并集。
             ...
             **如果当前编译模块混淆配置未包含-enable-lib-obfuscation-options选项**：合并对象为当前模块的所有混淆规则与依赖模块混淆规则中的[保留选项](#保留选项)。
...
```
     ...
     **HSP和HAR中混淆注意事项**
             ...
             如果consumerFiles指定的混淆配置文件中包含上述混淆选项，当其他模块依赖该模块时，这些选项会与主模块的混淆规则合并，从而影响主模块。因此不建议开发者在consumer-rules.txt文件中配置混淆选项，建议仅配置保留选项。

             如果在consumerFiles指定的混淆配置文件中添加-keep-dts选项，该选项会被转换成-keep-global-name和-keep-property-name。
                 ...
                 | -enable-filename-obfuscation                    HAR包文件/文件夹名称混淆
          ...
          12
                 ...
                 | -keep-file-name                    保留HAR包的文件/文件夹的名称

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `xml-generation`  |  **DocID**: `5aaa9cb98a284edebd771566525a9ce5`  |  **NodeID**: `000201775795788884445eafdfec2974`
...
# XML生成
    ...
    XML还可以作为消息传递格式，用于分布式系统中不同节点的通信。
     ...
     使用XmlSerializer生成XML示例如下：
       ...
       import { xml, util } from '@kit.ArkTS';             创建缓冲区，构造XmlSerializer对象。可以基于ArrayBuffer构造XmlSerializer对象，也可以基于DataView构造XmlSerializer对象。
...
```
     ...
     使用XmlDynamicSerializer生成XML示例如下：
       ...
       import { xml, util } from '@kit.ArkTS';             调用XML元素生成函数。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `sendable-constraints`  |  **DocID**: `392861f3ad89465eadfc7d833573eccb`  |  **NodeID**: `000201775795788884908a445eddceda`
...
```
                  ...
                  #### [h2]Sendable类的内部不允许使用当前模块内上下文环境中定义的变量
...
type D = C; // 编译报错
```
                  #### [h2]Sendable类和Sendable函数禁止使用除@Sendable外的装饰器
...
```
                 ...
                 @Sendable和其他自定义装饰器叠加使用可能造成运行时异常，需要开发者适配装饰器函数的实现。
                  ...
                  #### [h2]禁止使用对象字面量/数组字面量初始化Sendable对象

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ndk-listen-to-component-events`  |  **DocID**: `b91d45df6a2a43a5a3c4af6ceda64af0`  |  **NodeID**: `000201775795788884367bda6f0302af`
...
# 监听组件事件
      ...
      ArkUI_NativeNodeAPI_1 *nodeAPI = nullptr;      调用[OH_ArkUI_GetModuleInterface](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-native-interface-h#oh_arkui_getmoduleinterface)接口给定义的指针赋值：

      OH_ArkUI_GetModuleInterface(ARKUI_NATIVE_NODE, ArkUI_NativeNodeAPI_1, nodeAPI);      定义事件触发回调函数：
...
```
      ...
      nodeAPI->unregisterNodeEvent(button, NODE_ON_CLICK_EVENT);      卸载事件处理函数：

      nodeAPI->removeNodeEventReceiver(button, NodeEventReceiver);           全局事件监听
...
```
      ...
      nodeAPI->unregisterNodeEventReceiver();           获取事件信息
...
```
      ...
      nodeAPI->unregisterNodeEventReceiver();      解注册对应的事件类型：

      nodeAPI->unregisterNodeEvent(button, NODE_ON_CLICK_EVENT);           深浅色变更事件

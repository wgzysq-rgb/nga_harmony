...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `arkts-common-events-focus-event`  |  **DocID**: `75b8326f927a414f80d2c7432ea02c6a`  |  **NodeID**: `0002017757957888846004c6bef7bdf8`
...
```
     ...
     示例操作步骤：
      ...
      - 点击Set Active按钮，调用[activate](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-focuscontroller#activate14)接口进入焦点激活态。
      - Tab键走焦至Set Not Active按钮，Enter键触发按键事件，调用[activate](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-uicontext-focuscontroller#activate14)接口退出焦点激活态。
...
```
     ...
     **主动走焦**
     ...
     指开发者/用户���观行为导致的焦点移动，包括：使用外接键盘的按键走焦（Tab键/Shift+Tab键/方向键）、使用requestFocus申请焦点、clearFocus清除焦点、focusOnTouch点击申请焦点等接口导致的焦点转移。
...
```
     ...
     **自定义走焦算法**
     ...
     onFocus(event: () => void)     获焦事件回调，绑定该接口的组件获焦时，回调响应。

     onBlur(event:() => void)     失焦事件回调，绑定该接口的组件失焦时，回调响应。
...
```
     ...
     focusable(value: boolean)     设置组件是否可获焦。
     ...
     enabled(value: boolean)     设置组件可交互性属性[enabled](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-enable#enabled)为false，则组件不可交互，无法获焦。

     visibility(value: Visibility)     设置组件可见性属性[visibility](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-visibility#visibility)为Visibility.None或Visibility.Hidden，则组件不可见，无法获焦。
...
```
     ...
     tabStop(isTabStop: boolean)     设置当前容器组件的[tabStop](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/ts-universal-attributes-focus#tabstop14)属性，可决定在走焦时焦点是否会停留在当前容器。
...
```
     ...
     defaultFocus(value: boolean)     设置当前组件是否为当前[层级页面](#基础概念)上的默认焦点。
     ...
     **整体获焦的焦点链形成**
     ...
     3.获焦链上存在配置了获焦优先级的组件和容器：
             ...
             容器内不存在优先级大于PREVIOUS的组件，由上次获焦的节点获焦。例如，窗口失焦后重新获焦。
                ...
                focusBox(style: FocusBoxStyle)     设置当前组件系统焦点框样式。
...
```
     ...
     上述示例包含以下2步：
      ...
      - 按下Tab键，走焦到第二个Button，焦点框样式为远离边缘的红色粗框。
       ...
       requestFocus(value: string): boolean       调用此接口可以主动让焦点转移至参数指定的组件上，焦点转移生效时间为下一个帧信号。
...
```
     ...
     nextFocus(nextStep: Optional<FocusMovement>): T     若存在配置了nextFocus的组件，则走焦只会按照设置的nextFocus走焦顺序走焦，没有设置自定义走焦或者设置自定义走焦的组件或容器不存在时，仍进行默认走焦规则。
...
```
     ...
     tabIndex(index: number)     tabIndex自定义组件Tab键走焦顺序。
...
```
     ...
     focusScopePriority(scopeId: string, priority?: FocusPriority)     设置当前组件在指定容器内获焦的优先级。需要配合focusScopeId一起使用。

     focusScopeId(id: string, isGroup?: boolean)     设置当前容器组件的id标识，设置当前容器组件是否为焦点组。焦点组与tabIndex不能混用。

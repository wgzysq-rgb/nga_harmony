...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `arkts-new-binding`  |  **DocID**: `62b94c87dc134d689e95cf3f0842eaa0`  |  **NodeID**: `000201775795788884723360de7780c7`
    ...
    在状态管理V2中，推荐使用!!语法糖统一处理双向绑定。
     ...
     !!双向绑定语法，是一个语法糖方便开发者实现数据双向绑定，用于初始化子组件的[@Param](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-new-param)和[@Event](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-new-event)。其中@Event方法名需要声明为“$”+ @Param属性名，详见[使用场景](#使用场景)。
      ...
      - 父组件未使用!!时，变化是单向的。
       ...
       Child({ value: this.value, $value: (val: number) => { this.value = val; } })       上述语法可以简化为!!双向绑定语法糖。

       Star({ value: this.value!! })             使用@Param value与@Event $value语法实现自定义组件双向绑定。

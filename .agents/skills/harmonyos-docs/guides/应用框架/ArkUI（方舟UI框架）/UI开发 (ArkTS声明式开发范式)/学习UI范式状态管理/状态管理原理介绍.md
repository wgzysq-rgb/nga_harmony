...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `arkts-state-management-introduce`  |  **DocID**: `e018c71eeddb4f66b7ec3f71c47f9185`  |  **NodeID**: `000201775795788884524838c4c806df`
...
- 自定义组件Index被创建并首次调用build方法创建组件。
- 当框架执行到Text(`${this.name}'s age is ${this.age}`)时，为了显示文本内容，需要读取this.name和this.age的值。
- name和age都是被@State装饰器装饰的状态变量，状态变量在被读取时会收集当前正在渲染的系统组件的唯一标识elementId，并将其存储到一个Set集合中。因此，状态变量name和age均收集到Text1的唯一标识elementId1。
- 同理，当框架执行到Text(`${this.name}'s grade is ${this.grade}`)时，状态变量name和grade收集到了Text2的唯一标识elementId2。
...
- 点击Button1，触发 onClick 事件。
- 在事件处理函数中执行this.age++。由于age是状态变量，在改值的过程中会执行状态管理内部的更新操作。
- 由于在一个UI更新周期中，自定义组件中可能存在多个状态变量发生改变，而更新是以自定义组件为单位的，所以每个自定义组件中维护了一个标脏的系统组件集合（下称脏系统组件集合），用于保存在当前UI更新周期中标脏的系统组件的elementId。在状态变量age的更新操作中，将其依赖集合中系统组件的elementId加入到其所属的自定义组件Index的脏系统组件集合中。
- 完成系统组件标脏后，将状态变量age所属的自定义组件Index标脏，加入到标脏的自定义组件节点列表（下称脏自定义组件列表）中，并请求一个刷新信号。
- 在下一个UI更新周期中，框架遍历脏自定义组件列表，重新调用它们的rerender方法（rerender方法是由系统生成的），执行Index自定义组件的rerender方法时，遍历脏系统组件，刷新Text1组件并更新依赖。
- 同理，点击Button2修改状态变量grade的值，对应刷新Text2组件并更新依赖。
...
- 事件触发状态变量发生改变，执行状态变量的set方法，将自定义组件和系统组件标脏，并请求一个刷新信号。
- 刷新脏节点：刷新标脏的自定义组件和系统组件。
- 布局：根据标脏局部刷新组件树，触发子树上节点的尺寸测量和位置确认。
...
- 点击Button1按钮，产生手势事件的trace点。
- 手势事件触发onClick回调。
- 在onClick回调中改变状态变量age的值。
- 下一帧信号到来，执行VSync回调。
- 刷新脏自定义组件Index。
- 遍历自定义组件中的脏系统组件，重新渲染Text1组件。
- 执行后续布局流程。

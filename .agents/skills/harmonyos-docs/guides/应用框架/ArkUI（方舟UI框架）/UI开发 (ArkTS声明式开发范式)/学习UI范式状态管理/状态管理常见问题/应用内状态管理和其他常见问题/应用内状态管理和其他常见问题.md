...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `arkts-state-management-faq-application-and-others`  |  **DocID**: `4877ac598a174191a43203994631ff14`  |  **NodeID**: `000201775795788884743df3269bdcdf`
  ...
  build() {
    Button('New Worker')
      .onClick(() => {
        ...
        // 注册onAllErrors回调，捕获Worker线程的onmessage回调、timer回调以及文件执行等流程产生的全局异常。该���调在宿主线程执行。
        newWorker.onAllErrors = (err: ErrorEvent) => {
          console.error('workerInstance onAllErrors message is: ' + err.message);
        };

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `arkts-shared-element-transition`  |  **DocID**: `6e3f62bfb9bd4ee292ca957880847546`  |  **NodeID**: `0002017757957888848012f36c868fcc`
...
```
     ...
     以展开收起缩略图的场景为例，实现步骤为：
             ...
             由全屏状态返回到缩略图时，触发由PageTwo至PageOne的自定义转场动画，即PageTwo的共享元素组件从全屏状态做动画到原PageOne的缩略图��态，转场结束后共享元素组件从PageTwo被迁移回PageOne。
...
```
     ...
     以点击图片展开半模态页的场景为例，实现步骤为：
             ...
             还可以���合一镜到底动画，给初始界面图片也增加一个从透明到出现的动画，使得动效更为流畅。

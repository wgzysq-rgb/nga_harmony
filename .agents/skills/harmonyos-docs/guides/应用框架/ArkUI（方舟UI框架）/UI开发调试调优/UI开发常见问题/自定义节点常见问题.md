...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `arkts-user-defined-node-faq`  |  **DocID**: `2f44bfdcf7be403789db23b0e54679ab`  |  **NodeID**: `0002017757957888843943036935059f`
...
# 自定义节点常见问题
     ...
     **解决措施**
      ...
      - 对于使用OH_ArkUI_GetNodeHandleFromNapiValue获取BuilderNode或ComponentContent对象的root节点，调用[disposeNode](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-arkui-nativemodule-arkui-nativenodeapi-1#disposenode)减少OH_ArkUI_GetNodeHandleFromNapiValue增加的引用计数。
...
```
     ...
     **问题现象**
     ...
     使用[BuilderNode](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-user-defined-arktsnode-buildernode)创建自定义组件节点时，有��能出现前后端（ArkTS UI层与Native UI引擎层）之间的循环引用，使得自定义节点无法被销毁，进而引发内存泄漏。

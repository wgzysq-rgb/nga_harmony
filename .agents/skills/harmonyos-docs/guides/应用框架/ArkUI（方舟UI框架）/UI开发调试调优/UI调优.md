...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ui-inspector-profiler`  |  **DocID**: `9217ebd522c640258f1b547dc488515c`  |  **NodeID**: `000201775795788884269287c88f0fdd`
...
```
     ...
     hdc shell hidumper -s WindowManagerService -a '-a'     步骤2：执行打印状态变量dump信息的命令。
     ...
     假定激活的窗口id是90，可通过下面的命令dump出自定义组件树和根节点的状态变量的信息。
       ...
       hdc shell hidumper -s WindowManagerService -a '-w 90 -jsdump -dumpAll -r'       执行上述命令后，dump信息代表的含义如下：
...
```
       ...
       hdc shell hidumper -s WindowManagerService -a '-w 90 -jsdump -dumpAll -viewId=7'       输出信息如下。
...
```
     ...
     **图2** ArkUI State泳道图示意图
     ...
     DevEco Studio的ArkUI Inspector可以显示当前页面自定义组件内的状态变量的详细信息，具体包括以下内容。
          ...
          Compatible Mode：状态管理V1状态变量，且其装饰变量的类型没有[@Track](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/arkts-track)装饰的属性。
          ...
          V2：状态管理V2状态变量。
                 ...
                 | elmtIds                    状态变量关联的组件。声明式UI语法规则中，状态会驱动UI刷新。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `js-framework-multiple-languages`  |  **DocID**: `d66db2c8960e4bc7b23d0a7ac7c8fadc`  |  **NodeID**: `00020177579578888460931bfba5e4a5`
...
# 多语言支持
    ...
    开发者仅需要通过[定义资源文件](#定义资源文件)和[引用资源](#引用资源)两个步骤，就可以使用开发框架的多语言能力；如果需要在应用中获取当前系统语言，请参考[获取语言](#获取语言)。
     ...
     language[-script-region].json     限定词的取值需符合下表要求。
...
```
       ...
       表4 单复数格式化
                              ...
                              | 属性            | 类型            | 参数            | 必填            | 描述           
                                       | $tc            | Function            | 请见表 $tc参数说明            | 是                        根据系统语言完成单复数替换：this.$tc('strings.people')。
...
```
     ...
     获取语言功能请参考[@ohos.app.ability.Configuration (环境变量)](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-app-ability-configuration)。

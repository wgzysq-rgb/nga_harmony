...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `preferences-guidelines`  |  **DocID**: `26f7040b71ec4f30a6cb49f76e335fba`  |  **NodeID**: `000201775795788884221faec50664af`
...
```
     ...
     下列实例展示如何通过Preferences实现对键值数据的修改与持久化。
             ...
             创建Preferences配置选项（PreferencesOption）对象并设置配置选项成员（名称、应用组ID、包名、存储模式）。使用完毕后，调用OH_PreferencesOption_Destroy销毁配置选项实例。

             调用OH_Preferences_Open打开一个Preferences实例，该实例使用完后需要调用OH_Preferences_Close关闭。

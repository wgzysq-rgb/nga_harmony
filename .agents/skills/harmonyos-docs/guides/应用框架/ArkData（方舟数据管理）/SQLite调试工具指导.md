...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `sqlite-database-debug-tool`  |  **DocID**: `f5c24984d37d46f38f9fd25ce190d05b`  |  **NodeID**: `00020177579578888408642a2cca0164`
...
```
  ...
  sqlite3 /data/app/el1/100/base/com.test.myapplication/mydb.db  # 直接打开数据库（跳过进入Shell步骤）  创建并打开新数据库
...
SQL语句DROP TABLE语句用于删除表定义及其关联的所有数据、索引、触发器、约束和权限规范。
 ...
 sqlite>DROP TABLE COMPANY; 执行后，使用 .tables 命令将无法查询到 COMPANY 表，显示结果为空即表示删除成功。
...
在 sqlite> 提示符下，输入以下SQL语句插入单条数据：
  ...
  INSERT INTO COMPANY (ID, NAME, AGE, ADDRESS, SALARY) VALUES (1, '张三', 28, '北京市朝阳区', 20000.5);  通过SQL语句SELECT * FROM 表的名称可获取表的完整信息，具体如下：

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `vector-store-debug-tool`  |  **DocID**: `4041c6522cad4942a35ccbbaffbb907b`  |  **NodeID**: `000201775795788884177b1580b6b175`
...
| 快捷操作  | 命令  
  | [创建表](#创建表)  | create table  
 | [查询表](#查询表)  | .table或者.schema  
 | [重命名表名](#重命名表名)  | alter table t1 rename to new_t1;  
 | [增加表字段](#增加表字段)  | alter table t1 add column c text;  
 | [重命名表字段](#重命名表字段)  | alter table t1 rename b to new_b;  
 | [删除表字段](#删除表字段)  | alter table t1 drop column c;  
 | [删除表](#删除表)  | drop table t1;  
 | [添加表索引](#添加表索引)  | create index idx_1 on t3(a);  
 | [插入数据](#插入数据)  | insert into t2 values(1,'xx'),(2,'yy');  
 | [查询数据](#查询数据)  | select * from t1;  
 | [更新数据](#更新数据)  | update t1 set b = 'z' where a =3;  
 | [删除数据](#删除数据)  | delete from t1 where b = 'z';  
...
执行hdc shell命令进入shell交互模式。

  必须要先切换至目标调试应用路径下, 再进入已有db文件层级，执行"arkdata  -t vector"新建一个数据库。
...
vector>>>
```
  创建新的数据库时，系统会自动生成以下类型的文件， 标签debug_hap_data_file代表属于调试应用。

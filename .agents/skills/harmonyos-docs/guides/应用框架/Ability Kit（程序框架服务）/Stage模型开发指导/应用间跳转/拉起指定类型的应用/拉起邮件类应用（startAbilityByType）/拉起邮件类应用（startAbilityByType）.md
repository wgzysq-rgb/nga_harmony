...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `start-email-apps`  |  **DocID**: `aa6e0243fc894c488ae3fdfb7b7735df`  |  **NodeID**: `00020177579578888475340f4efd5eed`
...
# 拉起邮件类应用（startAbilityByType）
     ...
     startAbilityByType接口中type字段为mail，对应的wantParam参数：
       ...
       import { common, wantConstant } from '@kit.AbilityKit';             构造接口参数并调用startAbilityByType接口。
...
```
       ...
       UIAbility.onCreate(want: Want, launchParam: AbilityConstant.LaunchParam): void       在参数**want.parameters**中会携带Caller方传入的参数（与调用方传入的有些差异），如下表所示：

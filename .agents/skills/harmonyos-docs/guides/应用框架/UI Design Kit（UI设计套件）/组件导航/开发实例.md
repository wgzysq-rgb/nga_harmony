...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ui-design-navigation-dynamic-blur-demo`  |  **DocID**: `265117ce6306484f9fb48bd62fd8f6a6`  |  **NodeID**: `00020177579578888433137bdd9703e0`
...
在首页创建一级导航。通过titleBar接口设置HdsNavigation标题栏样式及内容设置。通过pushPath路由方法跳转二级页面。
...
const  TITLE_BAR_HEIGHT_MINI: number = 64;
...
export struct PageOne {
  ...
  build() {
      ...
      Scroll(this.scroller) { // HdsNavDestination内容区设置可滚动容器组件，用于实现内容区滚动联动标题栏动态模糊样式
        Column() {
          ...
          Button('pushPath', { stateEffect: true, type: ButtonType.Capsule })
            ...
            .margin({ top: '5%', right: '50vp', left: '50vp' , bottom: '5%'})
            .onClick(() => {
              console.info('HDS_BASE_COMPONENT', `onClick pageOne`);
          ...
          Button('popToName', { stateEffect: true, type: ButtonType.Capsule })
            ...
            .margin(20)
            .onClick(() => {
              this.pageInfos.popToName('pageTwo') // 回退路由栈到第一个名为name的HdsNavDestination页面
    ...
    .titleBar({ // HdsNavDestination标题栏配置
        style: { // HdsNavDestination标题栏样式配置
          // 标题栏动态模糊样式，包括是否使能滚动动态模糊，动态模糊类型，动态模糊生效的滚动距离等
          scrollEffectOpts: {
            enableScrollEffect: true,
            scrollEffectType: ScrollEffectType.COMMON_BLUR,
...
export struct PageTwo {
  ...
  build() {
    HdsNavDestination() { // 创建HdsNavDestination组件
      Column() { // HdsNavDestination组件内容区设置
        Blank().height(TITLE_BAR_HEIGHT_MINI)
    ...
    .titleBar({ // HdsNavDestination组件标题栏设置
      content: {
        title: {
          mainTitle: 'PageTwo'
        },
...
```
  ...
  "routerMap": "$profile:route_map"  工程entry/src/main/resources/base/profile目录下增加route_map.json文件。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ui-design-snackbar-resident-notification`  |  **DocID**: `4785bbe8854748e3b1bf0bca770a8a9c`  |  **NodeID**: `000201775795788884134d2ea3461ef2`
...
```
             创建UIContext，创建HdsSnackBar对象hdsSnackBar，调用HdsSnackBar对象的show方法可以显示HdsSnackBar弹窗，入参是左侧图标icon、中间文本message、右侧操作区operation、样式style，其中右侧操作区设置类型是带有关闭按钮的文本按钮，其中style中设置duration是-1表示HdsSnackBar弹窗常驻。

             设置textButtonId和nextFocusId两个属性，支持开发者自定义Tab键走焦能力。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `hypium-perf-python-guidelines`  |  **DocID**: `aa2b27a91dfe4220a599fd970bba80a4`  |  **NodeID**: `0008017737326395592526675713b0c2`
...
class OH_PerfDemoTest(PerfBaseCase):
    ...
    def teardown(self):
        ...
        # 此处为用例结尾时执��的PerfBaseCase的teardown方法，处理一些结束操作
...
参数名称
 ...
 is_watch
  ...
  默认False，不做视频观看操作，设置为True会对页面进行30S观看，在scene_type为WATCH时生效。
 ...
 is_short_video
  ...
  默认False，在scene_type字段为PLAY_VIDEO时生效，标记起播视频为长视频（False）或者短视频（True）。
 ...
 ad_monitor
  ...
  默认为False，在scene_type字段为COLD_START时生效，设置为True时，会进行广告检测。

  
 cold_start_wait_time

  非必填，在scene_type字段为COLD_START时生效，表示冷启动后，需要额外等待的时间。

  
 pop_up_window_enable

  默认True，在scene_type字段为COLD_START时生效，表示冷启动后，是否执行一次弹窗检测操作。
 ...
 **1、点击****操作**
 ...
 def touch_perf(self, target: **Union**[By, UiComponent, tuple], wait_time: float = 0.1, tag: Tag = None) **接口说明**
...
序号
 ...
 3
  ...
  对应性能场景tag，点击如果进入新界面，需使用WITH_PAGE_SWITCH（有界面切换的场景类型）。
 ...
 **2、长按****操作**
 ...
 def long_touch_perf(self, target: **Union**[By, UiComponent, tuple], tag: Tag = None) **接口说明**
 ...
 **3、双击****操作**
 ...
 def double_touch_perf(self, target: **Union**[By, UiComponent, tuple], tag: Tag = None) **接口说明**
 ...
 **接口说明**
 ...
 在���幕上或者指定区域area中执行朝向指定方向direction的滑动操作。该接口用于执行不太精准的滑动操作。
...
序号
 ...
 5
  ...
  对应性能场景tag，若该操作不涉及页面切换，需使用NO_PAGE_SWITCH。
 ...
 **5、执行精准的滑动操作**
 ...
 def **slide_perf**(self, start: **Union**[By, tuple], end: Union[By, tuple], slide_time: float = 2, tag: Tag = None) **接口说明**
 ...
 **7、屏幕侧边滑动返回****操作**
 ...
 def **swipe_to_back_perf**(self, side=UiParam.RIGHT, times: int = 1, height: float = 0.5, tag: Tag = None) **接口说明**
 ...
 **8、从屏幕底部上滑返回桌面**
 ...
 def **swipe_to_home_perf**(self, times: int = 1, tag: Tag = None) **接口说明**
...
序号
 ...
 2
  ...
  对应性能场景tag，返回桌面一般带有界面切换，需使用WITH_PAGE_SWITCH。
 ...
 **9、观看视频**
 ...
 def watch_perf(self, watch_time, is_bullet_screen=False, is_full_screen=False, watch_tag_desc=None) **接口说明**
 ...
 **10、在桌面滑动查找APP，并打开应用**
 ...
 def start_application_perf(self, app_name, scene_type, tag=None) **接口说明**
...
序号
 ...
 2
  ...
  应用启动方式，为SceneType.COLD_START或者SceneType.HOT_START。

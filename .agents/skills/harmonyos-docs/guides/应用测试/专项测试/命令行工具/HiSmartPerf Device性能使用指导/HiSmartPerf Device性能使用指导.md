...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `smartperf-guidelines`  |  **DocID**: `841cedad5f3147ab8fb7a8aa514e83cb`  |  **NodeID**: `000801773732639559253235789f2d1d`
...
Device daemon端从API version 9开始预置使用。

 Device daemon端执行需连接硬件设备，Device hap端需在有屏幕设备使用。

 Device daemon端执行前需完成[hdc环境配置](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/hdc)，并进入shell环境拉起daemon进程。
...
$
```
 全量采集示例2，设置包名并采集指定应用信息，包括cpu、gpu、温度、功耗、fps、内存信息、DDR信��、网络速率、屏幕截图。
...
开始采集示例1（采整机cpu、gpu、温度、功耗、fps、内存信息、DDR信息、网络速率、屏幕截图）：SP_daemon -start -c -g -t -p -r -d -net -snapshot。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `using-audiocapturer-for-recording`  |  **DocID**: `5dc5ed5b20ad4e27bdaf29079a242f90`  |  **NodeID**: `000201775795526928710d315993c94b`
...
# 使用AudioCapturer开发音频录制功能(ArkTs)
     ...
     **图1** AudioCapturer状态变化示意图
     ...
     以下各步骤示例为片段代码，可通过示例代码右下方链接获取[完整示例](https://gitcode.com/openharmony/applications_app_samples/blob/master/code/DocsSample/Media/Audio/AudioCaptureSampleJS)。
             ...
             配置音频采集参数并创建AudioCapturer实例，音频采集参数的详细信息可以查看[AudioCapturerOptions](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-i#audiocaptureroptions8)。
...
```
             调用[on('readData')](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiocapturer#onreaddata11)方法，订阅监听音频数据读入回调。
...
// ...
  ...
  // ...
      audioCapturer.on('readData', readDataCallback);
```
             调用[start](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiocapturer#start8)方法进入running状态，开始录制音频。
...
```
             调用[stop](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiocapturer#stop8)方法停止录制。
...
```
             调用[release](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-audio-audiocapturer#release8)方法销毁实例，释放资源。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `using-opensl-es-for-recording`  |  **DocID**: `6655ed2a667644818a3b91963f0ed2ad`  |  **NodeID**: `00020177579552692870650de62ab5d4`
...
HarmonyOS上的OpenSL ES接口，是早期SDK8版本开始提供，用于支持应用Native层音频开发的接口。但随着版本演进，接口定义的可扩展性不足，不再能满足音频系统的能力拓展，因此当前已不再推荐应用开发者继续使用此接口进行音频功能开发，会存在一些接口能力不足的缺陷。
 ...
 OHAudio的开发指南请参考：[推荐使用OHAudio开发音频录制功能(C/C++)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/using-ohaudio-for-recording)。

 考虑到一些接入HarmonyOS较早的应用开发者，这里提供了一份OpenSL ES接口切换到OHAudio的对照参考：[从OpenSL ES切换到OHAudio(C/C++)](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/replace-opensles-by-ohaudio)，便于开发者能够更快的在新版本切换到使用新接口。
...
target_link_libraries(sample PUBLIC libOpenSLES.so) 参考下列示例代码，完成音频录制。
...
```
  配置录音器信息（配置输入源audiosource、输出源audiosink），创建录音对象pcmCapturerObject。
...
```
  ...
  (*recordItf)->SetRecordState(recordItf, SL_RECORDSTATE_RECORDING);  结束音频录制。

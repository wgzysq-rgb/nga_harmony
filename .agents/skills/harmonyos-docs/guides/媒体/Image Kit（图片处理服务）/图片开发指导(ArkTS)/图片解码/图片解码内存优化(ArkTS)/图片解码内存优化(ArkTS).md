...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `image-allocator-type`  |  **DocID**: `aa65ec5b34b54e44ab49a74a12061a80`  |  **NodeID**: `000201775795526928854156924f0b34`
...
# 图片解码内存优化(ArkTS)
    ...
    应用侧通过解码API接口获取PixelMap，并将其传递给[Image组件](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-components-basic-image)以进行显示。
    ...
    当PixelMap较大且使用共享内存时，RS主线程将经历较长的纹理上传时间，导致卡顿现象。图形侧提供了DMA内存零拷贝功能，可在绘制图片时避免纹理上传时间消耗。
     ...
     stride的值可以通过[getImageInfo()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-imagesource#getimageinfo-1) 接口获取。
             ...
             调用[getImageInfo()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-imagesource#getimageinfo-1)方法，获取ImageInfo对象。

             从ImageInfo对象中访问stride值：info.stride。
...
```
     ...
     图片框架内，不同图片格式的下采样解码支持情况如下所示。
                 ...
                 | 不支持          | .gif .bmp .webp .dng [.svg10+](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-image-f#svg标签说明) .ico11+

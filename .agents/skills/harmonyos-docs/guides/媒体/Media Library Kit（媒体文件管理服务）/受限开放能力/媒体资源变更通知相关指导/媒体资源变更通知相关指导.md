...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `photoaccesshelper-notify-guidelines`  |  **DocID**: `0cd8b9cf72244431a950e3d260a8ca02`  |  **NodeID**: `000201775795526928736eebd21490a8`
...
```
     ...
     **开发步骤**
      ...
      - [获取用户相册](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/photoaccesshelper-useralbum-guidelines#获取用户相册)。
      - 对指定Album注册监听。
      - 将指定用户相册重命名。
...
```
      ...
      - 通过设置forChildUris值为true来注册模糊监听，uri为相册uri时，forChildUris为true能监听到相册中文件的变化，如果是false只能监听相册本身变化。
      - uri为photoAsset时，forChildUris为true、false没有区别。
      - uri为DefaultChangeUri时，forChildUris必须为true，如果为false将找不到该uri，收不到任何消息。

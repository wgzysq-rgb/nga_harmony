...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `using-avrecorder-for-recording`  |  **DocID**: `b8a812dd35f8472b920f32a3ff14da0f`  |  **NodeID**: `00020177579552692881400f24a3ddb1`
...
```
             配置音频录制参数，调用[prepare](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avrecorder#prepare9-1)接口，此时进入prepared状态。
...
```
             开始录制，调用[start](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avrecorder#start9-1)接口，此时进入started状态。

       ```
// 开始录制。
await this.avRecorder?.start();
```
             暂停录制，调用[pause](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avrecorder#pause9-1)接口，此时进入paused状态。
...
```
             停止录制，调用[stop](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avrecorder#stop9-1)接口，此时进入stopped状态。
...
```
             销毁实例，调用[release](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-avrecorder#release9-1)接口，进入released状态，退出录制。

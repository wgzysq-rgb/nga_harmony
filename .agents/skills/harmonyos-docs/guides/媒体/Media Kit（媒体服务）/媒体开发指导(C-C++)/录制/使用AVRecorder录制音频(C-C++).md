...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `using-ndk-avrecorder-for-audio-recording`  |  **DocID**: `18dbe011448045fcb41b8a07dfe71683`  |  **NodeID**: `0002017757955269288167e26bbbfc95`
...
# 使用AVRecorder录制音频(C/C++)
    ...
    **图1** 录制状态变化示意图
    ...
    状态的详细说明请参考[AVRecorderState](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-media-t#avrecorderstate9)。
     ...
     #include <multimedia/player_framework/native_avformat.h>     并在 CMake 脚本中链接如下动态库。
...
```
             配置音频录制参数，调用OH_AVRecorder_Prepare()接口，此时进入prepared状态。
...
```
             开始录制，调用OH_AVRecorder_Start()接口，此时AVRecorder进入started状态。

       OH_AVRecorder_Start(g_avRecorder);             暂停录制，调用OH_AVRecorder_Pause()接口，此时AVRecorder进入paused状态，同时暂停输入源输入数据。

       OH_AVRecorder_Pause(g_avRecorder);             恢复录制，调用OH_AVRecorder_Resume()接口，此时再次进入started状态。

       OH_AVRecorder_Resume(g_avRecorder);             停止录制，调用OH_AVRecorder_Stop()接口，此时进入stopped状态。

       OH_AVRecorder_Stop(g_avRecorder);             重置资源，调用OH_AVRecorder_Reset()重新进入idle状态，允许重新配置录制参数。

       OH_AVRecorder_Reset(g_avRecorder);             销毁实例，调用OH_AVRecorder_Release()进入released状态，退出录制。

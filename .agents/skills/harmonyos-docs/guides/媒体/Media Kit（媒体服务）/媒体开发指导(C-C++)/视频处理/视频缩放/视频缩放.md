...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `generate-super-resolution-video`  |  **DocID**: `9290c1bfab4d4e62bf36780101a2d22f`  |  **NodeID**: `000201775795526928827b413009023c`
...
```
 ...
 一般在进程内第一次使用时调用，可提前完成部分耗时操作。
  ...
  VideoProcessing_ErrorCode ret = OH_VideoProcessing_InitializeEnvironment();  创建细节增强模块。
...
```
  调用[OH_VideoProcessing_Stop()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-video-processing-h#oh_videoprocessing_stop)停止细节增强。

  VideoProcessing_ErrorCode ret = OH_VideoProcessing_Stop(videoProcessor);  释放处理实例。

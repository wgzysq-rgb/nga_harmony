...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ide-hvigor-errorcode-00306`  |  **DocID**: `507e207fea9e4a62aa8c00fe06ac7183`  |  **NodeID**: `00020177579013591893500445105f43`
...
# 规格错误码
     ...
     **处理步骤**
      ...
      - 确保工程级build-profile.json5中的compatibleSdkVersion版本为11或以上。
      - 确保模块级build-profile.json5中的apiType为stageMode。
     ...
     **可能原因**
      ...
      - 工程级build-profile.json5的buildOption.strictMode下未配置useNormalizedOHMUrl，或者该属性值设置为false。
      - 在模块级build-profile.json5文件中，buildOption.arkOptions.byteCodeHar设置为false。
     
     **处理步骤**

     
      - 确保工程级build-profile.json5的buildOption.strictMode包含useNormalizedOHMUrl属性，并将其设置为true。
      - 确保模块级build-profile.json5文件中，buildOption.arkOptions.byteCodeHar设置为true。
     ...
     **处理步骤**
      ...
      - 在[hvigor-config.json5](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-set-options)中调整线程相关配置：hvigor.pool.maxSize、ohos.arkCompile.maxSize、hvigor.enableMemoryCache。
      - 执行任务时关闭[parallel模式](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-improve-performance)。
     ...
     **处理步骤**
      ...
      - 执行“hvigorw --help”命令，了解有关hvigor命令的更多信息。
      - 执行“hvigorw tasks”命令，查看可用任务列表。
      - 执行命令时增加“--info”或“--debug”选项，获取更多日志输出。
     ...
     **处理步骤**
      ...
      - 修改hvigor-config.json5中nodeOptions下的maxOldSpaceSize，增加守护进程的内存。
      - [关闭守护进程](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-daemon)后重新编译，验证该问题是否与守护进程有关。

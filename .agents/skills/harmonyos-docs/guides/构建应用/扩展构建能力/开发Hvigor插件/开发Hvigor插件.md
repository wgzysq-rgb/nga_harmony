...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ide-hvigor-plugin`  |  **DocID**: `4ac7dc9053bb43e9a87a431dbec6f4e6`  |  **NodeID**: `00020177579013591889648522da1fd5`
...
# 开发Hvigor插件
    ...
    关于插件开发的具体实践请参考[定制hvigor插件开发实践](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-custom-hvigor-plugin)。
     ...
     以工程级hvigorfile.ts脚本为例，开发步骤如下。
      ...
      导入模块依赖，更多接口请参考[扩展构建API](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-hvigor-apis)。       
...
```
     ...
     配置npm镜像仓库地址，用于安装@ohos/hvigor插件。      
...
```
    ...
    发布npm包流程：
     ...
     编译npm包。      
      ...
      tsc      如果编译时报以下错误，请检查初始化项目时是否[删除了verbatimModuleSyntax](#li88369101451)。
...
```
     ...
     安装依赖。      
       ...
       - 方式2：使用hvigorw命令行工具执行任一命令，命令行工具会自动执行安装构建依赖。        hvigorw --sync

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `ide-hvigor-compilation-options-customizing-sample`  |  **DocID**: `bbd3523aa9ec408d949696b2addb741e`  |  **NodeID**: `000201775790135918926619a6a978cd`
...
示例1：构建APP时，构建产物为default，指定构建模式为debug，可执行如下命令：
 ...
 hvigorw --mode project -p product=default -p buildMode=debug assembleApp 编译产物示例如下：

 

 示例2：构建APP时，构建产物为default，指定构建模式为release，可执行如下命令：

 hvigorw --mode project -p product=default -p buildMode=release assembleApp 编译产物示例如下：

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `debug-lldb`  |  **DocID**: `6b6c3a98419b4a8f93de35ae3f86c2a0`  |  **NodeID**: `000201775795460123289aa12c412a25`
...
此处列举LLDB调试器支持的部分功能，更多命令参考：[LLDB工具使用指导](https://gitcode.com/openharmony/third_party_llvm-project/blob/master/lldb/README_zh.md)和[LLDB官网手册](https://lldb.llvm.org/use/map.ditamap)。Windows、Linux x86_64和Mac平台的LLDB工具有些许差异，以实际应用为准。
...
HarmonyOS设备需获取root权限
  ...
  hdc file send \DevEco Studio\sdk\default\HarmonyOS\native\llvm\lib\clang\current\bin\aarch64-linux-ohos\lldb /data/local/tmp/debugserver  选择lldb server路径（根据设备CPU架构选择，参考）并使用hdc传输到设备：
...
- 准备lldb-server，建议使用DevEco Studio推送。如手动推送，选择lldb-server路径（根据设备CPU架构选择，参考）并使用hdc传输到设备：hdc file send \DevEco Studio\sdk\default\hms\native\lldb\aarch64-linux-ohos\lldb-server /data/local/tmp/debugserver  
...
获取到与LLDB同一版本的clang编译器生成的带有调试信息的可执行文件a.out。
  ...
  使用**hdc shell**进入手机交互命令环境，进入lldb路径。

  运行LLDB工具，并指定要调试的文件为a.out。

  ./lldb a.out  在代码中main函数处设置断点。

  (lldb) b main  运行应用，使其停在断点处。

  (lldb) run  继续运行应用。

  (lldb) continue  列出所有断点。

  (lldb) breakpoint list  显示当前帧的参数和局部变量。

  (lldb) frame variable  按需执行调试命令进行后续调试操作。
...
使用**hdc shell**进入手机交互命令环境，进入lldb路径。
  ...
  ./a.out  在终端窗口2运行LLDB工具。

  ./lldb  attach应用。

  (lldb) process attach --name a.out  在hello.cpp的第10行设置断点。

  (lldb) breakpoint set --file hello.cpp --line 10  在终端窗口1，输入一个int类型的数。

  88  在终端行窗口2继续运行应用，使应用停在断点处。

  (lldb) continue  按需执行调试命令进行后续调试操作。
...
打开命令行窗口1，关闭SELinux。
  ...
  hdc shell setenforce 0  在命令行窗口1，将lldb-server和可执行文件a.out推送到设备。
...
```
  ...
  hdc shell /data/local/tmp/debugserver/lldb-server p --server --listen "*:8080"  打开命令行窗口2，运行二进制文件lldb。

  ./lldb  在LLDB命令行窗口进行远端选择与连接。
...
```
  ...
  (lldb) target create /data/local/tmp/debugserver/a.out  在代码中main函数处设置断点。

  (lldb) b main  启动应用。

  (lldb) run  查看当前目标进程的源码。

  (lldb) source list  按需执行调试命令进行后续调试操作。
...
```
  ...
  hdc shell aa process -a EntryAbility -b com.example.myapplication -D "/data/local/tmp/debugserver/com.example.myapplication/lldb-server platform --listen unix-abstract:///lldb-server/platform.sock"  打开命令行窗口2，运行二进制文件lldb。
...
```
  ...
  (lldb) settings append target.exec-search-paths "C:\Users\xxx\DevEcoStudioProjects\MyApplication\entry\build\default\intermediates\cmake\default\obj\arm64-v8a"  在源代码第6行处设置断点。

  (lldb) breakpoint set --file "C:/Users/xxx/DevEcoStudioProjects/MyApplication/entry/src/main/cpp/napi_init.cpp" --line 6  指定要调试的设备上的hap包对应的应用pid。

  (lldb) attach <pid>  点击设备应用，使其继续响应，并停止在断点处。

  继续调试。

  (lldb) continue  显示当前线程的堆栈回溯。

  (lldb) bt  按需执行调试命令进行后续调试操作。

...
> **分区**: 指南  |  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  |  **Slug**: `fdsan`  |  **DocID**: `3e3d236bbd654238bc9b6ffec201e0a5`  |  **NodeID**: `0002017757954601233136ad95d0ff16`
...
设计思路：当打开已有文件或创建一个新文件的时候，在得到返回fd后，设置一个关联的tag，来标记fd的属主信息；关闭文件前，检测fd关联的tag，判断是否符合预期(属主信息一致)，符合就继续走正常文件关闭流程；如果不符合就是检测到异常，根据设置，调用对应的异常处理。
 ...
 value用于标识实际的owner tag。
  ...
  #### [h2]fdsan_set_error_level

enum fdsan_error_level fdsan_set_error_level(enum fdsan_error_level new_level); **描述：** 可以通过fdsan_set_error_level设定error_level，error_level用于控制检测到异常后的处理行为。默认error_level为FDSAN_ERROR_LEVEL_WARN_ALWAYS。
...
| 名称  | 说明  
  ...
  **返回值：** 返回旧的error_level。
   ...
   #### [h2]fdsan_get_error_level

enum fdsan_error_level fdsan_get_error_level(); **描述：** 可以通过fdsan_get_error_level获取error level。

 **返回值：** 当前的error_level。

   #### [h2]fdsan_create_owner_tag

uint64_t fdsan_create_owner_tag(enum fdsan_owner_type type, uint64_t tag); **描述：** 通过传入的type和tag字段，拼接成一个有效的文件描述符的关闭tag。
...
| 名称  | 说明  
  ...
  **返回值：** 返回创建的tag，可以用于fdsan_exchange_owner_tag函数的输入。
   ...
   #### [h2]fdsan_exchange_owner_tag

void fdsan_exchange_owner_tag(int fd, uint64_t expected_tag, uint64_t new_tag); **描述：** 修改文件描述符的关闭tag。
...
| 名称  | 类型  | 说明  
 ...
 | new_tag  | uint64_t  | 设置新的ownership tag值。  
    ...
    #### [h2]fdsan_close_with_tag

int fdsan_close_with_tag(int fd, uint64_t tag); **描述：** 根据tag描述符关闭文件描述符。
...
| 名称  | 类型  | 说明  
  ...
  **返回值：** 0或者-1，0表示close成功，-1表示close失败。
   ...
   #### [h2]fdsan_get_owner_tag

uint64_t fdsan_get_owner_tag(int fd); **描述：** 根据文件描述符获取tag信息。
...
| 名称  | 类型  | 说明  
  ...
  **返回值：** 返回对应fd的tag。
   ...
   #### [h2]fdsan_get_tag_type

const char* fdsan_get_tag_type(uint64_t tag); **描述：** 根据tag计算出对应的type类型。
...
| 名称  | 类型  | 说明  
  ...
  **返回值：** 返回对应tag的type。
   ...
   #### [h2]fdsan_get_tag_value

uint64_t fdsan_get_tag_value(uint64_t tag); **描述：** 根据tag计算出对应的owner value。

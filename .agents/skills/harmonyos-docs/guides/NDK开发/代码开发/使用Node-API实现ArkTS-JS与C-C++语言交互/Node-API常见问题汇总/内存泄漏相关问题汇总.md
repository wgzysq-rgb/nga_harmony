...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `napi-faq-about-memory-leak`  |  **DocID**: `cac3e2e4e3cd44e893bc851769aa11f7`  |  **NodeID**: `0002017757954601233393136857af7c`
...
napi_value不在napi_handle_scope管理中，导致napi_value持有的ArkTS对象无法释放，该问题常见于[直接使用uv_queue_work的场景](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/napi-guidelines#异步任务)中。解决方法是添加napi_open_handle_scope和napi_close_handle_scope接口。
  ...
  使用 napi_create_reference 为 ArkTS 对象创建了强引用（initial_refcount 参数大于 0），且一直未删除，导致 ArkTS 对象无法被回收。napi_create_reference 接口内部会创建一个 C++ 对象，因此这种泄漏通常表现为ArkTS对象与Native对象的双重泄漏。可以使用 Allocation 工具捕获Native对象泄漏栈，检查是否存在 napi_create_reference 相关的栈帧。

   [基础内存分析：Allocation分析](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-insight-session-allocations)

  被其它存活的ArkTS对象持有时，使用snapshot查看泄漏对象的持有者。

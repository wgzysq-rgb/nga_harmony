...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `openmp-guideline`  |  **DocID**: `5c6febc0c77a464d98519b45fd94bcad`  |  **NodeID**: `0002017757954601233122351df549ce`
...
OpenMP库的引入可以通过静态链接和动态链接两种方式实现。
 ...
 target_link_libraries(entry PUBLIC libomp.a libace_napi.z.so libhilog_ndk.z.so) （2）打开entry/build-profile.json5，在buildOption->externalNativeOptions->cppFlags下添加编译参数"-static-openmp -fopenmp"。
 ...
 **动态链接**
 ...
 target_link_libraries(entry PUBLIC libomp.so libace_napi.z.so libhilog_ndk.z.so) （2）打开entry/build-profile.json5，在buildOption->externalNativeOptions->cppFlags下添加编译参数"-fopenmp"。
...
```
 ...
 export const ompTest: () => null; （3）Ts侧调用，修改entry/src/main/ets/pages/Index.ets，调用ompTest函数。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `jsvm-frequently-questions`  |  **DocID**: `9caf1ec81d884be0bb58f5e466f97b06`  |  **NodeID**: `00020177579546012333301683e96bb4`
...
```
       在上述示例代码中，JS引擎实例在函数结束前被关闭，因此可以直接使用栈上的param。
             ...
             Q：OH_JSVM_ReferenceRef、OH_JSVM_ReferenceUnRef、OH_JSVM_CreateReference、OH_JSVM_DeleteReference时程序崩溃
       ...
       A：检查是否同时有多个线程持有和释放JSVM_Ref，见[多线程共享引擎实例](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/jsvm-guidelines#多线程共享引擎实例)
             ...
             Q：在虚拟机引擎实例中创建JS类型实例崩溃（如OH_JSVM_CreateDouble），调用栈如下
...
```
       A：检查HandleScope的使用是否正确，参考[生命周期管理](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/jsvm-guidelines#生命周期管理)
             ...
             Q: 调用JSVM-API时出现 JSVM Fatal Error Message:(openHandleScopes)==(openHandleScopesBefore) 错误
...
```
       A: 这两种报错可能指向同一问题，即应用侧执行[OH_JSVM_DestroyEnv()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-jsvm-h#oh_jsvm_destroyenv)(释放JSVM环境)后，仍在执行业务逻辑，尝试调用JSVM-API，触发报错。该报错可能由三种情况产生：
       ...
       A：JSVM_PENDING_EXCEPTION表明当前虚拟机环境中存在未处理的异常，可能是由于本次调用产生的JS异常，也可能是之前调用产生的未被清理的异常。可以通过在函数调用前插入OH_JSVM_GetAndClearLastException排查之前是否有未清除的异常。如果为之前的未清理异常，检查是否有JSVM接口调用未处理异常返回值；如果是本次产生的异常，需清理异常，避免影响后续的函数调用。获取并清理异常的函数为OH_JSVM_GetAndClearLastException
             ...
             Q：JS执行时无法找到OH_JSVM_DefineClass定义的类

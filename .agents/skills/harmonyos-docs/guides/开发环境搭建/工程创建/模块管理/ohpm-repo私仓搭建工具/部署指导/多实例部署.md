...
> **分区**: Guides  |  **Slug**: `ide-ohpm-deploy-multiple-instances`  |  **DocID**: `a35b9ad1add240ac8065bfdff00aa5ea`
...
进入ohpm-repo解压目录的bin目录下，执行安装命令:ohpm-repo install   不配置参数--config，则默认使用ohpm-repo解压目录中conf目录内自带的配置文件config.yaml。
...
在完成了多实例配置并首次启动过ohpm-repo服务实例的机器上，执行ohpm-repo pack <deploy_root>。
 ...
 ohpm-repo pack D:\ohpm-repo  该命令用来打包备份ohpm-repo的<deploy_root>/conf，<deploy_root>/meta目录，并在命令行工作目录下生成压缩包。
...
- 在ohpm-repo工具的bin目录下创建自动运行脚本run-repo.sh：touch run-repo.sh 
写入下面内容，保存并关闭文件： 当mysql或sftp服务与ohpm-repo部署在同一服务器上时，请将mysql和sftp的启动命令放在ohpm-repo start命令之前。

  ```
#!/bin/bash
ohpm-repo start
```
  - 将该脚本设置为可执行文件：chmod +x run-repo.sh 
...
新建run-repo.bat文件，写入下面内容： 当mysql或sftp服务与ohpm-repo部署在同一服务器上时，请将mysql和sftp的启动命令放在ohpm-repo start命令之前。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `xengine-kit-rt-reflection`  |  **DocID**: `a8dbbba0197f45faaab529a60cd27b05`  |  **NodeID**: `000201775795491975920f0bc814c302`
...
- 用户进入光线追踪反射适用的游戏场景。
- 游戏应用调用HMS_XEG_CreateRTReflection接口创建光线追踪反射实例。如果光线追踪反射创建失败，直接调用后处理并送显当前帧即可。
- 游戏应用调用HMS_XEG_CmdRenderRTReflection接口计算光线追踪反射命中信息，并返回错误码。如果计算失败，直接调用后处理并送显当前帧即可。
- 游戏应用根据3中计算的反射命中信息，生成反射图像。
- 游戏应用将反射图像和游戏的主场景渲染结果进行融合。
- 游戏应用完成其余后处理，并送显当前帧。
- 用户退出光线追踪反射适用的游戏场景。
- 游戏应用调用HMS_XEG_HMS_XEG_DestroyRTReflection接口销毁光线追踪反射实例。
...
调用[HMS_XEG_EnumerateDeviceExtensionProperties](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/xengine-kit-xengine#hms_xeg_enumeratedeviceextensionproperties)接口，获取XEngine支持的扩展信息，只有在支持XEG_RT_REFLECTION_EXTENSION_NAME扩展时才可以使用光线追踪反射的相关接口。
...
```
  ...
  XEG_RTReflection xegRTReflection;  调用[HMS_XEG_CreateRTReflection](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/xengine-kit-xengine#hms_xeg_creatertreflection)接口，创建光线追踪反射实例。
...
```
  调用[HMS_XEG_CmdRenderRTReflection](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/xengine-kit-xengine#hms_xeg_cmdrenderrtreflection)接口下发渲染命令，每帧都需要调用。
...
```
  调用[HMS_XEG_DestroyRTReflection](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/xengine-kit-xengine#hms_xeg_destroyrtreflection)接口销毁实例，释放资源，当特性不再使用或应用退出时需要调用。

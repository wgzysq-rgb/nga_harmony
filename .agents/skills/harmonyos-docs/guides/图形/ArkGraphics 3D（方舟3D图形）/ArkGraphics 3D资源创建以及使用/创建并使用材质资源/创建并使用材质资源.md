...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `arkgraphics3d-resource-material`  |  **DocID**: `051c13c5686c4ad599913d82d2518903`  |  **NodeID**: `000201775795491975932d1d6312d30d`
...
# 创建并使用材质资源
    ...
    ArkGraphics 3D采用基于物理的渲染（PBR, Physically-Based Rendering）模型，其材质实现遵循通用的PBR原理。开发者既可以使用标准材质快速实现真实感效果，也可以通过自定义Shader材质灵活控制渲染逻辑。
     ...
     ArkGraphics 3D中的材质类型通过[MaterialType](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-scene-resources#materialtype)枚举指定，目前支持以下两种类型：
     ...
     材质的创建一般通过[SceneResourceFactory.createMaterial()](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-scene#creatematerial)方法完成，需指定材质名称和类型。不同类型的材质支持不同的参数配置，开发者可按需选择以实现预期的渲染效果。
     ...
     所有材质均具备以下基础属性（如materialType、shadowReceiver、blend等）可通过设置[Material](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-scene-resources#material)类型对象实现，用于控制材质的类型及其渲染基础行为：
     ...
     符合glTF标准的基于物理渲染（PBR）的金属-粗糙度材质，通过设置[MetallicRoughnessMaterial](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-scene-resources#metallicroughnessmaterial20)实现，其中各项属性采用[MaterialProperty](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/js-apis-inner-scene-resources#materialproperty20)类型封装，支持绑定纹理和设置因子（factor）值。具体属性包括：

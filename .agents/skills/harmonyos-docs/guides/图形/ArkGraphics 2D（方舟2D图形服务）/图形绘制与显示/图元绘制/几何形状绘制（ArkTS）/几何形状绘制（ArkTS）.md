...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `geometric-shape-drawing-arkts`  |  **DocID**: `95a3788392274daa9e51fae4898ea4f4`  |  **NodeID**: `0002017757954919750134ba4e13a388`
...
```
     ...
     绘制路径的相关接口和实现如下，详细的使用和参数说明请见[Path](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/arkts-apis-graphics-drawing-path)。常用的接口如下：
             ...
             使用new drawing.Path()可以创建一个路径对象。

             使用moveTo()接口可以设置自定义路径的起始点位置。

             使用lineTo()接口可以添加一条从起始点或路径的最后点位置（若路径没有内容则默认为(0,0)）到目标点位置的线段。

...
> **分区**: 指南  |  |  **API级别**: API 23 (HarmonyOS 6.0)  |  **Slug**: `basic-drawing-effect-c`  |  **DocID**: `0bb2b0c92bd740f1bca382bd0f632587`  |  **NodeID**: `00020177579549197502878b872cb2a3`
...
# 基础绘制效果（C/C++）
     ...
     使用画刷（Brush）设置绘制效果的常用接口如下表所示，详细的使用和参数请见[drawing_brush](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-drawing-brush-h)。
             ...
             使用OH_Drawing_BrushCreate()接口创建画刷Brush对象。

       ```
// 创建画刷
OH_Drawing_Brush* brush = OH_Drawing_BrushCreate();
```
             使用画刷设置基础绘制效果（可选以下的一个或者多个效果）。
...
```
             ...
             使用OH_Drawing_CanvasAttachBrush()接口给Canvas画布设置画刷。接口接受两个参数，一个是画布对象Canvas，请确保已创建或获取得到画布Canvas，具体可见[画布的获取与绘制结果的显示（C/C++）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/canvas-get-result-draw-c)；另一个是要设置的画刷对象。画布将会使用设置的画刷样式和颜色等填充图形。

       ```
// 设置画布的画刷
OH_Drawing_CanvasAttachBrush(canvas, brush);
```
             按需绘制图元，具体可见[图元绘制](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/primitive-drawing-overview)一节。

             当不需要填充效果时，可以使用OH_Drawing_CanvasDetachBrush()去除。入参为画布对象Canvas。

       ```
// 去除画布中的画刷
OH_Drawing_CanvasDetachBrush(canvas);
```
             当不再需要画刷进行效果填充时，请及时使用OH_Drawing_BrushDestroy()接口销毁Brush对象。
...
```
     ...
     使用画笔（Pen）设置绘制效果的常用接口如下表所示，详细的使用和参数请见[drawing_pen](https://developer.huawei.com/consumer/cn/doc/harmonyos-references/capi-drawing-pen-h)。
             ...
             使用OH_Drawing_PenCreate()接口创建画笔Pen对象。

       ```
// 创建画笔
OH_Drawing_Pen* pen = OH_Drawing_PenCreate();
```
             使用OH_Drawing_CanvasAttachPen()接口给Canvas画布设置画笔。接口接受两个参数，一个是画布对象Canvas，请确保已创建或获取得到画布Canvas，具体可见[画布的获取与绘制结果的显示（C/C++）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/canvas-get-result-draw-c)；另一个是要设置的画笔对象。画布将会使用设置的画笔样式和颜色等绘制图形轮廓。

       ```
// 设置画布的画笔
OH_Drawing_CanvasAttachPen(canvas, pen);
```
             使用画笔设置具体的描边效果（可选以下的一个或者多个效果）。
...
```
         OH_Drawing_PenLineJoinStyle转角样式可选分类对应如下：
             ...
             当不需要描边效果时，可以使用OH_Drawing_CanvasDetachPen()去除。入参为画布对象Canvas，请确保已创建或获取得到画布Canvas，具体可见[画布的获取与绘制结果的显示（C/C++）](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/canvas-get-result-draw-c)。

       ```
// 去除掉画布中的画笔
OH_Drawing_CanvasDetachPen(canvas);
```
             当不再需要画笔进行描边时，请及时使用OH_Drawing_PenDestroy()接口销毁Pen对象。

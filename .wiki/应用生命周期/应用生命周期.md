...
`EntryAbility.ets:14-38` — 在 loadContent 前尽可能早地设置主题色，减少启动闪烁。

```typescript
// EntryAbility.ets:14-38 — 启动初始化
...
`EntryAbility.ets:54-87` — 窗口创建后执行页面加载和环境设置：
...
`EntryAbility.ets:40-48` — 仅在 `themeName === 'system'` 时更新主题色：

```typescript
// EntryAbility.ets:40-48 — 系统配置变更
...
`EntryAbility.ets:105-126` 的 `updateBreakpoint` 方法实时计算响应式断点：
...
`EntryAbility.ets:31-33` 中 `appStore.init` 的 catch 分支仅记录日志，不阻塞应用启动。`Index` 页面需通过 `AppStorage('appInitialized')` 的值为 false 时显示加载中或超时提示。如果持久化数据损坏，`AppStore.init` 会跳过损坏数据，使用默认值继续运行。

### 避让区计算异常

沉浸式布局的避让区计算（`EntryAbility.ets:72-77`）可能因窗口未完全就绪或 DisplayManager 异常而失败。外层 try-catch （`EntryAbility.ets:83-85`）保证此异常不影响页面加载，仅记录 hilog.error。

### 主题切换闪烁

`onConfigurationUpdate` 仅在 `themeName === 'system'` 时更新系统主题。如果从 light 切换到 dark 时出现短暂闪烁，是因为 `appStore.init` 的异步完成晚于 `loadContent`。`onCreate` 中的预设置（`EntryAbility.ets:17-24`）可以部分缓解。

...
`common/` 目录是项目中最大的模块（57 个文件，含 13 个子目录），提供可复用的 UI 组件、媒体播放器、并发原语、分页管理器、数据源适配层、工具函数与常量定义。页面组件（`pages/`）与状态层（`store/`）通过 import 引入这些公共能力。
...
| `security/` | 凭据加密导出（`CredentialExportCrypto`） |
| `cache/` | `LruCache` 泛型缓存 |
| `dialogs/` | 系统弹窗工具 |
...
| `components/MdContentView.ets` | 组件 | Markdown 渲染组件，输入原始 Markdown → `MdParser` 解析为 AST → `@Builder` 渲染为 ArkUI 组件树。支持流式输出：`@State` text 属性更新时全量重解析并刷新 UI |

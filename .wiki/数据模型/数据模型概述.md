...
| `ThreadResult.ets` | `ThreadPaging`, `ThreadResult`, `PostAuthResult`, `PostReplyResult` | 帖子详情接口返回结果，P0-1 从 `service/NgaApi.ets` 迁入 |
...
| `AiConfig.ets` | `AiProviderProfile`, `AiTestResult` | 自定义 AI 提供方配置（端点/密钥/模型/流式开关/温度等），见 [AI 服务](../服务层/API通信.md#ai-服务模块) |
| `MdNode.ets` | `MdNodeType(enum)`, `MdNode` | Markdown AST 节点定义，由 `MdParser` 解析产生、`MdContentView` 消费渲染 |
...
`model/BBCodeNode.ets:1-40` 定义了 AST 节点类型枚举 `BBNodeType`（38 种节点类型），`model/BBCodeNode.ets:42-59` 定义了节点类 `BBNode`，全部字段显式初始化默认值：
...
> 注意：`ThreadResult.ets` 中另有一个 class 版本的 `ThreadPaging`（`model/ThreadResult.ets:17-21`，全字段默认值），与 `Thread.ets` 中 interface 版本的 `ThreadPagination` 并存——前者用于业务层结果封装，后者用于解析器内部数据流。
...
5. **ThreadPagination / ThreadPaging 双定义**：`Thread.ets`（interface `ThreadPagination`，解析器内部）与 `ThreadResult.ets`（class `ThreadPaging`，业务结果封装），修改字段时需同步两处

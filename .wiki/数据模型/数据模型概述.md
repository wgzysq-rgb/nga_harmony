...
> 注意：`ThreadResult.ets` 中另有一个 class 版本的 `ThreadPagination`（`model/ThreadResult.ets:12-16`，全字段默认值），与 `Thread.ets` 中 interface 版本的 `ThreadPagination` 并存——前者用于业务层结果封装，后者用于解析器内部数据流。

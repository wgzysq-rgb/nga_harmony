...
### 设置域（`store/settings/`，2 文件 + domain/ 8 文件）
...
- 多账户会话（Session）以 token 为 key 持久化，7 天自动过期清理（`AuthStore.ets:35` `SESSION_MAX_AGE`，清理逻辑见 `AuthStore.ets:191-205`）
- 旧版扁平 Key 格式自动迁移至 JSON 格式（`AuthStore.ets:104-123` `migrateAuthIfNeeded`）

### SettingsStore（`SettingsStore.ets:35`）
...
2. **持久化根状态 `SettingsState`（`settings/SettingsState.ets:24-49`）**——`@Observed` 装饰，24 个字段对应 8 个设置域。子 store 通过共享引用直接读写其字段，保持响应式不断。

3. **8 个设置域子 store（`settings/domain/`）**——各持 setter + `load` 分片。`load`（如 `MediaSettings.ets:64-76`）从反序列化对象按字段恢复；setter（如 `MediaSettings.ets:37-41`）改字段、必要时同步 AppStorage、最后调用 `ctx.persist()` 触发批量刷盘。
...
### ProfileStore（`ProfileStore.ets:16`）
...
投票记录持久化。每个用户最多保留 200 条帖子的投票记录（`VoteStore.ets:12` `VOTE_MAX_ENTRIES`，淘汰逻辑 `VoteStore.ets:116-127`），超出时淘汰最早记录。

### HistoryStore（`HistoryStore.ets:25`）

浏览历史持久化。每条记录包含帖子 ID、标题、作者信息等，最多 500 条（`HistoryStore.ets:20` `HISTORY_MAX_COUNT`）。按用户隔离存储，key 为 `history_{uid}`。

### NotificationStore（`NotificationStore.ets:15`）

通知缓存与未读数：

- 缓存通知列表，首次读取时从 Preferences 恢复
- `refreshUnreadCount()`（`NotificationStore.ets:42`）有 30 秒查询间隔限流（`NOTI_QUERY_DELAY`，`NotificationStore.ets:22`）
- 未读数通过 `AppStorage.setOrCreate(KEY_UNREAD_NOTI_COUNT, ...)`（`NotificationStore.ets:57`）驱动角标

### CategoryStore（`CategoryStore.ets:15`）

论坛分类列表缓存。首次从 Preferences 加载，后台 10 秒延迟发起网络刷新（`CategoryStore.ets:22` `REFRESH_DELAY`）。数据无变化时不写盘（`CategoryStore.ets:72-81` 用 `JSON.stringify` 深度比较），减少持久化频率。

### ToastManager（`common/feedback/ToastManager.ets:10`）

Toast 消息提示状态管理（P1-4 从 `store/` 迁至 `common/feedback/`）。显示新 Toast 时清除上一个定时器（`clearTimeout`），保证同一时间只有一个 Toast 显示，2.5 秒后自动消失。仍由 `AppStore` 持有实例并通过 `showToast` 转发（`AppStore.ets:228`）。
...
1. `PreferencesStore.init()`（`AppStore.ets:52`）— 基础设施就绪
2. `VoteStore.init()`, `HistoryStore.init()`（`AppStore.ets:55-56`）— 无状态依赖的子 Store
3. `AuthStore.init()`（`AppStore.ets:59`）— 认证 + Session
4. `CategoryStore.init()`（`AppStore.ets:60`）— 论坛分类
5. (已登录) `loadUserData()`（`AppStore.ets:74-78`）：`SettingsStore` → `ProfileStore` → `NotificationStore` — 用户数据
6. `applyCurrentTheme()`（`AppStore.ets:66`）— 主题应用（需要 context）
...
`AppStore.ets:69-71` 中 init 的 catch 分支仅 `hilog.error`，不影响应用启动。`AuthState.initialized` 在此状态下可能仍为 false，`Index` 页面需做等待超时处理。
...
A: `AppStore.ets:69-71` 中 init 的 catch 分支仅 `hilog.error`，不影响应用启动。`AuthState.initialized` 在此状态下可能仍为 false，`Index` 页面需做等待超时处理。

...
`pages/` 目录共 22 个文件，分为三类：`@Entry` 路由页（`MainPage`/`Index`/`LoginPage`/`AgentAnalysisPage`/`PostSummaryPage`/`WebViewPanel`）、`Router` 转发容器（`ActivityRouter`/`BoardRouter`，P2-4 命名归一化）、`Panel` 子页面（`TopicListPanel`/`ThreadPanel`/`SettingsPanel` 等）以及 `CommunityPanel`（社区内容）。本页涉及的文件：

| 文件 | 职责 |
|------|------|
| `pages/MainPage.ets` | 主页面三列布局容器（`@Entry`） |
| `pages/Index.ets` | 启动页，轮询 `appStore.auth.initialized` 后按认证状态跳转 |
| `common/components/SidebarComponent.ets` | 侧边栏第一列（头像 + 搜索框 + 内嵌 `CommunityPanel`） |
| `pages/BoardRouter.ets` | 板块列转发容器（原 `BoardPanelComponent`，P2-4 重命名） |
| `pages/ActivityRouter.ets` | 活动列转发容器（原 `ActivityPanelComponent`，P2-4 重命名） |
| `pages/CommunityPanel.ets` | 社区内容（板块分类列表/收藏网格/搜索入口，原 `CommunityTabContent`，P2-4 重命名） |
| `common/components/FloatingLayerComponent.ets` | 浮层渲染容器（原 `pages/FloatingLayerComponent`，P2-4 迁入 `common/components`） |
...
`EntryAbility.ets:112-133` 的 `updateBreakpoint` 根据窗口宽度计算断点：
...
```typescript
// EntryAbility.ets:120-127 — 断点判定逻辑
let newBp: string = 'sm'
if (widthVp < 600) {
  newBp = 'sm'
} else if (widthVp < 840) {
...
`MainPage.ets:76-123` 的核心构建方法（`SidebarComponent()` 与 `FloatingLayerComponent()` 现从 `common/components` 导入，见 `MainPage.ets:24`、`MainPage.ets:26`）：

```typescript
// MainPage.ets:76-123 — 三列容器
build() {
  Stack() {
    Row() {
...
4. **默认**：返回 `false` 交由系统处理（`MainPage.ets:73`）。
...
`pages/Index.ets` 作为应用入口页面，通过 **轮询** 等待 `AppStore` 初始化完成后，按认证状态跳转（`Index.ets:20-36`）：

```typescript
// Index.ets:20-36 — 轮询 initialized 后路由
...
> **与旧文档差异**：等待的标志是 `appStore.auth.initialized`（`Index.ets:21`），并非某个 AppStorage key。`verifyToken` 失败会 `clearAuth()` 后跳登录页（`Index.ets:27-29`）。
...
> **与旧文档差异**：`SidebarComponent` 不再提供「社区/我的」两个 Tab 切换的 UI；`RouterStore` 中仍保留 `sidebarTab` 字段与 `setSidebarTab()`（`RouterStore.ets:210`、`RouterStore.ets:216`），但当前已无 UI 调用方，仅由 `reset()` 在登出时复位（`RouterStore.ets:267-268`）。Tab 化的板块浏览逻辑已下沉到 `CommunityPanel` 自有的 `selectedSectionId`（`CommunityPanel.ets:13`）。
...
> **与旧文档差异**：token 有效路径不止"恢复"，还会触发 `ensureProfile`/`ensureFavorites`/`autoCheckinIfNeeded`（`MainPage.ets:162-164`）。所有 `replaceUrl` 调用均包裹 `try/catch` 防止路由异常中断（`MainPage.ets:145`、`MainPage.ets:152`）。

### 布局避让区计算失败

`EntryAbility.ets:79-84` 在 `onWindowStageCreate` 内读取系统避让区并换算为 `statusBarHeight`/`navBarHeight`（vp）写入 AppStorage：

```typescript
// EntryAbility.ets:79-84 — 避让区换算
...
整段沉浸式模式初始化（`EntryAbility.ets:70-92`）被 `try/catch` 包裹：若 `getMainWindowSync`/`getWindowAvoidArea` 抛异常（如窗口尚未就绪），仅记录日志（`EntryAbility.ets:91`），不会阻塞页面加载；此时 `statusBarHeight`/`navBarHeight` 保持默认值 0，页面顶部可能出现轻微偏移（侧边栏头部 `padding: top 14 + statusBarHeight`）。`updateBreakpoint` 内部对 `getDefaultDisplaySync` 也有独立 `try/catch`（`EntryAbility.ets:113-118`），density 获取失败时回退为 1。

## 常见问题

**Q: sm 模式下看不到侧边栏？**
A: 这是正常行为。sm 断点（< 600vp）隐藏侧边栏以最大化内容区域（`MainPage.ets:82`）。切换到 md/lg 断点或旋转设备可恢复。

**Q: 断点切换时布局闪烁？**
A: `windowSizeChange` 在拖动窗口时会高频触发。`updateBreakpoint` 通过 `this.curBp !== newBp` 过滤不必要的更新（`EntryAbility.ets:128`），不会在每次像素变化时都写 AppStorage / 触发布局重排。

**Q: Index 页面卡住不跳转？**
A: `Index.ets` 每 50ms 轮询 `appStore.auth.initialized`（`Index.ets:34`）。如果 `AppStore.init` 失败（日志 `AppStore init failed`，`EntryAbility.ets:39`），`auth.initialized` 永远不会被置为 `true`，`Index` 会一直停留在等待状态。当前无超时回退逻辑，需要重启应用。
